...
0008               ***************************************************************
0009               * File: stevie_b3.asm               ; Version 210919-3438025
0010               *
0011               * Bank 3 "John"
...
0260 6217 ....             text  'Build-ID  210919-3438025'
0261                       even
0262               
**** **** ****     > runlib.asm
...
0075 60F2 6D2A                   data   txt.stevie     ; Show Stevie version
0076               
...
0107 68B6 06A0  32         bl    @putstr
     68B8 242C 
0108 68BA 3622                   data txt.keymarker    ; Show key marker
0109               
...
0160 6900 361C                   data   txt.alpha.down
0161                       ;------------------------------------------------------
...
0054 694A 06A0  32         bl    @putstr
     694C 242C 
0055 694E 3718                   data txt.cmdb.prompt
0056                       ;------------------------------------------------------
0057                       ; Exit
0058                       ;------------------------------------------------------
...
0022 6A60 06A0  32         bl    @scroff               ; Turn off screen
     6A62 269C 
0023               
0024 6A64 06A0  32         bl    @f18rst               ; Reset and lock the F18A
     6A66 27AA 
0025               
0026 6A68 06A0  32         bl    @vidtab               ; Load video mode table into VDP
     6A6A 2306 
0027 6A6C 21F6                   data tibasic          ; Equate selected video mode table
0028                       ;-------------------------------------------------------
0029                       ; Setup VDP memory for TI Basic in 32x24 mode
0030                       ;-------------------------------------------------------
0031 6A6E 06A0  32         bl    @ldfnt
     6A70 236E 
0032 6A72 0900                   data >0900,fnopt3     ; Load font (upper & lower case)
     6A74 000C 
0033               
0034 6A76 06A0  32         bl    @filv
     6A78 229C 
0035 6A7A 0300                   data >0300,>D0,2      ; No sprites
     6A7C 00D0 
     6A7E 0002 
0036                       ;-------------------------------------------------------
0037                       ; Setup scratchpad memory for TI Basic
0038                       ;-------------------------------------------------------
0039 6A80 06A0  32         bl    @cpu.scrpad.pgout     ; \ Copy 256 bytes of scratchpad memory to
     6A82 2C58 
0040 6A84 AD00                   data scrpad.copy      ; | >ad00 and then load TI Basic layout from
0041                                                   ; / predefined address @cpu.scrpad.target
0042               
0043 6A86 04CB  14         clr   r11
0044 6A88 C820  54         mov   @run.tibasic.83d4,@>83d4
     6A8A 6AEE 
     6A8C 83D4 
0045 6A8E C820  54         mov   @run.tibasic.83fa,@>83fa
     6A90 6AF0 
     6A92 83FA 
0046 6A94 C820  54         mov   @run.tibasic.83fc,@>83fc
     6A96 6AF2 
     6A98 83FC 
0047 6A9A C820  54         mov   @run.tibasic.83fe,@>83fe
     6A9C 6AF4 
     6A9E 83FE 
0048                       ;-------------------------------------------------------
0049                       ; Register our ISR hook
0050                       ;-------------------------------------------------------
0051 6AA0 0201  20         li    r1,isr                ; \
     6AA2 6AF6 
0052 6AA4 C801  38         mov   r1,@>83c4             ; | >83c4 = Pointer to start address of
     6AA6 83C4 
0053                                                   ; /         User Interrupt Routine
0054                       ;-------------------------------------------------------
0055                       ; Run TI Basic in GPL Interpreter
0056                       ;-------------------------------------------------------
0057 6AA8 02E0  18         lwpi  >83e0
     6AAA 83E0 
0058 6AAC 0201  20         li    r1,>216f              ; Entrypoint for GPL TI Basic interpreter
     6AAE 216F 
0059 6AB0 D801  38         movb  r1,@grmwa             ; \
     6AB2 9C02 
0060 6AB4 06C1  14         swpb  r1                    ; | Set GPL address
0061 6AB6 D801  38         movb  r1,@grmwa             ; /
     6AB8 9C02 
0062 6ABA 1000  14         nop
0063 6ABC 0460  28         b     @>70                  ; Start GPL interpreter
     6ABE 0070 
0064                       ;-------------------------------------------------------
0065                       ; Return from TI Basic (got here from the ISR)
0066                       ;-------------------------------------------------------
0067               run.tibasic.return:
0068 6AC0 02E0  18         lwpi  >ad00                 ; Activate Stevie workspace
     6AC2 AD00 
0069               
0070 6AC4 06A0  32         bl    @cpu.scrpad.pgin
     6AC6 2C96 
0071 6AC8 AD00                   data scrpad.copy
0072               
0073 6ACA 06A0  32         bl    @scroff               ; Turn screen off
     6ACC 269C 
0074 6ACE 06A0  32         bl    @f18unl               ; Unlock the F18a
     6AD0 2740 
0076               
0077 6AD2 06A0  32         bl    @putvr                ; Turn on 30 rows mode.
     6AD4 2340 
0078 6AD6 3140                   data >3140            ; F18a VR49 (>31), bit 40
0079               
0081               
0082 6AD8 06A0  32         bl    @vidtab               ; Load video mode table into VDP
     6ADA 2306 
0083 6ADC 3446                   data stevie.tx8030    ; Equate selected video mode table
0084               
0085 6ADE 06A0  32         bl    @putvr                ; Turn on position based attributes
     6AE0 2340 
0086 6AE2 3202                   data >3202            ; F18a VR50 (>32), bit 2
0087               
0088 6AE4 06A0  32         BL    @putvr                ; Set VDP TAT base address for position
     6AE6 2340 
0089 6AE8 0360                   data >0360            ; based attributes (>40 * >60 = >1800)
0090                       ;------------------------------------------------------
0091                       ; Exit
0092                       ;------------------------------------------------------
0093               run.tibasic.exit:
0094 6AEA C2F9  30         mov   *stack+,r11           ; Pop r11
0095 6AEC 045B  20         b     *r11                  ; Return
0096               
0097               
0098               
0099                       ;-------------------------------------------------------
0100                       ; Required values for scratchpad
0101                       ;-------------------------------------------------------
0102               run.tibasic.83d4:
0103 6AEE E0D5             data  >e0d5
0104               run.tibasic.83fa:
0105 6AF0 9800             data  >9800
0106               run.tibasic.83fc:
0107 6AF2 0108             data  >0108
0108               run.tibasic.83fe:
0109 6AF4 8C02             data  >8c02
0110               
0111               
0112               
0113               ***************************************************************
0114               * isr
0115               * Interrupt Service Routine for returning to Stevie
0116               ***************************************************************
0117               * Called from monitor user interrupt
0118               *--------------------------------------------------------------
0119               * OUTPUT
0120               * none
0121               *--------------------------------------------------------------
...
0124               ********|*****|*********************|**************************
0125               isr:
0126 6AF6 C807  38         mov   r7,@rambuf+20         ; Backup R7
     6AF8 A154 
0127                       ;-------------------------------------------------------
0128                       ; Read TI Basic crunch buffer VDP >320
0129                       ;-------------------------------------------------------
0130 6AFA 0207  20         li    r7,>0320
     6AFC 0320 
0131 6AFE 06C7  14         swpb  r7                    ; \
0132 6B00 D807  38         movb  r7,@vdpa              ; | Set VDP read address
     6B02 8C02 
0133 6B04 06C7  14         swpb  r7                    ; | inlined @vdra call
0134 6B06 D807  38         movb  r7,@vdpa              ; /
     6B08 8C02 
0135                       ;-------------------------------------------------------
0136                       ; Copy TI Basic crunch buffer to Stevie ram buffer
0137                       ;-------------------------------------------------------
0138 6B0A 0207  20         li    r7,rambuf
     6B0C A140 
0139 6B0E DDE0  48         movb  @vdpr,*r7+            ; Read byte 1
     6B10 8800 
0140 6B12 DDE0  48         movb  @vdpr,*r7+            ; Read byte 2
     6B14 8800 
0141 6B16 DDE0  48         movb  @vdpr,*r7+            ; Read byte 3
     6B18 8800 
0142 6B1A DDE0  48         movb  @vdpr,*r7+            ; Read byte 4
     6B1C 8800 
0143                       ;-------------------------------------------------------
0144                       ; Check if 'EXIT' in Stevie ram buffer
0145                       ;-------------------------------------------------------
0146 6B1E 8820  54         c     @rambuf,@data.isr.exit
     6B20 A140 
     6B22 6B36 
0147 6B24 1605  14         jne   isr.exit              ; Skip unless 'EX'
0148 6B26 8820  54         c     @rambuf+2,@data.isr.exit+2
     6B28 A142 
     6B2A 6B38 
0149 6B2C 1601  14         jne   isr.exit              ; Skip unless 'IT'
0150                       ;-------------------------------------------------------
0151                       ; Return to Stevie
0152                       ;-------------------------------------------------------
0153 6B2E 10C8  14         jmp   run.tibasic.return
0154                       ;-------------------------------------------------------
0155                       ; Return from ISR
0156                       ;-------------------------------------------------------
0157               isr.exit:
0158 6B30 C1E0  34         mov   @rambuf+20,r7         ; Restore R7
     6B32 A154 
0159 6B34 045B  20         b     *r11                  ; Return from ISR
0160               data.isr.exit:
0161 6B36 ....             text  'EXIT'
**** **** ****     > stevie_b3.asm.3438025
0094                       ;-----------------------------------------------------------------------
...
0011               ;--------------------------------------------------------------
0012 6B3A 1301     txt.head.load      byte 19,1,3
     6B3C 0320 
0013 6B3D ....                        text ' Open DV80 file '
0014                                  byte 2
0015               txt.hint.load
0016 6B4E 3D53             byte  61
0017 6B4F ....             text  'Select Fastmode for file buffer in CPU RAM (HRD/HDX/IDE only)'
0018                       even
0019               
0020               
0021 6B8C 3146     txt.keys.load      byte 49
0022 6B8D ....                        text 'F9=Back  F3=Clear  F5=Fastmode  F-H=Home  F-L=End'
0023               
0024 6BBE 3246     txt.keys.load2     byte 50
0025 6BBF ....                        text 'F9=Back  F3=Clear  *F5=Fastmode  F-H=Home  F-L=End'
0026               
0027               ;--------------------------------------------------------------
0028               ; Dialog Save DV 80 file
0029               ;--------------------------------------------------------------
0030 6BF2 0103     txt.head.save      byte 19,1,3
0031 6BF4 ....                        text ' Save DV80 file '
0032 6C04 0223                        byte 2
0033 6C06 0103     txt.head.save2     byte 35,1,3,32
     6C08 2053 
0034 6C09 ....                        text 'Save marked block to DV80 file '
0035 6C28 0201                        byte 2
0036               txt.hint.save
0037                       byte  1
0038 6C2A ....             text  ' '
0039                       even
0040               
0041               
0042 6C2C 2446     txt.keys.save      byte 36
0043 6C2D ....                        text 'F9=Back  F3=Clear  F-H=Home  F-L=End'
0044               
0045               ;--------------------------------------------------------------
0046               ; Dialog "Unsaved changes"
0047               ;--------------------------------------------------------------
0048 6C52 0103     txt.head.unsaved   byte 20,1,3
0049 6C54 ....                        text ' Unsaved changes '
0050                                  byte 2
0051               txt.info.unsaved
0052 6C66 2157             byte  33
0053 6C67 ....             text  'Warning! Unsaved changes in file.'
0054                       even
0055               
0056               txt.hint.unsaved
0057 6C88 2A50             byte  42
0058 6C89 ....             text  'Press F6 to proceed or ENTER to save file.'
0059                       even
0060               
0061               txt.keys.unsaved
0062 6CB4 2843             byte  40
0063 6CB5 ....             text  'Confirm: F9=Back  F6=Proceed  ENTER=Save'
0064                       even
0065               
0066               
0067               ;--------------------------------------------------------------
0068               ; Dialog "About"
0069               ;--------------------------------------------------------------
0070 6CDE 0A01     txt.head.about     byte 10,1,3
     6CE0 0320 
0071 6CE1 ....                        text ' About '
0072 6CE8 0200                        byte 2
0073               
0074               txt.info.about
0075                       byte  0
0076 6CEA ....             text
...
0078               
0079               txt.hint.about
0080 6CEA 1D50             byte  29
0081 6CEB ....             text  'Press F9 to return to editor.'
0082                       even
0083               
0084 6D08 2148     txt.keys.about     byte 33
0085 6D09 ....                        text 'Help: F9=Back  '
0086 6D18 0E0F                        byte 14,15
0087 6D1A ....                        text '=Alpha Lock down'
0088               
0089               txt.stevie
0090 6D2A 0B53             byte  11
0091 6D2B ....             text  'STEVIE 1.1U'
0092                       even
0093               
0094               txt.about.build
0095 6D36 4C42             byte  76
0096 6D37 ....             text  'Build: 210919-3438025 / 2018-2021 Filip Van Vooren / retroclouds on Atariage'
0097                       even
0098               
0099               
0100               
...
0102               ; Dialog "Menu"
0103               ;--------------------------------------------------------------
0104 6D84 1001     txt.head.menu      byte 16,1,3
     6D86 0320 
0105 6D87 ....                        text ' Stevie 1.1U '
0106 6D94 021A                        byte 2
0107               
0108               txt.info.menu
0109                       byte  26
0110 6D96 ....             text  'File / Basic / Help / Quit'
0111                       even
0112               
0113 6DB0 0007     pos.info.menu      byte 0,7,15,22,>ff
     6DB2 0F16 
     6DB4 FF28 
0114               txt.hint.menu
0115                       byte  40
0116 6DB6 ....             text  'Press F,B,H,Q or F9 to return to editor.'
0117                       even
0118               
0119               txt.keys.menu
0120 6DDE 0746             byte  7
0121 6DDF ....             text  'F9=Back'
0122                       even
0123               
0124               
0125               
...
0127               ; Dialog "File"
0128               ;--------------------------------------------------------------
0129 6DE6 0901     txt.head.file      byte 9,1,3
     6DE8 0320 
0130 6DE9 ....                        text ' File '
0131                                  byte 2
0132               
0133               txt.info.file
0134 6DF0 114E             byte  17
0135 6DF1 ....             text  'New / Open / Save'
0136                       even
0137               
0138 6E02 0006     pos.info.file      byte 0,6,13,>ff
     6E04 0DFF 
0139               txt.hint.file
0140 6E06 2650             byte  38
0141 6E07 ....             text  'Press N,O,S or F9 to return to editor.'
0142                       even
0143               
0144               txt.keys.file
0145 6E2E 0746             byte  7
0146 6E2F ....             text  'F9=Back'
0147                       even
0148               
0149               
0150               ;--------------------------------------------------------------
0151               ; Dialog "Basic"
0152               ;--------------------------------------------------------------
0153 6E36 0E01     txt.head.basic     byte 14,1,3
     6E38 0320 
0154 6E39 ....                        text ' Run basic '
0155 6E44 021C                        byte 2
0156               
0157               txt.info.basic
0158                       byte  28
0159 6E46 ....             text  'TI Basic / TI Extended Basic'
0160                       even
0161               
0162 6E62 030E     pos.info.basic     byte 3,14,>ff
     6E64 FF3E 
0163               txt.hint.basic
0164                       byte  62
0165 6E66 ....             text  'Press B,E for running basic dialect or F9 to return to editor.'
0166                       even
0167               
0168               txt.keys.basic
0169 6EA4 0746             byte  7
0170 6EA5 ....             text  'F9=Back'
0171                       even
0172               
**** **** ****     > stevie_b3.asm.3438025
0098                       ;-----------------------------------------------------------------------
...
0122      3446     spvmod  equ   stevie.tx8030         ; Video mode.   See VIDTAB for details.
