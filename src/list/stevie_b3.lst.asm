...
0008               ***************************************************************
0009               * File: stevie_b3.asm               ; Version 210922-282810
0010               *
0011               * Bank 3 "John"
...
0260 6217 ....             text  'Build-ID  210922-282810'
0261                       even
0262               
**** **** ****     > runlib.asm
...
0075 60F2 6DE0                   data   txt.stevie     ; Show Stevie version
0076               
...
0107 68E6 06A0  32         bl    @putstr
     68E8 242A 
0108 68EA 3638                   data txt.keymarker    ; Show key marker
0109               
...
0160 6930 3632                   data   txt.alpha.down
0161                       ;------------------------------------------------------
...
0054 697A 06A0  32         bl    @putstr
     697C 242A 
0055 697E 372E                   data txt.cmdb.prompt
0056                       ;------------------------------------------------------
0057                       ; Exit
0058                       ;------------------------------------------------------
...
0019                       ;-------------------------------------------------------
0020                       ; Setup SAMS for TI Basic
0021                       ;-------------------------------------------------------
0022 6A90 06A0  32         bl    @sams.layout.copy     ; Backup Stevie SAMS page layout
     6A92 2652 
0023 6A94 A200                   data tv.sams.2000     ; \ @i = target address of 8 words table
0024                                                   ; /      that contains SAMS layout
0025               
0026 6A96 06A0  32         bl    @sams.layout
     6A98 25EE 
0027 6A9A 3474                   data mem.sams.tibasic ; Load SAMS page layout for TI Basic
0028               
0029 6A9C 06A0  32         bl    @cpyv2m
     6A9E 24C4 
0030 6AA0 0000                   data vdp.sit.base,>f000,vdp.sit.size
     6AA2 F000 
     6AA4 0960 
0031                                                   ; Dump Stevie SIT 80x30 to RAM buffer
0032                                                   ; >f000 (SAMS page #08)
0033                       ;-------------------------------------------------------
0034                       ; Put VDP in TI Basic compatible mode (32x30)
0035                       ;-------------------------------------------------------
0036 6AA6 06A0  32         bl    @scroff               ; Turn off screen
     6AA8 269A 
0037               
0038 6AAA 06A0  32         bl    @f18rst               ; Reset and lock the F18A
     6AAC 27A8 
0039               
0040 6AAE 06A0  32         bl    @vidtab               ; Load video mode table into VDP
     6AB0 2304 
0041 6AB2 21F4                   data tibasic          ; Equate selected video mode table
0042                       ;-------------------------------------------------------
0043                       ; Setup VDP memory for TI Basic in 32x24 mode
0044                       ;-------------------------------------------------------
0045 6AB4 06A0  32         bl    @ldfnt
     6AB6 236C 
0046 6AB8 0900                   data >0900,fnopt3     ; Load font (upper & lower case)
     6ABA 000C 
0047               
0048 6ABC 06A0  32         bl    @filv
     6ABE 229A 
0049 6AC0 0300                   data >0300,>D0,2      ; No sprites
     6AC2 00D0 
     6AC4 0002 
0050                       ;-------------------------------------------------------
0051                       ; Setup scratchpad memory for TI Basic
0052                       ;-------------------------------------------------------
0053 6AC6 06A0  32         bl    @cpu.scrpad.pgout     ; \ Copy 256 bytes of scratchpad memory to
     6AC8 2C4E 
0054 6ACA AD00                   data scrpad.copy      ; | >ad00 and then load TI Basic layout from
0055                                                   ; / predefined address @cpu.scrpad.target
0056               
0057 6ACC 04CB  14         clr   r11
0058 6ACE C820  54         mov   @run.tibasic.83d4,@>83d4
     6AD0 6B8E 
     6AD2 83D4 
0059 6AD4 C820  54         mov   @run.tibasic.83fa,@>83fa
     6AD6 6B90 
     6AD8 83FA 
0060 6ADA C820  54         mov   @run.tibasic.83fc,@>83fc
     6ADC 6B92 
     6ADE 83FC 
0061 6AE0 C820  54         mov   @run.tibasic.83fe,@>83fe
     6AE2 6B94 
     6AE4 83FE 
0062                       ;-------------------------------------------------------
0063                       ; Register our ISR hook
0064                       ;-------------------------------------------------------
0065 6AE6 0201  20         li    r1,isr                ; \
     6AE8 6B96 
0066 6AEA C801  38         mov   r1,@>83c4             ; | >83c4 = Pointer to start address of
     6AEC 83C4 
0067                                                   ; /         User Interrupt Routine
0068                       ;-------------------------------------------------------
0069                       ; Run TI Basic in GPL Interpreter
0070                       ;-------------------------------------------------------
0071 6AEE 02E0  18         lwpi  >83e0
     6AF0 83E0 
0072 6AF2 0201  20         li    r1,>216f              ; Entrypoint for GPL TI Basic interpreter
     6AF4 216F 
0073 6AF6 D801  38         movb  r1,@grmwa             ; \
     6AF8 9C02 
0074 6AFA 06C1  14         swpb  r1                    ; | Set GPL address
0075 6AFC D801  38         movb  r1,@grmwa             ; /
     6AFE 9C02 
0076 6B00 1000  14         nop
0077 6B02 0460  28         b     @>70                  ; Start GPL interpreter
     6B04 0070 
0078                       ;-------------------------------------------------------
0079                       ; Return from TI Basic (got here from the ISR)
0080                       ;-------------------------------------------------------
0081               run.tibasic.return:
0082 6B06 02E0  18         lwpi  >ad00                 ; Activate Stevie workspace in core RAM 2
     6B08 AD00 
0083               
0084 6B0A 06A0  32         bl    @cpu.scrpad.pgin      ; Page in copy of scratch pad memory and
     6B0C 2C8C 
0085 6B0E AD00                   data scrpad.copy      ; activate workspace at >8300
0086               
0087 6B10 06A0  32         bl    @scroff               ; Turn screen off
     6B12 269A 
0088                       ;-------------------------------------------------------
0089                       ; Cleanup after return from TI Basic
0090                       ;-------------------------------------------------------
0091 6B14 06A0  32         bl    @cpyv2m
     6B16 24C4 
0092 6B18 0000                   data >0000,>b000,16384
     6B1A B000 
     6B1C 4000 
0093                                                   ; Dump TI Basic 16K VDP memory to
0094                                                   ; RAM buffer >b000->ffff (SAMS pages #04-07)
0095               
0096 6B1E 06A0  32         bl    @film
     6B20 2242 
0097 6B22 A140                   data rambuf,>00,20    ; Clear crunch buffer copy in RAM
     6B24 0000 
     6B26 0014 
0098               
0099 6B28 06A0  32         bl    @cpym2v
     6B2A 2492 
0100 6B2C 0000                   data vdp.sit.base,>f000,vdp.sit.size
     6B2E F000 
     6B30 0960 
0101                                                   ; Dump SIT to VDP from RAM buffer
0102                                                   ; >f000 (SAMS page #08)
0103                       ;-------------------------------------------------------
0104                       ; Restore SAMS memory layout for Stevie
0105                       ;-------------------------------------------------------
0106 6B32 C120  34         mov   @tv.sams.b000,tmp0
     6B34 A206 
0107 6B36 0205  20         li    tmp1,>b000
     6B38 B000 
0108 6B3A 06A0  32         bl    @xsams.page.set       ; Set sams page for address >b000
     6B3C 2582 
0109               
0110 6B3E C120  34         mov   @tv.sams.c000,tmp0
     6B40 A208 
0111 6B42 0205  20         li    tmp1,>c000
     6B44 C000 
0112 6B46 06A0  32         bl    @xsams.page.set       ; Set sams page for address >c000
     6B48 2582 
0113               
0114 6B4A C120  34         mov   @tv.sams.d000,tmp0
     6B4C A20A 
0115 6B4E 0205  20         li    tmp1,>d000
     6B50 D000 
0116 6B52 06A0  32         bl    @xsams.page.set       ; Set sams page for address >d000
     6B54 2582 
0117               
0118 6B56 C120  34         mov   @tv.sams.e000,tmp0
     6B58 A20C 
0119 6B5A 0205  20         li    tmp1,>e000
     6B5C E000 
0120 6B5E 06A0  32         bl    @xsams.page.set       ; Set sams page for address >e000
     6B60 2582 
0121               
0122 6B62 C120  34         mov   @tv.sams.f000,tmp0
     6B64 A20E 
0123 6B66 0205  20         li    tmp1,>f000
     6B68 F000 
0124 6B6A 06A0  32         bl    @xsams.page.set       ; Set sams page for address >f000
     6B6C 2582 
0125                       ;-------------------------------------------------------
0126                       ; Setup F18a 80x30 mode again
0127                       ;-------------------------------------------------------
0128 6B6E 06A0  32         bl    @f18unl               ; Unlock the F18a
     6B70 273E 
0130               
0131 6B72 06A0  32         bl    @putvr                ; Turn on 30 rows mode.
     6B74 233E 
0132 6B76 3140                   data >3140            ; F18a VR49 (>31), bit 40
0133               
0135               
0136 6B78 06A0  32         bl    @vidtab               ; Load video mode table into VDP
     6B7A 2304 
0137 6B7C 343C                   data stevie.tx8030    ; Equate selected video mode table
0138               
0139 6B7E 06A0  32         bl    @putvr                ; Turn on position based attributes
     6B80 233E 
0140 6B82 3202                   data >3202            ; F18a VR50 (>32), bit 2
0141               
0142 6B84 06A0  32         BL    @putvr                ; Set VDP TAT base address for position
     6B86 233E 
0143 6B88 0360                   data >0360            ; based attributes (>40 * >60 = >1800)
0144                       ;------------------------------------------------------
0145                       ; Exit
0146                       ;------------------------------------------------------
0147               run.tibasic.exit:
0148 6B8A C2F9  30         mov   *stack+,r11           ; Pop r11
0149 6B8C 045B  20         b     *r11                  ; Return
0150               
0151                       ;-------------------------------------------------------
0152                       ; Required values for scratchpad
0153                       ;-------------------------------------------------------
0154               run.tibasic.83d4:
0155 6B8E E0D5             data  >e0d5
0156               run.tibasic.83fa:
0157 6B90 9800             data  >9800
0158               run.tibasic.83fc:
0159 6B92 0108             data  >0108
0160               run.tibasic.83fe:
0161 6B94 8C02             data  >8c02
0162               
0163               
0164               
0165               
0166               ; TODO slow down hotkey check, messes up basic keyboard scan.
0167               
0168               ***************************************************************
0169               * isr
0170               * Interrupt Service Routine for returning to Stevie
0171               ***************************************************************
0172               * Called from monitor user interrupt
0173               *--------------------------------------------------------------
0174               * OUTPUT
0175               * none
0176               *--------------------------------------------------------------
...
0179               ********|*****|*********************|**************************
0180               isr:
0181 6B96 C807  38         mov   r7,@rambuf+20         ; Backup R7
     6B98 A154 
0182 6B9A C80C  38         mov   r12,@rambuf+22        ; Backup R12
     6B9C A156 
0183                       ;-------------------------------------------------------
0184                       ; Hotkey pressed?
0185                       ;-------------------------------------------------------
0186 6B9E C1E0  34         mov   @>8374,r7             ; Get keyboard scancode
     6BA0 8374 
0187 6BA2 0247  22         andi  r7,>00ff              ; LSB only
     6BA4 00FF 
0188 6BA6 0287  22         ci    r7,>bb                ; Hotkey ctrl + '/' pressed?
     6BA8 00BB 
0189 6BAA 13AD  14         jeq   run.tibasic.return    ; Yes, return to Stevie
0190                       ;-------------------------------------------------------
0191                       ; Read TI Basic crunch buffer VDP >320
0192                       ;-------------------------------------------------------
0193 6BAC 0207  20         li    r7,>0320
     6BAE 0320 
0194 6BB0 06C7  14         swpb  r7                    ; \
0195 6BB2 D807  38         movb  r7,@vdpa              ; | Set VDP read address
     6BB4 8C02 
0196 6BB6 06C7  14         swpb  r7                    ; | inlined @vdra call
0197 6BB8 D807  38         movb  r7,@vdpa              ; /
     6BBA 8C02 
0198                       ;-------------------------------------------------------
0199                       ; Copy TI Basic crunch buffer to Stevie ram buffer
0200                       ;-------------------------------------------------------
0201               isr.crunchbuf:
0202 6BBC 0207  20         li    r7,rambuf
     6BBE A140 
0203 6BC0 DDE0  48         movb  @vdpr,*r7+            ; Read byte 1
     6BC2 8800 
0204 6BC4 DDE0  48         movb  @vdpr,*r7+            ; Read byte 2
     6BC6 8800 
0205 6BC8 DDE0  48         movb  @vdpr,*r7+            ; Read byte 3
     6BCA 8800 
0206 6BCC DDE0  48         movb  @vdpr,*r7+            ; Read byte 4
     6BCE 8800 
0207                       ;-------------------------------------------------------
0208                       ; Check if 'EXIT' in Stevie ram buffer
0209                       ;-------------------------------------------------------
0210 6BD0 8820  54         c     @rambuf,@data.isr.exit
     6BD2 A140 
     6BD4 6BEC 
0211 6BD6 1605  14         jne   isr.exit              ; Skip unless 'EX'
0212 6BD8 8820  54         c     @rambuf+2,@data.isr.exit+2
     6BDA A142 
     6BDC 6BEE 
0213 6BDE 1601  14         jne   isr.exit              ; Skip unless 'IT'
0214 6BE0 1092  14         jmp   run.tibasic.return    ; Return to Stevie
0215                       ;-------------------------------------------------------
0216                       ; Return from ISR
0217                       ;-------------------------------------------------------
0218               isr.exit:
0219 6BE2 C320  34         mov   @rambuf+22,r12        ; Restore R12
     6BE4 A156 
0220 6BE6 C1E0  34         mov   @rambuf+20,r7         ; Restore R7
     6BE8 A154 
0221 6BEA 045B  20         b     *r11                  ; Return from ISR
0222               data.isr.exit:
0223 6BEC ....             text  'EXIT'
**** **** ****     > stevie_b3.asm.282810
0094                       ;-----------------------------------------------------------------------
...
0011               ;--------------------------------------------------------------
0012 6BF0 1301     txt.head.load      byte 19,1,3
     6BF2 0320 
0013 6BF3 ....                        text ' Open DV80 file '
0014                                  byte 2
0015               txt.hint.load
0016 6C04 3D53             byte  61
0017 6C05 ....             text  'Select Fastmode for file buffer in CPU RAM (HRD/HDX/IDE only)'
0018                       even
0019               
0020               
0021 6C42 3146     txt.keys.load      byte 49
0022 6C43 ....                        text 'F9=Back  F3=Clear  F5=Fastmode  F-H=Home  F-L=End'
0023               
0024 6C74 3246     txt.keys.load2     byte 50
0025 6C75 ....                        text 'F9=Back  F3=Clear  *F5=Fastmode  F-H=Home  F-L=End'
0026               
0027               ;--------------------------------------------------------------
0028               ; Dialog Save DV 80 file
0029               ;--------------------------------------------------------------
0030 6CA8 0103     txt.head.save      byte 19,1,3
0031 6CAA ....                        text ' Save DV80 file '
0032 6CBA 0223                        byte 2
0033 6CBC 0103     txt.head.save2     byte 35,1,3,32
     6CBE 2053 
0034 6CBF ....                        text 'Save marked block to DV80 file '
0035 6CDE 0201                        byte 2
0036               txt.hint.save
0037                       byte  1
0038 6CE0 ....             text  ' '
0039                       even
0040               
0041               
0042 6CE2 2446     txt.keys.save      byte 36
0043 6CE3 ....                        text 'F9=Back  F3=Clear  F-H=Home  F-L=End'
0044               
0045               ;--------------------------------------------------------------
0046               ; Dialog "Unsaved changes"
0047               ;--------------------------------------------------------------
0048 6D08 0103     txt.head.unsaved   byte 20,1,3
0049 6D0A ....                        text ' Unsaved changes '
0050                                  byte 2
0051               txt.info.unsaved
0052 6D1C 2157             byte  33
0053 6D1D ....             text  'Warning! Unsaved changes in file.'
0054                       even
0055               
0056               txt.hint.unsaved
0057 6D3E 2A50             byte  42
0058 6D3F ....             text  'Press F6 to proceed or ENTER to save file.'
0059                       even
0060               
0061               txt.keys.unsaved
0062 6D6A 2843             byte  40
0063 6D6B ....             text  'Confirm: F9=Back  F6=Proceed  ENTER=Save'
0064                       even
0065               
0066               
0067               ;--------------------------------------------------------------
0068               ; Dialog "About"
0069               ;--------------------------------------------------------------
0070 6D94 0A01     txt.head.about     byte 10,1,3
     6D96 0320 
0071 6D97 ....                        text ' About '
0072 6D9E 0200                        byte 2
0073               
0074               txt.info.about
0075                       byte  0
0076 6DA0 ....             text
...
0078               
0079               txt.hint.about
0080 6DA0 1D50             byte  29
0081 6DA1 ....             text  'Press F9 to return to editor.'
0082                       even
0083               
0084 6DBE 2148     txt.keys.about     byte 33
0085 6DBF ....                        text 'Help: F9=Back  '
0086 6DCE 0E0F                        byte 14,15
0087 6DD0 ....                        text '=Alpha Lock down'
0088               
0089               txt.stevie
0090 6DE0 0B53             byte  11
0091 6DE1 ....             text  'STEVIE 1.1U'
0092                       even
0093               
0094               txt.about.build
0095 6DEC 4B42             byte  75
0096 6DED ....             text  'Build: 210922-282810 / 2018-2021 Filip Van Vooren / retroclouds on Atariage'
0097                       even
0098               
0099               
0100               
...
0102               ; Dialog "Menu"
0103               ;--------------------------------------------------------------
0104 6E38 1001     txt.head.menu      byte 16,1,3
     6E3A 0320 
0105 6E3B ....                        text ' Stevie 1.1U '
0106 6E48 021A                        byte 2
0107               
0108               txt.info.menu
0109                       byte  26
0110 6E4A ....             text  'File / Basic / Help / Quit'
0111                       even
0112               
0113 6E64 0007     pos.info.menu      byte 0,7,15,22,>ff
     6E66 0F16 
     6E68 FF28 
0114               txt.hint.menu
0115                       byte  40
0116 6E6A ....             text  'Press F,B,H,Q or F9 to return to editor.'
0117                       even
0118               
0119               txt.keys.menu
0120 6E92 0746             byte  7
0121 6E93 ....             text  'F9=Back'
0122                       even
0123               
0124               
0125               
...
0127               ; Dialog "File"
0128               ;--------------------------------------------------------------
0129 6E9A 0901     txt.head.file      byte 9,1,3
     6E9C 0320 
0130 6E9D ....                        text ' File '
0131                                  byte 2
0132               
0133               txt.info.file
0134 6EA4 114E             byte  17
0135 6EA5 ....             text  'New / Open / Save'
0136                       even
0137               
0138 6EB6 0006     pos.info.file      byte 0,6,13,>ff
     6EB8 0DFF 
0139               txt.hint.file
0140 6EBA 2650             byte  38
0141 6EBB ....             text  'Press N,O,S or F9 to return to editor.'
0142                       even
0143               
0144               txt.keys.file
0145 6EE2 0746             byte  7
0146 6EE3 ....             text  'F9=Back'
0147                       even
0148               
0149               
0150               ;--------------------------------------------------------------
0151               ; Dialog "Basic"
0152               ;--------------------------------------------------------------
0153 6EEA 0E01     txt.head.basic     byte 14,1,3
     6EEC 0320 
0154 6EED ....                        text ' Run basic '
0155 6EF8 021C                        byte 2
0156               
0157               txt.info.basic
0158                       byte  28
0159 6EFA ....             text  'TI Basic / TI Extended Basic'
0160                       even
0161               
0162 6F16 030E     pos.info.basic     byte 3,14,>ff
     6F18 FF3E 
0163               txt.hint.basic
0164                       byte  62
0165 6F1A ....             text  'Press B,E for running basic dialect or F9 to return to editor.'
0166                       even
0167               
0168               txt.keys.basic
0169 6F58 0746             byte  7
0170 6F59 ....             text  'F9=Back'
0171                       even
0172               
**** **** ****     > stevie_b3.asm.282810
0098                       ;-----------------------------------------------------------------------
...
0122      343C     spvmod  equ   stevie.tx8030         ; Video mode.   See VIDTAB for details.
