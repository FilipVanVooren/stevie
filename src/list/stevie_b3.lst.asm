...
0008               ***************************************************************
0009               * File: stevie_b3.asm               ; Version 210822-91491
0010               *
0011               * Bank 3 "John"
0012               *
0013               ***************************************************************
...
0260 21D3 ....             text  'Build-ID  210822-91491'
0261                       even
0262               
**** **** ****     > runlib.asm
...
0018 3459 ....             text  'Build: 210822-91491 / 2018-2021 Filip Van Vooren / retroclouds on Atariage'
0019                       even
0020               
0021               
0022               txt.delim
...
**** **** ****     > stevie_b3.asm.91491
0091                       ;-----------------------------------------------------------------------
0092                       ; Basic interpreter handling
0093                       ;-----------------------------------------------------------------------
0094                       copy  "run.tibasic.asm"     ; Run TI Basic in GPL Interpreter
**** **** ****     > run.tibasic.asm
...
0020 6700 06A0  32         bl    @f18rst               ; Reset and lock the F18A
     6702 279E 
0021               
0022 6704 06A0  32         bl    @vidtab               ; Load video mode table into VDP
     6706 22FA 
0023 6708 21EA                   data graph1           ; Equate selected video mode table
0024               
0025 670A 06A0  32         bl    @ldfnt
     670C 2362 
0026 670E 0900                   data >0900,fnopt3     ; Load font (upper & lower case)
     6710 000C 
0027               
0028 6712 06A0  32         bl    @filv
     6714 2290 
0029 6716 0380                   data >0380,>f0,32*24  ; Load color table
     6718 00F0 
     671A 0300 
0030               
0031                       ;clr  @bank0.rom            ; Activate bank 0
0032               
0033 671C 06A0  32         bl    @cpu.scrpad.restore
     671E F034 
0034               
0035               
0036 6720 C820  54         mov   @run.tibasic.83fa,@>83fa
     6722 674A 
     6724 83FA 
0037 6726 C820  54         mov   @run.tibasic.83fc,@>83fc
     6728 674C 
     672A 83FC 
0038 672C C820  54         mov   @run.tibasic.83fe,@>83fe
     672E 674E 
     6730 83FE 
0039                       ;-------------------------------------------------------
0040                       ; Run TI Basic in GPL Interpreter
0041                       ;-------------------------------------------------------
0042 6732 02E0  18         lwpi  >83e0
     6734 83E0 
0043 6736 0201  20         li    r1,>216f              ; Entrypoint for GPL TI Basic interpreter
     6738 216F 
0044 673A D801  38         movb  r1,@grmwa             ; \
     673C 9C02 
0045 673E 06C1  14         swpb  r1                    ; | Set GPL address
0046 6740 D801  38         movb  r1,@grmwa             ; /
     6742 9C02 
0047 6744 1000  14         nop
0048 6746 0460  28         b     @>70                  ; Start GPL interpreter
     6748 0070 
0049               
0050               run.tibasic.83fa:
0051 674A 9800             data  >9800
0052               run.tibasic.83fc:
0053 674C 0001             data  >0001
0054               run.tibasic.83fe:
0055 674E 8C02             data  >8c02
**** **** ****     > stevie_b3.asm.91491
0095                       ;-----------------------------------------------------------------------
0096                       ; Command buffer handling
0097                       ;-----------------------------------------------------------------------
...
0020               ********|*****|*********************|**************************
0021               cmdb.refresh:
0022 6750 0649  14         dect  stack
0023 6752 C64B  30         mov   r11,*stack            ; Save return address
0024 6754 0649  14         dect  stack
0025 6756 C644  30         mov   tmp0,*stack           ; Push tmp0
0026 6758 0649  14         dect  stack
0027 675A C645  30         mov   tmp1,*stack           ; Push tmp1
0028 675C 0649  14         dect  stack
0029 675E C646  30         mov   tmp2,*stack           ; Push tmp2
0030 6760 0649  14         dect  stack
0031 6762 C660  46         mov   @wyx,*stack           ; Push cursor position
     6764 832A 
0032                       ;------------------------------------------------------
0033                       ; Dump Command buffer content
0034                       ;------------------------------------------------------
0035 6766 C820  54         mov   @cmdb.yxprompt,@wyx   ; Screen position of command line prompt
     6768 A310 
     676A 832A 
0036               
0037 676C 05A0  34         inc   @wyx                  ; X +1 for prompt
     676E 832A 
0038               
0039 6770 06A0  32         bl    @yx2pnt               ; Get VDP PNT address for current YX pos.
     6772 23FC 
0040                                                   ; \ i  @wyx = Cursor position
0041                                                   ; / o  tmp0 = VDP target address
0042               
0043 6774 0205  20         li    tmp1,cmdb.cmd         ; Address of current command
     6776 A329 
0044 6778 0206  20         li    tmp2,1*79             ; Command length
     677A 004F 
0045               
0046 677C 06A0  32         bl    @xpym2v               ; \ Copy CPU memory to VDP memory
     677E 248E 
...
0052                       ;------------------------------------------------------
0053 6780 C820  54         mov   @cmdb.yxprompt,@wyx
     6782 A310 
     6784 832A 
0054 6786 06A0  32         bl    @putstr
     6788 2420 
0055 678A 3A2A                   data txt.cmdb.prompt
0056                       ;------------------------------------------------------
0057                       ; Exit
0058                       ;------------------------------------------------------
0059               cmdb.refresh.exit:
0060 678C C839  50         mov   *stack+,@wyx          ; Pop cursor position
     678E 832A 
0061 6790 C1B9  30         mov   *stack+,tmp2          ; Pop tmp2
0062 6792 C179  30         mov   *stack+,tmp1          ; Pop tmp1
0063 6794 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0064 6796 C2F9  30         mov   *stack+,r11           ; Pop r11
0065 6798 045B  20         b     *r11                  ; Return to caller
**** **** ****     > stevie_b3.asm.91491
...
0026 67A2 0649  14         dect  stack
0027 67A4 C645  30         mov   tmp1,*stack           ; Push tmp1
0028 67A6 0649  14         dect  stack
0029 67A8 C646  30         mov   tmp2,*stack           ; Push tmp2
0030                       ;------------------------------------------------------
0031                       ; Clear command
0032                       ;------------------------------------------------------
0033 67AA 04E0  34         clr   @cmdb.cmdlen          ; Reset length
     67AC A328 
0034 67AE 06A0  32         bl    @film                 ; Clear command
     67B0 2238 
0035 67B2 A329                   data  cmdb.cmd,>00,80
     67B4 0000 
     67B6 0050 
0036                       ;------------------------------------------------------
0037                       ; Put cursor at beginning of line
0038                       ;------------------------------------------------------
0039 67B8 C120  34         mov   @cmdb.yxprompt,tmp0
     67BA A310 
0040 67BC 0584  14         inc   tmp0
0041 67BE C804  38         mov   tmp0,@cmdb.cursor     ; Position cursor
     67C0 A30A 
0042                       ;------------------------------------------------------
0043                       ; Exit
0044                       ;------------------------------------------------------
0045               cmdb.cmd.clear.exit:
0046 67C2 C1B9  30         mov   *stack+,tmp2          ; Pop tmp2
0047 67C4 C179  30         mov   *stack+,tmp1          ; Pop tmp1
0048 67C6 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0049 67C8 C2F9  30         mov   *stack+,r11           ; Pop r11
0050 67CA 045B  20         b     *r11                  ; Return to caller
0051               
0052               
0053               
0054               
...
0075 67CC 0649  14         dect  stack
0076 67CE C64B  30         mov   r11,*stack            ; Save return address
0077                       ;-------------------------------------------------------
0078                       ; Get length of null terminated string
0079                       ;-------------------------------------------------------
0080 67D0 06A0  32         bl    @string.getlenc      ; Get length of C-style string
     67D2 2AD0 
0081 67D4 A329                   data cmdb.cmd,0      ; \ i  p0    = Pointer to C-style string
     67D6 0000 
0082                                                  ; | i  p1    = Termination character
0083                                                  ; / o  waux1 = Length of string
0084 67D8 C820  54         mov   @waux1,@outparm1     ; Save length of string
     67DA 833C 
     67DC 2F30 
0085                       ;------------------------------------------------------
0086                       ; Exit
...
0089 67DE C2F9  30         mov   *stack+,r11           ; Pop r11
0090 67E0 045B  20         b     *r11                  ; Return to caller
0091               
0092               
0093               
0094               
...
0115 67E2 0649  14         dect  stack
0116 67E4 C64B  30         mov   r11,*stack            ; Save return address
0117 67E6 0649  14         dect  stack
0118 67E8 C644  30         mov   tmp0,*stack           ; Push tmp0
0119               
0120 67EA 06A0  32         bl    @cmdb.cmd.getlength   ; Get length of command
     67EC 67CC 
0121                                                   ; \ i  @cmdb.cmd
0122                                                   ; / o  @outparm1
0123                       ;------------------------------------------------------
0124                       ; Assert
0125                       ;------------------------------------------------------
0126 67EE C120  34         mov   @outparm1,tmp0        ; Check length
     67F0 2F30 
0127 67F2 1300  14         jeq   cmdb.cmd.history.add.exit
...
0139 67F4 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0140 67F6 C2F9  30         mov   *stack+,r11           ; Pop r11
0141 67F8 045B  20         b     *r11                  ; Return to caller
**** **** ****     > stevie_b3.asm.91491
0100                       ;-----------------------------------------------------------------------
...
0007               ********|*****|*********************|**************************
0008               edb.line.pack:
0009 67FA 0649  14         dect  stack
0010 67FC C64B  30         mov   r11,*stack            ; Save return address
0011                       ;------------------------------------------------------
0012                       ; Call function in bank 1
0013                       ;------------------------------------------------------
0014 67FE 06A0  32         bl    @rom.farjump          ; \ Trampoline jump to bank
     6800 3008 
0015 6802 6002                   data bank1.rom        ; | i  p0 = bank address
0016 6804 7FD2                   data vec.10           ; | i  p1 = Vector with target address
0017 6806 6006                   data bankid           ; / i  p2 = Source ROM bank for return
0018                       ;------------------------------------------------------
0019                       ; Exit
0020                       ;------------------------------------------------------
0021 6808 C2F9  30         mov   *stack+,r11           ; Pop r11
0022 680A 045B  20         b     *r11                  ; Return to caller
0023               
0024               
0025               ***************************************************************
...
0030 680C 0649  14         dect  stack
0031 680E C64B  30         mov   r11,*stack            ; Save return address
0032                       ;------------------------------------------------------
0033                       ; Call function in bank 1
0034                       ;------------------------------------------------------
0035 6810 06A0  32         bl    @rom.farjump          ; \ Trampoline jump to bank
     6812 3008 
0036 6814 6002                   data bank1.rom        ; | i  p0 = bank address
0037 6816 7FDC                   data vec.15           ; | i  p1 = Vector with target address
0038 6818 6006                   data bankid           ; / i  p2 = Source ROM bank for return
0039                       ;------------------------------------------------------
0040                       ; Exit
0041                       ;------------------------------------------------------
0042 681A C2F9  30         mov   *stack+,r11           ; Pop r11
0043 681C 045B  20         b     *r11                  ; Return to caller
0044               
0045               
0046               
0047               ***************************************************************
...
0052 681E 0649  14         dect  stack
0053 6820 C64B  30         mov   r11,*stack            ; Save return address
0054                       ;------------------------------------------------------
0055                       ; Call function in bank 1
0056                       ;------------------------------------------------------
0057 6822 06A0  32         bl    @rom.farjump          ; \ Trampoline jump to bank
     6824 3008 
0058 6826 6002                   data bank1.rom        ; | i  p0 = bank address
0059 6828 7FF6                   data vec.28           ; | i  p1 = Vector with target address
0060 682A 6006                   data bankid           ; / i  p2 = Source ROM bank for return
0061                       ;------------------------------------------------------
0062                       ; Exit
0063                       ;------------------------------------------------------
0064 682C C2F9  30         mov   *stack+,r11           ; Pop r11
0065 682E 045B  20         b     *r11                  ; Return to caller
0066               
0067               ***************************************************************
...
0072 6830 0649  14         dect  stack
0073 6832 C64B  30         mov   r11,*stack            ; Save return address
0074                       ;------------------------------------------------------
0075                       ; Call function in bank 1
0076                       ;------------------------------------------------------
0077 6834 06A0  32         bl    @rom.farjump          ; \ Trampoline jump to bank
     6836 3008 
0078 6838 6002                   data bank1.rom        ; | i  p0 = bank address
0079 683A 7FF8                   data vec.29           ; | i  p1 = Vector with target address
0080 683C 6006                   data bankid           ; / i  p2 = Source ROM bank for return
0081                       ;------------------------------------------------------
0082                       ; Exit
0083                       ;------------------------------------------------------
0084 683E C2F9  30         mov   *stack+,r11           ; Pop r11
0085 6840 045B  20         b     *r11                  ; Return to caller
**** **** ****     > stevie_b3.asm.91491
0104                       ;-----------------------------------------------------------------------
...
0030 7FEE 6750     vec.24  data  cmdb.refresh          ;
0031 7FF0 679A     vec.25  data  cmdb.cmd.clear        ;
0032 7FF2 67CC     vec.26  data  cmdb.cmd.getlength    ;
0033 7FF4 67E2     vec.27  data  cmdb.cmd.history.add  ;
...
0123      33B0     spvmod  equ   stevie.tx8030         ; Video mode.   See VIDTAB for details.
