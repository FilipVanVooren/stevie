...
0008               ***************************************************************
0009               * File: stevie_b3.asm               ; Version 210717-2080625
0010               *
0011               * Bank 3 "John"
0012               *
0013               ***************************************************************
...
0029                       ;-------------------------------------------------------
0030                       ; Vector table bank 3: >6000 - >603f
0031                       ;-------------------------------------------------------
0032                       copy  "rom.vectors.bank3.asm"
...
0006               *--------------------------------------------------------------
0007 6000 608C     vec.1   data  dialog.about          ; Dialog "About"
0008 6002 65C0     vec.2   data  dialog.load           ; Dialog "Load DV80 file"
0009 6004 6618     vec.3   data  dialog.save           ; Dialog "Save DV80 file"
0010 6006 6680     vec.4   data  dialog.unsaved        ; Dialog "Unsaved changes"
0011 6008 657E     vec.5   data  dialog.file           ; Dialog "File"
0012 600A 604A     vec.6   data  dialog.menu           ; Dialog "Stevie Menu"
...
0260 21D3 ....             text  'Build-ID  210717-2080625'
0261                       even
0262               
**** **** ****     > runlib.asm
...
0018 3443 ....             text  'Build: 210717-2080625 / 2018-2021 Filip Van Vooren / retroclouds on Atariage'
0019                       even
0020               
0021               
0022               txt.delim
...
0075 6046 06A0  32         bl    @cpu.crash            ; Should never get here
     6048 2026 
0076                       ;-----------------------------------------------------------------------
0077                       ; Include files - Shared code
0078                       ;-----------------------------------------------------------------------
0079               
0080                       ;-----------------------------------------------------------------------
0081                       ; Include files - Dialogs
0082                       ;-----------------------------------------------------------------------
0083                       copy  "dialog.menu.asm"     ; Dialog "Stevie Menu"
**** **** ****     > dialog.menu.asm
...
**** **** ****     > stevie_b3.asm.2080625
0084                       copy  "dialog.about.asm"    ; Dialog "About"
**** **** ****     > dialog.about.asm
...
0075 60F2 3436                   data   txt.stevie     ; Show Stevie version
0076               
...
0329               
**** **** ****     > stevie_b3.asm.2080625
0085                       copy  "dialog.file.asm"     ; Dialog "File"
**** **** ****     > dialog.file.asm
...
**** **** ****     > stevie_b3.asm.2080625
0086                       copy  "dialog.load.asm"     ; Dialog "Load DV80 file"
**** **** ****     > dialog.load.asm
...
**** **** ****     > stevie_b3.asm.2080625
0087                       copy  "dialog.save.asm"     ; Dialog "Save DV80 file"
**** **** ****     > dialog.save.asm
...
**** **** ****     > stevie_b3.asm.2080625
0088                       copy  "dialog.unsaved.asm"  ; Dialog "Unsaved changes"
**** **** ****     > dialog.unsaved.asm
...
0007               ********|*****|*********************|**************************
0008               edb.line.pack:
0009 66BE 0649  14         dect  stack
0010 66C0 C64B  30         mov   r11,*stack            ; Save return address
0011                       ;------------------------------------------------------
0012                       ; Call function in bank 1
0013                       ;------------------------------------------------------
0014 66C2 06A0  32         bl    @rom.farjump          ; \ Trampoline jump to bank
     66C4 3008 
0015 66C6 6002                   data bank1.rom        ; | i  p0 = bank address
0016 66C8 6012                   data vec.10           ; | i  p1 = Vector with target address
0017 66CA 6006                   data bankid           ; / i  p2 = Source ROM bank for return
0018                       ;------------------------------------------------------
0019                       ; Exit
0020                       ;------------------------------------------------------
0021 66CC C2F9  30         mov   *stack+,r11           ; Pop r11
0022 66CE 045B  20         b     *r11                  ; Return to caller
0023               
0024               
0025               ***************************************************************
...
0030 66D0 0649  14         dect  stack
0031 66D2 C64B  30         mov   r11,*stack            ; Save return address
0032                       ;------------------------------------------------------
0033                       ; Call function in bank 1
0034                       ;------------------------------------------------------
0035 66D4 06A0  32         bl    @rom.farjump          ; \ Trampoline jump to bank
     66D6 3008 
0036 66D8 6002                   data bank1.rom        ; | i  p0 = bank address
0037 66DA 601C                   data vec.15           ; | i  p1 = Vector with target address
0038 66DC 6006                   data bankid           ; / i  p2 = Source ROM bank for return
0039                       ;------------------------------------------------------
0040                       ; Exit
0041                       ;------------------------------------------------------
0042 66DE C2F9  30         mov   *stack+,r11           ; Pop r11
0043 66E0 045B  20         b     *r11                  ; Return to caller
0044               
0045               
0046               ***************************************************************
...
0049               ********|*****|*********************|**************************
0050               cmdb.cmd.clear:
0051 66E2 0649  14         dect  stack
0052 66E4 C64B  30         mov   r11,*stack            ; Save return address
0053                       ;------------------------------------------------------
0054                       ; Call function in bank 1
0055                       ;------------------------------------------------------
0056 66E6 06A0  32         bl    @rom.farjump          ; \ Trampoline jump to bank
     66E8 3008 
0057 66EA 6002                   data bank1.rom        ; | i  p0 = bank address
0058 66EC 6024                   data vec.19           ; | i  p1 = Vector with target address
0059 66EE 6006                   data bankid           ; / i  p2 = Source ROM bank for return
0060                       ;------------------------------------------------------
0061                       ; Exit
0062                       ;------------------------------------------------------
0063 66F0 C2F9  30         mov   *stack+,r11           ; Pop r11
0064 66F2 045B  20         b     *r11                  ; Return to caller
0065               
0066               
0067               ***************************************************************
...
0072 66F4 0649  14         dect  stack
0073 66F6 C64B  30         mov   r11,*stack            ; Save return address
0074                       ;------------------------------------------------------
0075                       ; Call function in bank 1
0076                       ;------------------------------------------------------
0077 66F8 06A0  32         bl    @rom.farjump          ; \ Trampoline jump to bank
     66FA 3008 
0078 66FC 6002                   data bank1.rom        ; | i  p0 = bank address
0079 66FE 6036                   data vec.28           ; | i  p1 = Vector with target address
0080 6700 6006                   data bankid           ; / i  p2 = Source ROM bank for return
0081                       ;------------------------------------------------------
0082                       ; Exit
0083                       ;------------------------------------------------------
0084 6702 C2F9  30         mov   *stack+,r11           ; Pop r11
0085 6704 045B  20         b     *r11                  ; Return to caller
0086               
0087               ***************************************************************
...
0092 6706 0649  14         dect  stack
0093 6708 C64B  30         mov   r11,*stack            ; Save return address
0094                       ;------------------------------------------------------
0095                       ; Call function in bank 1
0096                       ;------------------------------------------------------
0097 670A 06A0  32         bl    @rom.farjump          ; \ Trampoline jump to bank
     670C 3008 
0098 670E 6002                   data bank1.rom        ; | i  p0 = bank address
0099 6710 6038                   data vec.29           ; | i  p1 = Vector with target address
0100 6712 6006                   data bankid           ; / i  p2 = Source ROM bank for return
0101                       ;------------------------------------------------------
0102                       ; Exit
0103                       ;------------------------------------------------------
0104 6714 C2F9  30         mov   *stack+,r11           ; Pop r11
0105 6716 045B  20         b     *r11                  ; Return to caller
**** **** ****     > stevie_b3.asm.2080625
0093                       ;-----------------------------------------------------------------------
0094                       ; Bank specific vector table
0095                       ;-----------------------------------------------------------------------
0099 6718 6718                   data $                ; Bank 3 ROM size OK.
0101               
0102               *--------------------------------------------------------------
...
0107      339A     spvmod  equ   stevie.tx8030         ; Video mode.   See VIDTAB for details.
