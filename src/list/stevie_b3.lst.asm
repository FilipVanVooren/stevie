...
0008               ***************************************************************
0009               * File: stevie_b3.asm               ; Version 210605-1391643
0010               *
0011               * Bank 3 "John"
0012               *
0013               ***************************************************************
...
0260 21D3 ....             text  'Build-ID  210605-1391643'
0261                       even
0262               
**** **** ****     > runlib.asm
...
0018 3443 ....             text  'Build: 210605-1391643 / 2018-2021 Filip Van Vooren / retroclouds on Atariage'
0019                       even
0020               
0021               
0022               txt.delim
...
0027                       ; Setup dialog
0028                       ;-------------------------------------------------------
0029 6526 0204  20         li    tmp0,id.dialog.file
     6528 0068 
0030 652A C804  38         mov   tmp0,@cmdb.dialog     ; Set dialog ID
     652C A31A 
0031               
0032 652E 0204  20         li    tmp0,txt.head.file
     6530 3828 
0033 6532 C804  38         mov   tmp0,@cmdb.panhead    ; Header for dialog
     6534 A31C 
0034               
0035 6536 0204  20         li    tmp0,txt.info.file
     6538 3832 
0036 653A C804  38         mov   tmp0,@cmdb.paninfo    ; Info message instead of input prompt
     653C A31E 
0037               
0038 653E 0204  20         li    tmp0,txt.hint.file
     6540 3858 
0039 6542 C804  38         mov   tmp0,@cmdb.panhint    ; Hint in bottom line
     6544 A320 
0040               
0041 6546 0204  20         li    tmp0,txt.keys.file
     6548 3882 
0042 654A C804  38         mov   tmp0,@cmdb.pankeys    ; Keylist in status line
     654C A322 
0043               
0044 654E 06A0  32         bl    @pane.cursor.hide     ; Hide cursor
     6550 6698 
0045                       ;-------------------------------------------------------
0046                       ; Exit
0047                       ;-------------------------------------------------------
0048               dialog.file.exit:
0049 6552 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0050 6554 C2F9  30         mov   *stack+,r11           ; Pop R11
0051 6556 045B  20         b     *r11                  ; Return to caller
**** **** ****     > stevie_b3.asm.1391643
...
0020               ********|*****|*********************|**************************
0021               dialog.load:
0022 6558 0649  14         dect  stack
0023 655A C64B  30         mov   r11,*stack            ; Save return address
0024 655C 0649  14         dect  stack
0025 655E C644  30         mov   tmp0,*stack           ; Push tmp0
0026                       ;-------------------------------------------------------
0027                       ; Show dialog "Unsaved changes" if editor buffer dirty
0028                       ;-------------------------------------------------------
0029 6560 C120  34         mov   @edb.dirty,tmp0       ; Editor dirty?
     6562 A206 
0030 6564 1303  14         jeq   dialog.load.setup     ; No, skip "Unsaved changes"
0031               
0032 6566 06A0  32         bl    @dialog.unsaved       ; Show dialog
     6568 6616 
0033 656A 101D  14         jmp   dialog.load.exit      ; Exit early
0034                       ;-------------------------------------------------------
0035                       ; Setup dialog
0036                       ;-------------------------------------------------------
0037               dialog.load.setup:
0038 656C 0204  20         li    tmp0,id.dialog.load
     656E 000A 
0039 6570 C804  38         mov   tmp0,@cmdb.dialog     ; Set dialog ID
     6572 A31A 
0040               
0041 6574 0204  20         li    tmp0,txt.head.load
     6576 35FA 
0042 6578 C804  38         mov   tmp0,@cmdb.panhead    ; Header for dialog
     657A A31C 
0043               
0044 657C 04E0  34         clr   @cmdb.paninfo         ; No info message, do input prompt
     657E A31E 
0045               
0046 6580 0204  20         li    tmp0,txt.hint.load
     6582 360E 
0047 6584 C804  38         mov   tmp0,@cmdb.panhint    ; Hint line in dialog
     6586 A320 
0048               
0049 6588 0760  38         abs   @fh.offsetopcode      ; FastMode is off ?
     658A A44E 
0050 658C 1303  14         jeq   !
0051                       ;-------------------------------------------------------
0052                       ; Show that FastMode is on
0053                       ;-------------------------------------------------------
0054 658E 0204  20         li    tmp0,txt.keys.load2   ; Highlight FastMode
     6590 368A 
0055 6592 1002  14         jmp   dialog.load.keylist
0056                       ;-------------------------------------------------------
0057                       ; Show that FastMode is off
0058                       ;-------------------------------------------------------
0059 6594 0204  20 !       li    tmp0,txt.keys.load
     6596 364C 
0060                       ;-------------------------------------------------------
0061                       ; Show dialog
0062                       ;-------------------------------------------------------
0063               dialog.load.keylist:
0064 6598 C804  38         mov   tmp0,@cmdb.pankeys    ; Keylist in status line
     659A A322 
0065                       ;-------------------------------------------------------
0066                       ; Set cursor shape
0067                       ;-------------------------------------------------------
0068 659C 06A0  32         bl    @pane.cursor.blink    ; Show cursor
     659E 6686 
0069 65A0 C820  54         mov   @tv.curshape,@ramsat+2
     65A2 A014 
     65A4 2F5C 
...
0075 65A6 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0076 65A8 C2F9  30         mov   *stack+,r11           ; Pop R11
0077 65AA 045B  20         b     *r11                  ; Return to caller
**** **** ****     > stevie_b3.asm.1391643
...
0020               ********|*****|*********************|**************************
0021               dialog.save:
0022 65AC 0649  14         dect  stack
0023 65AE C64B  30         mov   r11,*stack            ; Save return address
0024 65B0 0649  14         dect  stack
0025 65B2 C644  30         mov   tmp0,*stack           ; Push tmp0
0026                       ;-------------------------------------------------------
0027                       ; Crunch current row if dirty
0028                       ;-------------------------------------------------------
0029 65B4 8820  54         c     @fb.row.dirty,@w$ffff
     65B6 A10A 
     65B8 2022 
0030 65BA 1604  14         jne   !                     ; Skip crunching if clean
0031 65BC 06A0  32         bl    @edb.line.pack        ; Copy line to editor buffer
     65BE 6650 
0032 65C0 04E0  34         clr   @fb.row.dirty         ; Current row no longer dirty
     65C2 A10A 
0033                       ;-------------------------------------------------------
0034                       ; Setup dialog
0035                       ;-------------------------------------------------------
0036 65C4 8820  54 !       c     @edb.block.m2,@w$ffff ; Marker M2 unset?
     65C6 A20E 
     65C8 2022 
0037 65CA 1309  14         jeq   dialog.save.default   ; Yes, so show default dialog
0038                       ;-------------------------------------------------------
0039                       ; Setup dialog title
0040                       ;-------------------------------------------------------
0041 65CC 06A0  32         bl    @cmdb.cmd.clear       ; Clear current CMDB command
     65CE 6674 
0042               
0043 65D0 0204  20         li    tmp0,id.dialog.saveblock
     65D2 000C 
0044 65D4 C804  38         mov   tmp0,@cmdb.dialog     ; Set dialog ID
     65D6 A31A 
0045 65D8 0204  20         li    tmp0,txt.head.save2   ; Title "Save code block to DV80 file"
     65DA 36DD 
0046               
0047 65DC 1006  14         jmp   dialog.save.header
0048                       ;-------------------------------------------------------
0049                       ; Default dialog
0050                       ;-------------------------------------------------------
0051               dialog.save.default:
0052 65DE 0204  20         li    tmp0,id.dialog.save
     65E0 000B 
0053 65E2 C804  38         mov   tmp0,@cmdb.dialog     ; Set dialog ID
     65E4 A31A 
0054 65E6 0204  20         li    tmp0,txt.head.save    ; Title "Save DV80 file"
     65E8 36C9 
0055                       ;-------------------------------------------------------
0056                       ; Setup header
0057                       ;-------------------------------------------------------
0058               dialog.save.header:
0059 65EA C804  38         mov   tmp0,@cmdb.panhead    ; Header for dialog
     65EC A31C 
0060               
0061 65EE 04E0  34         clr   @cmdb.paninfo         ; No info message, do input prompt
     65F0 A31E 
0062               
0063 65F2 0204  20         li    tmp0,txt.hint.save
     65F4 3701 
0064 65F6 C804  38         mov   tmp0,@cmdb.panhint    ; Hint line in dialog
     65F8 A320 
0065               
0066 65FA 0204  20         li    tmp0,txt.keys.save
     65FC 3704 
0067 65FE C804  38         mov   tmp0,@cmdb.pankeys    ; Keylist in status line
     6600 A322 
0068               
0069 6602 04E0  34         clr   @fh.offsetopcode      ; Data buffer in VDP RAM
     6604 A44E 
0070                       ;-------------------------------------------------------
0071                       ; Set cursor shape
0072                       ;-------------------------------------------------------
0073 6606 06A0  32         bl    @pane.cursor.blink    ; Show cursor
     6608 6686 
0074 660A C820  54         mov   @tv.curshape,@ramsat+2
     660C A014 
     660E 2F5C 
...
0080 6610 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0081 6612 C2F9  30         mov   *stack+,r11           ; Pop R11
0082 6614 045B  20         b     *r11                  ; Return to caller
**** **** ****     > stevie_b3.asm.1391643
...
0027                       ; Setup dialog
0028                       ;-------------------------------------------------------
0029 661E 0204  20         li    tmp0,id.dialog.unsaved
     6620 0065 
0030 6622 C804  38         mov   tmp0,@cmdb.dialog     ; Set dialog ID
     6624 A31A 
0031               
0032 6626 0204  20         li    tmp0,txt.head.unsaved
     6628 3734 
0033 662A C804  38         mov   tmp0,@cmdb.panhead    ; Header for dialog
     662C A31C 
0034               
0035 662E 0204  20         li    tmp0,txt.info.unsaved
     6630 3749 
0036 6632 C804  38         mov   tmp0,@cmdb.paninfo    ; Info message instead of input prompt
     6634 A31E 
0037               
0038 6636 0204  20         li    tmp0,txt.hint.unsaved
     6638 376C 
0039 663A C804  38         mov   tmp0,@cmdb.panhint    ; Hint in bottom line
     663C A320 
0040               
0041 663E 0204  20         li    tmp0,txt.keys.unsaved
     6640 3798 
0042 6642 C804  38         mov   tmp0,@cmdb.pankeys    ; Keylist in status line
     6644 A322 
0043               
0044 6646 06A0  32         bl    @pane.cursor.hide     ; Hide cursor
     6648 6698 
0045                       ;-------------------------------------------------------
0046                       ; Exit
0047                       ;-------------------------------------------------------
0048               dialog.unsaved.exit:
0049 664A C139  30         mov   *stack+,tmp0          ; Pop tmp0
0050 664C C2F9  30         mov   *stack+,r11           ; Pop R11
0051 664E 045B  20         b     *r11                  ; Return to caller
**** **** ****     > stevie_b3.asm.1391643
0084                       ;-----------------------------------------------------------------------
...
0007               ********|*****|*********************|**************************
0008               edb.line.pack:
0009 6650 0649  14         dect  stack
0010 6652 C64B  30         mov   r11,*stack            ; Save return address
0011                       ;------------------------------------------------------
0012                       ; Call function in bank 1
0013                       ;------------------------------------------------------
0014 6654 06A0  32         bl    @rom.farjump          ; \ Trampoline jump to bank
     6656 3008 
0015 6658 6002                   data bank1.rom        ; | i  p0 = bank address
0016 665A 7FAE                   data vec.10           ; | i  p1 = Vector with target address
0017 665C 6006                   data bankid           ; / i  p2 = Source ROM bank for return
0018                       ;------------------------------------------------------
0019                       ; Exit
0020                       ;------------------------------------------------------
0021 665E C2F9  30         mov   *stack+,r11           ; Pop r11
0022 6660 045B  20         b     *r11                  ; Return to caller
0023               
0024               
0025               ***************************************************************
...
0030 6662 0649  14         dect  stack
0031 6664 C64B  30         mov   r11,*stack            ; Save return address
0032                       ;------------------------------------------------------
0033                       ; Call function in bank 1
0034                       ;------------------------------------------------------
0035 6666 06A0  32         bl    @rom.farjump          ; \ Trampoline jump to bank
     6668 3008 
0036 666A 6002                   data bank1.rom        ; | i  p0 = bank address
0037 666C 7FB8                   data vec.15           ; | i  p1 = Vector with target address
0038 666E 6006                   data bankid           ; / i  p2 = Source ROM bank for return
0039                       ;------------------------------------------------------
0040                       ; Exit
0041                       ;------------------------------------------------------
0042 6670 C2F9  30         mov   *stack+,r11           ; Pop r11
0043 6672 045B  20         b     *r11                  ; Return to caller
0044               
0045               
0046               ***************************************************************
...
0049               ********|*****|*********************|**************************
0050               cmdb.cmd.clear:
0051 6674 0649  14         dect  stack
0052 6676 C64B  30         mov   r11,*stack            ; Save return address
0053                       ;------------------------------------------------------
0054                       ; Call function in bank 1
0055                       ;------------------------------------------------------
0056 6678 06A0  32         bl    @rom.farjump          ; \ Trampoline jump to bank
     667A 3008 
0057 667C 6002                   data bank1.rom        ; | i  p0 = bank address
0058 667E 7FC0                   data vec.19           ; | i  p1 = Vector with target address
0059 6680 6006                   data bankid           ; / i  p2 = Source ROM bank for return
0060                       ;------------------------------------------------------
0061                       ; Exit
0062                       ;------------------------------------------------------
0063 6682 C2F9  30         mov   *stack+,r11           ; Pop r11
0064 6684 045B  20         b     *r11                  ; Return to caller
0065               
0066               
0067               ***************************************************************
...
0072 6686 0649  14         dect  stack
0073 6688 C64B  30         mov   r11,*stack            ; Save return address
0074                       ;------------------------------------------------------
0075                       ; Call function in bank 1
0076                       ;------------------------------------------------------
0077 668A 06A0  32         bl    @rom.farjump          ; \ Trampoline jump to bank
     668C 3008 
0078 668E 6002                   data bank1.rom        ; | i  p0 = bank address
0079 6690 7FD2                   data vec.28           ; | i  p1 = Vector with target address
0080 6692 6006                   data bankid           ; / i  p2 = Source ROM bank for return
0081                       ;------------------------------------------------------
0082                       ; Exit
0083                       ;------------------------------------------------------
0084 6694 C2F9  30         mov   *stack+,r11           ; Pop r11
0085 6696 045B  20         b     *r11                  ; Return to caller
0086               
0087               ***************************************************************
...
0092 6698 0649  14         dect  stack
0093 669A C64B  30         mov   r11,*stack            ; Save return address
0094                       ;------------------------------------------------------
0095                       ; Call function in bank 1
0096                       ;------------------------------------------------------
0097 669C 06A0  32         bl    @rom.farjump          ; \ Trampoline jump to bank
     669E 3008 
0098 66A0 6002                   data bank1.rom        ; | i  p0 = bank address
0099 66A2 7FD4                   data vec.29           ; | i  p1 = Vector with target address
0100 66A4 6006                   data bankid           ; / i  p2 = Source ROM bank for return
0101                       ;------------------------------------------------------
0102                       ; Exit
0103                       ;------------------------------------------------------
0104 66A6 C2F9  30         mov   *stack+,r11           ; Pop r11
0105 66A8 045B  20         b     *r11                  ; Return to caller
**** **** ****     > stevie_b3.asm.1391643
0088                       ;-----------------------------------------------------------------------
0089                       ; Bank specific vector table
0090                       ;-----------------------------------------------------------------------
0094 66AA 66AA                   data $                ; Bank 1 ROM size OK.
0096                       ;-------------------------------------------------------
...
0010 7F9E 6558     vec.2   data  dialog.load           ; Dialog "Load DV80 file"
0011 7FA0 65AC     vec.3   data  dialog.save           ; Dialog "Save DV80 file"
0012 7FA2 6616     vec.4   data  dialog.unsaved        ; Dialog "Unsaved changes"
0013 7FA4 651E     vec.5   data  dialog.file           ; Dialog "File"
