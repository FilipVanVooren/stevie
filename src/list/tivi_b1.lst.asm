...
0008               ***************************************************************
0009               * File: tivi_b1.asm                 ; Version 200322-22048
0010               
0011               
0012               ***************************************************************
...
0007               *              (c)2018-2020 // Filip van Vooren
0008               ***************************************************************
0009               * File: equates.asm                 ; Version 200322-22048
0010               *--------------------------------------------------------------
...
0031 6015 ....             text  'TIVI 200322-22048'
0032                       even
0033               
0041               
0042               *--------------------------------------------------------------
...
0249 21C1 ....             text  'Build-ID  200322-22048'
0250                       even
0251               
**** **** ****     > runlib.asm
...
0102 60F4 06A0  32         bl    @mkhook
     60F6 2CD0 
0103 60F8 6FE4                   data editor           ; Setup user hook
0104               
...
0325 6626 0460  28         b     @ed_wait              ; Back to editor main
     6628 7008 
0326               
0327               
0328               
...
0235 6FE0 0460  28         b     @poprt                ; Return to caller
     6FE2 2212 
**** **** ****     > tivi_b1.asm.22048
0048                       copy  "task.asm"            ; *****        - Code shared between tasks
**** **** ****     > task.asm
0001               * FILE......: task.asm
...
0010               ****************************************************************
0011 6FE4 20A0  38 editor  coc   @wbit11,config        ; ANYKEY pressed ?
     6FE6 2014 
0012 6FE8 160B  14         jne   ed_clear_kbbuffer     ; No, clear buffer and exit
0013               *---------------------------------------------------------------
0014               * Identical key pressed ?
0015               *---------------------------------------------------------------
0016 6FEA 40A0  34         szc   @wbit11,config        ; Reset ANYKEY
     6FEC 2014 
0017 6FEE 8820  54         c     @waux1,@waux2         ; Still pressing previous key?
     6FF0 833C 
     6FF2 833E 
0018 6FF4 1309  14         jeq   ed_wait
0019               *--------------------------------------------------------------
0020               * New key pressed
0021               *--------------------------------------------------------------
0022               ed_new_key
0023 6FF6 C820  54         mov   @waux1,@waux2         ; Save as previous key
     6FF8 833C 
     6FFA 833E 
0024 6FFC 0460  28         b     @edkey                ; Process key
     6FFE 6180 
0025               *--------------------------------------------------------------
0026               * Clear keyboard buffer if no key pressed
0027               *--------------------------------------------------------------
0028               ed_clear_kbbuffer
0029 7000 04E0  34         clr   @waux1
     7002 833C 
0030 7004 04E0  34         clr   @waux2
     7006 833E 
0031               *--------------------------------------------------------------
0032               * Delay to avoid key bouncing
0033               *--------------------------------------------------------------
0034               ed_wait
0035 7008 0204  20         li    tmp0,1800             ; Key delay to avoid bouncing keys
     700A 0708 
0036                       ;------------------------------------------------------
0037                       ; Delay loop
0038                       ;------------------------------------------------------
0039               ed_wait_loop
0040 700C 0604  14         dec   tmp0
0041 700E 16FE  14         jne   ed_wait_loop
0042               *--------------------------------------------------------------
0043               * Exit
0044               *--------------------------------------------------------------
0045 7010 0460  28 ed_exit b     @hookok               ; Return
     7012 2C2A 
0046               
0047               
0048               
...
0056               
0057               *--------------------------------------------------------------
0058               * Copy ramsat to VDP SAT and show bottom line - Tasks 1,2
0059               *--------------------------------------------------------------
0060               task.sub_copy_ramsat:
0061 7014 0649  14         dect  stack
0062 7016 C644  30         mov   tmp0,*stack            ; Push tmp0
0063               
0064 7018 06A0  32         bl    @cpym2v                ; Copy sprite SAT to VDP
     701A 2418 
0065 701C 2000                   data sprsat,ramsat,4   ; \ i  tmp0 = VDP destination
     701E 8380 
     7020 0004 
0066                                                    ; | i  tmp1 = ROM/RAM source
0067                                                    ; / i  tmp2 = Number of bytes to write
0068               
0069 7022 C820  54         mov   @wyx,@fb.yxsave
     7024 832A 
     7026 A294 
0070                       ;------------------------------------------------------
0071                       ; Show buffer number
0072                       ;------------------------------------------------------
0073               task.botline.bufnum:
0074 7028 06A0  32         bl    @putat
     702A 2410 
0075 702C 1D00                   byte  29,0
0076 702E 72BA                   data  txt.bufnum
0077                       ;------------------------------------------------------
0078                       ; Show current file
0079                       ;------------------------------------------------------
0080               task.botline.show_file:
0081 7030 06A0  32         bl    @at
     7032 264E 
0082 7034 1D03                   byte  29,3             ; Position cursor
0083 7036 C160  34         mov   @edb.filename.ptr,tmp1 ; Get string to display
     7038 A30E 
0084 703A 06A0  32         bl    @xutst0                ; Display string
     703C 2400 
0085               
0086 703E 06A0  32         bl    @at
     7040 264E 
0087 7042 1D23                   byte  29,35            ; Position cursor
0088               
0089 7044 C160  34         mov   @edb.filetype.ptr,tmp1 ; Get string to display
     7046 A310 
0090 7048 06A0  32         bl    @xutst0                ; Display Filetype string
     704A 2400 
0091                       ;------------------------------------------------------
0092                       ; Show text editing mode
0093                       ;------------------------------------------------------
0094               task.botline.show_mode:
0095 704C C120  34         mov   @edb.insmode,tmp0
     704E A30A 
0096 7050 1605  14         jne   task.botline.show_mode.insert
0097                       ;------------------------------------------------------
0098                       ; Overwrite mode
0099                       ;------------------------------------------------------
0100               task.botline.show_mode.overwrite:
0101 7052 06A0  32         bl    @putat
     7054 2410 
0102 7056 1D32                   byte  29,50
0103 7058 726C                   data  txt.ovrwrite
0104 705A 1004  14         jmp   task.botline.show_changed
0105                       ;------------------------------------------------------
0106                       ; Insert  mode
0107                       ;------------------------------------------------------
0108               task.botline.show_mode.insert:
0109 705C 06A0  32         bl    @putat
     705E 2410 
0110 7060 1D32                   byte  29,50
0111 7062 7270                   data  txt.insert
0112                       ;------------------------------------------------------
0113                       ; Show if text was changed in editor buffer
0114                       ;------------------------------------------------------
0115               task.botline.show_changed:
0116 7064 C120  34         mov   @edb.dirty,tmp0
     7066 A306 
0117 7068 1305  14         jeq   task.botline.show_changed.clear
0118                       ;------------------------------------------------------
0119                       ; Show "*"
0120                       ;------------------------------------------------------
0121 706A 06A0  32         bl    @putat
     706C 2410 
0122 706E 1D36                   byte 29,54
0123 7070 7274                   data txt.star
0124 7072 1001  14         jmp   task.botline.show_linecol
0125                       ;------------------------------------------------------
0126                       ; Show "line,column"
0127                       ;------------------------------------------------------
0128               task.botline.show_changed.clear:
0129 7074 1000  14         nop
0130               task.botline.show_linecol:
0131 7076 C820  54         mov   @fb.row,@parm1
     7078 A286 
     707A 8350 
0132 707C 06A0  32         bl    @fb.row2line
     707E 680E 
0133 7080 05A0  34         inc   @outparm1
     7082 8360 
0134                       ;------------------------------------------------------
0135                       ; Show line
0136                       ;------------------------------------------------------
0137 7084 06A0  32         bl    @putnum
     7086 29CE 
0138 7088 1D40                   byte  29,64            ; YX
0139 708A 8360                   data  outparm1,rambuf
     708C 8390 
0140 708E 3020                   byte  48               ; ASCII offset
0141                             byte  32               ; Padding character
0142                       ;------------------------------------------------------
0143                       ; Show comma
0144                       ;------------------------------------------------------
0145 7090 06A0  32         bl    @putat
     7092 2410 
0146 7094 1D45                   byte  29,69
0147 7096 725E                   data  txt.delim
0148                       ;------------------------------------------------------
0149                       ; Show column
0150                       ;------------------------------------------------------
0151 7098 06A0  32         bl    @film
     709A 2216 
0152 709C 8396                   data rambuf+6,32,12    ; Clear work buffer with space character
     709E 0020 
     70A0 000C 
0153               
0154 70A2 C820  54         mov   @fb.column,@waux1
     70A4 A28C 
     70A6 833C 
0155 70A8 05A0  34         inc   @waux1                 ; Offset 1
     70AA 833C 
0156               
0157 70AC 06A0  32         bl    @mknum                 ; Convert unsigned number to string
     70AE 2950 
0158 70B0 833C                   data  waux1,rambuf
     70B2 8390 
0159 70B4 3020                   byte  48               ; ASCII offset
0160                             byte  32               ; Fill character
0161               
0162 70B6 06A0  32         bl    @trimnum               ; Trim number to the left
     70B8 29A8 
0163 70BA 8390                   data  rambuf,rambuf+6,32
     70BC 8396 
     70BE 0020 
0164               
0165 70C0 0204  20         li    tmp0,>0200
     70C2 0200 
0166 70C4 D804  38         movb  tmp0,@rambuf+6         ; "Fix" number length to clear junk chars
     70C6 8396 
0167               
0168 70C8 06A0  32         bl    @putat
     70CA 2410 
0169 70CC 1D46                   byte 29,70
0170 70CE 8396                   data rambuf+6          ; Show column
0171                       ;------------------------------------------------------
0172                       ; Show lines in buffer unless on last line in file
0173                       ;------------------------------------------------------
0174 70D0 C820  54         mov   @fb.row,@parm1
     70D2 A286 
     70D4 8350 
0175 70D6 06A0  32         bl    @fb.row2line
     70D8 680E 
0176 70DA 8820  54         c     @edb.lines,@outparm1
     70DC A304 
     70DE 8360 
0177 70E0 1605  14         jne   task.botline.show_lines_in_buffer
0178               
0179 70E2 06A0  32         bl    @putat
     70E4 2410 
0180 70E6 1D4B                   byte 29,75
0181 70E8 7266                   data txt.bottom
0182               
0183 70EA 100B  14         jmp   task.botline.exit
0184                       ;------------------------------------------------------
0185                       ; Show lines in buffer
0186                       ;------------------------------------------------------
0187               task.botline.show_lines_in_buffer:
0188 70EC C820  54         mov   @edb.lines,@waux1
     70EE A304 
     70F0 833C 
0189 70F2 05A0  34         inc   @waux1                 ; Offset 1
     70F4 833C 
0190 70F6 06A0  32         bl    @putnum
     70F8 29CE 
0191 70FA 1D4B                   byte 29,75             ; YX
0192 70FC 833C                   data waux1,rambuf
     70FE 8390 
0193 7100 3020                   byte 48
0194                             byte 32
0195                       ;------------------------------------------------------
0196                       ; Exit
0197                       ;------------------------------------------------------
0198               task.botline.exit:
0199 7102 C820  54         mov   @fb.yxsave,@wyx
     7104 A294 
     7106 832A 
0200 7108 C139  30         mov   *stack+,tmp0           ; Pop tmp0
0201 710A 0460  28         b     @slotok                ; Exit running task
     710C 2CA6 
**** **** ****     > tivi_b1.asm.22048
0049                       copy  "task.vdp.panes.asm"  ; *****        - VDP draw editor panes
**** **** ****     > task.vdp.panes.asm
0001               * FILE......: tasks.vdp.panes.asm
...
0134 71D4 0460  28         b     @slotok
     71D6 2CA6 
**** **** ****     > tivi_b1.asm.22048
0050                       copy  "task.vdp.sat.asm"    ; *****        - VDP copy SAT
**** **** ****     > task.vdp.sat.asm
0001               * FILE......: tasks.vdp.panes.asm
0002               * Purpose...: TiVi Editor - VDP draw editor panes
0003               
...
0015 71E4 0460  28         b     @task.sub_copy_ramsat  ; Update VDP SAT and exit task
     71E6 7014 
**** **** ****     > tivi_b1.asm.22048
0051                       copy  "task.vdp.cursor.asm" ; *****        - VDP set cursor shape
**** **** ****     > task.vdp.cursor.asm
0001               ***************************************************************
0002               * Task - Update cursor shape (blink)
0003               ***************************************************************
0004               task.vdp.cursor:
0005 71E8 0560  34         inv   @fb.curtoggle          ; Flip cursor shape flag
     71EA A292 
0006 71EC 1304  14         jeq   task.vdp.cursor.visible
0007 71EE 04E0  34         clr   @ramsat+2              ; Hide cursor
     71F0 8382 
0008 71F2 0460  28         b     @task.sub_copy_ramsat  ; Update VDP SAT and exit task
     71F4 7014 
0009               task.vdp.cursor.visible:
0010 71F6 C120  34         mov   @edb.insmode,tmp0      ; Get Editor buffer insert mode
     71F8 A30A 
0011 71FA 1302  14         jeq   task.vdp.cursor.visible.overwrite_mode
0012                       ;------------------------------------------------------
0013                       ; Cursor in insert mode
0014                       ;------------------------------------------------------
0015               task.vdp.cursor.visible.insert_mode:
0016 71FC 04C4  14         clr   tmp0
0017 71FE 1002  14         jmp   task.vdp.cursor.visible.cursorshape
0018                       ;------------------------------------------------------
0019                       ; Cursor in overwrite mode
0020                       ;------------------------------------------------------
0021               task.vdp.cursor.visible.overwrite_mode:
0022 7200 0204  20         li    tmp0,>0200
     7202 0200 
0023                       ;------------------------------------------------------
0024                       ; Set cursor shape
0025                       ;------------------------------------------------------
0026               task.vdp.cursor.visible.cursorshape:
0027 7204 D804  38         movb  tmp0,@tv.curshape      ; Save cursor shape
     7206 A214 
0028 7208 C820  54         mov   @tv.curshape,@ramsat+2 ; Get cursor shape and color
     720A A214 
     720C 8382 
0029 720E 0460  28         b     @task.sub_copy_ramsat  ; Update VDP SAT and exit task
     7210 7014 
**** **** ****     > tivi_b1.asm.22048
0052                       copy  "data.asm"            ; data         - Data segment
**** **** ****     > data.asm
0001               * FILE......: data.asm
...
0124                            byte    4
0125 7302 ....                  text    'TiVi beta 200322-22048'
0126 7318 0500                  byte    5
0127 731A 731A     end          data    $
0128               
0129               
0130               fdname0
0131 731C 0D44             byte  13
0132 731D ....             text  'DSK1.INVADERS'
0133                       even
0134               
0135               fdname1
0136 732A 0F44             byte  15
0137 732B ....             text  'DSK1.SPEECHDOCS'
0138                       even
0139               
0140               fdname2
0141 733A 0C44             byte  12
0142 733B ....             text  'DSK1.XBEADOC'
0143                       even
0144               
0145               fdname3
0146 7348 0C44             byte  12
0147 7349 ....             text  'DSK3.XBEADOC'
0148                       even
0149               
0150               fdname4
0151 7356 0C44             byte  12
0152 7357 ....             text  'DSK3.C99MAN1'
0153                       even
0154               
0155               fdname5
0156 7364 0C44             byte  12
0157 7365 ....             text  'DSK3.C99MAN2'
0158                       even
0159               
0160               fdname6
0161 7372 0C44             byte  12
0162 7373 ....             text  'DSK3.C99MAN3'
0163                       even
0164               
0165               fdname7
0166 7380 0D44             byte  13
0167 7381 ....             text  'DSK3.C99SPECS'
0168                       even
0169               
0170               fdname8
0171 738E 0D44             byte  13
0172 738F ....             text  'DSK3.RANDOM#C'
0173                       even
0174               
0175               fdname9
0176 739C 0D44             byte  13
0177 739D ....             text  'DSK1.INVADERS'
0178                       even
0179               
**** **** ****     > tivi_b1.asm.22048
0053               
0057 73AA 73AA                   data $                ; Bank 1 ROM size OK.
0059               
0060               *--------------------------------------------------------------
