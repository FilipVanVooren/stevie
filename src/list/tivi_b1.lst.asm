...
0008               ***************************************************************
0009               * File: tivi_b1.asm                 ; Version 200414-19082
0010               
0011               
0012               ***************************************************************
...
0007               *              (c)2018-2020 // Filip van Vooren
0008               ***************************************************************
0009               * File: equates.asm                 ; Version 200414-19082
0010               *--------------------------------------------------------------
...
0031 6015 ....             text  'TIVI 200414-19082'
0032                       even
0033               
0041               
0042               *--------------------------------------------------------------
...
0249 21C1 ....             text  'Build-ID  200414-19082'
0250                       even
0251               
**** **** ****     > runlib.asm
...
0064 6094 06A0  32         bl    @cpym2m
     6096 2460 
0065 6098 7256                   data romsat,ramsat,4  ; Load sprite SAT
     609A 8380 
     609C 0004 
0066               
...
0103 60F4 06A0  32         bl    @mkhook
     60F6 2D20 
0104 60F8 6FA2                   data hook.keyscan     ; Setup user hook
0105               
...
0106                                                   ; Nothing to page-in if NULL pointer
0107                                                   ; (=empty line)
0108                       ;------------------------------------------------------
0109                       ; 1. Determine if requested SAMS page is already active
0110                       ;------------------------------------------------------
0111 6792 0245  22         andi  tmp1,>f000            ; Reduce address to 4K chunks
     6794 F000 
0112 6796 04C6  14         clr   tmp2                  ; Offset in SAMS shadow registers table
0113 6798 0207  20         li    tmp3,sams.copy.layout.data + 6
     679A 2604 
0114                                                   ; Entry >b000  in SAMS memory range table
0115                       ;------------------------------------------------------
0116                       ; Loop over memory ranges
0117                       ;------------------------------------------------------
0118               mem.edb.sams.pagein.compare.loop:
0119 679C 8177  30         c     *tmp3+,tmp1           ; Does memory range match?
0120 679E 1308  14         jeq   !                     ; Yes, now check SAMS page
0121               
0122 67A0 05C6  14         inct  tmp2                  ; Next range
0123 67A2 0286  22         ci    tmp2,12               ; All ranges checked?
     67A4 000C 
0124 67A6 16FA  14         jne   mem.edb.sams.pagein.compare.loop
0125                                                   ; Not yet, check next range
0126                       ;------------------------------------------------------
0127                       ; Invalid memory range. Should never get here
0128                       ;------------------------------------------------------
0129 67A8 C80B  38         mov   r11,@>ffce            ; \ Save caller address
     67AA FFCE 
0130 67AC 06A0  32         bl    @cpu.crash            ; / Crash and halt system
     67AE 2030 
0131                       ;------------------------------------------------------
0132                       ; 2. Determine if requested SAMS page is already active
0133                       ;------------------------------------------------------
0134 67B0 0226  22 !       ai    tmp2,tv.sams.2000     ; Add offset for SAMS shadow register
     67B2 A000 
0135 67B4 8116  26         c     *tmp2,tmp0            ; Requested SAMS page already active?
0136 67B6 1302  14         jeq   mem.edb.sams.pagein.exit
0137                                                   ; Yes, so exit
0138                       ;------------------------------------------------------
0139                       ; Activate requested SAMS page
0140                       ;-----------------------------------------------------
0141 67B8 06A0  32         bl    @xsams.page.set       ; Switch SAMS memory page
     67BA 24FC 
0142                                                   ; \ i  tmp0 = SAMS page
0143                                                   ; / i  tmp1 = Memory address
0144                       ;------------------------------------------------------
0145                       ; Exit
0146                       ;------------------------------------------------------
0147               mem.edb.sams.pagein.exit:
0148 67BC C1F9  30         mov   *stack+,tmp3          ; Pop tmp1
0149 67BE C1B9  30         mov   *stack+,tmp2          ; Pop tmp1
0150 67C0 C179  30         mov   *stack+,tmp1          ; Pop tmp1
0151 67C2 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0152 67C4 C2F9  30         mov   *stack+,r11           ; Pop r11
0153 67C6 045B  20         b     *r11                  ; Return to caller
0154               
**** **** ****     > tivi_b1.asm.19082
...
0047 67FC 06A0  32         bl    @film
     67FE 2216 
0048 6800 A600             data  fb.top,>00,fb.size    ; Clear it all the way
     6802 0000 
     6804 0960 
0049                       ;------------------------------------------------------
0050                       ; Exit
...
0164 684A 06A0  32         bl    @sams.copy.layout     ; Copy SAMS memory layout
     684C 25C6 
0165 684E A000                   data tv.sams.2000     ; \ i  p0 = Pointer to 8 words RAM buffer
0166                                                   ; /
0167                       ;------------------------------------------------------
...
0031               *
0032               * First line in editor buffer starts at index slot 2 (c002), this
0033               * allows the index to contain "null" pointers, aka empty lines
0034               * without reference to editor buffer.
0035               ***************************************************************
0036               
0037               
0038               ***************************************************************
0039               * idx.init
0040               * Initialize index
0041               ***************************************************************
0042               * bl @idx.init
0043               *--------------------------------------------------------------
0044               * INPUT
...
0052               ***************************************************************
0053               idx.init:
0054 68FA 0649  14         dect  stack
0055 68FC C64B  30         mov   r11,*stack            ; Save return address
0056                       ;------------------------------------------------------
0057                       ; Initialize
0058                       ;------------------------------------------------------
0059 68FE 0204  20         li    tmp0,idx.top
     6900 C000 
0060 6902 C804  38         mov   tmp0,@edb.index.ptr   ; Set pointer to index in editor structure
     6904 A202 
0061                       ;------------------------------------------------------
0062                       ; Clear index page
0063                       ;------------------------------------------------------
0064 6906 06A0  32         bl    @film
     6908 2216 
0065 690A C000             data  idx.top,>00,idx.size  ; Clear index
     690C 0000 
     690E 1000 
0066                       ;------------------------------------------------------
0067                       ; Exit
0068                       ;------------------------------------------------------
0069               idx.init.exit:
0070 6910 0460  28         b     @poprt                ; Return to caller
     6912 2212 
0071               
0072               
0073               
0074               ***************************************************************
0075               * idx.entry.update
0076               * Update index entry - Each entry corresponds to a line
0077               ***************************************************************
0078               * bl @idx.entry.update
0079               *--------------------------------------------------------------
0080               * INPUT
0081               * @parm1    = Line number in editor buffer
0082               * @parm2    = Pointer to line in editor buffer
0083               * @parm3    = SAMS page
0084               *--------------------------------------------------------------
0085               * OUTPUT
0086               * @outparm1 = Pointer to updated index entry
0087               *--------------------------------------------------------------
0088               * Register usage
0089               * tmp0,tmp1,tmp2
0090               *--------------------------------------------------------------
0091               idx.entry.update:
0092 6914 C120  34         mov   @parm1,tmp0           ; Get line number
     6916 8350 
0093 6918 C160  34         mov   @parm2,tmp1           ; Get pointer
     691A 8352 
0094                       ;------------------------------------------------------
0095                       ; Calculate LSB value index slot (pointer offset)
0096                       ;------------------------------------------------------
0097 691C 0245  22         andi  tmp1,>0fff            ; Remove high-nibble from pointer
     691E 0FFF 
0098 6920 0945  56         srl   tmp1,4                ; Get offset (divide by 16)
0099                       ;------------------------------------------------------
0100                       ; Calculate MSB value index slot (SAMS page)
0101                       ;------------------------------------------------------
0102 6922 06E0  34         swpb  @parm3
     6924 8354 
0103 6926 D160  34         movb  @parm3,tmp1           ; Put SAMS page in MSB
     6928 8354 
0104                       ;------------------------------------------------------
0105                       ; Update index slot
0106                       ;------------------------------------------------------
0107               idx.entry.update.save:
0108 692A 0A14  56         sla   tmp0,1                ; line number * 2
0109 692C C905  38         mov   tmp1,@idx.top(tmp0)   ; Update index slot
     692E C000 
0110                       ;------------------------------------------------------
0111                       ; Exit
0112                       ;------------------------------------------------------
0113               idx.entry.update.exit:
0114 6930 C804  38         mov   tmp0,@outparm1        ; Pointer to updated index entry
     6932 8360 
0115 6934 045B  20         b     *r11                  ; Return
0116               
0117               
0118               ***************************************************************
...
0135 6936 C120  34         mov   @parm1,tmp0           ; Line number in editor buffer
     6938 8350 
0136                       ;------------------------------------------------------
0137                       ; Calculate address of index entry and save pointer
0138                       ;------------------------------------------------------
0139 693A 0A14  56         sla   tmp0,1                ; line number * 2
0140 693C C824  54         mov   @idx.top(tmp0),@outparm1
     693E C000 
     6940 8360 
...
0144                       ;------------------------------------------------------
0145 6942 C1A0  34         mov   @parm2,tmp2           ; Get last line to check
     6944 8352 
0146 6946 61A0  34         s     @parm1,tmp2           ; Calculate loop
     6948 8350 
0147 694A 1601  14         jne   idx.entry.delete.reorg
0148                       ;------------------------------------------------------
0149                       ; Special treatment if last line
0150                       ;------------------------------------------------------
0151 694C 1006  14         jmp   idx.entry.delete.lastline
0152                       ;------------------------------------------------------
0153                       ; Reorganize index entries
0154                       ;------------------------------------------------------
0155               idx.entry.delete.reorg:
0156 694E C924  54         mov   @idx.top+2(tmp0),@idx.top+0(tmp0)
     6950 C002 
     6952 C000 
0157 6954 05C4  14         inct  tmp0                  ; Next index entry
0158 6956 0606  14         dec   tmp2                  ; tmp2--
0159 6958 16FA  14         jne   idx.entry.delete.reorg
0160                                                   ; Loop unless completed
0161                       ;------------------------------------------------------
0162                       ; Last line
0163                       ;------------------------------------------------------
0164               idx.entry.delete.lastline:
0165 695A 04E4  34         clr   @idx.top(tmp0)
     695C C000 
0166                       ;------------------------------------------------------
0167                       ; Exit
0168                       ;------------------------------------------------------
0169               idx.entry.delete.exit:
0170 695E 045B  20         b     *r11                  ; Return
0171               
0172               
0173               ***************************************************************
0174               * idx.entry.insert
0175               * Insert index entry
0176               ***************************************************************
0177               * bl @idx.entry.insert
0178               *--------------------------------------------------------------
0179               * INPUT
0180               * @parm1    = Line number in editor buffer to insert
0181               * @parm2    = Line number of last line to check for reorg
0182               *--------------------------------------------------------------
0183               * OUTPUT
0184               * NONE
0185               *--------------------------------------------------------------
...
0187               * tmp0,tmp2
0188               *--------------------------------------------------------------
0189               idx.entry.insert:
0190 6960 C120  34         mov   @parm2,tmp0           ; Last line number in editor buffer
     6962 8352 
0191                       ;------------------------------------------------------
0192                       ; Calculate address of index entry and save pointer
0193                       ;------------------------------------------------------
0194 6964 0A14  56         sla   tmp0,1                ; line number * 2
0195                       ;------------------------------------------------------
0196                       ; Prepare for index reorg
0197                       ;------------------------------------------------------
0198 6966 C1A0  34         mov   @parm2,tmp2           ; Get last line to check
     6968 8352 
0199 696A 61A0  34         s     @parm1,tmp2           ; Calculate loop
     696C 8350 
0200 696E 1606  14         jne   idx.entry.insert.reorg
0201                       ;------------------------------------------------------
0202                       ; Special treatment if last line
0203                       ;------------------------------------------------------
0204 6970 C924  54         mov   @idx.top+0(tmp0),@idx.top+2(tmp0)
     6972 C000 
     6974 C002 
0205                                                   ; Move index entry
0206 6976 04E4  34         clr   @idx.top+0(tmp0)      ; Clear new index entry
     6978 C000 
0207               
0208 697A 1009  14         jmp   idx.entry.insert.exit
0209                       ;------------------------------------------------------
0210                       ; Reorganize index entries
0211                       ;------------------------------------------------------
0212               idx.entry.insert.reorg:
0213 697C 05C6  14         inct  tmp2                  ; Adjust one time
0214               
0215 697E C924  54 !       mov   @idx.top+0(tmp0),@idx.top+2(tmp0)
     6980 C000 
     6982 C002 
0216                                                   ; Move index entry
0217               
0218 6984 0644  14         dect  tmp0                  ; Previous index entry
0219 6986 0606  14         dec   tmp2                  ; tmp2--
0220 6988 16FA  14         jne   -!                    ; Loop unless completed
0221               
0222 698A 04E4  34         clr   @idx.top+4(tmp0)      ; Clear new index entry
     698C C004 
0223                       ;------------------------------------------------------
0224                       ; Exit
0225                       ;------------------------------------------------------
0226               idx.entry.insert.exit:
0227 698E 045B  20         b     *r11                  ; Return
0228               
0229               
0230               
0231               ***************************************************************
...
0242               * @outparm2 = SAMS page
0243               *--------------------------------------------------------------
0244               * Register usage
0245               * tmp0,tmp1,tmp2
0246               *--------------------------------------------------------------
0247               idx.pointer.get:
0248 6990 0649  14         dect  stack
0249 6992 C64B  30         mov   r11,*stack            ; Save return address
0250                       ;------------------------------------------------------
0251                       ; Get slot entry
0252                       ;------------------------------------------------------
0253 6994 C120  34         mov   @parm1,tmp0           ; Line number in editor buffer
     6996 8350 
0254 6998 0A14  56         sla   tmp0,1                ; line number * 2
0255 699A C164  34         mov   @idx.top(tmp0),tmp1   ; Get slot entry
     699C C000 
0256                       ;------------------------------------------------------
0257                       ; Calculate MSB (SAMS page)
0258                       ;------------------------------------------------------
0259 699E C185  18         mov   tmp1,tmp2             ; \
0260 69A0 0986  56         srl   tmp2,8                ; / Right align SAMS page
0261                       ;------------------------------------------------------
0262                       ; Calculate LSB (pointer address)
0263                       ;------------------------------------------------------
0264 69A2 0245  22         andi  tmp1,>00ff            ; Get rid of MSB (SAMS page)
     69A4 00FF 
0265 69A6 0A45  56         sla   tmp1,4                ; Multiply with 16
0266 69A8 0225  22         ai    tmp1,edb.top          ; Add editor buffer base address
     69AA D000 
0267                       ;------------------------------------------------------
0268                       ; Return parameters
0269                       ;------------------------------------------------------
0270               idx.pointer.get.parm:
0271 69AC C805  38         mov   tmp1,@outparm1        ; Index slot -> Pointer
     69AE 8360 
0272 69B0 C806  38         mov   tmp2,@outparm2        ; Index slot -> SAMS page
     69B2 8362 
0273                       ;------------------------------------------------------
0274                       ; Exit
0275                       ;------------------------------------------------------
0276               idx.pointer.get.exit:
0277 69B4 0460  28         b     @poprt                ; Return to caller
     69B6 2212 
**** **** ****     > tivi_b1.asm.19082
...
0024               ***************************************************************
0025               edb.init:
0026 69B8 0649  14         dect  stack
0027 69BA C64B  30         mov   r11,*stack            ; Save return address
0028                       ;------------------------------------------------------
0029                       ; Initialize
0030                       ;------------------------------------------------------
0031 69BC 0204  20         li    tmp0,edb.top+2        ; Skip offset 0 so that it can't be confused
     69BE D002 
0032                                                   ; with null pointer (has offset 0)
0033               
0034 69C0 C804  38         mov   tmp0,@edb.top.ptr     ; Set pointer to top of editor buffer
     69C2 A200 
0035 69C4 C804  38         mov   tmp0,@edb.next_free.ptr
     69C6 A208 
0036                                                   ; Set pointer to next free line in
0037                                                   ; editor buffer
0038               
0039 69C8 0720  34         seto  @edb.insmode          ; Turn on insert mode for this editor buffer
     69CA A20A 
0040 69CC 04E0  34         clr   @edb.lines            ; Lines=0
     69CE A204 
0041 69D0 04E0  34         clr   @edb.rle              ; RLE compression off
     69D2 A20C 
0042               
0043 69D4 0204  20         li    tmp0,txt.newfile      ; "New file"
     69D6 7332 
0044 69D8 C804  38         mov   tmp0,@edb.filename.ptr
     69DA A20E 
0045               
0046 69DC 0204  20         li    tmp0,txt.filetype.none
     69DE 737E 
0047 69E0 C804  38         mov   tmp0,@edb.filetype.ptr
     69E2 A210 
0048               
0049               edb.init.exit:
0050                       ;------------------------------------------------------
0051                       ; Exit
0052                       ;------------------------------------------------------
0053 69E4 0460  28         b     @poprt                ; Return to caller
     69E6 2212 
0054               
0055               
0056               
...
0078               ********|*****|*********************|**************************
0079               edb.line.pack:
0080 69E8 0649  14         dect  stack
0081 69EA C64B  30         mov   r11,*stack            ; Save return address
0082                       ;------------------------------------------------------
0083                       ; Get values
0084                       ;------------------------------------------------------
0085 69EC C820  54         mov   @fb.column,@rambuf    ; Save @fb.column
     69EE A10C 
     69F0 8390 
0086 69F2 04E0  34         clr   @fb.column
     69F4 A10C 
0087 69F6 06A0  32         bl    @fb.calc_pointer      ; Beginning of row
     69F8 681E 
0088                       ;------------------------------------------------------
0089                       ; Prepare scan
0090                       ;------------------------------------------------------
0091 69FA 04C4  14         clr   tmp0                  ; Counter
0092 69FC C160  34         mov   @fb.current,tmp1      ; Get position
     69FE A102 
0093 6A00 C805  38         mov   tmp1,@rambuf+2        ; Save beginning of row
     6A02 8392 
0094               
0095                       ;------------------------------------------------------
...
0099 6A04 D1B5  28         movb  *tmp1+,tmp2           ; Get char
0100 6A06 0986  56         srl   tmp2,8                ; Right justify
0101 6A08 1302  14         jeq   edb.line.pack.prepare ; Stop scan if >00 found
0102 6A0A 0584  14         inc   tmp0                  ; Increase string length
0103 6A0C 10FB  14         jmp   edb.line.pack.scan    ; Next character
0104               
0105                       ;------------------------------------------------------
0106                       ; Prepare for storing line
0107                       ;------------------------------------------------------
0108               edb.line.pack.prepare:
0109 6A0E C820  54         mov   @fb.topline,@parm1    ; \ parm1 = fb.topline + fb.row
     6A10 A104 
     6A12 8350 
0110 6A14 A820  54         a     @fb.row,@parm1        ; /
     6A16 A106 
     6A18 8350 
0111               
0112 6A1A C804  38         mov   tmp0,@rambuf+4        ; Save length of line
     6A1C 8394 
0113               
0114                       ;------------------------------------------------------
0115                       ; 1. Update index
...
0118 6A1E C120  34         mov   @edb.next_free.ptr,tmp0
     6A20 A208 
0119 6A22 C804  38         mov   tmp0,@parm2           ; Block where line will reside
     6A24 8352 
0120               
0121 6A26 06A0  32         bl    @xsams.page.get       ; Get SAMS page
     6A28 24C4 
0122                                                   ; \ i  tmp0  = Memory address
0123                                                   ; | o  waux1 = SAMS page number
0124                                                   ; / o  waux2 = Address of SAMS register
0125               
0126 6A2A C820  54         mov   @waux1,@parm3
     6A2C 833C 
     6A2E 8354 
0127 6A30 06A0  32         bl    @idx.entry.update     ; Update index
     6A32 6914 
...
0135                       ;------------------------------------------------------
0136 6A34 8820  54         c     @edb.sams.page,@parm3 ; Stay on page?
     6A36 A212 
     6A38 8354 
0137 6A3A 1308  14         jeq   !                     ; Yes, skip setting page
0138               
0139 6A3C C120  34         mov   @parm3,tmp0           ; get SAMS page
     6A3E 8354 
0140 6A40 C160  34         mov   @edb.next_free.ptr,tmp1
     6A42 A208 
0141                                                   ; Pointer to line in editor buffer
0142 6A44 06A0  32         bl    @xsams.page.set       ; Switch to SAMS page
     6A46 24FC 
0143                                                   ; \ i  tmp0 = SAMS page
0144                                                   ; / i  tmp1 = Memory address
0145               
0146 6A48 C804  38         mov   tmp0,@fh.sams.page    ; Save current SAMS page
     6A4A A438 
0147               
0148                       ;------------------------------------------------------
0149                       ; 3. Set line prefix in editor buffer
0150                       ;------------------------------------------------------
0151 6A4C C120  34 !       mov   @rambuf+2,tmp0        ; Source for memory copy
     6A4E 8392 
0152 6A50 C160  34         mov   @edb.next_free.ptr,tmp1
     6A52 A208 
0153                                                   ; Address of line in editor buffer
0154               
0155 6A54 05E0  34         inct  @edb.next_free.ptr    ; Adjust pointer
     6A56 A208 
0156               
0157 6A58 C1A0  34         mov   @rambuf+4,tmp2        ; Get line length
     6A5A 8394 
0158 6A5C 0585  14         inc   tmp1                  ; Skip MSB for now (compressed length)
0159 6A5E 06C6  14         swpb  tmp2
0160 6A60 DD46  32         movb  tmp2,*tmp1+           ; Set line length as line prefix
0161 6A62 06C6  14         swpb  tmp2
0162 6A64 1310  14         jeq   edb.line.pack.exit    ; Nothing to copy if empty line
0163               
0164                       ;------------------------------------------------------
0165                       ; 4. Copy line from framebuffer to editor buffer
0166                       ;------------------------------------------------------
0167               edb.line.pack.copyline:
0168 6A66 0286  22         ci    tmp2,2
     6A68 0002 
0169 6A6A 1603  14         jne   edb.line.pack.copyline.checkbyte
0170 6A6C DD74  42         movb  *tmp0+,*tmp1+         ; \ Copy single word on possible
0171 6A6E DD74  42         movb  *tmp0+,*tmp1+         ; / uneven address
0172 6A70 1007  14         jmp   !
0173               edb.line.pack.copyline.checkbyte:
0174 6A72 0286  22         ci    tmp2,1
     6A74 0001 
0175 6A76 1602  14         jne   edb.line.pack.copyline.block
0176 6A78 D554  38         movb  *tmp0,*tmp1           ; Copy single byte
0177 6A7A 1002  14         jmp   !
0178               edb.line.pack.copyline.block:
0179 6A7C 06A0  32         bl    @xpym2m               ; Copy memory block
     6A7E 2466 
0180                                                   ; \ i  tmp0 = source
0181                                                   ; | i  tmp1 = destination
0182                                                   ; / i  tmp2 = bytes to copy
0183               
0184 6A80 A820  54 !       a     @rambuf+4,@edb.next_free.ptr
     6A82 8394 
     6A84 A208 
...
0191 6A86 C820  54         mov   @rambuf,@fb.column    ; Retrieve @fb.column
     6A88 8390 
     6A8A A10C 
0192 6A8C 0460  28         b     @poprt                ; Return to caller
     6A8E 2212 
0193               
0194               
0195               
...
0219               ********|*****|*********************|**************************
0220               edb.line.unpack:
0221 6A90 0649  14         dect  stack
0222 6A92 C64B  30         mov   r11,*stack            ; Save return address
0223                       ;------------------------------------------------------
0224                       ; Sanity check
0225                       ;------------------------------------------------------
0226 6A94 8820  54         c     @parm1,@edb.lines     ; Beyond editor buffer ?
     6A96 8350 
     6A98 A204 
0227 6A9A 1104  14         jlt   !
0228 6A9C C80B  38         mov   r11,@>ffce            ; \ Save caller address
     6A9E FFCE 
0229 6AA0 06A0  32         bl    @cpu.crash            ; / Crash and halt system
     6AA2 2030 
0230                       ;------------------------------------------------------
0231                       ; Save parameters
0232                       ;------------------------------------------------------
0233 6AA4 C820  54 !       mov   @parm1,@rambuf
     6AA6 8350 
     6AA8 8390 
0234 6AAA C820  54         mov   @parm2,@rambuf+2
     6AAC 8352 
     6AAE 8392 
0235                       ;------------------------------------------------------
0236                       ; Calculate offset in frame buffer
0237                       ;------------------------------------------------------
0238 6AB0 C120  34         mov   @fb.colsline,tmp0
     6AB2 A10E 
0239 6AB4 3920  72         mpy   @parm2,tmp0           ; Offset is in tmp1!
     6AB6 8352 
0240 6AB8 C1A0  34         mov   @fb.top.ptr,tmp2
     6ABA A100 
0241 6ABC A146  18         a     tmp2,tmp1             ; Add base to offset
0242 6ABE C805  38         mov   tmp1,@rambuf+6        ; Destination row in frame buffer
     6AC0 8396 
0243                       ;------------------------------------------------------
0244                       ; Get pointer to line & page-in editor buffer page
0245                       ;------------------------------------------------------
0246 6AC2 C120  34         mov   @parm1,tmp0
     6AC4 8350 
0247 6AC6 06A0  32         bl    @xmem.edb.sams.pagein ; Activate editor buffer SAMS page for line
     6AC8 6762 
0248                                                   ; \ i  tmp0     = Line number
0249                                                   ; | o  outparm1 = Pointer to line
0250                                                   ; / o  outparm2 = SAMS page
0251               
0252 6ACA C820  54         mov   @outparm2,@edb.sams.page
     6ACC 8362 
     6ACE A212 
0253                                                   ; Save current SAMS page
0254               
0255 6AD0 05E0  34         inct  @outparm1             ; Skip line prefix
     6AD2 8360 
0256 6AD4 C820  54         mov   @outparm1,@rambuf+4   ; Source memory address for block copy
     6AD6 8360 
     6AD8 8394 
0257                       ;------------------------------------------------------
0258                       ; Get length of line to unpack
0259                       ;------------------------------------------------------
0260 6ADA 06A0  32         bl    @edb.line.getlength   ; Get length of line
     6ADC 6B20 
0261                                                   ; \ i  parm1    = Line number
0262                                                   ; | o  outparm1 = Line length (uncompressed)
0263                                                   ; | o  outparm2 = Line length (compressed)
0264                                                   ; / o  outparm3 = SAMS page
0265               
0266 6ADE C820  54         mov   @outparm1,@rambuf+8   ; Save length of RLE (uncompressed) line
     6AE0 8360 
     6AE2 8398 
0267                       ;------------------------------------------------------
0268                       ; Erase chars from last column until column 80
0269                       ;------------------------------------------------------
0270               edb.line.unpack.clear:
0271 6AE4 C120  34         mov   @rambuf+6,tmp0        ; Start of row in frame buffer
     6AE6 8396 
0272 6AE8 A120  34         a     @rambuf+8,tmp0        ; Skip until end of row in frame buffer
     6AEA 8398 
0273               
0274 6AEC 04C5  14         clr   tmp1                  ; Fill with >00
0275 6AEE C1A0  34         mov   @fb.colsline,tmp2
     6AF0 A10E 
0276 6AF2 61A0  34         s     @rambuf+8,tmp2        ; Calculate number of bytes to clear
     6AF4 8398 
0277 6AF6 0586  14         inc   tmp2
0278               
0279 6AF8 06A0  32         bl    @xfilm                ; Fill CPU memory
     6AFA 221C 
0280                                                   ; \ i  tmp0 = Target address
0281                                                   ; | i  tmp1 = Byte to fill
0282                                                   ; / i  tmp2 = Repeat count
0283                       ;------------------------------------------------------
0284                       ; Prepare for unpacking data
0285                       ;------------------------------------------------------
0286               edb.line.unpack.prepare:
0287 6AFC C1A0  34         mov   @rambuf+8,tmp2        ; Line length
     6AFE 8398 
0288 6B00 130D  14         jeq   edb.line.unpack.exit  ; Exit if length = 0
0289 6B02 C120  34         mov   @rambuf+4,tmp0        ; Pointer to line in editor buffer
     6B04 8394 
0290 6B06 C160  34         mov   @rambuf+6,tmp1        ; Pointer to row in frame buffer
     6B08 8396 
0291                       ;------------------------------------------------------
0292                       ; Check before copy
0293                       ;------------------------------------------------------
0294               edb.line.unpack.copy:
0295 6B0A 0286  22         ci    tmp2,80               ; Check line length
     6B0C 0050 
0296 6B0E 1204  14         jle   !
0297 6B10 C80B  38         mov   r11,@>ffce            ; \ Save caller address
     6B12 FFCE 
0298 6B14 06A0  32         bl    @cpu.crash            ; / Crash and halt system
     6B16 2030 
0299                       ;------------------------------------------------------
0300                       ; Copy memory block
0301                       ;------------------------------------------------------
0302 6B18 06A0  32 !       bl    @xpym2m               ; Copy line to frame buffer
     6B1A 2466 
0303                                                   ; \ i  tmp0 = Source address
0304                                                   ; | i  tmp1 = Target address
0305                                                   ; / i  tmp2 = Bytes to copy
0306                       ;------------------------------------------------------
0307                       ; Exit
0308                       ;------------------------------------------------------
0309               edb.line.unpack.exit:
0310 6B1C 0460  28         b     @poprt                ; Return to caller
     6B1E 2212 
0311               
0312               
0313               
0314               
0315               ***************************************************************
0316               * edb.line.getlength
0317               * Get length of specified line
0318               ***************************************************************
0319               *  bl   @edb.line.getlength
0320               *--------------------------------------------------------------
0321               * INPUT
0322               * @parm1 = Line number
0323               *--------------------------------------------------------------
0324               * OUTPUT
0325               * @outparm1 = Length of line (uncompressed)
0326               * @outparm2 = Length of line (compressed)
0327               * @outparm3 = SAMS page
0328               *--------------------------------------------------------------
0329               * Register usage
0330               * tmp0,tmp1,tmp2
0331               ********|*****|*********************|**************************
0332               edb.line.getlength:
0333 6B20 0649  14         dect  stack
0334 6B22 C64B  30         mov   r11,*stack            ; Save return address
0335                       ;------------------------------------------------------
0336                       ; Initialisation
0337                       ;------------------------------------------------------
0338 6B24 04E0  34         clr   @outparm1             ; Reset uncompressed length
     6B26 8360 
0339 6B28 04E0  34         clr   @outparm2             ; Reset compressed length
     6B2A 8362 
0340 6B2C 04E0  34         clr   @outparm3             ; Reset SAMS bank
     6B2E 8364 
0341                       ;------------------------------------------------------
0342                       ; Get length
0343                       ;------------------------------------------------------
0344 6B30 06A0  32         bl    @idx.pointer.get      ; Get pointer to line
     6B32 6990 
0345                                                   ; \ i  parm1    = Line number
0346                                                   ; | o  outparm1 = Pointer to line
0347                                                   ; / o  outparm2 = SAMS page
0348               
0349 6B34 C120  34         mov   @outparm1,tmp0        ; Is pointer set?
     6B36 8360 
0350 6B38 130D  14         jeq   edb.line.getlength.exit
0351                                                   ; Exit early if NULL pointer
0352 6B3A C820  54         mov   @outparm2,@outparm3   ; Save SAMS page
     6B3C 8362 
     6B3E 8364 
0353                       ;------------------------------------------------------
0354                       ; Process line prefix
0355                       ;------------------------------------------------------
0356 6B40 04C5  14         clr   tmp1
0357 6B42 D174  28         movb  *tmp0+,tmp1           ; Get compressed length
0358 6B44 06C5  14         swpb  tmp1
0359 6B46 C805  38         mov   tmp1,@outparm2        ; Save length
     6B48 8362 
0360               
0361 6B4A 04C5  14         clr   tmp1
0362 6B4C D174  28         movb  *tmp0+,tmp1           ; Get uncompressed length
0363 6B4E 06C5  14         swpb  tmp1
0364 6B50 C805  38         mov   tmp1,@outparm1        ; Save length
     6B52 8360 
0365                       ;------------------------------------------------------
0366                       ; Exit
0367                       ;------------------------------------------------------
0368               edb.line.getlength.exit:
0369 6B54 0460  28         b     @poprt                ; Return to caller
     6B56 2212 
0370               
0371               
0372               
0373               
0374               ***************************************************************
0375               * edb.line.getlength2
0376               * Get length of current row (as seen from editor buffer side)
0377               ***************************************************************
0378               *  bl   @edb.line.getlength2
0379               *--------------------------------------------------------------
0380               * INPUT
0381               * @fb.row = Row in frame buffer
0382               *--------------------------------------------------------------
0383               * OUTPUT
0384               * @fb.row.length = Length of row
0385               *--------------------------------------------------------------
0386               * Register usage
0387               * tmp0
0388               ********|*****|*********************|**************************
0389               edb.line.getlength2:
0390 6B58 0649  14         dect  stack
0391 6B5A C64B  30         mov   r11,*stack            ; Save return address
0392                       ;------------------------------------------------------
0393                       ; Calculate line in editor buffer
0394                       ;------------------------------------------------------
0395 6B5C C120  34         mov   @fb.topline,tmp0      ; Get top line in frame buffer
     6B5E A104 
0396 6B60 A120  34         a     @fb.row,tmp0          ; Get current row in frame buffer
     6B62 A106 
0397                       ;------------------------------------------------------
0398                       ; Get length
0399                       ;------------------------------------------------------
0400 6B64 C804  38         mov   tmp0,@parm1
     6B66 8350 
0401 6B68 06A0  32         bl    @edb.line.getlength
     6B6A 6B20 
0402 6B6C C820  54         mov   @outparm1,@fb.row.length
     6B6E 8360 
     6B70 A108 
0403                                                   ; Save row length
0404                       ;------------------------------------------------------
0405                       ; Exit
0406                       ;------------------------------------------------------
0407               edb.line.getlength2.exit:
0408 6B72 0460  28         b     @poprt                ; Return to caller
     6B74 2212 
0409               
**** **** ****     > tivi_b1.asm.19082
...
0025               ********|*****|*********************|**************************
0026               cmdb.init:
0027 6B76 0649  14         dect  stack
0028 6B78 C64B  30         mov   r11,*stack            ; Save return address
0029                       ;------------------------------------------------------
0030                       ; Initialize
0031                       ;------------------------------------------------------
0032 6B7A 0204  20         li    tmp0,cmdb.top         ; \ Set pointer to command buffer
     6B7C B000 
0033 6B7E C804  38         mov   tmp0,@cmdb.top.ptr    ; /
     6B80 A300 
0034               
0035 6B82 04E0  34         clr   @cmdb.visible         ; Hide command buffer
     6B84 A302 
0036 6B86 0204  20         li    tmp0,10
     6B88 000A 
0037 6B8A C804  38         mov   tmp0,@cmdb.scrrows    ; Set current command buffer size
     6B8C A304 
0038 6B8E C804  38         mov   tmp0,@cmdb.default    ; Set default command buffer size
     6B90 A306 
0039               
0040 6B92 0204  20         li    tmp0,>1b02            ; Y=27, X=2
     6B94 1B02 
0041 6B96 C804  38         mov   tmp0,@cmdb.cursor     ; Screen position of cursor in cmdb pane
     6B98 A308 
0042               
0043 6B9A 04E0  34         clr   @cmdb.lines           ; Number of lines in cmdb buffer
     6B9C A30E 
0044 6B9E 04E0  34         clr   @cmdb.dirty           ; Command buffer is clean
     6BA0 A310 
0045                       ;------------------------------------------------------
0046                       ; Clear command buffer
0047                       ;------------------------------------------------------
0048 6BA2 06A0  32         bl    @film
     6BA4 2216 
0049 6BA6 B000             data  cmdb.top,>00,cmdb.size
     6BA8 0000 
     6BAA 1000 
...
0053                       ; Exit
0054                       ;------------------------------------------------------
0055 6BAC C2F9  30         mov   *stack+,r11           ; Pop r11
0056 6BAE 045B  20         b     *r11                  ; Return to caller
0057               
0058               
0059               
0060               
...
0077               ********|*****|*********************|**************************
0078               cmdb.show:
0079 6BB0 0649  14         dect  stack
0080 6BB2 C64B  30         mov   r11,*stack            ; Save return address
0081 6BB4 0649  14         dect  stack
0082 6BB6 C644  30         mov   tmp0,*stack           ; Push tmp0
0083                       ;------------------------------------------------------
0084                       ; Show command buffer pane
0085                       ;------------------------------------------------------
0086 6BB8 C820  54         mov   @wyx,@cmdb.fb.yxsave
     6BBA 832A 
     6BBC A312 
0087                                                   ; Save YX position in frame buffer
0088               
0089 6BBE C120  34         mov   @fb.scrrows.max,tmp0
     6BC0 A11A 
0090 6BC2 6120  34         s     @cmdb.scrrows,tmp0
     6BC4 A304 
0091 6BC6 C804  38         mov   tmp0,@fb.scrrows      ; Resize framebuffer
     6BC8 A118 
0092               
0093 6BCA 05C4  14         inct  tmp0                  ; Line below cmdb top border line
0094 6BCC 0A84  56         sla   tmp0,8                ; LSB to MSB (Y), X=0
0095 6BCE 0584  14         inc   tmp0                  ; X=1
0096 6BD0 C804  38         mov   tmp0,@cmdb.yxtop      ; Set command buffer cursor
     6BD2 A30C 
0097               
0098 6BD4 0720  34         seto  @cmdb.visible         ; Show pane
     6BD6 A302 
0099               
0100 6BD8 0204  20         li    tmp0,pane.focus.cmdb  ; \ CMDB pane has focus
     6BDA 0001 
0101 6BDC C804  38         mov   tmp0,@tv.pane.focus   ; /
     6BDE A016 
0102               
0103 6BE0 0720  34         seto  @fb.dirty             ; Redraw framebuffer
     6BE2 A116 
0104               
0105               cmdb.show.exit:
0106                       ;------------------------------------------------------
0107                       ; Exit
0108                       ;------------------------------------------------------
0109 6BE4 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0110 6BE6 C2F9  30         mov   *stack+,r11           ; Pop r11
0111 6BE8 045B  20         b     *r11                  ; Return to caller
0112               
0113               
0114               
0115               ***************************************************************
...
0132               ********|*****|*********************|**************************
0133               cmdb.hide:
0134 6BEA 0649  14         dect  stack
0135 6BEC C64B  30         mov   r11,*stack            ; Save return address
0136                       ;------------------------------------------------------
0137                       ; Hide command buffer pane
0138                       ;------------------------------------------------------
0139 6BEE C820  54         mov   @fb.scrrows.max,@fb.scrrows
     6BF0 A11A 
     6BF2 A118 
0140                                                   ; Resize framebuffer
0141               
0142 6BF4 C820  54         mov   @cmdb.fb.yxsave,@wyx  ; Position cursor in framebuffer
     6BF6 A312 
     6BF8 832A 
0143               
0144 6BFA 04E0  34         clr   @cmdb.visible         ; Hide command buffer pane
     6BFC A302 
0145 6BFE 0720  34         seto  @fb.dirty             ; Redraw framebuffer
     6C00 A116 
0146 6C02 04E0  34         clr   @tv.pane.focus        ; Framebuffer has focus!
     6C04 A016 
0147               
0148               cmdb.hide.exit:
0149                       ;------------------------------------------------------
0150                       ; Exit
0151                       ;------------------------------------------------------
0152 6C06 C2F9  30         mov   *stack+,r11           ; Pop r11
0153 6C08 045B  20         b     *r11                  ; Return to caller
0154               
0155               
0156               
0157               ***************************************************************
...
0173               ********|*****|*********************|**************************
0174               cmdb.refresh:
0175 6C0A 0649  14         dect  stack
0176 6C0C C64B  30         mov   r11,*stack            ; Save return address
0177 6C0E 0649  14         dect  stack
0178 6C10 C644  30         mov   tmp0,*stack           ; Push tmp0
0179 6C12 0649  14         dect  stack
0180 6C14 C645  30         mov   tmp1,*stack           ; Push tmp1
0181 6C16 0649  14         dect  stack
0182 6C18 C646  30         mov   tmp2,*stack           ; Push tmp2
0183                       ;------------------------------------------------------
0184                       ; Dump Command buffer content
0185                       ;------------------------------------------------------
0186 6C1A C820  54         mov   @wyx,@cmdb.yxsave     ; Save YX position
     6C1C 832A 
     6C1E A30A 
0187               
0188 6C20 C820  54         mov   @cmdb.yxtop,@wyx
     6C22 A30C 
     6C24 832A 
0189 6C26 06A0  32         bl    @yx2pnt               ; Get VDP PNT address for current YX pos.
     6C28 23DA 
0190               
0191 6C2A C160  34         mov   @cmdb.top.ptr,tmp1    ; Top of command buffer
     6C2C A300 
0192 6C2E 0206  20         li    tmp2,9*80
     6C30 02D0 
0193               
0194 6C32 06A0  32         bl    @xpym2v               ; \ Copy CPU memory to VDP memory
     6C34 241E 
...
0201                       ;------------------------------------------------------
0202 6C36 06A0  32         bl    @putat
     6C38 2410 
0203 6C3A 1B01                   byte 27,1
0204 6C3C 733E                   data txt.cmdb.prompt
0205               
0206 6C3E C820  54         mov   @cmdb.yxsave,@fb.yxsave
     6C40 A30A 
     6C42 A114 
0207 6C44 C820  54         mov   @cmdb.yxsave,@wyx
     6C46 A30A 
     6C48 832A 
...
0211                       ; Exit
0212                       ;------------------------------------------------------
0213 6C4A C1B9  30         mov   *stack+,tmp2          ; Pop tmp2
0214 6C4C C179  30         mov   *stack+,tmp1          ; Pop tmp1
0215 6C4E C139  30         mov   *stack+,tmp0          ; Pop tmp0
0216 6C50 C2F9  30         mov   *stack+,r11           ; Pop r11
0217 6C52 045B  20         b     *r11                  ; Return to caller
0218               
**** **** ****     > tivi_b1.asm.19082
...
0031 6C54 0649  14         dect  stack
0032 6C56 C64B  30         mov   r11,*stack            ; Save return address
0033                       ;------------------------------------------------------
0034                       ; Initialisation
0035                       ;------------------------------------------------------
0036 6C58 04E0  34         clr   @fh.rleonload         ; No RLE compression!
     6C5A A444 
0037 6C5C 04E0  34         clr   @fh.records           ; Reset records counter
     6C5E A42E 
0038 6C60 04E0  34         clr   @fh.counter           ; Clear internal counter
     6C62 A434 
0039 6C64 04E0  34         clr   @fh.kilobytes         ; Clear kilobytes processed
     6C66 A432 
0040 6C68 04C8  14         clr   tmp4                  ; Clear kilobytes processed display counter
0041 6C6A 04E0  34         clr   @fh.pabstat           ; Clear copy of VDP PAB status byte
     6C6C A42A 
0042 6C6E 04E0  34         clr   @fh.ioresult          ; Clear status register contents
     6C70 A42C 
0043               
0044 6C72 C120  34         mov   @edb.top.ptr,tmp0
     6C74 A200 
0045 6C76 06A0  32         bl    @xsams.page.get       ; Get SAMS page
     6C78 24C4 
0046                                                   ; \ i  tmp0  = Memory address
0047                                                   ; | o  waux1 = SAMS page number
0048                                                   ; / o  waux2 = Address of SAMS register
0049               
0050 6C7A C820  54         mov   @waux1,@fh.sams.page  ; Set current SAMS page
     6C7C 833C 
     6C7E A438 
0051 6C80 C820  54         mov   @waux1,@fh.sams.hpage ; Set highest SAMS page in use
     6C82 833C 
     6C84 A43A 
0052                       ;------------------------------------------------------
0053                       ; Save parameters / callback functions
0054                       ;------------------------------------------------------
0055 6C86 C820  54         mov   @parm1,@fh.fname.ptr  ; Pointer to file descriptor
     6C88 8350 
     6C8A A436 
0056 6C8C C820  54         mov   @parm2,@fh.callback1  ; Loading indicator 1
     6C8E 8352 
     6C90 A43C 
0057 6C92 C820  54         mov   @parm3,@fh.callback2  ; Loading indicator 2
     6C94 8354 
     6C96 A43E 
0058 6C98 C820  54         mov   @parm4,@fh.callback3  ; Loading indicator 3
     6C9A 8356 
     6C9C A440 
0059 6C9E C820  54         mov   @parm5,@fh.callback4  ; File I/O error handler
     6CA0 8358 
     6CA2 A442 
0060                       ;------------------------------------------------------
0061                       ; Sanity check
0062                       ;------------------------------------------------------
0063 6CA4 C120  34         mov   @fh.callback1,tmp0
     6CA6 A43C 
0064 6CA8 0284  22         ci    tmp0,>6000            ; Insane address ?
     6CAA 6000 
0065 6CAC 1114  14         jlt   fh.file.read.crash    ; Yes, crash!
0066               
0067 6CAE 0284  22         ci    tmp0,>7fff            ; Insane address ?
     6CB0 7FFF 
0068 6CB2 1511  14         jgt   fh.file.read.crash    ; Yes, crash!
0069               
0070 6CB4 C120  34         mov   @fh.callback2,tmp0
     6CB6 A43E 
0071 6CB8 0284  22         ci    tmp0,>6000            ; Insane address ?
     6CBA 6000 
0072 6CBC 110C  14         jlt   fh.file.read.crash    ; Yes, crash!
0073               
0074 6CBE 0284  22         ci    tmp0,>7fff            ; Insane address ?
     6CC0 7FFF 
0075 6CC2 1509  14         jgt   fh.file.read.crash    ; Yes, crash!
0076               
0077 6CC4 C120  34         mov   @fh.callback3,tmp0
     6CC6 A440 
0078 6CC8 0284  22         ci    tmp0,>6000            ; Insane address ?
     6CCA 6000 
0079 6CCC 1104  14         jlt   fh.file.read.crash    ; Yes, crash!
0080               
0081 6CCE 0284  22         ci    tmp0,>7fff            ; Insane address ?
     6CD0 7FFF 
0082 6CD2 1501  14         jgt   fh.file.read.crash    ; Yes, crash!
0083               
0084 6CD4 1004  14         jmp   fh.file.read.sams.load1
0085                                                   ; All checks passed, continue.
0086                                                   ;--------------------------
0087                                                   ; Check failed, crash CPU!
0088                                                   ;--------------------------
0089               fh.file.read.crash:
0090 6CD6 C80B  38         mov   r11,@>ffce            ; \ Save caller address
     6CD8 FFCE 
0091 6CDA 06A0  32         bl    @cpu.crash            ; / Crash and halt system
     6CDC 2030 
0092                       ;------------------------------------------------------
0093                       ; Show "loading indicator 1"
0094                       ;------------------------------------------------------
0095               fh.file.read.sams.load1:
0096 6CDE C120  34         mov   @fh.callback1,tmp0
     6CE0 A43C 
0097 6CE2 0694  24         bl    *tmp0                 ; Run callback function
0098                       ;------------------------------------------------------
0099                       ; Copy PAB header to VDP
0100                       ;------------------------------------------------------
0101               fh.file.read.sams.pabheader:
0102 6CE4 06A0  32         bl    @cpym2v
     6CE6 2418 
0103 6CE8 0A60                   data fh.vpab,fh.file.pab.header,9
     6CEA 6E44 
     6CEC 0009 
...
0107                       ;------------------------------------------------------
0108 6CEE 0204  20         li    tmp0,fh.vpab + 9      ; VDP destination
     6CF0 0A69 
0109 6CF2 C160  34         mov   @fh.fname.ptr,tmp1    ; Get pointer to file descriptor
     6CF4 A436 
0110 6CF6 D195  26         movb  *tmp1,tmp2            ; Get file descriptor length
0111 6CF8 0986  56         srl   tmp2,8                ; Right justify
0112 6CFA 0586  14         inc   tmp2                  ; Include length byte as well
0113 6CFC 06A0  32         bl    @xpym2v               ; Append file descriptor to VDP PAB
     6CFE 241E 
0114                       ;------------------------------------------------------
0115                       ; Load GPL scratchpad layout
0116                       ;------------------------------------------------------
0117 6D00 06A0  32         bl    @cpu.scrpad.pgout     ; \ Swap scratchpad memory (SPECTRA->GPL)
     6D02 2AA2 
0118 6D04 3F00                   data scrpad.backup2   ; / 8300->2100, 2000->8300
0119                       ;------------------------------------------------------
0120                       ; Open file
0121                       ;------------------------------------------------------
0122 6D06 06A0  32         bl    @file.open
     6D08 2BF0 
0123 6D0A 0A60                   data fh.vpab          ; Pass file descriptor to DSRLNK
0124 6D0C 21A0  38         coc   @wbit2,tmp2           ; Equal bit set?
     6D0E 2026 
0125 6D10 1602  14         jne   fh.file.read.sams.record
0126 6D12 0460  28         b     @fh.file.read.sams.error
     6D14 6E0E 
...
0132 6D16 05A0  34         inc   @fh.records           ; Update counter
     6D18 A42E 
0133 6D1A 04E0  34         clr   @fh.reclen            ; Reset record length
     6D1C A430 
0134               
0135 6D1E 06A0  32         bl    @file.record.read     ; Read file record
     6D20 2C32 
0136 6D22 0A60                   data fh.vpab          ; \ i  p0   = Address of PAB in VDP RAM
...
0142               
0143 6D24 C804  38         mov   tmp0,@fh.pabstat      ; Save VDP PAB status byte
     6D26 A42A 
0144 6D28 C805  38         mov   tmp1,@fh.reclen       ; Save bytes read
     6D2A A430 
0145 6D2C C806  38         mov   tmp2,@fh.ioresult     ; Save status register contents
     6D2E A42C 
0146                       ;------------------------------------------------------
0147                       ; 1a: Calculate kilobytes processed
0148                       ;------------------------------------------------------
0149 6D30 A805  38         a     tmp1,@fh.counter
     6D32 A434 
0150 6D34 A160  34         a     @fh.counter,tmp1
     6D36 A434 
0151 6D38 0285  22         ci    tmp1,1024
     6D3A 0400 
0152 6D3C 1106  14         jlt   !
0153 6D3E 05A0  34         inc   @fh.kilobytes
     6D40 A432 
0154 6D42 0225  22         ai    tmp1,-1024            ; Remove KB portion and keep bytes
     6D44 FC00 
0155 6D46 C805  38         mov   tmp1,@fh.counter
     6D48 A434 
0156                       ;------------------------------------------------------
0157                       ; 1b: Load spectra scratchpad layout
0158                       ;------------------------------------------------------
0159 6D4A 06A0  32 !       bl    @cpu.scrpad.backup    ; Backup GPL layout to @cpu.scrpad.tgt
     6D4C 2A28 
0160 6D4E 06A0  32         bl    @cpu.scrpad.pgin      ; \ Swap scratchpad memory (GPL->SPECTRA)
     6D50 2AC4 
0161 6D52 3F00                   data scrpad.backup2   ; / @scrpad.backup2 to >8300
0162                       ;------------------------------------------------------
0163                       ; 1c: Check if a file error occured
0164                       ;------------------------------------------------------
0165               fh.file.read.sams.check_fioerr:
0166 6D54 C1A0  34         mov   @fh.ioresult,tmp2
     6D56 A42C 
0167 6D58 21A0  38         coc   @wbit2,tmp2           ; IO error occured?
     6D5A 2026 
0168 6D5C 1602  14         jne   fh.file.read.sams.check_setpage
0169                                                   ; No, goto (1d)
0170 6D5E 0460  28         b     @fh.file.read.sams.error
     6D60 6E0E 
...
0176 6D62 C120  34         mov   @edb.next_free.ptr,tmp0
     6D64 A208 
0177                                                   ;--------------------------
0178                                                   ; Sanity check
0179                                                   ;--------------------------
0180 6D66 0284  22         ci    tmp0,edb.top + edb.size
     6D68 E000 
0181                                                   ; Insane address ?
0182 6D6A 15B5  14         jgt   fh.file.read.crash    ; Yes, crash!
0183                                                   ;--------------------------
0184                                                   ; Check overflow
0185                                                   ;--------------------------
0186 6D6C 0244  22         andi  tmp0,>0fff            ; Get rid off highest nibble
     6D6E 0FFF 
0187 6D70 A120  34         a     @fh.reclen,tmp0       ; Add length of line just read
     6D72 A430 
0188 6D74 05C4  14         inct  tmp0                  ; +2 for line prefix
0189 6D76 0284  22         ci    tmp0,>1000 - 16       ; 4K boundary reached?
     6D78 0FF0 
0190 6D7A 110E  14         jlt   fh.file.read.sams.process_line
0191                                                   ; Not yet so skip SAMS page switch
0192                       ;------------------------------------------------------
0193                       ; 1e: Increase SAMS page
0194                       ;------------------------------------------------------
0195 6D7C 05A0  34         inc   @fh.sams.page         ; Next SAMS page
     6D7E A438 
0196 6D80 C820  54         mov   @fh.sams.page,@fh.sams.hpage
     6D82 A438 
     6D84 A43A 
0197                                                   ; Set highest SAMS page
0198 6D86 C820  54         mov   @edb.top.ptr,@edb.next_free.ptr
     6D88 A200 
     6D8A A208 
...
0202                       ;------------------------------------------------------
0203 6D8C C120  34         mov   @fh.sams.page,tmp0
     6D8E A438 
0204 6D90 C160  34         mov   @edb.top.ptr,tmp1
     6D92 A200 
0205 6D94 06A0  32         bl    @xsams.page.set       ; Set SAMS page
     6D96 24FC 
...
0212 6D98 0204  20         li    tmp0,fh.vrecbuf       ; VDP source address
     6D9A 0960 
0213 6D9C C160  34         mov   @edb.next_free.ptr,tmp1
     6D9E A208 
0214                                                   ; RAM target in editor buffer
0215               
0216 6DA0 C805  38         mov   tmp1,@parm2           ; Needed in step 4b (index update)
     6DA2 8352 
0217               
0218 6DA4 C1A0  34         mov   @fh.reclen,tmp2       ; Number of bytes to copy
     6DA6 A430 
0219 6DA8 1318  14         jeq   fh.file.read.sams.prepindex.emptyline
0220                                                   ; Handle empty line
0221                       ;------------------------------------------------------
0222                       ; 2a: Copy line from VDP to CPU editor buffer
0223                       ;------------------------------------------------------
0224                                                   ; Put line length word before string
0225 6DAA DD46  32         movb  tmp2,*tmp1+           ; \ MSB to line prefix
0226 6DAC 06C6  14         swpb  tmp2                  ; |
0227 6DAE DD46  32         movb  tmp2,*tmp1+           ; | LSB to line prefix
0228 6DB0 06C6  14         swpb  tmp2                  ; /
0229               
0230 6DB2 05E0  34         inct  @edb.next_free.ptr    ; Keep pointer synced with tmp1
     6DB4 A208 
0231 6DB6 A806  38         a     tmp2,@edb.next_free.ptr
     6DB8 A208 
0232                                                   ; Add line length
0233               
0234 6DBA 06A0  32         bl    @xpyv2m               ; Copy memory block from VDP to CPU
     6DBC 2444 
...
0241                       ;------------------------------------------------------
0242 6DBE C120  34         mov   @edb.next_free.ptr,tmp0  ; \ Round up to next multiple of 16.
     6DC0 A208 
0243 6DC2 0504  16         neg   tmp0                     ; | tmp0 = tmp0 + (-tmp0 & 15)
0244 6DC4 0244  22         andi  tmp0,15                  ; | Hacker's Delight 2nd Edition
     6DC6 000F 
0245 6DC8 A804  38         a     tmp0,@edb.next_free.ptr  ; / Chapter 2
     6DCA A208 
0246               
0247               
0248                       ;------------------------------------------------------
...
0252 6DCC C820  54         mov   @edb.lines,@parm1     ; parm1 = Line number
     6DCE A204 
     6DD0 8350 
0253                                                   ; parm2 = Must allready be set!
0254 6DD2 C820  54         mov   @fh.sams.page,@parm3  ; parm3 = SAMS page number
     6DD4 A438 
     6DD6 8354 
0255               
0256 6DD8 1009  14         jmp   fh.file.read.sams.updindex
0257                                                   ; Update index
0258                       ;------------------------------------------------------
0259                       ; 3a: Special handling for empty line
0260                       ;------------------------------------------------------
0261               fh.file.read.sams.prepindex.emptyline:
0262 6DDA C820  54         mov   @fh.records,@parm1    ; parm1 = Line number
     6DDC A42E 
     6DDE 8350 
0263 6DE0 0620  34         dec   @parm1                ;         Adjust for base 0 index
     6DE2 8350 
0264 6DE4 04E0  34         clr   @parm2                ; parm2 = Pointer to >0000
     6DE6 8352 
0265 6DE8 0720  34         seto  @parm3                ; parm3 = SAMS not used >FFFF
     6DEA 8354 
0266                       ;------------------------------------------------------
0267                       ; 3b: Do actual index update
0268                       ;------------------------------------------------------
0269               fh.file.read.sams.updindex:
0270 6DEC 06A0  32         bl    @idx.entry.update     ; Update index
     6DEE 6914 
...
0276                                                   ; /               entry
0277               
0278 6DF0 05A0  34         inc   @edb.lines            ; lines=lines+1
     6DF2 A204 
0279                       ;------------------------------------------------------
0280                       ; Step 4: Display results
0281                       ;------------------------------------------------------
0282               fh.file.read.sams.display:
0283 6DF4 C120  34         mov   @fh.callback2,tmp0    ; Get pointer to "Loading indicator 2"
     6DF6 A43E 
0284 6DF8 0694  24         bl    *tmp0                 ; Run callback function
0285                       ;------------------------------------------------------
0286                       ; 4a: Next record
0287                       ;------------------------------------------------------
0288               fh.file.read.sams.next:
0289 6DFA 06A0  32         bl    @cpu.scrpad.pgout     ; \ Swap scratchpad memory (SPECTRA->GPL)
     6DFC 2AA2 
0290 6DFE 3F00                   data scrpad.backup2   ; / 8300->2100, 2000->8300
0291                       ;-------------------------------------------------------
0292                       ; ** TEMPORARY FIX for 4KB INDEX LIMIT **
0293                       ;-------------------------------------------------------
0294 6E00 C120  34         mov   @edb.lines,tmp0
     6E02 A204 
0295 6E04 0284  22         ci    tmp0,2047
     6E06 07FF 
0296 6E08 1311  14         jeq   fh.file.read.sams.eof
0297               
0298 6E0A 0460  28         b     @fh.file.read.sams.record
     6E0C 6D16 
0299                                                   ; Next record
0300                       ;------------------------------------------------------
0301                       ; Error handler
...
0304 6E0E C120  34         mov   @fh.pabstat,tmp0      ; Get VDP PAB status byte
     6E10 A42A 
0305 6E12 0984  56         srl   tmp0,8                ; Right align VDP PAB 1 status byte
0306 6E14 0284  22         ci    tmp0,io.err.eof       ; EOF reached ?
     6E16 0005 
0307 6E18 1309  14         jeq   fh.file.read.sams.eof
0308                                                   ; All good. File closed by DSRLNK
0309                       ;------------------------------------------------------
0310                       ; File error occured
0311                       ;------------------------------------------------------
0312 6E1A 06A0  32         bl    @cpu.scrpad.pgin      ; \ Swap scratchpad memory (GPL->SPECTRA)
     6E1C 2AC4 
0313 6E1E 3F00                   data scrpad.backup2   ; / >2100->8300
0314               
0315 6E20 06A0  32         bl    @mem.setup.sams.layout
     6E22 6732 
0316                                                   ; Restore SAMS default memory layout
0317               
0318 6E24 C120  34         mov   @fh.callback4,tmp0    ; Get pointer to "File I/O error handler"
     6E26 A442 
0319 6E28 0694  24         bl    *tmp0                 ; Run callback function
0320 6E2A 100A  14         jmp   fh.file.read.sams.exit
0321                       ;------------------------------------------------------
0322                       ; End-Of-File reached
0323                       ;------------------------------------------------------
0324               fh.file.read.sams.eof:
0325 6E2C 06A0  32         bl    @cpu.scrpad.pgin      ; \ Swap scratchpad memory (GPL->SPECTRA)
     6E2E 2AC4 
0326 6E30 3F00                   data scrpad.backup2   ; / >2100->8300
0327               
0328 6E32 06A0  32         bl    @mem.setup.sams.layout
     6E34 6732 
0329                                                   ; Restore SAMS default memory layout
0330                       ;------------------------------------------------------
0331                       ; Show "loading indicator 3" (final)
0332                       ;------------------------------------------------------
0333 6E36 0720  34         seto  @edb.dirty            ; Text changed in editor buffer!
     6E38 A206 
0334               
0335 6E3A C120  34         mov   @fh.callback3,tmp0    ; Get pointer to "Loading indicator 3"
     6E3C A440 
0336 6E3E 0694  24         bl    *tmp0                 ; Run callback function
0337               *--------------------------------------------------------------
0338               * Exit
0339               *--------------------------------------------------------------
0340               fh.file.read.sams.exit:
0341 6E40 0460  28         b     @poprt                ; Return to caller
     6E42 2212 
0342               
0343               
0344               
...
0352 6E44 0014             byte  io.op.open            ;  0    - OPEN
0353                       byte  io.ft.sf.ivd          ;  1    - INPUT, VARIABLE, DISPLAY
0354 6E46 0960             data  fh.vrecbuf            ;  2-3  - Record buffer in VDP memory
0355 6E48 5000             byte  80                    ;  4    - Record length (80 chars max)
0356                       byte  00                    ;  5    - Character count
0357 6E4A 0000             data  >0000                 ;  6-7  - Seek record (only for fixed recs)
0358 6E4C 0000             byte  >00                   ;  8    - Screen offset (cassette DSR only)
0359                       ;------------------------------------------------------
...
0012               ********|*****|*********************|**************************
0013               fm.loadfile:
0014 6E4E 0649  14         dect  stack
0015 6E50 C64B  30         mov   r11,*stack            ; Save return address
0016               
0017 6E52 C804  38         mov   tmp0,@parm1           ; Setup file to load
     6E54 8350 
0018 6E56 06A0  32         bl    @edb.init             ; Initialize editor buffer
     6E58 69B8 
0019 6E5A 06A0  32         bl    @idx.init             ; Initialize index
     6E5C 68FA 
0020 6E5E 06A0  32         bl    @fb.init              ; Initialize framebuffer
     6E60 67C8 
0021 6E62 06A0  32         bl    @cmdb.hide            ; Hide command buffer
     6E64 6BEA 
0022 6E66 C820  54         mov   @parm1,@edb.filename.ptr
     6E68 8350 
     6E6A A20E 
0023                                                   ; Set filename
0024                       ;-------------------------------------------------------
...
0026                       ;-------------------------------------------------------
0027 6E6C 06A0  32         bl    @filv
     6E6E 226E 
0028 6E70 2000                   data sprsat,>0000,4   ; Turn off sprites (cursor)
     6E72 0000 
     6E74 0004 
0029               
0030 6E76 C160  34         mov   @fb.scrrows,tmp1
     6E78 A118 
0031 6E7A 3960  72         mpy   @fb.colsline,tmp1     ; columns per line * rows on screen
     6E7C A10E 
0032                                                   ; 16 bit part is in tmp2!
0033               
0034 6E7E 0204  20         li    tmp0,80               ; VDP target address (2nd line on screen!)
     6E80 0050 
0035 6E82 0205  20         li    tmp1,32               ; Character to fill
     6E84 0020 
0036               
0037 6E86 06A0  32         bl    @xfilv                ; Fill VDP memory
     6E88 2274 
...
0043                       ;-------------------------------------------------------
0044 6E8A 0204  20         li    tmp0,fm.loadfile.callback.indicator1
     6E8C 6EBE 
0045 6E8E C804  38         mov   tmp0,@parm2           ; Register callback 1
     6E90 8352 
0046               
0047 6E92 0204  20         li    tmp0,fm.loadfile.callback.indicator2
     6E94 6EF6 
0048 6E96 C804  38         mov   tmp0,@parm3           ; Register callback 2
     6E98 8354 
0049               
0050 6E9A 0204  20         li    tmp0,fm.loadfile.callback.indicator3
     6E9C 6F28 
0051 6E9E C804  38         mov   tmp0,@parm4           ; Register callback 3
     6EA0 8356 
0052               
0053 6EA2 0204  20         li    tmp0,fm.loadfile.callback.fioerr
     6EA4 6F5A 
0054 6EA6 C804  38         mov   tmp0,@parm5           ; Register callback 4
     6EA8 8358 
0055               
0056 6EAA 06A0  32         bl    @fh.file.read.sams    ; Read specified file with SAMS support
     6EAC 6C54 
...
0067               
0068 6EAE 04E0  34         clr   @edb.dirty            ; Editor buffer content replaced, not
     6EB0 A206 
0069                                                   ; longer dirty.
0070               
0071 6EB2 0204  20         li    tmp0,txt.filetype.DV80
     6EB4 7372 
0072 6EB6 C804  38         mov   tmp0,@edb.filetype.ptr
     6EB8 A210 
...
0078 6EBA 0460  28         b     @poprt                ; Return to caller
     6EBC 2212 
0079               
0080               
0081               
...
0088 6EBE 0649  14         dect  stack
0089 6EC0 C64B  30         mov   r11,*stack            ; Save return address
0090                       ;------------------------------------------------------
0091                       ; Show loading indicators and file descriptor
0092                       ;------------------------------------------------------
0093 6EC2 06A0  32         bl    @hchar
     6EC4 2742 
0094 6EC6 1D03                   byte 29,3,32,77
     6EC8 204D 
0095 6ECA FFFF                   data EOL
0096               
0097 6ECC 06A0  32         bl    @putat
     6ECE 2410 
0098 6ED0 1D03                   byte 29,3
0099 6ED2 72EA                   data txt.loading      ; Display "Loading...."
0100               
0101 6ED4 8820  54         c     @fh.rleonload,@w$ffff
     6ED6 A444 
     6ED8 202C 
0102 6EDA 1604  14         jne   !
0103 6EDC 06A0  32         bl    @putat
     6EDE 2410 
0104 6EE0 1D44                   byte 29,68
0105 6EE2 72FA                   data txt.rle          ; Display "RLE"
0106               
0107 6EE4 06A0  32 !       bl    @at
     6EE6 264E 
0108 6EE8 1D0E                   byte 29,14            ; Cursor YX position
0109 6EEA C160  34         mov   @parm1,tmp1           ; Get pointer to file descriptor
     6EEC 8350 
0110 6EEE 06A0  32         bl    @xutst0               ; Display device/filename
     6EF0 2400 
0111                       ;------------------------------------------------------
0112                       ; Exit
0113                       ;------------------------------------------------------
0114               fm.loadfile.callback.indicator1.exit:
0115 6EF2 0460  28         b     @poprt                ; Return to caller
     6EF4 2212 
0116               
0117               
0118               
...
0126 6EF6 0649  14         dect  stack
0127 6EF8 C64B  30         mov   r11,*stack            ; Save return address
0128               
0129 6EFA 06A0  32         bl    @putnum
     6EFC 2A1E 
0130 6EFE 1D4B                   byte 29,75            ; Show lines read
0131 6F00 A204                   data edb.lines,rambuf,>3020
     6F02 8390 
     6F04 3020 
0132               
0133 6F06 8220  34         c     @fh.kilobytes,tmp4
     6F08 A432 
0134 6F0A 130C  14         jeq   fm.loadfile.callback.indicator2.exit
0135               
0136 6F0C C220  34         mov   @fh.kilobytes,tmp4    ; Save for compare
     6F0E A432 
0137               
0138 6F10 06A0  32         bl    @putnum
     6F12 2A1E 
0139 6F14 1D38                   byte 29,56            ; Show kilobytes read
0140 6F16 A432                   data fh.kilobytes,rambuf,>3020
     6F18 8390 
     6F1A 3020 
0141               
0142 6F1C 06A0  32         bl    @putat
     6F1E 2410 
0143 6F20 1D3D                   byte 29,61
0144 6F22 72F6                   data txt.kb           ; Show "kb" string
0145                       ;------------------------------------------------------
0146                       ; Exit
0147                       ;------------------------------------------------------
0148               fm.loadfile.callback.indicator2.exit:
0149 6F24 0460  28         b     @poprt                ; Return to caller
     6F26 2212 
0150               
0151               
0152               
...
0161 6F28 0649  14         dect  stack
0162 6F2A C64B  30         mov   r11,*stack            ; Save return address
0163               
0164               
0165 6F2C 06A0  32         bl    @hchar
     6F2E 2742 
0166 6F30 1D03                   byte 29,3,32,50       ; Erase loading indicator
     6F32 2032 
0167 6F34 FFFF                   data EOL
0168               
0169 6F36 06A0  32         bl    @putnum
     6F38 2A1E 
0170 6F3A 1D38                   byte 29,56            ; Show kilobytes read
0171 6F3C A432                   data fh.kilobytes,rambuf,>3020
     6F3E 8390 
     6F40 3020 
0172               
0173 6F42 06A0  32         bl    @putat
     6F44 2410 
0174 6F46 1D3D                   byte 29,61
0175 6F48 72F6                   data txt.kb           ; Show "kb" string
0176               
0177 6F4A 06A0  32         bl    @putnum
     6F4C 2A1E 
0178 6F4E 1D4B                   byte 29,75            ; Show lines read
0179 6F50 A42E                   data fh.records,rambuf,>3020
     6F52 8390 
     6F54 3020 
0180                       ;------------------------------------------------------
0181                       ; Exit
...
0184 6F56 0460  28         b     @poprt                ; Return to caller
     6F58 2212 
0185               
0186               
0187               
...
0194 6F5A 0649  14         dect  stack
0195 6F5C C64B  30         mov   r11,*stack            ; Save return address
0196               
0197 6F5E 06A0  32         bl    @hchar
     6F60 2742 
0198 6F62 1D00                   byte 29,0,32,50       ; Erase loading indicator
     6F64 2032 
0199 6F66 FFFF                   data EOL
0200               
0201                       ;------------------------------------------------------
0202                       ; Display I/O error message
0203                       ;------------------------------------------------------
0204 6F68 06A0  32         bl    @cpym2m
     6F6A 2460 
0205 6F6C 7305                   data txt.ioerr+1
0206 6F6E B000                   data cmdb.top
0207 6F70 0029                   data 41               ; Error message
0208               
0209               
0210 6F72 C120  34         mov   @edb.filename.ptr,tmp0
     6F74 A20E 
0211 6F76 D194  26         movb  *tmp0,tmp2            ; Get length byte
0212 6F78 0986  56         srl   tmp2,8                ; Right align
0213 6F7A 0584  14         inc   tmp0                  ; Skip length byte
0214 6F7C 0205  20         li    tmp1,cmdb.top + 42    ; RAM destination address
     6F7E B02A 
0215               
0216 6F80 06A0  32         bl    @xpym2m               ; \ Copy CPU memory to CPU memory
     6F82 2466 
...
0220               
0221               
0222 6F84 0204  20         li    tmp0,txt.newfile      ; New file
     6F86 7332 
0223 6F88 C804  38         mov   tmp0,@edb.filename.ptr
     6F8A A20E 
0224               
0225 6F8C 0204  20         li    tmp0,txt.filetype.none
     6F8E 737E 
0226 6F90 C804  38         mov   tmp0,@edb.filetype.ptr
     6F92 A210 
0227                                                   ; Empty filetype string
0228               
0229 6F94 C820  54         mov   @cmdb.scrrows,@parm1
     6F96 A304 
     6F98 8350 
0230 6F9A 06A0  32         bl    @cmdb.show
     6F9C 6BB0 
0231                       ;------------------------------------------------------
0232                       ; Exit
0233                       ;------------------------------------------------------
0234               fm.loadfile.callback.fioerr.exit:
0235 6F9E 0460  28         b     @poprt                ; Return to caller
     6FA0 2212 
**** **** ****     > tivi_b1.asm.19082
...
0010               ****************************************************************
0011               hook.keyscan:
0012 6FA2 20A0  38         coc   @wbit11,config        ; ANYKEY pressed ?
     6FA4 2014 
0013 6FA6 160B  14         jne   hook.keyscan.clear_kbbuffer
0014                                                   ; No, clear buffer and exit
0015               *---------------------------------------------------------------
0016               * Identical key pressed ?
0017               *---------------------------------------------------------------
0018 6FA8 40A0  34         szc   @wbit11,config        ; Reset ANYKEY
     6FAA 2014 
0019 6FAC 8820  54         c     @waux1,@waux2         ; Still pressing previous key?
     6FAE 833C 
     6FB0 833E 
0020 6FB2 1309  14         jeq   hook.keyscan.bounce
0021               *--------------------------------------------------------------
0022               * New key pressed
0023               *--------------------------------------------------------------
0024               hook.keyscan.newkey:
0025 6FB4 C820  54         mov   @waux1,@waux2         ; Save as previous key
     6FB6 833C 
     6FB8 833E 
0026 6FBA 0460  28         b     @edkey.key.process    ; Process key
     6FBC 60FE 
0027               *--------------------------------------------------------------
0028               * Clear keyboard buffer if no key pressed
0029               *--------------------------------------------------------------
0030               hook.keyscan.clear_kbbuffer:
0031 6FBE 04E0  34         clr   @waux1
     6FC0 833C 
0032 6FC2 04E0  34         clr   @waux2
     6FC4 833E 
0033               *--------------------------------------------------------------
0034               * Delay to avoid key bouncing
0035               *--------------------------------------------------------------
0036               hook.keyscan.bounce:
0037 6FC6 0204  20         li    tmp0,2000             ; Avoid key bouncing
     6FC8 07D0 
0038                       ;------------------------------------------------------
0039                       ; Delay loop
0040                       ;------------------------------------------------------
0041               hook.keyscan.bounce.loop:
0042 6FCA 0604  14         dec   tmp0
0043 6FCC 16FE  14         jne   hook.keyscan.bounce.loop
0044               *--------------------------------------------------------------
0045               * Exit
0046               *--------------------------------------------------------------
0047 6FCE 0460  28         b     @hookok               ; Return
     6FD0 2C7A 
**** **** ****     > tivi_b1.asm.19082
...
0010               ***************************************************************
0011               task.vdp.panes:
0012 6FD2 C120  34         mov   @fb.dirty,tmp0        ; Is frame buffer dirty?
     6FD4 A116 
0013 6FD6 136E  14         jeq   task.vdp.panes.exit   ; No, skip update
0014                       ;------------------------------------------------------
0015                       ; Show banner line
0016                       ;------------------------------------------------------
0017 6FD8 06A0  32         bl    @pane.topline.draw
     6FDA 7138 
0018 6FDC C820  54         mov   @wyx,@fb.yxsave       ; Backup VDP cursor position
     6FDE 832A 
     6FE0 A114 
0019                       ;------------------------------------------------------
0020                       ; Determine how many rows to copy
0021                       ;------------------------------------------------------
0022 6FE2 8820  54         c     @edb.lines,@fb.scrrows
     6FE4 A204 
     6FE6 A118 
0023 6FE8 1103  14         jlt   task.vdp.panes.setrows.small
0024 6FEA C160  34         mov   @fb.scrrows,tmp1      ; Lines to copy
     6FEC A118 
0025 6FEE 1003  14         jmp   task.vdp.panes.copy.framebuffer
0026                       ;------------------------------------------------------
0027                       ; Less lines in editor buffer as rows in frame buffer
0028                       ;------------------------------------------------------
0029               task.vdp.panes.setrows.small:
0030 6FF0 C160  34         mov   @edb.lines,tmp1       ; Lines to copy
     6FF2 A204 
0031 6FF4 0585  14         inc   tmp1
0032                       ;------------------------------------------------------
0033                       ; Determine area to copy
0034                       ;------------------------------------------------------
0035               task.vdp.panes.copy.framebuffer:
0036 6FF6 3960  72         mpy   @fb.colsline,tmp1     ; columns per line * rows on screen
     6FF8 A10E 
0037                                                   ; 16 bit part is in tmp2!
0038 6FFA 0204  20         li    tmp0,80               ; VDP target address (2nd line on screen!)
     6FFC 0050 
0039 6FFE C160  34         mov   @fb.top.ptr,tmp1      ; RAM Source address
     7000 A100 
0040                       ;------------------------------------------------------
0041                       ; Copy memory block
0042                       ;------------------------------------------------------
0043 7002 06A0  32         bl    @xpym2v               ; Copy to VDP
     7004 241E 
0044                                                   ; \ i  tmp0 = VDP target address
0045                                                   ; | i  tmp1 = RAM source address
0046                                                   ; / i  tmp2 = Bytes to copy
0047 7006 04E0  34         clr   @fb.dirty             ; Reset frame buffer dirty flag
     7008 A116 
0048                       ;-------------------------------------------------------
0049                       ; Draw EOF marker at end-of-file
0050                       ;-------------------------------------------------------
0051 700A C120  34         mov   @edb.lines,tmp0
     700C A204 
0052 700E 6120  34         s     @fb.topline,tmp0      ; Y = @edb.lines - @fb.topline
     7010 A104 
0053 7012 05C4  14         inct  tmp0                  ; Y = Y + 2
0054 7014 8120  34         c     @fb.scrrows,tmp0      ; Hide if last line on screen
     7016 A118 
0055 7018 121C  14         jle   task.vdp.panes.draw_double.line
0056                       ;-------------------------------------------------------
0057                       ; Do actual drawing of EOF marker
0058                       ;-------------------------------------------------------
0059               task.vdp.panes.draw_marker:
0060 701A 0A84  56         sla   tmp0,8                ; Move LSB to MSB (Y), X=0
0061 701C C804  38         mov   tmp0,@wyx             ; Set VDP cursor
     701E 832A 
0062               
0063 7020 06A0  32         bl    @putstr
     7022 23FE 
0064 7024 72D4                   data txt.marker       ; Display *EOF*
0065                       ;-------------------------------------------------------
0066                       ; Draw empty line after (and below) EOF marker
0067                       ;-------------------------------------------------------
0068 7026 06A0  32         bl    @setx
     7028 2664 
0069 702A 0005                   data  5               ; Cursor after *EOF* string
0070               
0071 702C C120  34         mov   @wyx,tmp0
     702E 832A 
0072 7030 0984  56         srl   tmp0,8                ; Right justify
0073 7032 0584  14         inc   tmp0                  ; One time adjust
0074 7034 8120  34         c     @fb.scrrows,tmp0      ; Don't spill on last line on screen
     7036 A118 
0075 7038 1303  14         jeq   !
0076 703A 0206  20         li    tmp2,colrow+colrow-5  ; Repeat count for 2 lines
     703C 009B 
0077 703E 1002  14         jmp   task.vdp.panes.draw_marker.empty.line
0078 7040 0206  20 !       li    tmp2,colrow-5         ; Repeat count for 1 line
     7042 004B 
0079                       ;-------------------------------------------------------
0080                       ; Draw 1 or 2 empty lines
0081                       ;-------------------------------------------------------
0082               task.vdp.panes.draw_marker.empty.line:
0083 7044 0604  14         dec   tmp0                  ; One time adjust
0084 7046 06A0  32         bl    @yx2pnt               ; Set VDP address in tmp0
     7048 23DA 
0085 704A 0205  20         li    tmp1,32               ; Character to write (whitespace)
     704C 0020 
0086 704E 06A0  32         bl    @xfilv                ; Fill VDP memory
     7050 2274 
...
0094 7052 C120  34         mov   @fb.scrrows,tmp0
     7054 A118 
0095 7056 0584  14         inc   tmp0                  ; 1st Line after frame buffer boundary
0096 7058 06C4  14         swpb  tmp0                  ; LSB to MSB
0097 705A C804  38         mov   tmp0,@wyx             ; Save YX
     705C 832A 
0098               
0099 705E C120  34         mov   @cmdb.visible,tmp0    ; Command buffer hidden ?
     7060 A302 
0100 7062 1306  14         jeq   !                     ; Yes, full double line
0101                       ;-------------------------------------------------------
0102                       ; Double line with corners
0103                       ;-------------------------------------------------------
0104 7064 06A0  32         bl    @setx                 ; Set cursor to screen column 17
     7066 2664 
0105 7068 0001                   data 1
0106 706A 0206  20         li    tmp2,78               ; Repeat 78x
     706C 004E 
0107 706E 1005  14         jmp   task.vdp.panes.draw_double.draw
0108                       ;-------------------------------------------------------
0109                       ; Continuous double line (80 characters)
0110                       ;-------------------------------------------------------
0111 7070 06A0  32 !       bl    @setx                 ; Set cursor to screen column 0
     7072 2664 
0112 7074 0000                   data 0
0113 7076 0206  20         li    tmp2,80               ; Repeat 80x
     7078 0050 
0114                       ;-------------------------------------------------------
0115                       ; Do actual drawing
0116                       ;-------------------------------------------------------
0117               task.vdp.panes.draw_double.draw:
0118 707A 06A0  32         bl    @yx2pnt               ; Set VDP address in tmp0
     707C 23DA 
0119 707E 0205  20         li    tmp1,3                ; Character to write (double line)
     7080 0003 
0120 7082 06A0  32         bl    @xfilv                ; \ Fill VDP memory
     7084 2274 
0121                                                   ; | i  tmp0 = VDP destination
0122                                                   ; | i  tmp1 = Byte to write
0123                                                   ; / i  tmp2 = Number of bstes to write
0124 7086 C820  54         mov   @fb.yxsave,@wyx       ; Restore cursor postion
     7088 A114 
     708A 832A 
0125                       ;-------------------------------------------------------
0126                       ; Show command buffer
0127                       ;-------------------------------------------------------
0128 708C C120  34         mov   @cmdb.visible,tmp0     ; Show command buffer?
     708E A302 
0129 7090 1311  14         jeq   task.vdp.panes.exit    ; No, skip
0130               
0131 7092 06A0  32         bl    @cmdb.refresh          ; Refresh command buffer content
     7094 6C0A 
0132               
0133 7096 06A0  32         bl    @vchar
     7098 276A 
0134 709A 1200                   byte 18,0,4,1          ; Top left corner
     709C 0401 
0135 709E 124F                   byte 18,79,5,1         ; Top right corner
     70A0 0501 
0136 70A2 1300                   byte 19,0,6,9          ; Left vertical double line
     70A4 0609 
0137 70A6 134F                   byte 19,79,7,9         ; Right vertical double line
     70A8 0709 
0138 70AA 1C00                   byte 28,0,8,1          ; Bottom left corner
     70AC 0801 
0139 70AE 1C4F                   byte 28,79,9,1         ; Bottom right corner
     70B0 0901 
0140 70B2 FFFF                   data EOL
0141                       ;------------------------------------------------------
0142                       ; Exit task
0143                       ;------------------------------------------------------
0144               task.vdp.panes.exit:
0145 70B4 06A0  32         bl    @pane.botline.draw     ; Draw status bottom line
     70B6 7162 
0146 70B8 0460  28         b     @slotok
     70BA 2CF6 
**** **** ****     > tivi_b1.asm.19082
...
0012 70BC C120  34         mov   @tv.pane.focus,tmp0
     70BE A016 
0013 70C0 130A  14         jeq   !                     ; Frame buffer has focus
0014               
0015 70C2 0284  22         ci    tmp0,pane.focus.cmdb
     70C4 0001 
0016 70C6 1304  14         jeq   task.vdp.copy.sat.cmdb
0017                                                   ; Command buffer has focus
0018                       ;------------------------------------------------------
0019                       ; Assert failed. Invalid value
0020                       ;------------------------------------------------------
0021 70C8 C80B  38         mov   r11,@>ffce            ; \ Save caller address
     70CA FFCE 
0022 70CC 06A0  32         bl    @cpu.crash            ; / Halt system.
     70CE 2030 
0023                       ;------------------------------------------------------
0024                       ; Command buffer has focus, position cursor
0025                       ;------------------------------------------------------
0026               task.vdp.copy.sat.cmdb:
0027 70D0 C820  54         mov   @cmdb.cursor,@wyx     ; Position cursor in CMDB pane
     70D2 A308 
     70D4 832A 
0028                       ;------------------------------------------------------
0029                       ; Position cursor
0030                       ;------------------------------------------------------
0031 70D6 E0A0  34 !       soc   @wbit0,config         ; Sprite adjustment on
     70D8 202A 
0032 70DA 06A0  32         bl    @yx2px                ; \ Calculate pixel position
     70DC 2670 
0033                                                   ; | i  @WYX = Cursor YX
0034                                                   ; / o  tmp0 = Pixel YX
0035 70DE C804  38         mov   tmp0,@ramsat          ; Set cursor YX
     70E0 8380 
0036               
0037 70E2 06A0  32         bl    @cpym2v               ; Copy sprite SAT to VDP
     70E4 2418 
0038 70E6 2000                   data sprsat,ramsat,4  ; \ i  tmp0 = VDP destination
     70E8 8380 
     70EA 0004 
...
0045 70EC 0460  28         b     @slotok               ; Exit task
     70EE 2CF6 
**** **** ****     > tivi_b1.asm.19082
...
0012 70F0 0560  34         inv   @fb.curtoggle          ; Flip cursor shape flag
     70F2 A112 
0013 70F4 1303  14         jeq   task.vdp.cursor.visible
0014 70F6 04E0  34         clr   @ramsat+2              ; Hide cursor
     70F8 8382 
0015 70FA 1015  14         jmp   task.vdp.cursor.copy.sat
0016                                                    ; Update VDP SAT and exit task
0017               task.vdp.cursor.visible:
0018 70FC C120  34         mov   @edb.insmode,tmp0      ; Get Editor buffer insert mode
     70FE A20A 
0019 7100 130B  14         jeq   task.vdp.cursor.visible.overwrite_mode
0020                       ;------------------------------------------------------
0021                       ; Cursor in insert mode
0022                       ;------------------------------------------------------
0023               task.vdp.cursor.visible.insert_mode:
0024 7102 C120  34         mov   @tv.pane.focus,tmp0    ; Get pane with focus
     7104 A016 
0025 7106 1303  14         jeq   task.vdp.cursor.visible.insert_mode.fb
0026                                                    ; Framebuffer has focus
0027 7108 0284  22         ci    tmp0,pane.focus.cmdb
     710A 0001 
0028 710C 1302  14         jeq   task.vdp.cursor.visible.insert_mode.cmdb
0029                       ;------------------------------------------------------
0030                       ; Editor cursor (insert mode)
0031                       ;------------------------------------------------------
0032               task.vdp.cursor.visible.insert_mode.fb:
0033 710E 04C4  14         clr   tmp0                   ; Cursor editor insert mode
0034 7110 1005  14         jmp   task.vdp.cursor.visible.cursorshape
0035                       ;------------------------------------------------------
0036                       ; Command buffer cursor (insert mode)
0037                       ;------------------------------------------------------
0038               task.vdp.cursor.visible.insert_mode.cmdb:
0039 7112 0204  20         li    tmp0,>0100             ; Cursor CMDB insert mode
     7114 0100 
0040 7116 1002  14         jmp   task.vdp.cursor.visible.cursorshape
0041                       ;------------------------------------------------------
0042                       ; Cursor in overwrite mode
0043                       ;------------------------------------------------------
0044               task.vdp.cursor.visible.overwrite_mode:
0045 7118 0204  20         li    tmp0,>0200             ; Cursor overwrite mode
     711A 0200 
0046                       ;------------------------------------------------------
0047                       ; Set cursor shape
0048                       ;------------------------------------------------------
0049               task.vdp.cursor.visible.cursorshape:
0050 711C D804  38         movb  tmp0,@tv.curshape      ; Save cursor shape
     711E A014 
0051 7120 C820  54         mov   @tv.curshape,@ramsat+2 ; Get cursor shape and color
     7122 A014 
     7124 8382 
0052                       ;------------------------------------------------------
0053                       ; Copy SAT
...
0056 7126 06A0  32         bl    @cpym2v                ; Copy sprite SAT to VDP
     7128 2418 
0057 712A 2000                   data sprsat,ramsat,4   ; \ i  tmp0 = VDP destination
     712C 8380 
     712E 0004 
...
0062                       ;-------------------------------------------------------
0063 7130 06A0  32         bl    @pane.botline.draw     ; Draw status bottom line
     7132 7162 
0064                       ;------------------------------------------------------
0065                       ; Exit
0066                       ;------------------------------------------------------
0067               task.vdp.cursor.exit:
0068 7134 0460  28         b     @slotok                ; Exit task
     7136 2CF6 
**** **** ****     > tivi_b1.asm.19082
...
0021 7138 0649  14         dect  stack
0022 713A C64B  30         mov   r11,*stack            ; Save return address
0023 713C C820  54         mov   @wyx,@fb.yxsave
     713E 832A 
     7140 A114 
0024                       ;------------------------------------------------------
0025                       ; Show banner (line above frame buffer, not part of it)
0026                       ;------------------------------------------------------
0027 7142 06A0  32         bl    @hchar
     7144 2742 
0028 7146 0000                   byte 0,0,1,34         ; Double line at top (left)
     7148 0122 
0029 714A 002E                   byte 0,46,1,34        ; Double line at top (right)
     714C 0122 
0030 714E FFFF                   data EOL
0031               
0032 7150 06A0  32         bl    @putat
     7152 2410 
0033 7154 0022                   byte 0,34
0034 7156 738A                   data txt.tivi         ; TiVi banner (middle)
0035                       ;------------------------------------------------------
0036                       ; Exit
0037                       ;------------------------------------------------------
0038               pane.topline.exit:
0039 7158 C820  54         mov   @fb.yxsave,@wyx
     715A A114 
     715C 832A 
0040 715E C2F9  30         mov   *stack+,r11           ; Pop r11
0041 7160 045B  20         b     *r11                  ; Return
**** **** ****     > tivi_b1.asm.19082
...
0021 7162 0649  14         dect  stack
0022 7164 C64B  30         mov   r11,*stack            ; Save return address
0023 7166 0649  14         dect  stack
0024 7168 C644  30         mov   tmp0,*stack           ; Push tmp0
0025               
0026 716A C820  54         mov   @wyx,@fb.yxsave
     716C 832A 
     716E A114 
0027                       ;------------------------------------------------------
...
0031 7170 06A0  32         bl    @putat
     7172 2410 
0032 7174 1D00                   byte  29,0
0033 7176 732E                   data  txt.bufnum
0034                       ;------------------------------------------------------
0035                       ; Show current file
0036                       ;------------------------------------------------------
0037               pane.botline.show_file:
0038 7178 06A0  32         bl    @at
     717A 264E 
0039 717C 1D03                   byte  29,3            ; Position cursor
0040 717E C160  34         mov   @edb.filename.ptr,tmp1
     7180 A20E 
0041                                                   ; Get string to display
0042 7182 06A0  32         bl    @xutst0               ; Display string
     7184 2400 
0043               
0044 7186 06A0  32         bl    @at
     7188 264E 
0045 718A 1D23                   byte  29,35           ; Position cursor
0046               
0047 718C C160  34         mov   @edb.filetype.ptr,tmp1
     718E A210 
0048                                                   ; Get string to display
0049 7190 06A0  32         bl    @xutst0               ; Display Filetype string
     7192 2400 
0050                       ;------------------------------------------------------
0051                       ; Show text editing mode
0052                       ;------------------------------------------------------
0053               pane.botline.show_mode:
0054 7194 C120  34         mov   @edb.insmode,tmp0
     7196 A20A 
0055 7198 1605  14         jne   pane.botline.show_mode.insert
0056                       ;------------------------------------------------------
0057                       ; Overwrite mode
0058                       ;------------------------------------------------------
0059               pane.botline.show_mode.overwrite:
0060 719A 06A0  32         bl    @putat
     719C 2410 
0061 719E 1D32                   byte  29,50
0062 71A0 72E0                   data  txt.ovrwrite
0063 71A2 1004  14         jmp   pane.botline.show_changed
0064                       ;------------------------------------------------------
0065                       ; Insert  mode
0066                       ;------------------------------------------------------
0067               pane.botline.show_mode.insert:
0068 71A4 06A0  32         bl    @putat
     71A6 2410 
0069 71A8 1D32                   byte  29,50
0070 71AA 72E4                   data  txt.insert
0071                       ;------------------------------------------------------
0072                       ; Show if text was changed in editor buffer
0073                       ;------------------------------------------------------
0074               pane.botline.show_changed:
0075 71AC C120  34         mov   @edb.dirty,tmp0
     71AE A206 
0076 71B0 1305  14         jeq   pane.botline.show_changed.clear
0077                       ;------------------------------------------------------
0078                       ; Show "*"
0079                       ;------------------------------------------------------
0080 71B2 06A0  32         bl    @putat
     71B4 2410 
0081 71B6 1D36                   byte 29,54
0082 71B8 72E8                   data txt.star
0083 71BA 1001  14         jmp   pane.botline.show_linecol
0084                       ;------------------------------------------------------
0085                       ; Show "line,column"
0086                       ;------------------------------------------------------
0087               pane.botline.show_changed.clear:
0088 71BC 1000  14         nop
0089               pane.botline.show_linecol:
0090 71BE C820  54         mov   @fb.row,@parm1
     71C0 A106 
     71C2 8350 
0091 71C4 06A0  32         bl    @fb.row2line
     71C6 680A 
0092 71C8 05A0  34         inc   @outparm1
     71CA 8360 
0093                       ;------------------------------------------------------
0094                       ; Show line
0095                       ;------------------------------------------------------
0096 71CC 06A0  32         bl    @putnum
     71CE 2A1E 
0097 71D0 1D40                   byte  29,64           ; YX
0098 71D2 8360                   data  outparm1,rambuf
     71D4 8390 
0099 71D6 3020                   byte  48              ; ASCII offset
0100                             byte  32              ; Padding character
0101                       ;------------------------------------------------------
0102                       ; Show comma
0103                       ;------------------------------------------------------
0104 71D8 06A0  32         bl    @putat
     71DA 2410 
0105 71DC 1D45                   byte  29,69
0106 71DE 72D2                   data  txt.delim
0107                       ;------------------------------------------------------
0108                       ; Show column
0109                       ;------------------------------------------------------
0110 71E0 06A0  32         bl    @film
     71E2 2216 
0111 71E4 8396                   data rambuf+6,32,12   ; Clear work buffer with space character
     71E6 0020 
     71E8 000C 
0112               
0113 71EA C820  54         mov   @fb.column,@waux1
     71EC A10C 
     71EE 833C 
0114 71F0 05A0  34         inc   @waux1                ; Offset 1
     71F2 833C 
0115               
0116 71F4 06A0  32         bl    @mknum                ; Convert unsigned number to string
     71F6 29A0 
0117 71F8 833C                   data  waux1,rambuf
     71FA 8390 
0118 71FC 3020                   byte  48              ; ASCII offset
0119                             byte  32              ; Fill character
0120               
0121 71FE 06A0  32         bl    @trimnum              ; Trim number to the left
     7200 29F8 
0122 7202 8390                   data  rambuf,rambuf+6,32
     7204 8396 
     7206 0020 
0123               
0124 7208 0204  20         li    tmp0,>0200
     720A 0200 
0125 720C D804  38         movb  tmp0,@rambuf+6        ; "Fix" number length to clear junk chars
     720E 8396 
0126               
0127 7210 06A0  32         bl    @putat
     7212 2410 
0128 7214 1D46                   byte 29,70
0129 7216 8396                   data rambuf+6         ; Show column
0130                       ;------------------------------------------------------
0131                       ; Show lines in buffer unless on last line in file
0132                       ;------------------------------------------------------
0133 7218 C820  54         mov   @fb.row,@parm1
     721A A106 
     721C 8350 
0134 721E 06A0  32         bl    @fb.row2line
     7220 680A 
0135 7222 8820  54         c     @edb.lines,@outparm1
     7224 A204 
     7226 8360 
0136 7228 1605  14         jne   pane.botline.show_lines_in_buffer
0137               
0138 722A 06A0  32         bl    @putat
     722C 2410 
0139 722E 1D4B                   byte 29,75
0140 7230 72DA                   data txt.bottom
0141               
0142 7232 100B  14         jmp   pane.botline.exit
0143                       ;------------------------------------------------------
0144                       ; Show lines in buffer
0145                       ;------------------------------------------------------
0146               pane.botline.show_lines_in_buffer:
0147 7234 C820  54         mov   @edb.lines,@waux1
     7236 A204 
     7238 833C 
0148 723A 05A0  34         inc   @waux1                ; Offset 1
     723C 833C 
0149 723E 06A0  32         bl    @putnum
     7240 2A1E 
0150 7242 1D4B                   byte 29,75            ; YX
0151 7244 833C                   data waux1,rambuf
     7246 8390 
0152 7248 3020                   byte 48
0153                             byte 32
0154                       ;------------------------------------------------------
0155                       ; Exit
0156                       ;------------------------------------------------------
0157               pane.botline.exit:
0158 724A C820  54         mov   @fb.yxsave,@wyx
     724C A114 
     724E 832A 
0159 7250 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0160 7252 C2F9  30         mov   *stack+,r11           ; Pop r11
0161 7254 045B  20         b     *r11                  ; Return
**** **** ****     > tivi_b1.asm.19082
...
0006               ********|*****|*********************|**************************
0007               romsat:
0008 7256 0303             data  >0303,>000f           ; Cursor YX, initial shape and colour
     7258 000F 
0009               
0010               cursors:
0011 725A 0000             data >0000,>0000,>0000,>001c ; Cursor 1 - Insert mode
     725C 0000 
     725E 0000 
     7260 001C 
0012 7262 1010             data >1010,>1010,>1010,>1000 ; Cursor 2 - Insert mode
     7264 1010 
     7266 1010 
     7268 1000 
0013 726A 1C1C             data >1c1c,>1c1c,>1c1c,>1c00 ; Cursor 3 - Overwrite mode
     726C 1C1C 
     726E 1C1C 
     7270 1C00 
0014               
0015               patterns:
0016 7272 0000             data >0000,>ff00,>00ff,>0080 ; 01. Double line top + ruler
     7274 FF00 
     7276 00FF 
     7278 0080 
0017 727A 0080             data >0080,>0000,>ff00,>ff00 ; 02. Ruler + double line bottom
     727C 0000 
     727E FF00 
     7280 FF00 
0018               patterns.box:
0019 7282 0000             data >0000,>0000,>ff00,>ff00 ; 03. Double line bottom
     7284 0000 
     7286 FF00 
     7288 FF00 
0020 728A 0000             data >0000,>0000,>ff80,>bfa0 ; 04. Top left corner
     728C 0000 
     728E FF80 
     7290 BFA0 
0021 7292 0000             data >0000,>0000,>fc04,>f414 ; 05. Top right corner
     7294 0000 
     7296 FC04 
     7298 F414 
0022 729A A0A0             data >a0a0,>a0a0,>a0a0,>a0a0 ; 06. Left vertical double line
     729C A0A0 
     729E A0A0 
     72A0 A0A0 
0023 72A2 1414             data >1414,>1414,>1414,>1414 ; 07. Right vertical double line
     72A4 1414 
     72A6 1414 
     72A8 1414 
0024 72AA A0A0             data >a0a0,>a0a0,>bf80,>ff00 ; 08. Bottom left corner
     72AC A0A0 
     72AE BF80 
     72B0 FF00 
0025 72B2 1414             data >1414,>1414,>f404,>fc00 ; 09. Bottom right corner
     72B4 1414 
     72B6 F404 
     72B8 FC00 
0026 72BA 0000             data >0000,>c0c0,>c0c0,>0080 ; 10. Double line top left corner
     72BC C0C0 
     72BE C0C0 
     72C0 0080 
0027 72C2 0000             data >0000,>0f0f,>0f0f,>0000 ; 11. Double line top right corner
     72C4 0F0F 
     72C6 0F0F 
     72C8 0000 
0028               
...
0031 72CA F404             data  >f404                 ; White      | Dark blue  | Dark blue
0032 72CC F101             data  >f101                 ; White      | Black      | Black
0033 72CE 1707             data  >1707                 ; Black      | Cyan       | Cyan
0034 72D0 1F0F             data  >1f0f                 ; Black      | White      | White
**** **** ****     > tivi_b1.asm.19082
...
0006               ***************************************************************
0007               txt.delim
0008 72D2 012C             byte  1
0009 72D3 ....             text  ','
0010                       even
0011               
0012               txt.marker
0013 72D4 052A             byte  5
0014 72D5 ....             text  '*EOF*'
0015                       even
0016               
0017               txt.bottom
0018 72DA 0520             byte  5
0019 72DB ....             text  '  BOT'
0020                       even
0021               
0022               txt.ovrwrite
0023 72E0 034F             byte  3
0024 72E1 ....             text  'OVR'
0025                       even
0026               
0027               txt.insert
0028 72E4 0349             byte  3
0029 72E5 ....             text  'INS'
0030                       even
0031               
0032               txt.star
0033 72E8 012A             byte  1
0034 72E9 ....             text  '*'
0035                       even
0036               
0037               txt.loading
0038 72EA 0A4C             byte  10
0039 72EB ....             text  'Loading...'
0040                       even
0041               
0042               txt.kb
0043 72F6 026B             byte  2
0044 72F7 ....             text  'kb'
0045                       even
0046               
0047               txt.rle
0048 72FA 0352             byte  3
0049 72FB ....             text  'RLE'
0050                       even
0051               
0052               txt.lines
0053 72FE 054C             byte  5
0054 72FF ....             text  'Lines'
0055                       even
0056               
0057               txt.ioerr
0058 7304 2921             byte  41
0059 7305 ....             text  '! I/O error occured. Could not load file:'
0060                       even
0061               
0062               txt.bufnum
0063 732E 0223             byte  2
0064 732F ....             text  '#1'
0065                       even
0066               
0067               txt.newfile
0068 7332 0A5B             byte  10
0069 7333 ....             text  '[New file]'
0070                       even
0071               
0072               
0073               txt.cmdb.prompt
0074 733E 013E             byte  1
0075 733F ....             text  '>'
0076                       even
0077               
0078               txt.cmdb.hint
0079 7340 2348             byte  35
0080 7341 ....             text  'Hint: Type "help" for command list.'
0081                       even
0082               
0083               txt.cmdb.catalog
0084 7364 0C46             byte  12
0085 7365 ....             text  'File catalog'
0086                       even
0087               
0088               
0089               
0090               txt.filetype.dv80
0091 7372 0A44             byte  10
0092 7373 ....             text  'DIS/VAR80 '
0093                       even
0094               
0095               txt.filetype.none
0096 737E 0A20             byte  10
0097 737F ....             text  '          '
0098                       even
0099               
0100               
0101 738A 0C0A     txt.tivi     byte    12
0102                            byte    10
0103 738C ....                  text    'TiVi v1.00'
0104 7396 0B00                  byte    11
0105                            even
0106               
0107               fdname0
0108 7398 0F44             byte  15
0109 7399 ....             text  'DSK1.FWDOC/PSRV'
0110                       even
0111               
0112               fdname1
0113 73A8 0F44             byte  15
0114 73A9 ....             text  'DSK1.SPEECHDOCS'
0115                       even
0116               
0117               fdname2
0118 73B8 0C44             byte  12
0119 73B9 ....             text  'DSK1.XBEADOC'
0120                       even
0121               
0122               fdname3
0123 73C6 0C44             byte  12
0124 73C7 ....             text  'DSK3.XBEADOC'
0125                       even
0126               
0127               fdname4
0128 73D4 0C44             byte  12
0129 73D5 ....             text  'DSK3.C99MAN1'
0130                       even
0131               
0132               fdname5
0133 73E2 0C44             byte  12
0134 73E3 ....             text  'DSK3.C99MAN2'
0135                       even
0136               
0137               fdname6
0138 73F0 0C44             byte  12
0139 73F1 ....             text  'DSK3.C99MAN3'
0140                       even
0141               
0142               fdname7
0143 73FE 0D44             byte  13
0144 73FF ....             text  'DSK3.C99SPECS'
0145                       even
0146               
0147               fdname8
0148 740C 0D44             byte  13
0149 740D ....             text  'DSK3.RANDOM#C'
0150                       even
0151               
0152               fdname9
0153 741A 0D44             byte  13
0154 741B ....             text  'DSK1.INVADERS'
0155                       even
0156               
0157               
0158               
...
0161               *---------------------------------------------------------------
0162               txt.fctn.0
0163 7428 0866             byte  8
0164 7429 ....             text  'fctn + 0'
0165                       even
0166               
0167               txt.fctn.1
0168 7432 0866             byte  8
0169 7433 ....             text  'fctn + 1'
0170                       even
0171               
0172               txt.fctn.2
0173 743C 0866             byte  8
0174 743D ....             text  'fctn + 2'
0175                       even
0176               
0177               txt.fctn.3
0178 7446 0866             byte  8
0179 7447 ....             text  'fctn + 3'
0180                       even
0181               
0182               txt.fctn.4
0183 7450 0866             byte  8
0184 7451 ....             text  'fctn + 4'
0185                       even
0186               
0187               txt.fctn.5
0188 745A 0866             byte  8
0189 745B ....             text  'fctn + 5'
0190                       even
0191               
0192               txt.fctn.6
0193 7464 0866             byte  8
0194 7465 ....             text  'fctn + 6'
0195                       even
0196               
0197               txt.fctn.7
0198 746E 0866             byte  8
0199 746F ....             text  'fctn + 7'
0200                       even
0201               
0202               txt.fctn.8
0203 7478 0866             byte  8
0204 7479 ....             text  'fctn + 8'
0205                       even
0206               
0207               txt.fctn.9
0208 7482 0866             byte  8
0209 7483 ....             text  'fctn + 9'
0210                       even
0211               
0212               txt.fctn.a
0213 748C 0866             byte  8
0214 748D ....             text  'fctn + a'
0215                       even
0216               
0217               txt.fctn.b
0218 7496 0866             byte  8
0219 7497 ....             text  'fctn + b'
0220                       even
0221               
0222               txt.fctn.c
0223 74A0 0866             byte  8
0224 74A1 ....             text  'fctn + c'
0225                       even
0226               
0227               txt.fctn.d
0228 74AA 0866             byte  8
0229 74AB ....             text  'fctn + d'
0230                       even
0231               
0232               txt.fctn.e
0233 74B4 0866             byte  8
0234 74B5 ....             text  'fctn + e'
0235                       even
0236               
0237               txt.fctn.f
0238 74BE 0866             byte  8
0239 74BF ....             text  'fctn + f'
0240                       even
0241               
0242               txt.fctn.g
0243 74C8 0866             byte  8
0244 74C9 ....             text  'fctn + g'
0245                       even
0246               
0247               txt.fctn.h
0248 74D2 0866             byte  8
0249 74D3 ....             text  'fctn + h'
0250                       even
0251               
0252               txt.fctn.i
0253 74DC 0866             byte  8
0254 74DD ....             text  'fctn + i'
0255                       even
0256               
0257               txt.fctn.j
0258 74E6 0866             byte  8
0259 74E7 ....             text  'fctn + j'
0260                       even
0261               
0262               txt.fctn.k
0263 74F0 0866             byte  8
0264 74F1 ....             text  'fctn + k'
0265                       even
0266               
0267               txt.fctn.l
0268 74FA 0866             byte  8
0269 74FB ....             text  'fctn + l'
0270                       even
0271               
0272               txt.fctn.m
0273 7504 0866             byte  8
0274 7505 ....             text  'fctn + m'
0275                       even
0276               
0277               txt.fctn.n
0278 750E 0866             byte  8
0279 750F ....             text  'fctn + n'
0280                       even
0281               
0282               txt.fctn.o
0283 7518 0866             byte  8
0284 7519 ....             text  'fctn + o'
0285                       even
0286               
0287               txt.fctn.p
0288 7522 0866             byte  8
0289 7523 ....             text  'fctn + p'
0290                       even
0291               
0292               txt.fctn.q
0293 752C 0866             byte  8
0294 752D ....             text  'fctn + q'
0295                       even
0296               
0297               txt.fctn.r
0298 7536 0866             byte  8
0299 7537 ....             text  'fctn + r'
0300                       even
0301               
0302               txt.fctn.s
0303 7540 0866             byte  8
0304 7541 ....             text  'fctn + s'
0305                       even
0306               
0307               txt.fctn.t
0308 754A 0866             byte  8
0309 754B ....             text  'fctn + t'
0310                       even
0311               
0312               txt.fctn.u
0313 7554 0866             byte  8
0314 7555 ....             text  'fctn + u'
0315                       even
0316               
0317               txt.fctn.v
0318 755E 0866             byte  8
0319 755F ....             text  'fctn + v'
0320                       even
0321               
0322               txt.fctn.w
0323 7568 0866             byte  8
0324 7569 ....             text  'fctn + w'
0325                       even
0326               
0327               txt.fctn.x
0328 7572 0866             byte  8
0329 7573 ....             text  'fctn + x'
0330                       even
0331               
0332               txt.fctn.y
0333 757C 0866             byte  8
0334 757D ....             text  'fctn + y'
0335                       even
0336               
0337               txt.fctn.z
0338 7586 0866             byte  8
0339 7587 ....             text  'fctn + z'
0340                       even
0341               
0342               *---------------------------------------------------------------
...
0344               *---------------------------------------------------------------
0345               txt.fctn.dot
0346 7590 0866             byte  8
0347 7591 ....             text  'fctn + .'
0348                       even
0349               
0350               txt.fctn.plus
0351 759A 0866             byte  8
0352 759B ....             text  'fctn + +'
0353                       even
0354               
0355               *---------------------------------------------------------------
...
0357               *---------------------------------------------------------------
0358               txt.ctrl.0
0359 75A4 0863             byte  8
0360 75A5 ....             text  'ctrl + 0'
0361                       even
0362               
0363               txt.ctrl.1
0364 75AE 0863             byte  8
0365 75AF ....             text  'ctrl + 1'
0366                       even
0367               
0368               txt.ctrl.2
0369 75B8 0863             byte  8
0370 75B9 ....             text  'ctrl + 2'
0371                       even
0372               
0373               txt.ctrl.3
0374 75C2 0863             byte  8
0375 75C3 ....             text  'ctrl + 3'
0376                       even
0377               
0378               txt.ctrl.4
0379 75CC 0863             byte  8
0380 75CD ....             text  'ctrl + 4'
0381                       even
0382               
0383               txt.ctrl.5
0384 75D6 0863             byte  8
0385 75D7 ....             text  'ctrl + 5'
0386                       even
0387               
0388               txt.ctrl.6
0389 75E0 0863             byte  8
0390 75E1 ....             text  'ctrl + 6'
0391                       even
0392               
0393               txt.ctrl.7
0394 75EA 0863             byte  8
0395 75EB ....             text  'ctrl + 7'
0396                       even
0397               
0398               txt.ctrl.8
0399 75F4 0863             byte  8
0400 75F5 ....             text  'ctrl + 8'
0401                       even
0402               
0403               txt.ctrl.9
0404 75FE 0863             byte  8
0405 75FF ....             text  'ctrl + 9'
0406                       even
0407               
0408               txt.ctrl.a
0409 7608 0863             byte  8
0410 7609 ....             text  'ctrl + a'
0411                       even
0412               
0413               txt.ctrl.b
0414 7612 0863             byte  8
0415 7613 ....             text  'ctrl + b'
0416                       even
0417               
0418               txt.ctrl.c
0419 761C 0863             byte  8
0420 761D ....             text  'ctrl + c'
0421                       even
0422               
0423               txt.ctrl.d
0424 7626 0863             byte  8
0425 7627 ....             text  'ctrl + d'
0426                       even
0427               
0428               txt.ctrl.e
0429 7630 0863             byte  8
0430 7631 ....             text  'ctrl + e'
0431                       even
0432               
0433               txt.ctrl.f
0434 763A 0863             byte  8
0435 763B ....             text  'ctrl + f'
0436                       even
0437               
0438               txt.ctrl.g
0439 7644 0863             byte  8
0440 7645 ....             text  'ctrl + g'
0441                       even
0442               
0443               txt.ctrl.h
0444 764E 0863             byte  8
0445 764F ....             text  'ctrl + h'
0446                       even
0447               
0448               txt.ctrl.i
0449 7658 0863             byte  8
0450 7659 ....             text  'ctrl + i'
0451                       even
0452               
0453               txt.ctrl.j
0454 7662 0863             byte  8
0455 7663 ....             text  'ctrl + j'
0456                       even
0457               
0458               txt.ctrl.k
0459 766C 0863             byte  8
0460 766D ....             text  'ctrl + k'
0461                       even
0462               
0463               txt.ctrl.l
0464 7676 0863             byte  8
0465 7677 ....             text  'ctrl + l'
0466                       even
0467               
0468               txt.ctrl.m
0469 7680 0863             byte  8
0470 7681 ....             text  'ctrl + m'
0471                       even
0472               
0473               txt.ctrl.n
0474 768A 0863             byte  8
0475 768B ....             text  'ctrl + n'
0476                       even
0477               
0478               txt.ctrl.o
0479 7694 0863             byte  8
0480 7695 ....             text  'ctrl + o'
0481                       even
0482               
0483               txt.ctrl.p
0484 769E 0863             byte  8
0485 769F ....             text  'ctrl + p'
0486                       even
0487               
0488               txt.ctrl.q
0489 76A8 0863             byte  8
0490 76A9 ....             text  'ctrl + q'
0491                       even
0492               
0493               txt.ctrl.r
0494 76B2 0863             byte  8
0495 76B3 ....             text  'ctrl + r'
0496                       even
0497               
0498               txt.ctrl.s
0499 76BC 0863             byte  8
0500 76BD ....             text  'ctrl + s'
0501                       even
0502               
0503               txt.ctrl.t
0504 76C6 0863             byte  8
0505 76C7 ....             text  'ctrl + t'
0506                       even
0507               
0508               txt.ctrl.u
0509 76D0 0863             byte  8
0510 76D1 ....             text  'ctrl + u'
0511                       even
0512               
0513               txt.ctrl.v
0514 76DA 0863             byte  8
0515 76DB ....             text  'ctrl + v'
0516                       even
0517               
0518               txt.ctrl.w
0519 76E4 0863             byte  8
0520 76E5 ....             text  'ctrl + w'
0521                       even
0522               
0523               txt.ctrl.x
0524 76EE 0863             byte  8
0525 76EF ....             text  'ctrl + x'
0526                       even
0527               
0528               txt.ctrl.y
0529 76F8 0863             byte  8
0530 76F9 ....             text  'ctrl + y'
0531                       even
0532               
0533               txt.ctrl.z
0534 7702 0863             byte  8
0535 7703 ....             text  'ctrl + z'
0536                       even
0537               
0538               *---------------------------------------------------------------
...
0548               *---------------------------------------------------------------
0549               txt.enter
0550 7716 0565             byte  5
0551 7717 ....             text  'enter'
0552                       even
0553               
**** **** ****     > tivi_b1.asm.19082
...
0103                       ; Movement keys
0104                       ;-------------------------------------------------------
0105 771C 0D00             data  key.enter, txt.enter, edkey.action.enter
     771E 7716 
     7720 6568 
0106 7722 0800             data  key.fctn.s, txt.fctn.s, edkey.action.left
     7724 7540 
     7726 615E 
0107 7728 0900             data  key.fctn.d, txt.fctn.d, edkey.action.right
     772A 74AA 
     772C 6174 
0108 772E 0B00             data  key.fctn.e, txt.fctn.e, edkey.action.up
     7730 74B4 
     7732 618C 
0109 7734 0A00             data  key.fctn.x, txt.fctn.x, edkey.action.down
     7736 7572 
     7738 61DE 
0110 773A 8100             data  key.ctrl.a, txt.ctrl.a, edkey.action.home
     773C 7608 
     773E 624A 
0111 7740 8600             data  key.ctrl.f, txt.ctrl.f, edkey.action.end
     7742 763A 
     7744 6262 
0112 7746 9300             data  key.ctrl.s, txt.ctrl.s, edkey.action.pword
     7748 76BC 
     774A 6276 
0113 774C 8400             data  key.ctrl.d, txt.ctrl.d, edkey.action.nword
     774E 7626 
     7750 62C8 
0114 7752 8500             data  key.ctrl.e, txt.ctrl.e, edkey.action.ppage
     7754 7630 
     7756 6328 
0115 7758 9800             data  key.ctrl.x, txt.ctrl.x, edkey.action.npage
     775A 76EE 
     775C 636E 
0116 775E 9400             data  key.ctrl.t, txt.ctrl.t, edkey.action.top
     7760 76C6 
     7762 639A 
0117 7764 8200             data  key.ctrl.b, txt.ctrl.b, edkey.action.bot
     7766 7612 
     7768 63CA 
0118                       ;-------------------------------------------------------
0119                       ; Modifier keys - Delete
0120                       ;-------------------------------------------------------
0121 776A 0300             data  key.fctn.1, txt.fctn.1, edkey.action.del_char
     776C 7432 
     776E 640A 
0122 7770 0000             data  key.ctrl.k, txt.ctrl.k, edkey.action.del_eol
     7772 766C 
     7774 6442 
0123 7776 0700             data  key.fctn.3, txt.fctn.3, edkey.action.del_line
     7778 7446 
     777A 6476 
0124                       ;-------------------------------------------------------
0125                       ; Modifier keys - Insert
0126                       ;-------------------------------------------------------
0127 777C 0400             data  key.fctn.2, txt.fctn.2, edkey.action.ins_char.ws
     777E 743C 
     7780 64CE 
0128 7782 B900             data  key.fctn.dot, txt.fctn.dot, edkey.action.ins_onoff
     7784 7590 
     7786 65D6 
0129 7788 0E00             data  key.fctn.5, txt.fctn.5, edkey.action.ins_line
     778A 745A 
     778C 6524 
0130                       ;-------------------------------------------------------
0131                       ; Other action keys
0132                       ;-------------------------------------------------------
0133 778E 0500             data  key.fctn.plus, txt.fctn.plus, edkey.action.quit
     7790 759A 
     7792 6626 
0134 7794 0F00             data  key.fctn.9, txt.fctn.9, edkey.action.cmdb.toggle
     7796 7482 
     7798 6632 
0135 779A 9A00             data  key.ctrl.z, txt.ctrl.z, edkey.action.color.cycle
     779C 7702 
     779E 6650 
0136                       ;-------------------------------------------------------
0137                       ; Editor/File buffer keys
0138                       ;-------------------------------------------------------
0139 77A0 B000             data  key.ctrl.0, txt.ctrl.0, edkey.action.buffer0
     77A2 75A4 
     77A4 6692 
0140 77A6 B100             data  key.ctrl.1, txt.ctrl.1, edkey.action.buffer1
     77A8 75AE 
     77AA 6698 
0141 77AC B200             data  key.ctrl.2, txt.ctrl.2, edkey.action.buffer2
     77AE 75B8 
     77B0 669E 
0142 77B2 B300             data  key.ctrl.3, txt.ctrl.3, edkey.action.buffer3
     77B4 75C2 
     77B6 66A4 
0143 77B8 B400             data  key.ctrl.4, txt.ctrl.4, edkey.action.buffer4
     77BA 75CC 
     77BC 66AA 
0144 77BE B500             data  key.ctrl.5, txt.ctrl.5, edkey.action.buffer5
     77C0 75D6 
     77C2 66B0 
0145 77C4 B600             data  key.ctrl.6, txt.ctrl.6, edkey.action.buffer6
     77C6 75E0 
     77C8 66B6 
0146 77CA B700             data  key.ctrl.7, txt.ctrl.7, edkey.action.buffer7
     77CC 75EA 
     77CE 66BC 
0147 77D0 9E00             data  key.ctrl.8, txt.ctrl.8, edkey.action.buffer8
     77D2 75F4 
     77D4 66C2 
0148 77D6 9F00             data  key.ctrl.9, txt.ctrl.9, edkey.action.buffer9
     77D8 75FE 
     77DA 66C8 
0149                       ;-------------------------------------------------------
0150                       ; End of list
0151                       ;-------------------------------------------------------
0152 77DC FFFF             data  EOL                           ; EOL
0153               
0154               
0155               
0156               
...
0162                       ; Movement keys
0163                       ;-------------------------------------------------------
0164 77DE 0D00             data  key.enter, txt.enter, edkey.action.enter
     77E0 7716 
     77E2 6568 
0165 77E4 0800             data  key.fctn.s, txt.fctn.s, edkey.action.left
     77E6 7540 
     77E8 615E 
0166 77EA 0900             data  key.fctn.d, txt.fctn.d, edkey.action.right
     77EC 74AA 
     77EE 6174 
0167 77F0 0B00             data  key.fctn.e, txt.fctn.e, edkey.action.noop
     77F2 74B4 
     77F4 662E 
0168 77F6 0A00             data  key.fctn.x, txt.fctn.x, edkey.action.noop
     77F8 7572 
     77FA 662E 
0169 77FC 8100             data  key.ctrl.a, txt.ctrl.a, edkey.action.noop
     77FE 7608 
     7800 662E 
0170 7802 8600             data  key.ctrl.f, txt.ctrl.f, edkey.action.noop
     7804 763A 
     7806 662E 
0171 7808 9300             data  key.ctrl.s, txt.ctrl.s, edkey.action.noop
     780A 76BC 
     780C 662E 
0172 780E 8400             data  key.ctrl.d, txt.ctrl.d, edkey.action.noop
     7810 7626 
     7812 662E 
0173 7814 8500             data  key.ctrl.e, txt.ctrl.e, edkey.action.noop
     7816 7630 
     7818 662E 
0174 781A 9800             data  key.ctrl.x, txt.ctrl.x, edkey.action.noop
     781C 76EE 
     781E 662E 
0175 7820 9400             data  key.ctrl.t, txt.ctrl.t, edkey.action.noop
     7822 76C6 
     7824 662E 
0176 7826 8200             data  key.ctrl.b, txt.ctrl.b, edkey.action.noop
     7828 7612 
     782A 662E 
0177                       ;-------------------------------------------------------
0178                       ; Modifier keys - Delete
0179                       ;-------------------------------------------------------
0180 782C 0300             data  key.fctn.1, txt.fctn.1, edkey.action.del_char
     782E 7432 
     7830 640A 
0181 7832 0000             data  key.ctrl.k, txt.ctrl.k, edkey.action.del_eol
     7834 766C 
     7836 6442 
0182 7838 0700             data  key.fctn.3, txt.fctn.3, edkey.action.noop
     783A 7446 
     783C 662E 
0183                       ;-------------------------------------------------------
0184                       ; Modifier keys - Insert
0185                       ;-------------------------------------------------------
0186 783E 0400             data  key.fctn.2, txt.fctn.2, edkey.action.ins_char.ws
     7840 743C 
     7842 64CE 
0187 7844 B900             data  key.fctn.dot, txt.fctn.dot, edkey.action.ins_onoff
     7846 7590 
     7848 65D6 
0188 784A 0E00             data  key.fctn.5, txt.fctn.5, edkey.action.noop
     784C 745A 
     784E 662E 
0189                       ;-------------------------------------------------------
0190                       ; Other action keys
0191                       ;-------------------------------------------------------
0192 7850 0500             data  key.fctn.plus, txt.fctn.plus, edkey.action.quit
     7852 759A 
     7854 6626 
0193 7856 0F00             data  key.fctn.9, txt.fctn.9, edkey.action.cmdb.toggle
     7858 7482 
     785A 6632 
0194 785C 9A00             data  key.ctrl.z, txt.ctrl.z, edkey.action.color.cycle
     785E 7702 
     7860 6650 
0195                       ;-------------------------------------------------------
0196                       ; Editor/File buffer keys
0197                       ;-------------------------------------------------------
0198 7862 B000             data  key.ctrl.0, txt.ctrl.0, edkey.action.buffer0
     7864 75A4 
     7866 6692 
0199 7868 B100             data  key.ctrl.1, txt.ctrl.1, edkey.action.buffer1
     786A 75AE 
     786C 6698 
0200 786E B200             data  key.ctrl.2, txt.ctrl.2, edkey.action.buffer2
     7870 75B8 
     7872 669E 
0201 7874 B300             data  key.ctrl.3, txt.ctrl.3, edkey.action.buffer3
     7876 75C2 
     7878 66A4 
0202 787A B400             data  key.ctrl.4, txt.ctrl.4, edkey.action.buffer4
     787C 75CC 
     787E 66AA 
0203 7880 B500             data  key.ctrl.5, txt.ctrl.5, edkey.action.buffer5
     7882 75D6 
     7884 66B0 
0204 7886 B600             data  key.ctrl.6, txt.ctrl.6, edkey.action.buffer6
     7888 75E0 
     788A 66B6 
0205 788C B700             data  key.ctrl.7, txt.ctrl.7, edkey.action.buffer7
     788E 75EA 
     7890 66BC 
0206 7892 9E00             data  key.ctrl.8, txt.ctrl.8, edkey.action.buffer8
     7894 75F4 
     7896 66C2 
0207 7898 9F00             data  key.ctrl.9, txt.ctrl.9, edkey.action.buffer9
     789A 75FE 
     789C 66C8 
0208                       ;-------------------------------------------------------
0209                       ; End of list
0210                       ;-------------------------------------------------------
0211 789E FFFF             data  EOL                           ; EOL
**** **** ****     > tivi_b1.asm.19082
0061               
0065 78A0 78A0                   data $                ; Bank 1 ROM size OK.
0067               
0068               *--------------------------------------------------------------
