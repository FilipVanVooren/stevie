...
0008               ***************************************************************
0009               * File: stevie_b7.asm               ; Version 220110-2052550
0010               *
0011               * Bank 7 "Jonas"
...
0270 21ED   42             text  'Build-ID  220110-2052550'
     21EE 7569     
     21F0 6C64     
     21F2 2D49     
     21F4 4420     
...
0030 327E 600E                   data bankid           ; / i  p2 = Source ROM bank for return
0031               
...
0053 6046 06A0  32         bl    @cpu.crash            ; Should never get here
     6048 2026     
0054                       ;-----------------------------------------------------------------------
0055                       ; SAMS support routines and utilities
0056                       ;-----------------------------------------------------------------------
0057                       copy  "mem.sams.layout.asm" ; Setup SAMS memory banks from cart space
     **** ****     > mem.sams.layout.asm
0001               
0002               ***************************************************************
...
0023                       ;-------------------------------------------------------
0024 604A 020C  20         li    r12,>1e00             ; SAMS CRU address
     604C 1E00     
0025 604E 1D00  20         sbo   0                     ; Enable access to SAMS registers
0026               
0027 6050 C830  50         mov   *r0+,@>4004           ; Set page for >2000 - >2fff
     6052 4004     
0028 6054 C830  50         mov   *r0+,@>4006           ; Set page for >3000 - >3fff
     6056 4006     
0029 6058 C830  50         mov   *r0+,@>4014           ; Set page for >a000 - >afff
     605A 4014     
0030 605C C830  50         mov   *r0+,@>4016           ; Set page for >b000 - >bfff
     605E 4016     
0031 6060 C830  50         mov   *r0+,@>4018           ; Set page for >c000 - >cfff
     6062 4018     
0032 6064 C830  50         mov   *r0+,@>401a           ; Set page for >d000 - >dfff
     6066 401A     
0033 6068 C830  50         mov   *r0+,@>401c           ; Set page for >e000 - >efff
     606A 401C     
0034 606C C830  50         mov   *r0+,@>401e           ; Set page for >f000 - >ffff
     606E 401E     
0035               
0036 6070 1E00  20         sbz   0                     ; Disable access to SAMS registers
0037                       ;------------------------------------------------------
0038                       ; Exit
0039                       ;------------------------------------------------------
0040               mem.sams.set.banks.exit:
0041 6072 045B  20         b     *r11                  ; Return
0042               
0043               
0044               
0045               
...
0068 6074 020C  20         li    r12,>1e00             ; SAMS CRU address
     6076 1E00     
0069 6078 1E01  20         sbz   1                     ; Disable SAMS mapper
...
0080                       ;-------------------------------------------------------
0081 6082 C820  54         mov   @mem.sams.set.legacy.code,@>8300
     6084 60A4     
     6086 8300     
0082 6088 C820  54         mov   @mem.sams.set.legacy.code+2,@>8302
     608A 60A6     
     608C 8302     
0083 608E C820  54         mov   @mem.sams.set.legacy.code+4,@>8304
     6090 60A8     
     6092 8304     
0084 6094 C820  54         mov   @mem.sams.set.legacy.code+6,@>8306
     6096 60AA     
     6098 8306     
0085 609A C820  54         mov   @mem.sams.set.legacy.code+8,@>8308
     609C 60AC     
     609E 8308     
0086 60A0 0460  28         b     @>8300                ; Run code. Bye bye.
     60A2 8300     
0087                       ;-------------------------------------------------------
0088                       ; Assembly language code for returning to monitor
0089                       ;-------------------------------------------------------
0090               mem.sams.set.legacy.code:
0091 60A4 04E0             data  >04e0,bank0.rom       ; Activate bank 0
     60A6 6000     
0092 60A8 0420             data  >0420,>0000           ; blwp @0
     60AA 0000     
0093               
0094               
0095               
...
0308                       ;-------------------------------------------------------
0309 60D6 020C  20         li    r12,>1e00             ; SAMS CRU address
     60D8 1E00     
0310 60DA 1D00  20         sbo   0                     ; Enable access to SAMS registers
0311               
0312 60DC 1D01  20         sbo   1                     ; Enable SAMS mapper
...
0317                                                   ; / to change.
0318               
0319 60DE C020  34         mov   @tv.sams.2000,r0      ; \
     60E0 A200     
0320 60E2 06C0  14         swpb  r0                    ; | Set page for >2000 - >2fff
0321 60E4 C800  38         mov   r0,@>4004             ; /
     60E6 4004     
0322               
0323 60E8 C020  34         mov   @tv.sams.3000,r0      ; \
     60EA A202     
0324 60EC 06C0  14         swpb  r0                    ; | Set page for >3000 - >3fff
0325 60EE C800  38         mov   r0,@>4006             ; /
     60F0 4006     
0326               
0327 60F2 C020  34         mov   @tv.sams.a000,r0      ; \
     60F4 A204     
0328 60F6 06C0  14         swpb  r0                    ; | Set page for >a000 - >afff
0329 60F8 C800  38         mov   r0,@>4014             ; /
     60FA 4014     
0330               
0331 60FC C020  34         mov   @tv.sams.b000,r0      ; \
     60FE A206     
0332 6100 06C0  14         swpb  r0                    ; | Set page for >b000 - >bfff
0333 6102 C800  38         mov   r0,@>4016             ; /
     6104 4016     
0334               
0335 6106 C020  34         mov   @tv.sams.c000,r0      ; \
     6108 A208     
0336 610A 06C0  14         swpb  r0                    ; | Set page for >c000 - >cfff
0337 610C C800  38         mov   r0,@>4018             ; /
     610E 4018     
0338               
0339 6110 C020  34         mov   @tv.sams.d000,r0      ; \
     6112 A20A     
0340 6114 06C0  14         swpb  r0                    ; | Set page for >d000 - >dfff
0341 6116 C800  38         mov   r0,@>401a             ; /
     6118 401A     
0342               
0343 611A C020  34         mov   @tv.sams.e000,r0      ; \
     611C A20C     
0344 611E 06C0  14         swpb  r0                    ; | Set page for >e000 - >efff
0345 6120 C800  38         mov   r0,@>401c             ; /
     6122 401C     
0346               
0347 6124 C020  34         mov   @tv.sams.f000,r0      ; \
     6126 A20E     
0348 6128 06C0  14         swpb  r0                    ; | Set page for >f000 - >ffff
0349 612A C800  38         mov   r0,@>401e             ; /
     612C 401E     
0350               
0351 612E 1E00  20         sbz   0                     ; Disable access to SAMS registers
0352 6130 1D01  20         sbo   1                     ; Enable SAMS mapper
0353                       ;------------------------------------------------------
0354                       ; Exit
0355                       ;------------------------------------------------------
...
0060                       ;-----------------------------------------------------------------------
0061                       copy  "tibasic.session.asm" ; Run TI Basic session
     **** ****     > tibasic.session.asm
0001               * FILE......: tibasic.session.asm
...
0006               ***************************************************************
0007               * tibasic
0008               * Run TI Basic session
0009               ***************************************************************
0010               * bl   @tibasic
0011               *--------------------------------------------------------------
0012               * INPUT
0013               * @tibasic.session = TI Basic session to start/resume
0014               *
0015               * OUTPUT
0016               * none
0017               *--------------------------------------------------------------
0018               * Register usage
0019               * r1 in GPL WS, tmp0, tmp1, tmp2, r12
0020               *--------------------------------------------------------------
0021               * Remarks
0022               * tibasic >> b @0070 (GPL interpreter/TI Basic)
0023               *         >> isr
0024               *         >> tibasic.return
0025               *
0026               * Uses scratchpad memory
0027               * >83b4   Hide Flag/ISR counter for triggering SID display.
0028               * >83b6   TI Basic Session ID
0029               ********|*****|*********************|**************************
0030               tibasic:
0031 6134 0649  14         dect  stack
0032 6136 C64B  30         mov   r11,*stack            ; Save return address
0033 6138 0649  14         dect  stack
0034 613A C644  30         mov   tmp0,*stack           ; Push tmp0
0035 613C 0649  14         dect  stack
0036 613E C645  30         mov   tmp1,*stack           ; Push tmp1
0037 6140 0649  14         dect  stack
0038 6142 C646  30         mov   tmp2,*stack           ; Push tmp2
0039 6144 0649  14         dect  stack
0040 6146 C64C  30         mov   r12,*stack            ; Push r12
0041                       ;-------------------------------------------------------
0042                       ; Setup SAMS memory
0043                       ;-------------------------------------------------------
0044 6148 C802  38         mov   config,@tv.sp2.conf   ; Backup the SP2 config register
     614A A22C     
0045               
0046 614C 06A0  32         bl    @sams.layout.copy     ; Backup Stevie SAMS page layout
     614E 2652     
0047 6150 A200                   data tv.sams.2000     ; \ @i = target address of 8 words table
0048                                                   ; /      that contains SAMS layout
0049               
0050 6152 06A0  32         bl    @scroff               ; Turn off screen
     6154 269A     
0051               
0052 6156 06A0  32         bl    @mem.sams.set.external
     6158 60B2     
0053                                                   ; Load SAMS page layout (from cart space)
0054                                                   ; before running external program.
0055               
0056 615A 06A0  32         bl    @cpyv2m
     615C 24CC     
0057 615E 0000                   data >0000,>b000,16384
     6160 B000     
     6162 4000     
...
0062                       ;-------------------------------------------------------
0063 6164 06A0  32         bl    @f18rst               ; Reset and lock the F18A
     6166 27BA     
0064               
0065 6168 06A0  32         bl    @vidtab               ; Load video mode table into VDP
     616A 230C     
0066 616C 35DA                   data tibasic.32x24    ; Equate selected video mode table
0067                       ;-------------------------------------------------------
0068                       ; Keep 'Hide SID' flag for later use
0069                       ;-------------------------------------------------------
0070 616E C120  34         mov   @tibasic.hidesid,tmp0 ; \
     6170 A02C     
...
0075                                                   ; |
0076 6172 C804  38         mov   tmp0,@>83fc           ; | Also store a copy in the Stevie
     6174 83FC     
0077                                                   ; | scratchpad >83ff for later use in
0078                                                   ; / TI Basic scratchpad.
0079                       ;-------------------------------------------------------
0080                       ; Keep TI Basic session ID for later use
0081                       ;-------------------------------------------------------
0082 6176 C120  34         mov   @tibasic.session,tmp0 ; \
     6178 A02E     
0083                                                   ; | Store TI Basic session ID in tmp0.
0084                                                   ; | Througout the subroutine tmp0 will
0085                                                   ; | keep this value, even when SAMS
0086                                                   ; | banks are switched.
0087                                                   ; |
0088 617A C804  38         mov   tmp0,@>83fe           ; | Also store a copy in the Stevie
     617C 83FE     
0089                                                   ; | scratchpad >83ff for later use in
0090                                                   ; / TI Basic scratchpad.
0091                       ;-------------------------------------------------------
0092                       ; Switch for TI Basic session
0093                       ;-------------------------------------------------------
0094 617E 0284  22         ci    tmp0,1
     6180 0001     
0095 6182 1310  14         jeq   tibasic.init.basic1
0096 6184 0284  22         ci    tmp0,2
     6186 0002     
0097 6188 131E  14         jeq   tibasic.init.basic2
0098 618A 0284  22         ci    tmp0,3
     618C 0003     
0099 618E 132A  14         jeq   tibasic.init.basic3
0100 6190 0284  22         ci    tmp0,4
     6192 0004     
0101 6194 1336  14         jeq   tibasic.init.basic4
0102 6196 0284  22         ci    tmp0,5
     6198 0005     
0103 619A 1342  14         jeq   tibasic.init.basic5
0104                       ;-------------------------------------------------------
0105                       ; Assert, should never get here
0106                       ;-------------------------------------------------------
0107 619C C80B  38         mov   r11,@>ffce            ; \ Save caller address
     619E FFCE     
0108 61A0 06A0  32         bl    @cpu.crash            ; / Crash and halt system
     61A2 2026     
0109                       ;-------------------------------------------------------
0110                       ; New TI Basic session 1
0111                       ;-------------------------------------------------------
0112               tibasic.init.basic1:
0113 61A4 C160  34         mov   @tibasic1.status,tmp1 ; Resume TI Basic session?
     61A6 A030     
0114 61A8 1302  14         jeq   !                     ; No, new session
0115 61AA 0460  28         b     @tibasic.resume.basic1
     61AC 62AE     
0116 61AE 0265  22 !       ori   tmp1,1                ; \
     61B0 0001     
0117 61B2 C805  38         mov   tmp1,@tibasic1.status ; / Set resume flag for next run
     61B4 A030     
0118               
0119 61B6 06A0  32         bl    @mem.sams.set.basic1  ; \ Load SAMS page layout (from cart space)
     61B8 60B8     
0120                                                   ; / for TI Basic session 1
0121               
...
0125               
0126 61C4 103C  14         jmp   tibasic.init.rest     ; Continue initialisation
0127                       ;-------------------------------------------------------
0128                       ; New TI Basic session 2
0129                       ;-------------------------------------------------------
0130               tibasic.init.basic2:
0131 61C6 C160  34         mov   @tibasic2.status,tmp1 ; Resume TI Basic session?
     61C8 A032     
0132 61CA 1574  14         jgt   tibasic.resume.basic2 ; yes, do resume
0133 61CC 0265  22         ori   tmp1,1                ; \
     61CE 0001     
0134 61D0 C805  38         mov   tmp1,@tibasic2.status ; / Set resume flag for next run
     61D2 A032     
0135               
0136 61D4 06A0  32         bl    @mem.sams.set.basic2  ; \ Load SAMS page layout (from cart space)
     61D6 60BE     
0137                                                   ; / for TI Basic session 2
0138               
...
0142               
0143 61E2 102D  14         jmp   tibasic.init.rest     ; Continue initialisation
0144                       ;-------------------------------------------------------
0145                       ; New TI Basic session 3
0146                       ;-------------------------------------------------------
0147               tibasic.init.basic3:
0148 61E4 C160  34         mov   @tibasic3.status,tmp1 ; Resume TI Basic session?
     61E6 A034     
0149 61E8 1568  14         jgt   tibasic.resume.basic3 ; yes, do resume
0150 61EA 0265  22         ori   tmp1,1                ; \
     61EC 0001     
0151 61EE C805  38         mov   tmp1,@tibasic3.status ; / Set resume flag for next run
     61F0 A034     
0152               
0153 61F2 06A0  32         bl    @mem.sams.set.basic3  ; \ Load SAMS page layout (from cart space)
     61F4 60C4     
0154                                                   ; / for TI Basic session 3
0155               
...
0159               
0160 6200 101E  14         jmp   tibasic.init.rest     ; Continue initialisation
0161                       ;-------------------------------------------------------
0162                       ; New TI Basic session 4
0163                       ;-------------------------------------------------------
0164               tibasic.init.basic4:
0165 6202 C160  34         mov   @tibasic4.status,tmp1 ; Resume TI Basic session?
     6204 A036     
0166 6206 155C  14         jgt   tibasic.resume.basic4 ; yes, do resume
0167 6208 0265  22         ori   tmp1,1                ; \
     620A 0001     
0168 620C C805  38         mov   tmp1,@tibasic4.status ; / Set resume flag for next run
     620E A036     
0169               
0170 6210 06A0  32         bl    @mem.sams.set.basic4  ; \ Load SAMS page layout (from cart space)
     6212 60CA     
0171                                                   ; / for TI Basic session 4
0172               
...
0176               
0177 621E 100F  14         jmp   tibasic.init.rest     ; Continue initialisation
0178                       ;-------------------------------------------------------
0179                       ; New TI Basic session 5
0180                       ;-------------------------------------------------------
0181               tibasic.init.basic5:
0182 6220 C160  34         mov   @tibasic5.status,tmp1 ; Resume TI Basic session?
     6222 A038     
0183 6224 1550  14         jgt   tibasic.resume.basic5 ; yes, do resume
0184 6226 0265  22         ori   tmp1,1                ; \
     6228 0001     
0185 622A C805  38         mov   tmp1,@tibasic5.status ; / Set resume flag for next run
     622C A038     
0186               
0187 622E 06A0  32         bl    @mem.sams.set.basic5  ; \ Load SAMS page layout (from cart space)
     6230 60D0     
...
0193               
0194 623C 1000  14         jmp   tibasic.init.rest     ; Continue initialisation
0195                       ;-------------------------------------------------------
0196                       ; New TI Basic session (part 2)
0197                       ;-------------------------------------------------------
0198               tibasic.init.rest:
0199 623E 06A0  32         bl    @cpym2m
     6240 24EE     
0200 6242 7E00                   data cpu.scrpad.src,cpu.scrpad.tgt,256
     6244 F960     
     6246 0100     
0201                                                   ; Initialize scratchpad memory for TI Basic
0202                                                   ; @cpu.scrpad.tgt (SAMS bank) with dump
0203                                                   ; of OS Monitor scratchpad stored at
0204                                                   ; @cpu.scrpad.src (ROM bank 7).
0205               
0206 6248 06A0  32         bl    @ldfnt
     624A 2374     
0207 624C 0900                   data >0900,fnopt3     ; Load font (upper & lower case)
     624E 000C     
0208               
0209 6250 06A0  32         bl    @filv
     6252 22A2     
0210 6254 0300                   data >0300,>D0,2      ; No sprites
     6256 00D0     
     6258 0002     
0211               
...
0332               
0333 62F4 020C  20         li    r12,>1e00             ; \ Disable SAMS mapper (transparent mode)
     62F6 1E00     
0334 62F8 1E01  20         sbz   1                     ; /
0335                       ;-------------------------------------------------------
0336                       ; Resume TI Basic interpreter
0337                       ;-------------------------------------------------------
0338 62FA 0460  28         b     @>0ab8                ; Return from interrupt routine.
     62FC 0AB8     
0339                                                   ; See TI Intern page 32 (german)
0340                       ;-------------------------------------------------------
0341                       ; Required values for TI Basic scratchpad
0342                       ;-------------------------------------------------------
0343               tibasic.scrpad.83d4:
0344 62FE E0D5             data  >e0d5
0345               tibasic.scrpad.83fa:
0346 6300 9800             data  >9800
0347               tibasic.scrpad.83fc:
0348 6302 0108             data  >0108
0349               tibasic.scrpad.83fe:
0350 6304 8C02             data  >8c02
0351               
0352               
0353               
0354               
...
0357               ***************************************************************
0358               * isr
0359               * Interrupt Service Routine in TI Basic
0360               ***************************************************************
0361               * Called from console rom at >0ab6
0362               * See TI Intern page 32 (german) for details
0363               *--------------------------------------------------------------
0364               * OUTPUT
0365               * none
0366               *--------------------------------------------------------------
...
0369               ********|*****|*********************|**************************
0370               isr:
0371 6306 0300  24         limi  0                     ; \ Turn off interrupts
     6308 0000     
0372                                                   ; / Prevent ISR reentry
0373               
0374 630A C807  38         mov   r7,@rambuf            ; Backup R7
     630C A100     
0375 630E C80C  38         mov   r12,@rambuf+2         ; Backup R12
     6310 A102     
0376                       ;--------------------------------------------------------------
0377                       ; Exit ISR if TI-Basic is busy running a program
0378                       ;--------------------------------------------------------------
0379 6312 C1E0  34         mov   @>8344,r7             ; Busy running program?
     6314 8344     
0380 6316 0287  22         ci    r7,>0100
     6318 0100     
0381 631A 1601  14         jne   isr.showid            ; No, TI-Basic is in command line mode.
0382                       ;--------------------------------------------------------------
0383                       ; TI-Basic program running
0384                       ;--------------------------------------------------------------
0385 631C 1023  14         jmp   isr.exit              ; Exit
0386                       ;--------------------------------------------------------------
0387                       ; Show TI-Basic session ID ?
0388                       ;--------------------------------------------------------------
0389               isr.showid:
0390 631E C1E0  34         mov   @>83b4,r7             ; Get counter/Hide flag
     6320 83B4     
0391 6322 0287  22         ci    r7,>ffff              ; Hide flag set?
     6324 FFFF     
0392 6326 1317  14         jeq   isr.hotkey            ; Yes, skip showing session ID
0393 6328 0287  22         ci    r7,>0010              ; Counter limit reached ?
     632A 0010     
0394 632C 1112  14         jlt   isr.counter           ; Not yet, skip showing Session ID
0395 632E 04E0  34         clr   @>83b4                ; Reset counter
     6330 83B4     
0396                       ;--------------------------------------------------------------
0397                       ; Setup VDP write address for column 30
0398                       ;--------------------------------------------------------------
0399 6332 0207  20         li    r7,>401e              ; \
     6334 401E     
0400 6336 06C7  14         swpb  r7                    ; | >1c is the VDP column position
0401 6338 D807  38         movb  r7,@vdpa              ; | where bytes should be written
     633A 8C02     
0402 633C 06C7  14         swpb  r7                    ; |
0403 633E D807  38         movb  r7,@vdpa              ; /
     6340 8C02     
0404                       ;--------------------------------------------------------------
0405                       ; Dump TI-Basic Session ID to screen
0406                       ;--------------------------------------------------------------
0407 6342 0207  20         li    r7,>83df              ; Char '#' and char >df
     6344 83DF     
0408 6346 D807  38         movb  r7,@vdpw              ; Write byte
     6348 8C00     
0409 634A 06C7  14         swpb  r7
0410 634C D807  38         movb  r7,@vdpw              ; Write byte
     634E 8C00     
0411 6350 1002  14         jmp   isr.hotkey
0412                       ;-------------------------------------------------------
0413                       ; Increase counter
0414                       ;-------------------------------------------------------
0415               isr.counter:
0416 6352 05A0  34         inc   @>83b4                ; Increase counter
     6354 83B4     
0417                       ;-------------------------------------------------------
0418                       ; Hotkey pressed?
0419                       ;-------------------------------------------------------
0420               isr.hotkey:
0421 6356 C1E0  34         mov   @>8374,r7             ; \ Get keyboard scancode from @>8375
     6358 8374     
0422 635A 0247  22         andi  r7,>00ff              ; / LSB only
     635C 00FF     
0423 635E 0287  22         ci    r7,>0f                ; Hotkey fctn + '9' pressed?
     6360 000F     
0424 6362 1305  14         jeq   tibasic.return        ; Yes, return to Stevie
0425                       ;-------------------------------------------------------
0426                       ; Return from ISR
0427                       ;-------------------------------------------------------
0428               isr.exit:
0429 6364 C320  34         mov   @rambuf+2,r12         ; Restore R12
     6366 A102     
0430 6368 C1E0  34         mov   @rambuf,r7            ; Restore R7
     636A A100     
0431 636C 045B  20         b     *r11                  ; Return from ISR
0432               
0433               
0434               ***************************************************************
0435               * tibasic.return
0436               * Return from TI Basic to Stevie
0437               ***************************************************************
0438               * bl   @tibasic.return
0439               *--------------------------------------------------------------
0440               * OUTPUT
0441               * none
0442               *--------------------------------------------------------------
0443               * Register usage
0444               * r1 in GPL WS, tmp0, tmp1
0445               *--------------------------------------------------------------
0446               * REMARKS
0447               * Called from ISR code
0448               ********|*****|*********************|**************************
0449               tibasic.return:
0450 636E 020C  20         li    r12,>1e00             ; \ Enable SAMS mapper again
     6370 1E00     
0451 6372 1D01  20         sbo   1                     ; | We stil have the SAMS banks layout
0452                                                   ; / mem.sams.layout.external
0453               
0454 6374 02E0  18         lwpi  cpu.scrpad.moved      ; Activate Stevie workspace that got
     6376 AD00     
0455                                                   ; paged out in tibasic.init
0456               
0457 6378 D820  54         movb  @w$ffff,@>8375        ; Reset keycode
     637A 2022     
     637C 8375     
0458               
0459 637E 06A0  32         bl    @cpym2m
     6380 24EE     
0460 6382 8300                   data >8300,cpu.scrpad.tgt,256
     6384 F960     
     6386 0100     
0461                                                   ; Backup TI Basic scratchpad to
0462                                                   ; @cpu.scrpad.tgt (SAMS bank)
0463               
0464 6388 06A0  32         bl    @cpu.scrpad.pgin      ; \ Page in copy of Stevie scratch pad memory
     638A 2B8E     
0465 638C AD00                   data cpu.scrpad.moved ; | and activate workspace at >8300
0466                                                   ; / Destroys registers tmp0-tmp2
0467               
0468 638E C0A0  34         mov   @tv.sp2.conf,config   ; Restore the SP2 config register
     6390 A22C     
0469               
0470 6392 06A0  32         bl    @mute                 ; Mute sound generators
     6394 280E     
0471                       ;-------------------------------------------------------
0472                       ; Cleanup after return from TI Basic
0473                       ;-------------------------------------------------------
0474 6396 06A0  32         bl    @scroff               ; Turn screen off
     6398 269A     
0475 639A 06A0  32         bl    @cpyv2m
     639C 24CC     
0476 639E 0000                   data >0000,>b000,16384
     63A0 B000     
     63A2 4000     
0477                                                   ; Dump TI Basic 16K VDP memory to ram buffer
0478                                                   ; >b000->efff
0479                       ;-------------------------------------------------------
0480                       ; Restore VDP screen with Stevie content
0481                       ;-------------------------------------------------------
0482               tibasic.return.stevie:
0483 63A4 06A0  32         bl    @mem.sams.set.external
     63A6 60B2     
0484                                                   ; Load SAMS page layout when returning from
0485                                                   ; external program.
0486               
0487 63A8 06A0  32         bl    @cpym2v
     63AA 249A     
0488 63AC 0000                   data >0000,>b000,16384
     63AE B000     
     63B0 4000     
0489                                                   ; Restore Stevie 16K to VDP from RAM buffer
0490                                                   ; >b000->efff
0491                       ;-------------------------------------------------------
0492                       ; Restore SAMS memory layout for editor buffer and index
0493                       ;-------------------------------------------------------
0494 63B2 06A0  32         bl    @mem.sams.set.stevie  ; Setup SAMS memory banks for stevie
     63B4 60D6     
0495                                                   ; \ For this to work the bank having the
0496                                                   ; | @tv.sams.xxxx variables must already
0497                                                   ; | be active and may not switch to
0498                                                   ; / another bank.
0499                       ;-------------------------------------------------------
0500                       ; Setup F18a 80x30 mode again
0501                       ;-------------------------------------------------------
0502 63B6 06A0  32         bl    @f18unl               ; Unlock the F18a
     63B8 273E     
0509               
0510 63BA 06A0  32         bl    @vidtab               ; Load video mode table into VDP
     63BC 230C     
0511 63BE 35D0                   data stevie.80x30     ; Equate selected video mode table
0512               
0513 63C0 06A0  32         bl    @putvr                ; Turn on position based attributes
     63C2 2346     
0514 63C4 3202                   data >3202            ; F18a VR50 (>32), bit 2
0515               
0516 63C6 06A0  32         bl    @putvr                ; Set VDP TAT base address for position
     63C8 2346     
0517 63CA 0360                   data >0360            ; based attributes (>40 * >60 = >1800)
0518               
0519 63CC 04E0  34         clr   @parm1                ; Screen off while reloading color scheme
     63CE A000     
0520 63D0 04E0  34         clr   @parm2                ; Don't skip colorizing marked lines
     63D2 A002     
0521 63D4 04E0  34         clr   @parm3                ; Colorize all panes
     63D6 A004     
0522               
0523 63D8 06A0  32         bl    @pane.action.colorscheme.load
     63DA 6410     
0524                                                   ; Reload color scheme
0525                                                   ; \ i  @parm1 = Skip screen off if >FFFF
0526                                                   ; | i  @parm2 = Skip colorizing marked lines
0527                                                   ; |             if >FFFF
0528                                                   ; | i  @parm3 = Only colorize CMDB pane
0529                                                   ; /             if >FFFF
0530                       ;------------------------------------------------------
0531                       ; Exit
0532                       ;------------------------------------------------------
0533               tibasic.return.exit:
0534 63DC C339  30         mov   *stack+,r12           ; Pop r12
0535 63DE C1B9  30         mov   *stack+,tmp2          ; Pop tmp2
0536 63E0 C179  30         mov   *stack+,tmp1          ; Pop tmp1
0537 63E2 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0538 63E4 C2F9  30         mov   *stack+,r11           ; Pop r11
0539 63E6 045B  20         b     *r11                  ; Return
0540               
0541               
0542               
0543               tibasic.patterns:
0544 63E8 007E             byte  >00,>7E,>E7,>C7,>E7,>E7,>C3,>7E ; 1
     63EA E7C7     
     63EC E7E7     
     63EE C37E     
0545 63F0 007E             byte  >00,>7E,>C3,>F3,>C3,>CF,>C3,>7E ; 2
     63F2 C3F3     
     63F4 C3CF     
     63F6 C37E     
0546 63F8 007E             byte  >00,>7E,>C3,>F3,>C3,>F3,>C3,>7E ; 3
     63FA C3F3     
     63FC C3F3     
     63FE C37E     
0547 6400 007E             byte  >00,>7E,>D3,>D3,>C3,>F3,>F3,>7E ; 4
     6402 D3D3     
     6404 C3F3     
     6406 F37E     
0548 6408 007E             byte  >00,>7E,>C3,>CF,>C3,>F3,>C3,>7E ; 5
     640A C3CF     
     640C C3F3     
     640E C37E     
                   < stevie_b7.asm.30981
...
0063                       ; Stubs
0064                       ;-----------------------------------------------------------------------
0065                       copy  "rom.stubs.bank7.asm" ; Bank specific stubs
     **** ****     > rom.stubs.bank7.asm
0001               * FILE......: rom.stubs.bank7.asm
0002               * Purpose...: Bank 7 stubs for functions in other banks
0003               
0004               ***************************************************************
0005               * Stub for "pane.action.colorscheme.load"
0006               * bank1 vec.31
0007               ********|*****|*********************|**************************
0008               pane.action.colorscheme.load:
0009 6410 0649  14         dect  stack
0010 6412 C64B  30         mov   r11,*stack            ; Save return address
0011                       ;------------------------------------------------------
0012                       ; Call function in bank 1
0013                       ;------------------------------------------------------
0014 6414 06A0  32         bl    @rom.farjump          ; \ Trampoline jump to bank
     6416 2F88     
0015 6418 6002                   data bank1.rom        ; | i  p0 = bank address
0016 641A 7FFC                   data vec.31           ; | i  p1 = Vector with target address
0017 641C 600E                   data bankid           ; / i  p2 = Source ROM bank for return
0018                       ;------------------------------------------------------
0019                       ; Exit
0020                       ;------------------------------------------------------
0021 641E C2F9  30         mov   *stack+,r11           ; Pop r11
0022 6420 045B  20         b     *r11                  ; Return to caller
                   < stevie_b7.asm.30981
...
0013 6422 0649  14         dect  stack
0014 6424 C64B  30         mov   r11,*stack            ; Save return address
0015                       ;------------------------------------------------------
0016                       ; Call function in bank 1
0017                       ;------------------------------------------------------
0018 6426 06A0  32         bl    @rom.farjump          ; \ Trampoline jump to bank
     6428 2F88     
0019 642A 6002                   data bank1.rom        ; | i  p0 = bank address
0020 642C 7FC0                   data vec.1            ; | i  p1 = Vector with target address
0021 642E 600E                   data bankid           ; / i  p2 = Source ROM bank for return
0022                       ;------------------------------------------------------
0023                       ; Exit
0024                       ;------------------------------------------------------
0025 6430 C2F9  30         mov   *stack+,r11           ; Pop r11
0026 6432 045B  20         b     *r11                  ; Return to caller
0027               
0029               
0030               
0031               ***************************************************************
...
0068                       ; Program data
0069                       ;-----------------------------------------------------------------------
0070                       copy  "data.sams.layout.asm"; SAMS bank layout for multi-purpose
...
0009 6434 0200             data  >0200                 ; >2000-2fff, SAMS page >02
0010 6436 0300             data  >0300                 ; >3000-3fff, SAMS page >03
0011 6438 0A00             data  >0a00                 ; >a000-afff, SAMS page >0a
0012 643A 0B00             data  >0b00                 ; >b000-bfff, SAMS page >0b
0013 643C 0C00             data  >0c00                 ; >c000-cfff, SAMS page >0c
0014 643E 0D00             data  >0d00                 ; >d000-dfff, SAMS page >0d
0015 6440 0E00             data  >0e00                 ; >e000-efff, SAMS page >0e
0016 6442 0F00             data  >0f00                 ; >f000-ffff, SAMS page >0f
0017               
0018               
0019               ***************************************************************
...
0023 6444 0000             data  >0000                 ; >2000-2fff, SAMS page >00
0024 6446 0100             data  >0100                 ; >3000-3fff, SAMS page >01
0025 6448 0400             data  >0400                 ; >a000-afff, SAMS page >04
0026 644A 2000             data  >2000                 ; >b000-bfff, SAMS page >20
0027                                                   ; \
0028                                                   ; | Index can allocate
0029                                                   ; | pages >20 to >3f.
0030                                                   ; /
0031 644C 4000             data  >4000                 ; >c000-cfff, SAMS page >40
0032                                                   ; \
0033                                                   ; | Editor buffer can allocate
0034                                                   ; | pages >40 to >ff.
0035                                                   ; /
0036 644E 0500             data  >0500                 ; >d000-dfff, SAMS page >05
0037 6450 0600             data  >0600                 ; >e000-efff, SAMS page >06
0038 6452 0700             data  >0700                 ; >f000-ffff, SAMS page >07
0039               
0040               
0041               
0042               ***************************************************************
...
0046 6454 0000             data  >0000                 ; >2000-2fff, SAMS page >00
0047 6456 0100             data  >0100                 ; >3000-3fff, SAMS page >01
0048 6458 0400             data  >0400                 ; >a000-afff, SAMS page >04
0049               
0050 645A 1000             data  >1000                 ; >b000-efff, SAMS page >10
0051 645C 1100             data  >1100                 ; \
0052 645E 1200             data  >1200                 ; | Stevie session
0053 6460 1300             data  >1300                 ; | VDP content
0054                                                   ; /
0055 6462 0700             data  >0700                 ; >f000-ffff, SAMS page >07
0056               
0057               
0058               ***************************************************************
...
0062 6464 0000             data  >0000                 ; >2000-2fff, SAMS page >00
0063 6466 0100             data  >0100                 ; >3000-3fff, SAMS page >01
0064 6468 0400             data  >0400                 ; >a000-afff, SAMS page >04
0065               
0066 646A E700             data  >e700                 ; >b000-efff, SAMS page >e7
0067 646C E800             data  >e800                 ; \
0068 646E E900             data  >e900                 ; | TI Basic session 1
0069 6470 EA00             data  >ea00                 ; | VDP content
0070                                                   ; /
0071 6472 EB00             data  >eb00                 ; >f000-ffff, SAMS page >eb
0072               
0073               
0074               ***************************************************************
...
0078 6474 0000             data  >0000                 ; >2000-2fff, SAMS page >00
0079 6476 0100             data  >0100                 ; >3000-3fff, SAMS page >01
0080 6478 0400             data  >0400                 ; >a000-afff, SAMS page >04
0081               
0082 647A EC00             data  >ec00                 ; >b000-efff, SAMS page >ec
0083 647C ED00             data  >ed00                 ; \
0084 647E EE00             data  >ee00                 ; | TI Basic session 2
0085 6480 EF00             data  >ef00                 ; | VDP content
0086                                                   ; /
0087 6482 F000             data  >f000                 ; >f000-ffff, SAMS page >f0
0088               
0089               
0090               ***************************************************************
...
0094 6484 0000             data  >0000                 ; >2000-2fff, SAMS page >00
0095 6486 0100             data  >0100                 ; >3000-3fff, SAMS page >01
0096 6488 0400             data  >0400                 ; >a000-afff, SAMS page >04
0097               
0098 648A F100             data  >f100                 ; >b000-efff, SAMS page >f1
0099 648C F200             data  >f200                 ; \
0100 648E F300             data  >f300                 ; | TI Basic session 3
0101 6490 F400             data  >f400                 ; | VDP content
0102                                                   ; /
0103 6492 F500             data  >f500                 ; >f000-ffff, SAMS page >f5
0104               
0105               
0106               ***************************************************************
...
0110 6494 0000             data  >0000                 ; >2000-2fff, SAMS page >00
0111 6496 0100             data  >0100                 ; >3000-3fff, SAMS page >01
0112 6498 0400             data  >0400                 ; >a000-afff, SAMS page >04
0113               
0114 649A F600             data  >f600                 ; >b000-efff, SAMS page >f6
0115 649C F700             data  >f700                 ; \
0116 649E F800             data  >f800                 ; | TI Basic session 4
0117 64A0 F900             data  >f900                 ; | VDP content
0118                                                   ; /
0119 64A2 FA00             data  >fa00                 ; >f000-ffff, SAMS page >fa
0120               
0121               
0122               ***************************************************************
...
0126 64A4 0000             data  >0000                 ; >2000-2fff, SAMS page >00
0127 64A6 0100             data  >0100                 ; >3000-3fff, SAMS page >01
0128 64A8 0400             data  >0400                 ; >a000-afff, SAMS page >04
0129               
0130 64AA FB00             data  >fb00                 ; >b000-efff, SAMS page >fc
0131 64AC FC00             data  >fc00                 ; \
0132 64AE FD00             data  >fd00                 ; | TI Basic session 5
0133 64B0 FE00             data  >fe00                 ; | VDP content
0134                                                   ; /
0135 64B2 FF00             data  >ff00                 ; >f000-ffff, SAMS page >ff
                   < stevie_b7.asm.30981
0071                       ;-----------------------------------------------------------------------
0072                       ; Bank full check
0073                       ;-----------------------------------------------------------------------
0077                       ;-----------------------------------------------------------------------
0078                       ; Show ROM bank in CPU crash screen
0079                       ;-----------------------------------------------------------------------
0080               cpu.crash.showbank:
0081                       aorg >7fb0
0082 7FB0 06A0  32         bl    @putat
     7FB2 2456     
0083 7FB4 0314                   byte 3,20
0084 7FB6 7FBA                   data cpu.crash.showbank.bankstr
0085 7FB8 10FF  14         jmp   $
0086               cpu.crash.showbank.bankstr:
0087               
0088 7FBA 05               byte  5
0089 7FBB   52             text  'ROM#7'
     7FBC 4F4D     
     7FBE 2337     
0090                       even
0091               
0092                       ;-----------------------------------------------------------------------
0093                       ; Scratchpad memory dump
0094                       ;-----------------------------------------------------------------------
0095                       aorg >7e00
0096                       copy  "data.scrpad.asm"     ; Required for TI Basic
     **** ****     > data.scrpad.asm
...
0098                       ; Vector table
0099                       ;-----------------------------------------------------------------------
0100                       aorg  bankx.vectab
0101                       copy  "rom.vectors.bank7.asm"
...
0006               *--------------------------------------------------------------
0007 7FC0 6074     vec.1   data  mem.sams.set.legacy   ;
0008 7FC2 60AC     vec.2   data  mem.sams.set.boot     ;
0009 7FC4 60D6     vec.3   data  mem.sams.set.stevie   ;
0010 7FC6 60B2     vec.4   data  mem.sams.set.external ;
0011 7FC8 60B8     vec.5   data  mem.sams.set.basic1   ;
0012 7FCA 60BE     vec.6   data  mem.sams.set.basic2   ;
0013 7FCC 60C4     vec.7   data  mem.sams.set.basic3   ;
0014 7FCE 60CA     vec.8   data  mem.sams.set.basic4   ;
0015 7FD0 60D0     vec.9   data  mem.sams.set.basic5   ;
0016 7FD2 6134     vec.10  data  tibasic               ;
...
0109      35D0     spvmod  equ   stevie.80x30          ; Video mode.   See VIDTAB for details.
