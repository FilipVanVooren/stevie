...
0008               ***************************************************************
0009               * File: stevie_b7.asm               ; Version 210912-1053093
0010               *
0011               * Bank 7 "Jonas"
0012               *
0013               ***************************************************************
...
0260 621D ....             text  'Build-ID  210912-1053093'
0261                       even
0262               
**** **** ****     > runlib.asm
...
0037 6CA2 C17B  30         mov   *r11+,tmp1            ; tmp1 = Memory target address
0038                       ;------------------------------------------------------
0039                       ; Copy scratchpad memory to destination
0040                       ;------------------------------------------------------
0041               xcpu.scrpad.pgout:
0042 6CA4 0204  20         li    tmp0,>8300            ; tmp0 = Memory source address
     6CA6 8300 
0043 6CA8 C1C5  18         mov   tmp1,tmp3             ; tmp3 = copy of tmp1
0044 6CAA 0206  20         li    tmp2,16               ; tmp2 = 256/16
     6CAC 0010 
0045                       ;------------------------------------------------------
0046                       ; Copy memory
0047                       ;------------------------------------------------------
0048 6CAE CD74  46 !       mov   *tmp0+,*tmp1+         ; Copy word 1
0049 6CB0 CD74  46         mov   *tmp0+,*tmp1+         ; Copy word 2
0050 6CB2 CD74  46         mov   *tmp0+,*tmp1+         ; Copy word 3
0051 6CB4 CD74  46         mov   *tmp0+,*tmp1+         ; Copy word 4
0052 6CB6 CD74  46         mov   *tmp0+,*tmp1+         ; Copy word 5
0053 6CB8 CD74  46         mov   *tmp0+,*tmp1+         ; Copy word 6
0054 6CBA CD74  46         mov   *tmp0+,*tmp1+         ; Copy word 7
0055 6CBC CD74  46         mov   *tmp0+,*tmp1+         ; Copy word 8
0056 6CBE 0606  14         dec   tmp2
0057 6CC0 16F6  14         jne   -!                    ; Loop until done
0058                       ;------------------------------------------------------
0059                       ; Switch to new workspace
0060                       ;------------------------------------------------------
0061 6CC2 C347  18         mov   tmp3,r13              ; R13=WP   (pop tmp1 from stack)
0062 6CC4 020E  20         li    r14,cpu.scrpad.pgout.after.rtwp
     6CC6 6CCC 
0063                                                   ; R14=PC
0064 6CC8 04CF  14         clr   r15                   ; R15=STATUS
0065                       ;------------------------------------------------------
...
0069                       ;------------------------------------------------------
0070 6CCA 0380  18         rtwp                        ; Activate copied workspace
0071                                                   ; in non-scratchpad memory!
0072               
0073               cpu.scrpad.pgout.after.rtwp:
0074 6CCC 0460  28         b     @cpu.scrpad.restore   ; Restore scratchpad memory
     6CCE 6C6C 
...
0080 6CD0 045B  20         b     *r11                  ; Return to caller
0081               
0082               
0083               ***************************************************************
...
0103 6CD2 C13B  30         mov   *r11+,tmp0            ; tmp0 = Memory source address
0104                       ;------------------------------------------------------
0105                       ; Copy scratchpad memory to destination
0106                       ;------------------------------------------------------
0107               xcpu.scrpad.pgin:
0108 6CD4 0205  20         li    tmp1,>8300            ; tmp1 = Memory destination address
     6CD6 8300 
0109 6CD8 0206  20         li    tmp2,16               ; tmp2 = 256/16
     6CDA 0010 
0110                       ;------------------------------------------------------
0111                       ; Copy memory
0112                       ;------------------------------------------------------
0113 6CDC CD74  46 !       mov   *tmp0+,*tmp1+         ; Copy word 1
0114 6CDE CD74  46         mov   *tmp0+,*tmp1+         ; Copy word 2
0115 6CE0 CD74  46         mov   *tmp0+,*tmp1+         ; Copy word 3
0116 6CE2 CD74  46         mov   *tmp0+,*tmp1+         ; Copy word 4
0117 6CE4 CD74  46         mov   *tmp0+,*tmp1+         ; Copy word 5
0118 6CE6 CD74  46         mov   *tmp0+,*tmp1+         ; Copy word 6
0119 6CE8 CD74  46         mov   *tmp0+,*tmp1+         ; Copy word 7
0120 6CEA CD74  46         mov   *tmp0+,*tmp1+         ; Copy word 8
0121 6CEC 0606  14         dec   tmp2
0122 6CEE 16F6  14         jne   -!                    ; Loop until done
0123                       ;------------------------------------------------------
0124                       ; Switch workspace to scratchpad memory
0125                       ;------------------------------------------------------
0126 6CF0 02E0  18         lwpi  >8300                 ; Activate copied workspace
     6CF2 8300 
0127                       ;------------------------------------------------------
0128                       ; Exit
0129                       ;------------------------------------------------------
0130               cpu.scrpad.pgin.exit:
0131 6CF4 045B  20         b     *r11                  ; Return to caller
**** **** ****     > runlib.asm
0217               
...
0057 6CF8 6CFA             data  dsrlnk.init           ; Entry point
0058                       ;------------------------------------------------------
...
0310 6E24 6E26             data  dsrlnk.reuse.init     ; entry point
0311                       ;------------------------------------------------------
...
0020      6F20     hookok  equ   tmgr1                 ; Exit point for user hook
0021               
0022               
0023               ***************************************************************
...
0351 70B2 06A0  32 runlic  bl    @vidtab               ; Load video mode table into VDP
     70B4 6350 
0352 70B6 70DA             data  spvmod                ; Equate selected video mode table
...
0031               ***************************************************************
0032               stevie.tx8030:
0033 70DA 04F0             byte  >04,>f0,>00,>3f,>02,>43,>05,SPFCLR,0,80
     70DC 003F 
     70DE 0243 
     70E0 05F4 
     70E2 0050 
0034               
0035               romsat:
0036 70E4 0000             data  >0000,>0201             ; Cursor YX, initial shape and colour
     70E6 0201 
0037 70E8 0000             data  >0000,>0301             ; Current line indicator
     70EA 0301 
0038 70EC 0820             data  >0820,>0401             ; Current line indicator
     70EE 0401 
0039               nosprite:
0040 70F0 D000             data  >d000                   ; End-of-Sprites list
0041               
0042               
0043               ***************************************************************
...
0047 70F2 2000             data  >2000,>0002           ; >2000-2fff, SAMS page >02
     70F4 0002 
0048 70F6 3000             data  >3000,>0003           ; >3000-3fff, SAMS page >03
     70F8 0003 
0049 70FA A000             data  >a000,>000a           ; >a000-afff, SAMS page >0a
     70FC 000A 
0050               
0051 70FE B000             data  >b000,>0010           ; >b000-bfff, SAMS page >10
     7100 0010 
0052                                                   ; \ The index can allocate
0053                                                   ; / pages >10 to >2f.
0054               
0055 7102 C000             data  >c000,>0030           ; >c000-cfff, SAMS page >30
     7104 0030 
0056                                                   ; \ Editor buffer can allocate
0057                                                   ; / pages >30 to >ff.
0058               
0059 7106 D000             data  >d000,>000d           ; >d000-dfff, SAMS page >0d
     7108 000D 
0060 710A E000             data  >e000,>000e           ; >e000-efff, SAMS page >0e
     710C 000E 
0061 710E F000             data  >f000,>000f           ; >f000-ffff, SAMS page >0f
     7110 000F 
0062               
0063               
0064               
...
0117 7112 F417             data  >f417,>f171,>1b1f,>71b1 ; 1  White on blue with cyan touch
     7114 F171 
     7116 1B1F 
     7118 71B1 
0118 711A A11A             data  >a11a,>f0ff,>1f1a,>f1ff ; 2  Dark yellow on black
     711C F0FF 
     711E 1F1A 
     7120 F1FF 
0119 7122 2112             data  >2112,>f0ff,>1f12,>f1f6 ; 3  Dark green on black
     7124 F0FF 
     7126 1F12 
     7128 F1F6 
0120 712A F41F             data  >f41f,>1e11,>1a17,>1e11 ; 4  White on blue
     712C 1E11 
     712E 1A17 
     7130 1E11 
0121 7132 E11E             data  >e11e,>e1ff,>1f1e,>e1ff ; 5  Grey on black
     7134 E1FF 
     7136 1F1E 
     7138 E1FF 
0122 713A 1771             data  >1771,>1016,>1b71,>1711 ; 6  Black on cyan
     713C 1016 
     713E 1B71 
     7140 1711 
0123 7142 1FF1             data  >1ff1,>1011,>f1f1,>1f11 ; 7  Black on white
     7144 1011 
     7146 F1F1 
     7148 1F11 
0124 714A 1AF1             data  >1af1,>a1ff,>1f1f,>f11f ; 8  Black on dark yellow
     714C A1FF 
     714E 1F1F 
     7150 F11F 
0125 7152 21F0             data  >21f0,>12ff,>1b12,>12ff ; 9  Dark green on black
     7154 12FF 
     7156 1B12 
     7158 12FF 
0126 715A F5F1             data  >f5f1,>e1ff,>1b1f,>f131 ; 10 White on light blue
     715C E1FF 
     715E 1B1F 
     7160 F131 
0127                       even
0128               
0129               tv.tabs.table:
0130 7162 0007             byte  0,7,12,25               ; \   Default tab positions as used
     7164 0C19 
0131 7166 1E2D             byte  30,45,59,79             ; |   in Editor/Assembler module.
     7168 3B4F 
0132 716A FF00             byte  >ff,0,0,0               ; |
     716C 0000 
0133 716E 0000             byte  0,0,0,0                 ; |   Up to 20 positions supported.
     7170 0000 
0134 7172 0000             byte  0,0,0,0                 ; /   >ff means end-of-list.
     7174 0000 
0135                       even
**** **** ****     > stevie_b7.asm.1053093
0045                       ;-----------------------------------------------------------------------
...
0067      70DA     spvmod  equ   stevie.tx8030         ; Video mode.   See VIDTAB for details.
