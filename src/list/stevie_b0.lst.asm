...
0008               ***************************************************************
0009               * File: stevie_b0.asm               ; Version 201115-30943
0010               
...
0007               *              (c)2018-2020 // Filip van Vooren
0008               ***************************************************************
0009               * File: equates.equ                 ; Version 201115-30943
0010               *--------------------------------------------------------------
...
0260 6257 ....             text  'Build-ID  201115-30943'
0261                       even
0262               
**** **** ****     > runlib.asm
...
0091               ********|*****|*********************|**************************
0092               trimnum:
0093 6A76 C13B  30         mov   *r11+,tmp0            ; Get pointer to input string
0094 6A78 C17B  30         mov   *r11+,tmp1            ; Get pointer to output string
0095 6A7A C1BB  30         mov   *r11+,tmp2            ; Get padding character
0096 6A7C 06C6  14         swpb  tmp2                  ; LO <-> HI
0097 6A7E 0207  20         li    tmp3,5                ; Set counter
     6A80 0005 
0098                       ;------------------------------------------------------
0099                       ; Scan for padding character from left to right
0100                       ;------------------------------------------------------:
0101               trimnum_scan:
0102 6A82 9194  26         cb    *tmp0,tmp2            ; Matches padding character ?
0103 6A84 1604  14         jne   trimnum_setlen        ; No, exit loop
0104 6A86 0584  14         inc   tmp0                  ; Next character
0105 6A88 0607  14         dec   tmp3                  ; Last digit reached ?
0106 6A8A 1301  14         jeq   trimnum_setlen        ; yes, exit loop
0107 6A8C 10FA  14         jmp   trimnum_scan
0108                       ;------------------------------------------------------
0109                       ; Scan completed, set length byte new string
0110                       ;------------------------------------------------------
0111               trimnum_setlen:
0112 6A8E 06C7  14         swpb  tmp3                  ; LO <-> HI
0113 6A90 DD47  32         movb  tmp3,*tmp1+           ; Update string-length in work buffer
0114 6A92 06C7  14         swpb  tmp3                  ; LO <-> HI
0115                       ;------------------------------------------------------
0116                       ; Start filling new string
0117                       ;------------------------------------------------------
0118               trimnum_fill
0119 6A94 DD74  42         movb  *tmp0+,*tmp1+         ; Copy character
0120 6A96 0607  14         dec   tmp3                  ; Last character ?
0121 6A98 16FD  14         jne   trimnum_fill          ; Not yet, repeat
0122 6A9A 045B  20         b     *r11                  ; Return
0123               
0124               
0125               
0126               
...
0138               ********|*****|*********************|**************************
0139 6A9C C83B  50 putnum  mov   *r11+,@wyx            ; Set cursor
     6A9E 832A 
0140 6AA0 0262  22         ori   config,>8000          ; CONFIG register bit 0=1
     6AA2 8000 
0141 6AA4 10BC  14         jmp   mknum                 ; Convert number and display
**** **** ****     > runlib.asm
0187               
0191               
0195               
...
0350 6F16 06A0  32 runlic  bl    @vidtab               ; Load video mode table into VDP
     6F18 2304 
0351 6F1A 3276             data  spvmod                ; Equate selected video mode table
...
0018               ********|*****|*********************|**************************
0019               fb.init:
0020 707C 0649  14         dect  stack
0021 707E C64B  30         mov   r11,*stack            ; Save return address
0022                       ;------------------------------------------------------
0023                       ; Initialize
0024                       ;------------------------------------------------------
0025 7080 0204  20         li    tmp0,fb.top
     7082 A600 
0026 7084 C804  38         mov   tmp0,@fb.top.ptr      ; Set pointer to framebuffer
     7086 A100 
0027 7088 04E0  34         clr   @fb.topline           ; Top line in framebuffer
     708A A104 
0028 708C 04E0  34         clr   @fb.row               ; Current row=0
     708E A106 
0029 7090 04E0  34         clr   @fb.column            ; Current column=0
     7092 A10C 
0030               
0031 7094 0204  20         li    tmp0,colrow
     7096 0050 
0032 7098 C804  38         mov   tmp0,@fb.colsline     ; Columns per row=80
     709A A10E 
0033               
0034 709C 0204  20         li    tmp0,29
     709E 001D 
0035 70A0 C804  38         mov   tmp0,@fb.scrrows      ; Physical rows on screen = 29
     70A2 A118 
0036 70A4 C804  38         mov   tmp0,@fb.scrrows.max  ; Maximum number of physical rows for fb
     70A6 A11A 
0037               
0038 70A8 04E0  34         clr   @tv.pane.focus        ; Frame buffer has focus!
     70AA A01C 
0039 70AC 0720  34         seto  @fb.dirty             ; Set dirty flag (trigger screen update)
     70AE A116 
0040                       ;------------------------------------------------------
0041                       ; Clear frame buffer
0042                       ;------------------------------------------------------
0043 70B0 06A0  32         bl    @film
     70B2 2242 
0044 70B4 A600             data  fb.top,>00,fb.size    ; Clear it all the way
     70B6 0000 
     70B8 0960 
0045                       ;------------------------------------------------------
0046                       ; Exit
...
0044               ********|*****|*********************|**************************
0045               idx.init:
0046 70BE 0649  14         dect  stack
0047 70C0 C64B  30         mov   r11,*stack            ; Save return address
0048 70C2 0649  14         dect  stack
0049 70C4 C644  30         mov   tmp0,*stack           ; Push tmp0
0050                       ;------------------------------------------------------
0051                       ; Initialize
0052                       ;------------------------------------------------------
0053 70C6 0204  20         li    tmp0,idx.top
     70C8 B000 
0054 70CA C804  38         mov   tmp0,@edb.index.ptr   ; Set pointer to index in editor structure
     70CC A202 
0055               
0056 70CE C120  34         mov   @tv.sams.b000,tmp0
     70D0 A006 
0057 70D2 C804  38         mov   tmp0,@idx.sams.page   ; Set current SAMS page
     70D4 A500 
0058 70D6 C804  38         mov   tmp0,@idx.sams.lopage ; Set 1st SAMS page
     70D8 A502 
0059 70DA C804  38         mov   tmp0,@idx.sams.hipage ; Set last SAMS page
     70DC A504 
0060                       ;------------------------------------------------------
0061                       ; Clear index page
0062                       ;------------------------------------------------------
0063 70DE 06A0  32         bl    @film
     70E0 2242 
0064 70E2 B000                   data idx.top,>00,idx.size
     70E4 0000 
     70E6 1000 
0065                                                   ; Clear index
0066                       ;------------------------------------------------------
...
0068                       ;------------------------------------------------------
0069               idx.init.exit:
0070 70E8 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0071 70EA C2F9  30         mov   *stack+,r11           ; Pop r11
0072 70EC 045B  20         b     *r11                  ; Return to caller
0073               
0074               
0075               
0076               ***************************************************************
...
0097 70EE 0649  14         dect  stack
0098 70F0 C64B  30         mov   r11,*stack            ; Push return address
0099 70F2 0649  14         dect  stack
0100 70F4 C644  30         mov   tmp0,*stack           ; Push tmp0
0101 70F6 0649  14         dect  stack
0102 70F8 C645  30         mov   tmp1,*stack           ; Push tmp1
0103 70FA 0649  14         dect  stack
0104 70FC C646  30         mov   tmp2,*stack           ; Push tmp2
0105               *--------------------------------------------------------------
0106               * Map index pages into memory window  (b000-ffff)
0107               *--------------------------------------------------------------
0108 70FE C120  34         mov   @idx.sams.lopage,tmp0 ; Get lowest index page
     7100 A502 
0109 7102 0205  20         li    tmp1,idx.top
     7104 B000 
0110               
0111 7106 C1A0  34         mov   @idx.sams.hipage,tmp2 ; Get highest index page
     7108 A504 
0112 710A 0586  14         inc   tmp2                  ; +1 loop adjustment
0113 710C 61A0  34         s     @idx.sams.lopage,tmp2 ; Set loop counter
     710E A502 
0114                       ;-------------------------------------------------------
0115                       ; Sanity check
0116                       ;-------------------------------------------------------
0117 7110 0286  22         ci    tmp2,5                ; Crash if too many index pages
     7112 0005 
0118 7114 1104  14         jlt   !
0119 7116 C80B  38         mov   r11,@>ffce            ; \ Save caller address
     7118 FFCE 
0120 711A 06A0  32         bl    @cpu.crash            ; / Crash and halt system
     711C 2030 
0121                       ;-------------------------------------------------------
0122                       ; Loop over banks
0123                       ;-------------------------------------------------------
0124 711E 06A0  32 !       bl    @xsams.page.set       ; Set SAMS page
     7120 2546 
0125                                                   ; \ i  tmp0  = SAMS page number
0126                                                   ; / i  tmp1  = Memory address
0127               
0128 7122 0584  14         inc   tmp0                  ; Next SAMS index page
0129 7124 0225  22         ai    tmp1,>1000            ; Next memory region
     7126 1000 
0130 7128 0606  14         dec   tmp2                  ; Update loop counter
0131 712A 15F9  14         jgt   -!                    ; Next iteration
0132               *--------------------------------------------------------------
0133               * Exit
0134               *--------------------------------------------------------------
0135               _idx.sams.mapcolumn.on.exit:
0136 712C C1B9  30         mov   *stack+,tmp2          ; Pop tmp2
0137 712E C179  30         mov   *stack+,tmp1          ; Pop tmp1
0138 7130 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0139 7132 C2F9  30         mov   *stack+,r11           ; Pop return address
0140 7134 045B  20         b     *r11                  ; Return to caller
0141               
0142               
0143               ***************************************************************
...
0156 7136 0649  14         dect  stack
0157 7138 C64B  30         mov   r11,*stack            ; Push return address
0158 713A 0649  14         dect  stack
0159 713C C644  30         mov   tmp0,*stack           ; Push tmp0
0160 713E 0649  14         dect  stack
0161 7140 C645  30         mov   tmp1,*stack           ; Push tmp1
0162 7142 0649  14         dect  stack
0163 7144 C646  30         mov   tmp2,*stack           ; Push tmp2
0164 7146 0649  14         dect  stack
0165 7148 C647  30         mov   tmp3,*stack           ; Push tmp3
0166               *--------------------------------------------------------------
0167               * Map index pages into memory window  (b000-?????)
0168               *--------------------------------------------------------------
0169 714A 0205  20         li    tmp1,idx.top
     714C B000 
0170 714E 0206  20         li    tmp2,5                ; Always 5 pages
     7150 0005 
0171 7152 0207  20         li    tmp3,tv.sams.b000     ; Pointer to fist SAMS page
     7154 A006 
0172                       ;-------------------------------------------------------
0173                       ; Loop over table in memory (@tv.sams.b000:@tv.sams.f000)
0174                       ;-------------------------------------------------------
0175 7156 C137  30 !       mov   *tmp3+,tmp0           ; Get SAMS page
0176               
0177 7158 06A0  32         bl    @xsams.page.set       ; Set SAMS page
     715A 2546 
0178                                                   ; \ i  tmp0  = SAMS page number
0179                                                   ; / i  tmp1  = Memory address
0180               
0181 715C 0225  22         ai    tmp1,>1000            ; Next memory region
     715E 1000 
0182 7160 0606  14         dec   tmp2                  ; Update loop counter
0183 7162 15F9  14         jgt   -!                    ; Next iteration
0184               *--------------------------------------------------------------
0185               * Exit
0186               *--------------------------------------------------------------
0187               _idx.sams.mapcolumn.off.exit:
0188 7164 C1F9  30         mov   *stack+,tmp3          ; Pop tmp3
0189 7166 C1B9  30         mov   *stack+,tmp2          ; Pop tmp2
0190 7168 C179  30         mov   *stack+,tmp1          ; Pop tmp1
0191 716A C139  30         mov   *stack+,tmp0          ; Pop tmp0
0192 716C C2F9  30         mov   *stack+,r11           ; Pop return address
0193 716E 045B  20         b     *r11                  ; Return to caller
0194               
0195               
0196               
0197               ***************************************************************
...
0217 7170 0649  14         dect  stack
0218 7172 C64B  30         mov   r11,*stack            ; Save return address
0219 7174 0649  14         dect  stack
0220 7176 C644  30         mov   tmp0,*stack           ; Push tmp0
0221 7178 0649  14         dect  stack
0222 717A C645  30         mov   tmp1,*stack           ; Push tmp1
0223 717C 0649  14         dect  stack
0224 717E C646  30         mov   tmp2,*stack           ; Push tmp2
0225                       ;------------------------------------------------------
0226                       ; Determine SAMS index page
0227                       ;------------------------------------------------------
0228 7180 C184  18         mov   tmp0,tmp2             ; Line number
0229 7182 04C5  14         clr   tmp1                  ; MSW (tmp1) = 0 / LSW (tmp2) = Line number
0230 7184 0204  20         li    tmp0,2048             ; Index entries in 4K SAMS page
     7186 0800 
0231               
0232 7188 3D44  128         div   tmp0,tmp1             ; \ Divide 32 bit value by 2048
0233                                                   ; | tmp1 = quotient  (SAMS page offset)
0234                                                   ; / tmp2 = remainder
0235               
0236 718A 0A16  56         sla   tmp2,1                ; line number * 2
0237 718C C806  38         mov   tmp2,@outparm1        ; Offset index entry
     718E 2F30 
0238               
0239 7190 A160  34         a     @idx.sams.lopage,tmp1 ; Add SAMS page base
     7192 A502 
0240 7194 8805  38         c     tmp1,@idx.sams.page   ; Page already active?
     7196 A500 
0241               
0242 7198 130E  14         jeq   _idx.samspage.get.exit
0243                                                   ; Yes, so exit
0244                       ;------------------------------------------------------
0245                       ; Activate SAMS index page
0246                       ;------------------------------------------------------
0247 719A C805  38         mov   tmp1,@idx.sams.page   ; Set current SAMS page
     719C A500 
0248 719E C805  38         mov   tmp1,@tv.sams.b000    ; Also keep SAMS window synced in Stevie
     71A0 A006 
0249               
0250 71A2 C105  18         mov   tmp1,tmp0             ; Destination SAMS page
0251 71A4 0205  20         li    tmp1,>b000            ; Memory window for index page
     71A6 B000 
0252               
0253 71A8 06A0  32         bl    @xsams.page.set       ; Switch to SAMS page
     71AA 2546 
...
0258                       ;------------------------------------------------------
0259 71AC 8804  38         c     tmp0,@idx.sams.hipage ; New highest page?
     71AE A504 
0260 71B0 1202  14         jle   _idx.samspage.get.exit
0261                                                   ; No, exit
0262 71B2 C804  38         mov   tmp0,@idx.sams.hipage ; Yes, set highest SAMS index page
     71B4 A504 
0263                       ;------------------------------------------------------
0264                       ; Exit
0265                       ;------------------------------------------------------
0266               _idx.samspage.get.exit:
0267 71B6 C1B9  30         mov   *stack+,tmp2          ; Pop tmp2
0268 71B8 C179  30         mov   *stack+,tmp1          ; Pop tmp1
...
0020               ***************************************************************
0021               edb.init:
0022 71C0 0649  14         dect  stack
0023 71C2 C64B  30         mov   r11,*stack            ; Save return address
0024 71C4 0649  14         dect  stack
0025 71C6 C644  30         mov   tmp0,*stack           ; Push tmp0
0026                       ;------------------------------------------------------
0027                       ; Initialize
0028                       ;------------------------------------------------------
0029 71C8 0204  20         li    tmp0,edb.top          ; \
     71CA C000 
0030 71CC C804  38         mov   tmp0,@edb.top.ptr     ; / Set pointer to top of editor buffer
     71CE A200 
0031 71D0 C804  38         mov   tmp0,@edb.next_free.ptr
     71D2 A208 
0032                                                   ; Set pointer to next free line
0033               
0034 71D4 0720  34         seto  @edb.insmode          ; Turn on insert mode for this editor buffer
     71D6 A20A 
0035               
0036 71D8 0204  20         li    tmp0,1
     71DA 0001 
0037 71DC C804  38         mov   tmp0,@edb.lines       ; Lines=1
     71DE A204 
0038               
0039 71E0 0720  34         seto  @edb.block.m1         ; Reset block start line marker
     71E2 A20C 
0040 71E4 0720  34         seto  @edb.block.m2         ; Reset block end line marker
     71E6 A20E 
0041               
0042 71E8 0204  20         li    tmp0,txt.newfile      ; "New file"
     71EA 3540 
0043 71EC C804  38         mov   tmp0,@edb.filename.ptr
     71EE A210 
0044               
0045 71F0 0204  20         li    tmp0,txt.filetype.none
     71F2 3552 
0046 71F4 C804  38         mov   tmp0,@edb.filetype.ptr
     71F6 A212 
0047               
0048               edb.init.exit:
0049                       ;------------------------------------------------------
0050                       ; Exit
0051                       ;------------------------------------------------------
0052 71F8 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0053 71FA C2F9  30         mov   *stack+,r11           ; Pop r11
0054 71FC 045B  20         b     *r11                  ; Return to caller
0055               
0056               
0057               
0058               
...
0027                       ; Initialize
0028                       ;------------------------------------------------------
0029 7206 0204  20         li    tmp0,cmdb.top         ; \ Set pointer to command buffer
     7208 D000 
0030 720A C804  38         mov   tmp0,@cmdb.top.ptr    ; /
     720C A300 
0031               
0032 720E 04E0  34         clr   @cmdb.visible         ; Hide command buffer
     7210 A302 
0033 7212 0204  20         li    tmp0,4
     7214 0004 
0034 7216 C804  38         mov   tmp0,@cmdb.scrrows    ; Set current command buffer size
     7218 A306 
0035 721A C804  38         mov   tmp0,@cmdb.default    ; Set default command buffer size
     721C A308 
0036               
0037 721E 04E0  34         clr   @cmdb.lines           ; Number of lines in cmdb buffer
     7220 A316 
0038 7222 04E0  34         clr   @cmdb.dirty           ; Command buffer is clean
     7224 A318 
0039 7226 04E0  34         clr   @cmdb.action.ptr      ; Reset action to execute pointer
     7228 A324 
0040                       ;------------------------------------------------------
0041                       ; Clear command buffer
0042                       ;------------------------------------------------------
0043 722A 06A0  32         bl    @film
     722C 2242 
0044 722E D000             data  cmdb.top,>00,cmdb.size
     7230 0000 
     7232 1000 
...
0048                       ; Exit
0049                       ;------------------------------------------------------
0050 7234 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0051 7236 C2F9  30         mov   *stack+,r11           ; Pop r11
0052 7238 045B  20         b     *r11                  ; Return to caller
0053               
0054               
0055               
0056               
...
0076               ********|*****|*********************|**************************
0077               cmdb.refresh:
0078 723A 0649  14         dect  stack
0079 723C C64B  30         mov   r11,*stack            ; Save return address
0080 723E 0649  14         dect  stack
0081 7240 C644  30         mov   tmp0,*stack           ; Push tmp0
0082 7242 0649  14         dect  stack
0083 7244 C645  30         mov   tmp1,*stack           ; Push tmp1
0084 7246 0649  14         dect  stack
0085 7248 C646  30         mov   tmp2,*stack           ; Push tmp2
0086                       ;------------------------------------------------------
0087                       ; Dump Command buffer content
0088                       ;------------------------------------------------------
0089 724A C820  54         mov   @wyx,@cmdb.yxsave     ; Save YX position
     724C 832A 
     724E A30C 
0090 7250 C820  54         mov   @cmdb.yxprompt,@wyx   ; Screen position of command line prompt
     7252 A310 
     7254 832A 
0091               
0092 7256 05A0  34         inc   @wyx                  ; X +1 for prompt
     7258 832A 
0093               
0094 725A 06A0  32         bl    @yx2pnt               ; Get VDP PNT address for current YX pos.
     725C 2406 
0095                                                   ; \ i  @wyx = Cursor position
0096                                                   ; / o  tmp0 = VDP target address
0097               
0098 725E 0205  20         li    tmp1,cmdb.cmd         ; Address of current command
     7260 A327 
0099 7262 0206  20         li    tmp2,1*79             ; Command length
     7264 004F 
0100               
0101 7266 06A0  32         bl    @xpym2v               ; \ Copy CPU memory to VDP memory
     7268 245C 
...
0107                       ;------------------------------------------------------
0108 726A C820  54         mov   @cmdb.yxprompt,@wyx
     726C A310 
     726E 832A 
0109 7270 06A0  32         bl    @putstr
     7272 242A 
0110 7274 38CC                   data txt.cmdb.prompt
0111               
0112 7276 C820  54         mov   @cmdb.yxsave,@fb.yxsave
     7278 A30C 
     727A A114 
0113 727C C820  54         mov   @cmdb.yxsave,@wyx
     727E A30C 
     7280 832A 
...
0119 7282 C1B9  30         mov   *stack+,tmp2          ; Pop tmp2
0120 7284 C179  30         mov   *stack+,tmp1          ; Pop tmp1
0121 7286 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0122 7288 C2F9  30         mov   *stack+,r11           ; Pop r11
0123 728A 045B  20         b     *r11                  ; Return to caller
**** **** ****     > stevie_b0.asm.30943
...
0020               ***************************************************************
0021               errline.init:
0022 728C 0649  14         dect  stack
0023 728E C64B  30         mov   r11,*stack            ; Save return address
0024 7290 0649  14         dect  stack
0025 7292 C644  30         mov   tmp0,*stack           ; Push tmp0
0026                       ;------------------------------------------------------
0027                       ; Initialize
0028                       ;------------------------------------------------------
0029 7294 04E0  34         clr   @tv.error.visible     ; Set to hidden
     7296 A020 
0030               
0031 7298 06A0  32         bl    @film
     729A 2242 
0032 729C A022                   data tv.error.msg,0,160
     729E 0000 
     72A0 00A0 
0033               
0034 72A2 0204  20         li    tmp0,>A000            ; Length of error message (160 bytes)
     72A4 A000 
0035 72A6 D804  38         movb  tmp0,@tv.error.msg    ; Set length byte
     72A8 A022 
0036                       ;-------------------------------------------------------
0037                       ; Exit
0038                       ;-------------------------------------------------------
0039               errline.exit:
0040 72AA C139  30         mov   *stack+,tmp0          ; Pop tmp0
0041 72AC C2F9  30         mov   *stack+,r11           ; Pop R11
0042 72AE 045B  20         b     *r11                  ; Return to caller
0043               
**** **** ****     > stevie_b0.asm.30943
...
0023 72B2 C64B  30         mov   r11,*stack            ; Save return address
0024 72B4 0649  14         dect  stack
0025 72B6 C644  30         mov   tmp0,*stack           ; Push tmp0
0026                       ;------------------------------------------------------
0027                       ; Initialize
0028                       ;------------------------------------------------------
0029 72B8 0204  20         li    tmp0,1                ; \ Set default color scheme
     72BA 0001 
0030 72BC C804  38         mov   tmp0,@tv.colorscheme  ; /
     72BE A012 
0031               
0032 72C0 04E0  34         clr   @tv.task.oneshot      ; Reset pointer to oneshot task
     72C2 A01E 
0033 72C4 E0A0  34         soc   @wbit10,config        ; Assume ALPHA LOCK is down
     72C6 2016 
0034                       ;-------------------------------------------------------
0035                       ; Exit
0036                       ;-------------------------------------------------------
0037               tv.init.exit:
0038 72C8 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0039 72CA C2F9  30         mov   *stack+,r11           ; Pop R11
0040 72CC 045B  20         b     *r11                  ; Return to caller
0041               
0042               
0043               
0044               ***************************************************************
...
0060               ***************************************************************
0061               tv.reset:
0062 72CE 0649  14         dect  stack
0063 72D0 C64B  30         mov   r11,*stack            ; Save return address
0064                       ;------------------------------------------------------
0065                       ; Reset editor
0066                       ;------------------------------------------------------
0067 72D2 06A0  32         bl    @cmdb.init            ; Initialize command buffer
     72D4 318A 
0068 72D6 06A0  32         bl    @edb.init             ; Initialize editor buffer
     72D8 314C 
0069 72DA 06A0  32         bl    @idx.init             ; Initialize index
     72DC 304A 
0070 72DE 06A0  32         bl    @fb.init              ; Initialize framebuffer
     72E0 3008 
0071 72E2 06A0  32         bl    @errline.init         ; Initialize error line
     72E4 3218 
0072                       ;-------------------------------------------------------
0073                       ; Exit
0074                       ;-------------------------------------------------------
0075               tv.reset.exit:
0076 72E6 C2F9  30         mov   *stack+,r11           ; Pop R11
0077 72E8 045B  20         b     *r11                  ; Return to caller
**** **** ****     > stevie_b0.asm.30943
...
0031               ***************************************************************
0032               stevie.tx8030:
0033 72EA 04F0             byte  >04,>f0,>00,>3f,>02,>43,>05,SPFCLR,0,80
     72EC 003F 
     72EE 0243 
     72F0 05F4 
     72F2 0050 
0034               
0035               romsat:
0036 72F4 0303             data  >0303,>0001             ; Cursor YX, initial shape and colour
     72F6 0001 
0037               
0038               cursors:
0039 72F8 0000             data  >0000,>0000,>0000,>001c ; Cursor 1 - Insert mode
     72FA 0000 
     72FC 0000 
     72FE 001C 
0040 7300 1010             data  >1010,>1010,>1010,>1000 ; Cursor 2 - Insert mode
     7302 1010 
     7304 1010 
     7306 1000 
0041 7308 1C1C             data  >1c1c,>1c1c,>1c1c,>1c00 ; Cursor 3 - Overwrite mode
     730A 1C1C 
     730C 1C1C 
     730E 1C00 
0042               
0043               patterns:
0044 7310 0000             data  >0000,>0000,>00ff,>0000 ; 01. Single line
     7312 0000 
     7314 00FF 
     7316 0000 
0045 7318 0080             data  >0080,>0000,>ff00,>ff00 ; 02. Ruler + double line bottom
     731A 0000 
     731C FF00 
     731E FF00 
0046               
0047               patterns.box:
0048 7320 0000             data  >0000,>0000,>ff00,>ff00 ; 03. Double line bottom
     7322 0000 
     7324 FF00 
     7326 FF00 
0049 7328 0000             data  >0000,>0000,>ff80,>bfa0 ; 04. Top left corner
     732A 0000 
     732C FF80 
     732E BFA0 
0050 7330 0000             data  >0000,>0000,>fc04,>f414 ; 05. Top right corner
     7332 0000 
     7334 FC04 
     7336 F414 
0051 7338 A0A0             data  >a0a0,>a0a0,>a0a0,>a0a0 ; 06. Left vertical double line
     733A A0A0 
     733C A0A0 
     733E A0A0 
0052 7340 1414             data  >1414,>1414,>1414,>1414 ; 07. Right vertical double line
     7342 1414 
     7344 1414 
     7346 1414 
0053 7348 A0A0             data  >a0a0,>a0a0,>bf80,>ff00 ; 08. Bottom left corner
     734A A0A0 
     734C BF80 
     734E FF00 
0054 7350 1414             data  >1414,>1414,>f404,>fc00 ; 09. Bottom right corner
     7352 1414 
     7354 F404 
     7356 FC00 
0055 7358 0000             data  >0000,>c0c0,>c0c0,>0080 ; 10. Double line top left corner
     735A C0C0 
     735C C0C0 
     735E 0080 
0056 7360 0000             data  >0000,>0f0f,>0f0f,>0000 ; 11. Double line top right corner
     7362 0F0F 
     7364 0F0F 
     7366 0000 
0057               
0058               
0059               patterns.cr:
0060 7368 6C48             data  >6c48,>6c48,>4800,>7c00 ; 12. FF (Form Feed)
     736A 6C48 
     736C 4800 
     736E 7C00 
0061 7370 0024             data  >0024,>64fc,>6020,>0000 ; 13. CR (Carriage return) - arrow
     7372 64FC 
     7374 6020 
     7376 0000 
0062               
0063               
0064               alphalock:
0065 7378 0000             data  >0000,>00e0,>e0e0,>e0e0 ; 14. alpha lock down
     737A 00E0 
     737C E0E0 
     737E E0E0 
0066 7380 00E0             data  >00e0,>e0e0,>e0e0,>0000 ; 15. alpha lock up
     7382 E0E0 
     7384 E0E0 
     7386 0000 
0067               
0068               
0069               vertline:
0070 7388 1010             data  >1010,>1010,>1010,>1010 ; 16. Vertical line
     738A 1010 
     738C 1010 
     738E 1010 
0071               
...
0077 7390 2000             data  >2000,>0002           ; >2000-2fff, SAMS page >02
     7392 0002 
0078 7394 3000             data  >3000,>0003           ; >3000-3fff, SAMS page >03
     7396 0003 
0079 7398 A000             data  >a000,>000a           ; >a000-afff, SAMS page >0a
     739A 000A 
0080               
0081 739C B000             data  >b000,>0010           ; >b000-bfff, SAMS page >10
     739E 0010 
0082                                                   ; \ The index can allocate
0083                                                   ; / pages >10 to >2f.
0084               
0085 73A0 C000             data  >c000,>0030           ; >c000-cfff, SAMS page >30
     73A2 0030 
0086                                                   ; \ Editor buffer can allocate
0087                                                   ; / pages >30 to >ff.
0088               
0089 73A4 D000             data  >d000,>000d           ; >d000-dfff, SAMS page >0d
     73A6 000D 
0090 73A8 E000             data  >e000,>000e           ; >e000-efff, SAMS page >0e
     73AA 000E 
0091 73AC F000             data  >f000,>000f           ; >f000-ffff, SAMS page >0f
     73AE 000F 
0092               
0093               
0094               
...
0129 73B0 F41F      data  >f41f,>f001,>1b00,>0000 ; 1  whit/dblue  | black/whit  | whit  | black
     73B2 F001 
     73B4 1B00 
     73B6 0000 
0130 73B8 F41C      data  >f41c,>f00f,>1b00,>0000 ; 2  whit/dblue  | black/dgreen| whit  | whit
     73BA F00F 
     73BC 1B00 
     73BE 0000 
0131 73C0 A11A      data  >a11a,>f00f,>1f00,>0000 ; 3  yel/black   | black/dyel  | whit  | whit
     73C2 F00F 
     73C4 1F00 
     73C6 0000 
0132 73C8 2112      data  >2112,>f00f,>1b00,>0000 ; 4  mgreen/black| black/mgreen| white | whit
     73CA F00F 
     73CC 1B00 
     73CE 0000 
0133 73D0 E11E      data  >e11e,>f00f,>1b00,>0000 ; 5  grey/black  | black/grey  | white | whit
     73D2 F00F 
     73D4 1B00 
     73D6 0000 
0134 73D8 1771      data  >1771,>1006,>1b00,>0000 ; 6  black/cyan  | cyan/black  | black | ?
     73DA 1006 
     73DC 1B00 
     73DE 0000 
0135 73E0 1FF1      data  >1ff1,>1001,>1b00,>0000 ; 7  black/whit  | whit/black  | black | black
     73E2 1001 
     73E4 1B00 
     73E6 0000 
0136 73E8 A1F0      data  >a1f0,>1a0f,>1b00,>0000 ; 8  dyel/black  | whit/trnsp  | inver | whit
     73EA 1A0F 
     73EC 1B00 
     73EE 0000 
0137 73F0 21F0      data  >21f0,>f20f,>1b00,>0000 ; 9  mgreen/black| whit/trnsp  | inver | whit
     73F2 F20F 
     73F4 1B00 
     73F6 0000 
0138               
...
0012 73F8 0C53             byte  12
0013 73F9 ....             text  'Stevie v0.1E'
0014                       even
0015               
0016               txt.about.purpose
0017 7406 2350             byte  35
0018 7407 ....             text  'Programming Editor for the TI-99/4a'
0019                       even
0020               
0021               txt.about.author
0022 742A 1D32             byte  29
0023 742B ....             text  '2018-2020 by Filip Van Vooren'
0024                       even
0025               
0026               txt.about.website
0027 7448 1B68             byte  27
0028 7449 ....             text  'https://stevie.oratronik.de'
0029                       even
0030               
0031               txt.about.build
0032 7464 1342             byte  19
0033 7465 ....             text  'Build: 201115-30943'
0034                       even
0035               
0036               
0037               txt.about.msg1
0038 7478 2446             byte  36
0039 7479 ....             text  'FCTN-7 (F7)   Help, shortcuts, about'
0040                       even
0041               
0042               txt.about.msg2
0043 749E 2246             byte  34
0044 749F ....             text  'FCTN-9 (F9)   Toggle edit/cmd mode'
0045                       even
0046               
0047               txt.about.msg3
0048 74C2 1946             byte  25
0049 74C3 ....             text  'FCTN-+        Quit Stevie'
0050                       even
0051               
0052               txt.about.msg4
0053 74DC 1C43             byte  28
0054 74DD ....             text  'CTRL-L (^L)   Load DV80 file'
0055                       even
0056               
0057               txt.about.msg5
0058 74FA 1C43             byte  28
0059 74FB ....             text  'CTRL-K (^K)   Save DV80 file'
0060                       even
0061               
0062               txt.about.msg6
0063 7518 1A43             byte  26
0064 7519 ....             text  'CTRL-Z (^Z)   Cycle colors'
0065                       even
0066               
0067               
0068 7534 380E     txt.about.msg7     byte    56,14
0069 7536 ....                        text    ' ALPHA LOCK up     '
0070                                  byte    15
0071 754A ....                        text    ' ALPHA LOCK down   '
0072 755D ....                        text    '  * Text changed'
0073               
0074               
0075               ;--------------------------------------------------------------
...
0082               
0083               txt.marker
0084 7570 052A             byte  5
0085 7571 ....             text  '*EOF*'
0086                       even
0087               
0088               txt.bottom
0089 7576 0520             byte  5
0090 7577 ....             text  '  BOT'
0091                       even
0092               
0093               txt.ovrwrite
0094 757C 034F             byte  3
0095 757D ....             text  'OVR'
0096                       even
0097               
0098               txt.insert
0099 7580 0349             byte  3
0100 7581 ....             text  'INS'
0101                       even
0102               
0103               txt.star
0104 7584 012A             byte  1
0105 7585 ....             text  '*'
0106                       even
0107               
0108               txt.loading
0109 7586 0A4C             byte  10
0110 7587 ....             text  'Loading...'
0111                       even
0112               
0113               txt.saving
0114 7592 0953             byte  9
0115 7593 ....             text  'Saving...'
0116                       even
0117               
0118               txt.fastmode
0119 759C 0846             byte  8
0120 759D ....             text  'Fastmode'
0121                       even
0122               
0123               txt.kb
0124 75A6 026B             byte  2
0125 75A7 ....             text  'kb'
0126                       even
0127               
0128               txt.lines
0129 75AA 054C             byte  5
0130 75AB ....             text  'Lines'
0131                       even
0132               
0133               txt.bufnum
0134 75B0 0323             byte  3
0135 75B1 ....             text  '#1 '
0136                       even
0137               
0138               txt.newfile
0139 75B4 0A5B             byte  10
0140 75B5 ....             text  '[New file]'
0141                       even
0142               
0143               txt.filetype.dv80
0144 75C0 0444             byte  4
0145 75C1 ....             text  'DV80'
0146                       even
0147               
0148               txt.filetype.none
0149 75C6 0420             byte  4
0150 75C7 ....             text  '    '
0151                       even
0152               
0153               
0154 75CC 010F     txt.alpha.up       data >010f
0155 75CE 010E     txt.alpha.down     data >010e
0156 75D0 0110     txt.vertline       data >0110
0157               
0158               
0159               ;--------------------------------------------------------------
...
0161               ;--------------------------------------------------------------
0162               txt.head.load
0163 75D2 0F4C             byte  15
0164 75D3 ....             text  'Load DV80 file '
0165                       even
0166               
0167               txt.hint.load
0168 75E2 4D48             byte  77
0169 75E3 ....             text  'HINT: Fastmode uses CPU RAM instead of VDP RAM for file buffer (HRD/HDX/IDE).'
0170                       even
0171               
0172               txt.keys.load
0173 7630 3746             byte  55
0174 7631 ....             text  'F9=Back    F3=Clear    F5=Fastmode    ^A=Home    ^F=End'
0175                       even
0176               
0177               txt.keys.load2
0178 7668 3746             byte  55
0179 7669 ....             text  'F9=Back    F3=Clear   *F5=Fastmode    ^A=Home    ^F=End'
0180                       even
0181               
0182               
0183               ;--------------------------------------------------------------
...
0185               ;--------------------------------------------------------------
0186               txt.head.save
0187 76A0 0F53             byte  15
0188 76A1 ....             text  'Save DV80 file '
0189                       even
0190               
0191               txt.hint.save
0192 76B0 3F48             byte  63
0193 76B1 ....             text  'HINT: Fastmode uses CPU RAM instead of VDP RAM for file buffer.'
0194                       even
0195               
0196               txt.keys.save
0197 76F0 2846             byte  40
0198 76F1 ....             text  'F9=Back    F3=Clear    ^A=Home    ^F=End'
0199                       even
0200               
0201               
0202               ;--------------------------------------------------------------
...
0206 771A 1055             byte  16
0207 771B ....             text  'Unsaved changes '
0208                       even
0209               
0210               txt.info.unsaved
0211 772C 3259             byte  50
0212 772D ....             text  'You are about to lose changes to the current file!'
0213                       even
0214               
0215               txt.hint.unsaved
0216 7760 3F48             byte  63
0217 7761 ....             text  'HINT: Press F6 to proceed without saving or ENTER to save file.'
0218                       even
0219               
0220               txt.keys.unsaved
0221 77A0 2846             byte  40
0222 77A1 ....             text  'F9=Back    F6=Proceed    ENTER=Save file'
0223                       even
0224               
0225               
0226               
...
0230               ;--------------------------------------------------------------
0231               txt.head.block
0232 77CA 1742             byte  23
0233 77CB ....             text  'Block move/copy/delete '
0234                       even
0235               
0236               txt.info.block
0237 77E2 334D             byte  51
0238 77E3 ....             text  'Mark text block and press a control key for action.'
0239                       even
0240               
0241               txt.hint.block
0242 7816 2B48             byte  43
0243 7817 ....             text  'HINT: Mark block begin/end with FCTN-SPACE.'
0244                       even
0245               
0246               txt.keys.block
0247 7842 3646             byte  54
0248 7843 ....             text  'F9=Back  ^M=Move block  ^C=Copy block  ^D=Delete block'
0249                       even
0250               
0251               
0252               
...
0255               ;--------------------------------------------------------------
0256               txt.head.about
0257 787A 0D41             byte  13
0258 787B ....             text  'About Stevie '
0259                       even
0260               
0261               txt.hint.about
0262 7888 2C48             byte  44
0263 7889 ....             text  'HINT: Press F9 or ENTER to return to editor.'
0264                       even
0265               
0266               txt.keys.about
0267 78B6 1546             byte  21
0268 78B7 ....             text  'F9=Back    ENTER=Back'
0269                       even
0270               
0271               
0272               ;--------------------------------------------------------------
...
0274               ;--------------------------------------------------------------
0275               txt.ioerr.load
0276 78CC 2049             byte  32
0277 78CD ....             text  'I/O error. Failed loading file: '
0278                       even
0279               
0280               txt.ioerr.save
0281 78EE 1F49             byte  31
0282 78EF ....             text  'I/O error. Failed saving file: '
0283                       even
0284               
0285               txt.io.nofile
0286 790E 2149             byte  33
0287 790F ....             text  'I/O error. No filename specified.'
0288                       even
0289               
0290               
0291               
...
0294               ;--------------------------------------------------------------
0295               txt.cmdb.title
0296 7930 0E43             byte  14
0297 7931 ....             text  'Command buffer'
0298                       even
0299               
0300               txt.cmdb.prompt
0301 7940 013E             byte  1
0302 7941 ....             text  '>'
0303                       even
0304               
0305               
0306 7942 0C0A     txt.stevie         byte    12
0307                                  byte    10
0308 7944 ....                        text    'stevie v1.00'
0309 7950 0B00                        byte    11
0310                                  even
0311               
0312               txt.colorscheme
0313 7952 0E43             byte  14
0314 7953 ....             text  'Color scheme: '
0315                       even
0316               
0317               
**** **** ****     > stevie_b0.asm.30943
...
0103               *---------------------------------------------------------------
0104               txt.fctn.0
0105 7962 0866             byte  8
0106 7963 ....             text  'fctn + 0'
0107                       even
0108               
0109               txt.fctn.1
0110 796C 0866             byte  8
0111 796D ....             text  'fctn + 1'
0112                       even
0113               
0114               txt.fctn.2
0115 7976 0866             byte  8
0116 7977 ....             text  'fctn + 2'
0117                       even
0118               
0119               txt.fctn.3
0120 7980 0866             byte  8
0121 7981 ....             text  'fctn + 3'
0122                       even
0123               
0124               txt.fctn.4
0125 798A 0866             byte  8
0126 798B ....             text  'fctn + 4'
0127                       even
0128               
0129               txt.fctn.5
0130 7994 0866             byte  8
0131 7995 ....             text  'fctn + 5'
0132                       even
0133               
0134               txt.fctn.6
0135 799E 0866             byte  8
0136 799F ....             text  'fctn + 6'
0137                       even
0138               
0139               txt.fctn.7
0140 79A8 0866             byte  8
0141 79A9 ....             text  'fctn + 7'
0142                       even
0143               
0144               txt.fctn.8
0145 79B2 0866             byte  8
0146 79B3 ....             text  'fctn + 8'
0147                       even
0148               
0149               txt.fctn.9
0150 79BC 0866             byte  8
0151 79BD ....             text  'fctn + 9'
0152                       even
0153               
0154               txt.fctn.a
0155 79C6 0866             byte  8
0156 79C7 ....             text  'fctn + a'
0157                       even
0158               
0159               txt.fctn.b
0160 79D0 0866             byte  8
0161 79D1 ....             text  'fctn + b'
0162                       even
0163               
0164               txt.fctn.c
0165 79DA 0866             byte  8
0166 79DB ....             text  'fctn + c'
0167                       even
0168               
0169               txt.fctn.d
0170 79E4 0866             byte  8
0171 79E5 ....             text  'fctn + d'
0172                       even
0173               
0174               txt.fctn.e
0175 79EE 0866             byte  8
0176 79EF ....             text  'fctn + e'
0177                       even
0178               
0179               txt.fctn.f
0180 79F8 0866             byte  8
0181 79F9 ....             text  'fctn + f'
0182                       even
0183               
0184               txt.fctn.g
0185 7A02 0866             byte  8
0186 7A03 ....             text  'fctn + g'
0187                       even
0188               
0189               txt.fctn.h
0190 7A0C 0866             byte  8
0191 7A0D ....             text  'fctn + h'
0192                       even
0193               
0194               txt.fctn.i
0195 7A16 0866             byte  8
0196 7A17 ....             text  'fctn + i'
0197                       even
0198               
0199               txt.fctn.j
0200 7A20 0866             byte  8
0201 7A21 ....             text  'fctn + j'
0202                       even
0203               
0204               txt.fctn.k
0205 7A2A 0866             byte  8
0206 7A2B ....             text  'fctn + k'
0207                       even
0208               
0209               txt.fctn.l
0210 7A34 0866             byte  8
0211 7A35 ....             text  'fctn + l'
0212                       even
0213               
0214               txt.fctn.m
0215 7A3E 0866             byte  8
0216 7A3F ....             text  'fctn + m'
0217                       even
0218               
0219               txt.fctn.n
0220 7A48 0866             byte  8
0221 7A49 ....             text  'fctn + n'
0222                       even
0223               
0224               txt.fctn.o
0225 7A52 0866             byte  8
0226 7A53 ....             text  'fctn + o'
0227                       even
0228               
0229               txt.fctn.p
0230 7A5C 0866             byte  8
0231 7A5D ....             text  'fctn + p'
0232                       even
0233               
0234               txt.fctn.q
0235 7A66 0866             byte  8
0236 7A67 ....             text  'fctn + q'
0237                       even
0238               
0239               txt.fctn.r
0240 7A70 0866             byte  8
0241 7A71 ....             text  'fctn + r'
0242                       even
0243               
0244               txt.fctn.s
0245 7A7A 0866             byte  8
0246 7A7B ....             text  'fctn + s'
0247                       even
0248               
0249               txt.fctn.t
0250 7A84 0866             byte  8
0251 7A85 ....             text  'fctn + t'
0252                       even
0253               
0254               txt.fctn.u
0255 7A8E 0866             byte  8
0256 7A8F ....             text  'fctn + u'
0257                       even
0258               
0259               txt.fctn.v
0260 7A98 0866             byte  8
0261 7A99 ....             text  'fctn + v'
0262                       even
0263               
0264               txt.fctn.w
0265 7AA2 0866             byte  8
0266 7AA3 ....             text  'fctn + w'
0267                       even
0268               
0269               txt.fctn.x
0270 7AAC 0866             byte  8
0271 7AAD ....             text  'fctn + x'
0272                       even
0273               
0274               txt.fctn.y
0275 7AB6 0866             byte  8
0276 7AB7 ....             text  'fctn + y'
0277                       even
0278               
0279               txt.fctn.z
0280 7AC0 0866             byte  8
0281 7AC1 ....             text  'fctn + z'
0282                       even
0283               
0284               *---------------------------------------------------------------
...
0286               *---------------------------------------------------------------
0287               txt.fctn.dot
0288 7ACA 0866             byte  8
0289 7ACB ....             text  'fctn + .'
0290                       even
0291               
0292               txt.fctn.plus
0293 7AD4 0866             byte  8
0294 7AD5 ....             text  'fctn + +'
0295                       even
0296               
0297               
0298               txt.ctrl.dot
0299 7ADE 0863             byte  8
0300 7ADF ....             text  'ctrl + .'
0301                       even
0302               
0303               txt.ctrl.comma
0304 7AE8 0863             byte  8
0305 7AE9 ....             text  'ctrl + ,'
0306                       even
0307               
0308               *---------------------------------------------------------------
...
0310               *---------------------------------------------------------------
0311               txt.ctrl.0
0312 7AF2 0863             byte  8
0313 7AF3 ....             text  'ctrl + 0'
0314                       even
0315               
0316               txt.ctrl.1
0317 7AFC 0863             byte  8
0318 7AFD ....             text  'ctrl + 1'
0319                       even
0320               
0321               txt.ctrl.2
0322 7B06 0863             byte  8
0323 7B07 ....             text  'ctrl + 2'
0324                       even
0325               
0326               txt.ctrl.3
0327 7B10 0863             byte  8
0328 7B11 ....             text  'ctrl + 3'
0329                       even
0330               
0331               txt.ctrl.4
0332 7B1A 0863             byte  8
0333 7B1B ....             text  'ctrl + 4'
0334                       even
0335               
0336               txt.ctrl.5
0337 7B24 0863             byte  8
0338 7B25 ....             text  'ctrl + 5'
0339                       even
0340               
0341               txt.ctrl.6
0342 7B2E 0863             byte  8
0343 7B2F ....             text  'ctrl + 6'
0344                       even
0345               
0346               txt.ctrl.7
0347 7B38 0863             byte  8
0348 7B39 ....             text  'ctrl + 7'
0349                       even
0350               
0351               txt.ctrl.8
0352 7B42 0863             byte  8
0353 7B43 ....             text  'ctrl + 8'
0354                       even
0355               
0356               txt.ctrl.9
0357 7B4C 0863             byte  8
0358 7B4D ....             text  'ctrl + 9'
0359                       even
0360               
0361               txt.ctrl.a
0362 7B56 0863             byte  8
0363 7B57 ....             text  'ctrl + a'
0364                       even
0365               
0366               txt.ctrl.b
0367 7B60 0863             byte  8
0368 7B61 ....             text  'ctrl + b'
0369                       even
0370               
0371               txt.ctrl.c
0372 7B6A 0863             byte  8
0373 7B6B ....             text  'ctrl + c'
0374                       even
0375               
0376               txt.ctrl.d
0377 7B74 0863             byte  8
0378 7B75 ....             text  'ctrl + d'
0379                       even
0380               
0381               txt.ctrl.e
0382 7B7E 0863             byte  8
0383 7B7F ....             text  'ctrl + e'
0384                       even
0385               
0386               txt.ctrl.f
0387 7B88 0863             byte  8
0388 7B89 ....             text  'ctrl + f'
0389                       even
0390               
0391               txt.ctrl.g
0392 7B92 0863             byte  8
0393 7B93 ....             text  'ctrl + g'
0394                       even
0395               
0396               txt.ctrl.h
0397 7B9C 0863             byte  8
0398 7B9D ....             text  'ctrl + h'
0399                       even
0400               
0401               txt.ctrl.i
0402 7BA6 0863             byte  8
0403 7BA7 ....             text  'ctrl + i'
0404                       even
0405               
0406               txt.ctrl.j
0407 7BB0 0863             byte  8
0408 7BB1 ....             text  'ctrl + j'
0409                       even
0410               
0411               txt.ctrl.k
0412 7BBA 0863             byte  8
0413 7BBB ....             text  'ctrl + k'
0414                       even
0415               
0416               txt.ctrl.l
0417 7BC4 0863             byte  8
0418 7BC5 ....             text  'ctrl + l'
0419                       even
0420               
0421               txt.ctrl.m
0422 7BCE 0863             byte  8
0423 7BCF ....             text  'ctrl + m'
0424                       even
0425               
0426               txt.ctrl.n
0427 7BD8 0863             byte  8
0428 7BD9 ....             text  'ctrl + n'
0429                       even
0430               
0431               txt.ctrl.o
0432 7BE2 0863             byte  8
0433 7BE3 ....             text  'ctrl + o'
0434                       even
0435               
0436               txt.ctrl.p
0437 7BEC 0863             byte  8
0438 7BED ....             text  'ctrl + p'
0439                       even
0440               
0441               txt.ctrl.q
0442 7BF6 0863             byte  8
0443 7BF7 ....             text  'ctrl + q'
0444                       even
0445               
0446               txt.ctrl.r
0447 7C00 0863             byte  8
0448 7C01 ....             text  'ctrl + r'
0449                       even
0450               
0451               txt.ctrl.s
0452 7C0A 0863             byte  8
0453 7C0B ....             text  'ctrl + s'
0454                       even
0455               
0456               txt.ctrl.t
0457 7C14 0863             byte  8
0458 7C15 ....             text  'ctrl + t'
0459                       even
0460               
0461               txt.ctrl.u
0462 7C1E 0863             byte  8
0463 7C1F ....             text  'ctrl + u'
0464                       even
0465               
0466               txt.ctrl.v
0467 7C28 0863             byte  8
0468 7C29 ....             text  'ctrl + v'
0469                       even
0470               
0471               txt.ctrl.w
0472 7C32 0863             byte  8
0473 7C33 ....             text  'ctrl + w'
0474                       even
0475               
0476               txt.ctrl.x
0477 7C3C 0863             byte  8
0478 7C3D ....             text  'ctrl + x'
0479                       even
0480               
0481               txt.ctrl.y
0482 7C46 0863             byte  8
0483 7C47 ....             text  'ctrl + y'
0484                       even
0485               
0486               txt.ctrl.z
0487 7C50 0863             byte  8
0488 7C51 ....             text  'ctrl + z'
0489                       even
0490               
0491               *---------------------------------------------------------------
...
0501               *---------------------------------------------------------------
0502               txt.enter
0503 7C64 0565             byte  5
0504 7C65 ....             text  'enter'
0505                       even
0506               
**** **** ****     > stevie_b0.asm.30943
0153                       ;------------------------------------------------------
...
0155                       ;------------------------------------------------------
0156 7C6A DEAD             data  >dead,>beef,>dead,>beef
     7C6C BEEF 
     7C6E DEAD 
     7C70 BEEF 
0158               
0162 7C72 3BFE                   data $                ; Bank 0 ROM size OK.
0164               
0165               *--------------------------------------------------------------
...
0170      3276     spvmod  equ   stevie.tx8030         ; Video mode.   See VIDTAB for details.
