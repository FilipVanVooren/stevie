...
0008               ***************************************************************
0009               * File: stevie_b0.asm               ; Version 201227-977512
0010               *
0011               * Bank 0 "Jill"
0012               *
0013               ***************************************************************
...
0260 624D ....             text  'Build-ID  201227-977512'
0261                       even
0262               
**** **** ****     > runlib.asm
...
0350 6F0C 06A0  32 runlic  bl    @vidtab               ; Load video mode table into VDP
     6F0E 22FA 
0351 6F10 32E0             data  spvmod                ; Equate selected video mode table
...
0058 7146 C804  38         mov   tmp0,@idx.sams.lopage ; Set 1st SAMS page
     7148 A502 
0059 714A C804  38         mov   tmp0,@idx.sams.hipage ; Set last SAMS page
     714C A504 
0060                       ;------------------------------------------------------
0061                       ; Clear index page
0062                       ;------------------------------------------------------
0063 714E 06A0  32         bl    @film
     7150 2238 
0064 7152 B000                   data idx.top,>00,idx.size
     7154 0000 
     7156 1000 
0065                                                   ; Clear index
0066                       ;------------------------------------------------------
...
0068                       ;------------------------------------------------------
0069               idx.init.exit:
0070 7158 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0071 715A C2F9  30         mov   *stack+,r11           ; Pop r11
0072 715C 045B  20         b     *r11                  ; Return to caller
0073               
0074               
0075               
0076               ***************************************************************
...
0097 715E 0649  14         dect  stack
0098 7160 C64B  30         mov   r11,*stack            ; Push return address
0099 7162 0649  14         dect  stack
0100 7164 C644  30         mov   tmp0,*stack           ; Push tmp0
0101 7166 0649  14         dect  stack
0102 7168 C645  30         mov   tmp1,*stack           ; Push tmp1
0103 716A 0649  14         dect  stack
0104 716C C646  30         mov   tmp2,*stack           ; Push tmp2
0105               *--------------------------------------------------------------
0106               * Map index pages into memory window  (b000-ffff)
0107               *--------------------------------------------------------------
0108 716E C120  34         mov   @idx.sams.lopage,tmp0 ; Get lowest index page
     7170 A502 
0109 7172 0205  20         li    tmp1,idx.top
     7174 B000 
0110               
0111 7176 C1A0  34         mov   @idx.sams.hipage,tmp2 ; Get highest index page
     7178 A504 
0112 717A 0586  14         inc   tmp2                  ; +1 loop adjustment
0113 717C 61A0  34         s     @idx.sams.lopage,tmp2 ; Set loop counter
     717E A502 
0114                       ;-------------------------------------------------------
0115                       ; Sanity check
0116                       ;-------------------------------------------------------
0117 7180 0286  22         ci    tmp2,5                ; Crash if too many index pages
     7182 0005 
0118 7184 1104  14         jlt   !
0119 7186 C80B  38         mov   r11,@>ffce            ; \ Save caller address
     7188 FFCE 
0120 718A 06A0  32         bl    @cpu.crash            ; / Crash and halt system
     718C 2026 
0121                       ;-------------------------------------------------------
0122                       ; Loop over banks
0123                       ;-------------------------------------------------------
0124 718E 06A0  32 !       bl    @xsams.page.set       ; Set SAMS page
     7190 253C 
0125                                                   ; \ i  tmp0  = SAMS page number
0126                                                   ; / i  tmp1  = Memory address
0127               
0128 7192 0584  14         inc   tmp0                  ; Next SAMS index page
0129 7194 0225  22         ai    tmp1,>1000            ; Next memory region
     7196 1000 
0130 7198 0606  14         dec   tmp2                  ; Update loop counter
0131 719A 15F9  14         jgt   -!                    ; Next iteration
0132               *--------------------------------------------------------------
0133               * Exit
0134               *--------------------------------------------------------------
0135               _idx.sams.mapcolumn.on.exit:
0136 719C C1B9  30         mov   *stack+,tmp2          ; Pop tmp2
0137 719E C179  30         mov   *stack+,tmp1          ; Pop tmp1
0138 71A0 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0139 71A2 C2F9  30         mov   *stack+,r11           ; Pop return address
0140 71A4 045B  20         b     *r11                  ; Return to caller
0141               
0142               
0143               ***************************************************************
...
0156 71A6 0649  14         dect  stack
0157 71A8 C64B  30         mov   r11,*stack            ; Push return address
0158 71AA 0649  14         dect  stack
0159 71AC C644  30         mov   tmp0,*stack           ; Push tmp0
0160 71AE 0649  14         dect  stack
0161 71B0 C645  30         mov   tmp1,*stack           ; Push tmp1
0162 71B2 0649  14         dect  stack
0163 71B4 C646  30         mov   tmp2,*stack           ; Push tmp2
0164 71B6 0649  14         dect  stack
0165 71B8 C647  30         mov   tmp3,*stack           ; Push tmp3
0166               *--------------------------------------------------------------
0167               * Map index pages into memory window  (b000-?????)
0168               *--------------------------------------------------------------
0169 71BA 0205  20         li    tmp1,idx.top
     71BC B000 
0170 71BE 0206  20         li    tmp2,5                ; Always 5 pages
     71C0 0005 
0171 71C2 0207  20         li    tmp3,tv.sams.b000     ; Pointer to fist SAMS page
     71C4 A006 
0172                       ;-------------------------------------------------------
0173                       ; Loop over table in memory (@tv.sams.b000:@tv.sams.f000)
0174                       ;-------------------------------------------------------
0175 71C6 C137  30 !       mov   *tmp3+,tmp0           ; Get SAMS page
0176               
0177 71C8 06A0  32         bl    @xsams.page.set       ; Set SAMS page
     71CA 253C 
0178                                                   ; \ i  tmp0  = SAMS page number
0179                                                   ; / i  tmp1  = Memory address
0180               
0181 71CC 0225  22         ai    tmp1,>1000            ; Next memory region
     71CE 1000 
0182 71D0 0606  14         dec   tmp2                  ; Update loop counter
0183 71D2 15F9  14         jgt   -!                    ; Next iteration
0184               *--------------------------------------------------------------
0185               * Exit
0186               *--------------------------------------------------------------
0187               _idx.sams.mapcolumn.off.exit:
0188 71D4 C1F9  30         mov   *stack+,tmp3          ; Pop tmp3
0189 71D6 C1B9  30         mov   *stack+,tmp2          ; Pop tmp2
0190 71D8 C179  30         mov   *stack+,tmp1          ; Pop tmp1
0191 71DA C139  30         mov   *stack+,tmp0          ; Pop tmp0
0192 71DC C2F9  30         mov   *stack+,r11           ; Pop return address
0193 71DE 045B  20         b     *r11                  ; Return to caller
0194               
0195               
0196               
0197               ***************************************************************
...
0217 71E0 0649  14         dect  stack
0218 71E2 C64B  30         mov   r11,*stack            ; Save return address
0219 71E4 0649  14         dect  stack
0220 71E6 C644  30         mov   tmp0,*stack           ; Push tmp0
0221 71E8 0649  14         dect  stack
0222 71EA C645  30         mov   tmp1,*stack           ; Push tmp1
0223 71EC 0649  14         dect  stack
0224 71EE C646  30         mov   tmp2,*stack           ; Push tmp2
0225                       ;------------------------------------------------------
0226                       ; Determine SAMS index page
0227                       ;------------------------------------------------------
0228 71F0 C184  18         mov   tmp0,tmp2             ; Line number
0229 71F2 04C5  14         clr   tmp1                  ; MSW (tmp1) = 0 / LSW (tmp2) = Line number
0230 71F4 0204  20         li    tmp0,2048             ; Index entries in 4K SAMS page
     71F6 0800 
0231               
0232 71F8 3D44  128         div   tmp0,tmp1             ; \ Divide 32 bit value by 2048
0233                                                   ; | tmp1 = quotient  (SAMS page offset)
0234                                                   ; / tmp2 = remainder
0235               
0236 71FA 0A16  56         sla   tmp2,1                ; line number * 2
0237 71FC C806  38         mov   tmp2,@outparm1        ; Offset index entry
     71FE 2F30 
0238               
0239 7200 A160  34         a     @idx.sams.lopage,tmp1 ; Add SAMS page base
     7202 A502 
0240 7204 8805  38         c     tmp1,@idx.sams.page   ; Page already active?
     7206 A500 
0241               
0242 7208 130E  14         jeq   _idx.samspage.get.exit
0243                                                   ; Yes, so exit
0244                       ;------------------------------------------------------
0245                       ; Activate SAMS index page
0246                       ;------------------------------------------------------
0247 720A C805  38         mov   tmp1,@idx.sams.page   ; Set current SAMS page
     720C A500 
0248 720E C805  38         mov   tmp1,@tv.sams.b000    ; Also keep SAMS window synced in Stevie
     7210 A006 
0249               
0250 7212 C105  18         mov   tmp1,tmp0             ; Destination SAMS page
0251 7214 0205  20         li    tmp1,>b000            ; Memory window for index page
     7216 B000 
0252               
0253 7218 06A0  32         bl    @xsams.page.set       ; Switch to SAMS page
     721A 253C 
...
0258                       ;------------------------------------------------------
0259 721C 8804  38         c     tmp0,@idx.sams.hipage ; New highest page?
     721E A504 
0260 7220 1202  14         jle   _idx.samspage.get.exit
0261                                                   ; No, exit
0262 7222 C804  38         mov   tmp0,@idx.sams.hipage ; Yes, set highest SAMS index page
     7224 A504 
0263                       ;------------------------------------------------------
0264                       ; Exit
0265                       ;------------------------------------------------------
0266               _idx.samspage.get.exit:
0267 7226 C1B9  30         mov   *stack+,tmp2          ; Pop tmp2
0268 7228 C179  30         mov   *stack+,tmp1          ; Pop tmp1
...
0043 725C C804  38         mov   tmp0,@edb.filename.ptr
     725E A212 
0044               
0045 7260 0204  20         li    tmp0,txt.filetype.none
     7262 362E 
0046 7264 C804  38         mov   tmp0,@edb.filetype.ptr
     7266 A214 
0047               
0048               edb.init.exit:
0049                       ;------------------------------------------------------
0050                       ; Exit
0051                       ;------------------------------------------------------
0052 7268 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0053 726A C2F9  30         mov   *stack+,r11           ; Pop r11
0054 726C 045B  20         b     *r11                  ; Return to caller
0055               
0056               
0057               
0058               
...
0077                       ;------------------------------------------------------
0078 730C 06A0  32         bl    @hchar
     730E 2788 
0079 7310 0034                   byte 0,52,32,18           ; Remove markers
     7312 2012 
0080 7314 1D00                   byte pane.botrow,0,32,50  ; Remove block shortcuts
     7316 2032 
0081 7318 FFFF                   data eol
0082                       ;-------------------------------------------------------
0083                       ; Exit
0084                       ;-------------------------------------------------------
0085               tv.reset.exit:
0086 731A C2F9  30         mov   *stack+,r11           ; Pop R11
0087 731C 045B  20         b     *r11                  ; Return to caller
**** **** ****     > mem.resident.3000.asm
...
0025                       ; Initialize
0026                       ;------------------------------------------------------
0027 7326 06A0  32         bl    @mknum                ; Convert unsigned number to string
     7328 299A 
0028 732A 2F20                   data parm1            ; \ i p0  = Pointer to 16bit unsigned number
0029 732C 2F6A                   data rambuf           ; | i p1  = Pointer to 5 byte string buffer
0030 732E 3020                   byte 48               ; | i p2H = Offset for ASCII digit 0
0031                             byte 32               ; / i p2L = Char for replacing leading 0's
0032               
0033 7330 0204  20         li    tmp0,unpacked.string
     7332 2F44 
0034 7334 04F4  30         clr   *tmp0+                ; Clear string 01
0035 7336 04F4  30         clr   *tmp0+                ; Clear string 23
0036 7338 04F4  30         clr   *tmp0+                ; Clear string 34
0037               
0038 733A 06A0  32         bl    @trimnum              ; Trim unsigned number string
     733C 29F2 
0039 733E 2F6A                   data rambuf           ; \ i p0  = Pointer to 5 byte string buffer
0040 7340 2F44                   data unpacked.string  ; | i p1  = Pointer to output buffer
0041 7342 0020                   data 32               ; | i p2  = Padding char to match against
0042                       ;-------------------------------------------------------
0043                       ; Exit
0044                       ;-------------------------------------------------------
0045               tv.unpack.uint16.exit:
0046 7344 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0047 7346 C2F9  30         mov   *stack+,r11           ; Pop r11
0048 7348 045B  20         b     *r11                  ; Return to caller
**** **** ****     > mem.resident.3000.asm
...
0031               ***************************************************************
0032               stevie.tx8030:
0033 734A 04F0             byte  >04,>f0,>00,>3f,>02,>43,>05,SPFCLR,0,80
     734C 003F 
     734E 0243 
     7350 05F4 
     7352 0050 
0034               
0035               romsat:
0036 7354 0303             data  >0303,>0001             ; Cursor YX, initial shape and colour
     7356 0001 
0037               
0038               cursors:
0039 7358 0000             data  >0000,>0000,>0000,>001c ; Cursor 1 - Insert mode
     735A 0000 
     735C 0000 
     735E 001C 
0040 7360 1010             data  >1010,>1010,>1010,>1000 ; Cursor 2 - Insert mode
     7362 1010 
     7364 1010 
     7366 1000 
0041 7368 1C1C             data  >1c1c,>1c1c,>1c1c,>1c00 ; Cursor 3 - Overwrite mode
     736A 1C1C 
     736C 1C1C 
     736E 1C00 
0042               
0043               patterns:
0044 7370 0000             data  >0000,>0000,>00ff,>0000 ; 01. Single line
     7372 0000 
     7374 00FF 
     7376 0000 
0045 7378 0080             data  >0080,>0000,>ff00,>ff00 ; 02. Ruler + double line bottom
     737A 0000 
     737C FF00 
     737E FF00 
0046               
0047               patterns.box:
0048 7380 0000             data  >0000,>0000,>ff00,>ff00 ; 03. Double line bottom
     7382 0000 
     7384 FF00 
     7386 FF00 
0049 7388 0000             data  >0000,>0000,>ff80,>bfa0 ; 04. Top left corner
     738A 0000 
     738C FF80 
     738E BFA0 
0050 7390 0000             data  >0000,>0000,>fc04,>f414 ; 05. Top right corner
     7392 0000 
     7394 FC04 
     7396 F414 
0051 7398 A0A0             data  >a0a0,>a0a0,>a0a0,>a0a0 ; 06. Left vertical double line
     739A A0A0 
     739C A0A0 
     739E A0A0 
0052 73A0 1414             data  >1414,>1414,>1414,>1414 ; 07. Right vertical double line
     73A2 1414 
     73A4 1414 
     73A6 1414 
0053 73A8 A0A0             data  >a0a0,>a0a0,>bf80,>ff00 ; 08. Bottom left corner
     73AA A0A0 
     73AC BF80 
     73AE FF00 
0054 73B0 1414             data  >1414,>1414,>f404,>fc00 ; 09. Bottom right corner
     73B2 1414 
     73B4 F404 
     73B6 FC00 
0055 73B8 0000             data  >0000,>c0c0,>c0c0,>0080 ; 10. Double line top left corner
     73BA C0C0 
     73BC C0C0 
     73BE 0080 
0056 73C0 0000             data  >0000,>0f0f,>0f0f,>0000 ; 11. Double line top right corner
     73C2 0F0F 
     73C4 0F0F 
     73C6 0000 
0057               
0058               
0059               patterns.cr:
0060 73C8 6C48             data  >6c48,>6c48,>4800,>7c00 ; 12. FF (Form Feed)
     73CA 6C48 
     73CC 4800 
     73CE 7C00 
0061 73D0 0024             data  >0024,>64fc,>6020,>0000 ; 13. CR (Carriage return) - arrow
     73D2 64FC 
     73D4 6020 
     73D6 0000 
0062               
0063               
0064               alphalock:
0065 73D8 0000             data  >0000,>00e0,>e0e0,>e0e0 ; 14. alpha lock down
     73DA 00E0 
     73DC E0E0 
     73DE E0E0 
0066 73E0 00E0             data  >00e0,>e0e0,>e0e0,>0000 ; 15. alpha lock up
     73E2 E0E0 
     73E4 E0E0 
     73E6 0000 
0067               
0068               
0069               vertline:
0070 73E8 1010             data  >1010,>1010,>1010,>1010 ; 16. Vertical line
     73EA 1010 
     73EC 1010 
     73EE 1010 
0071               
...
0077 73F0 2000             data  >2000,>0002           ; >2000-2fff, SAMS page >02
     73F2 0002 
0078 73F4 3000             data  >3000,>0003           ; >3000-3fff, SAMS page >03
     73F6 0003 
0079 73F8 A000             data  >a000,>000a           ; >a000-afff, SAMS page >0a
     73FA 000A 
0080               
0081 73FC B000             data  >b000,>0010           ; >b000-bfff, SAMS page >10
     73FE 0010 
0082                                                   ; \ The index can allocate
0083                                                   ; / pages >10 to >2f.
0084               
0085 7400 C000             data  >c000,>0030           ; >c000-cfff, SAMS page >30
     7402 0030 
0086                                                   ; \ Editor buffer can allocate
0087                                                   ; / pages >30 to >ff.
0088               
0089 7404 D000             data  >d000,>000d           ; >d000-dfff, SAMS page >0d
     7406 000D 
0090 7408 E000             data  >e000,>000e           ; >e000-efff, SAMS page >0e
     740A 000E 
0091 740C F000             data  >f000,>000f           ; >f000-ffff, SAMS page >0f
     740E 000F 
0092               
0093               
0094               
...
0147 7410 F417      data  >f417,>f171,>1b1f,>0000 ; 1  White on blue with inversed cyan border
     7412 F171 
     7414 1B1F 
     7416 0000 
0148 7418 F41F      data  >f41f,>f011,>1a17,>0000 ; 2  White on blue with inversed white border
     741A F011 
     741C 1A17 
     741E 0000 
0149 7420 A11A      data  >a11a,>f0ff,>1f1a,>0000 ; 3  Dark yellow on black with inversed border
     7422 F0FF 
     7424 1F1A 
     7426 0000 
0150 7428 2112      data  >2112,>f0ff,>1b12,>0000 ; 4  Dark green on black with inversed border
     742A F0FF 
     742C 1B12 
     742E 0000 
0151 7430 E11E      data  >e11e,>f00f,>1b1e,>0000 ; 5  Grey on black with inversed grey border
     7432 F00F 
     7434 1B1E 
     7436 0000 
0152 7438 1771      data  >1771,>1006,>1b71,>0000 ; 6  Black on cyan with inversed black border
     743A 1006 
     743C 1B71 
     743E 0000 
0153 7440 1FF1      data  >1ff1,>1001,>1bf1,>0000 ; 7  Black on white with inversed black border
     7442 1001 
     7444 1BF1 
     7446 0000 
0154 7448 A1F0      data  >a1f0,>1a0f,>1b1a,>0000 ; 8  Dark yellow on black with white border
     744A 1A0F 
     744C 1B1A 
     744E 0000 
0155 7450 21F0      data  >21f0,>f20f,>1b12,>0000 ; 9  Dark green on black with white border
     7452 F20F 
     7454 1B12 
     7456 0000 
0156               
...
0012 7458 0C53             byte  12
0013 7459 ....             text  'Stevie V0.1I'
0014                       even
0015               
0016               txt.about.purpose
0017 7466 2350             byte  35
0018 7467 ....             text  'Programming Editor for the TI-99/4a'
0019                       even
0020               
0021               txt.about.author
0022 748A 1D32             byte  29
0023 748B ....             text  '2018-2020 by Filip Van Vooren'
0024                       even
0025               
0026               txt.about.website
0027 74A8 1B68             byte  27
0028 74A9 ....             text  'https://stevie.oratronik.de'
0029                       even
0030               
0031               txt.about.build
0032 74C4 1442             byte  20
0033 74C5 ....             text  'Build: 201227-977512'
0034                       even
0035               
0036               
0037               txt.about.msg1
0038 74DA 2466             byte  36
0039 74DB ....             text  'fctn-7 (F7)   Help, shortcuts, about'
0040                       even
0041               
0042               txt.about.msg2
0043 7500 2266             byte  34
0044 7501 ....             text  'fctn-9 (F9)   Toggle edit/cmd mode'
0045                       even
0046               
0047               txt.about.msg3
0048 7524 1966             byte  25
0049 7525 ....             text  'fctn-+        Quit Stevie'
0050                       even
0051               
0052               txt.about.msg4
0053 753E 1C43             byte  28
0054 753F ....             text  'CTRL-O (^O)   Open DV80 file'
0055                       even
0056               
0057               txt.about.msg5
0058 755C 1C43             byte  28
0059 755D ....             text  'CTRL-S (^S)   Save DV80 file'
0060                       even
0061               
0062               txt.about.msg6
0063 757A 1A43             byte  26
0064 757B ....             text  'CTRL-Z (^Z)   Cycle colors'
0065                       even
0066               
0067               
0068 7596 380F     txt.about.msg7     byte    56,15
0069 7598 ....                        text    ' ALPHA LOCK up     '
0070                                  byte    14
0071 75AC ....                        text    ' ALPHA LOCK down   '
0072 75BF ....                        text    '  * Text changed'
0073               
0074               
0075               ;--------------------------------------------------------------
...
0082               
0083               txt.marker
0084 75D2 052A             byte  5
0085 75D3 ....             text  '*EOF*'
0086                       even
0087               
0088               txt.bottom
0089 75D8 0520             byte  5
0090 75D9 ....             text  '  BOT'
0091                       even
0092               
0093               txt.ovrwrite
0094 75DE 034F             byte  3
0095 75DF ....             text  'OVR'
0096                       even
0097               
0098               txt.insert
0099 75E2 0349             byte  3
0100 75E3 ....             text  'INS'
0101                       even
0102               
0103               txt.star
0104 75E6 012A             byte  1
0105 75E7 ....             text  '*'
0106                       even
0107               
0108               txt.loading
0109 75E8 0A4C             byte  10
0110 75E9 ....             text  'Loading...'
0111                       even
0112               
0113               txt.saving
0114 75F4 0A53             byte  10
0115 75F5 ....             text  'Saving....'
0116                       even
0117               
0118               txt.block.del
0119 7600 1244             byte  18
0120 7601 ....             text  'Deleting block....'
0121                       even
0122               
0123               txt.block.save
0124 7614 1D53             byte  29
0125 7615 ....             text  'Saving block to DV80 file....'
0126                       even
0127               
0128               txt.fastmode
0129 7632 0846             byte  8
0130 7633 ....             text  'Fastmode'
0131                       even
0132               
0133               txt.kb
0134 763C 026B             byte  2
0135 763D ....             text  'kb'
0136                       even
0137               
0138               txt.lines
0139 7640 054C             byte  5
0140 7641 ....             text  'Lines'
0141                       even
0142               
0143               txt.bufnum
0144 7646 0323             byte  3
0145 7647 ....             text  '#1 '
0146                       even
0147               
0148               txt.newfile
0149 764A 0A5B             byte  10
0150 764B ....             text  '[New file]'
0151                       even
0152               
0153               txt.filetype.dv80
0154 7656 0444             byte  4
0155 7657 ....             text  'DV80'
0156                       even
0157               
0158               txt.m1
0159 765C 034D             byte  3
0160 765D ....             text  'M1='
0161                       even
0162               
0163               txt.m2
0164 7660 034D             byte  3
0165 7661 ....             text  'M2='
0166                       even
0167               
0168               
0169 7664 2D5E     txt.keys.block     byte    45
0170 7665 ....                        text    '^Del  ^Copy  ^N=Move  ^Goto M1  ^Reset  ^Save'
0171               
0172 7692 010F     txt.alpha.up       data >010f
0173 7694 010E     txt.alpha.down     data >010e
0174 7696 0110     txt.vertline       data >0110
0175               
0176               txt.clear
0177 7698 0420             byte  4
0178 7699 ....             text  '    '
0179                       even
0180               
...
0186               ;--------------------------------------------------------------
0187               txt.head.load
0188 769E 0F4F             byte  15
0189 769F ....             text  'Open DV80 file '
0190                       even
0191               
0192               txt.hint.load
0193 76AE 4D48             byte  77
0194 76AF ....             text  'HINT: Fastmode uses CPU RAM instead of VDP RAM for file buffer (HRD/HDX/IDE).'
0195                       even
0196               
0197               txt.keys.load
0198 76FC 3946             byte  57
0199 76FD ....             text  'F9=Back    F3=Clear    F5=Fastmode    F-H=Home    F-L=End'
0200                       even
0201               
0202               txt.keys.load2
0203 7736 3946             byte  57
0204 7737 ....             text  'F9=Back    F3=Clear   *F5=Fastmode    F-H=Home    F-L=End'
0205                       even
0206               
0207               
0208               ;--------------------------------------------------------------
...
0210               ;--------------------------------------------------------------
0211               txt.head.save
0212 7770 0F53             byte  15
0213 7771 ....             text  'Save DV80 file '
0214                       even
0215               
0216               txt.head.save2
0217 7780 1D53             byte  29
0218 7781 ....             text  'Save code block to DV80 file '
0219                       even
0220               
0221               txt.hint.save
0222 779E 3F48             byte  63
0223 779F ....             text  'HINT: Fastmode uses CPU RAM instead of VDP RAM for file buffer.'
0224                       even
0225               
0226               txt.keys.save
0227 77DE 3046             byte  48
0228 77DF ....             text  'F9=Back    F3=Clear    Fctn-H=Home    Fctn-L=End'
0229                       even
0230               
0231               
0232               ;--------------------------------------------------------------
...
0236 7810 1055             byte  16
0237 7811 ....             text  'Unsaved changes '
0238                       even
0239               
0240               txt.info.unsaved
0241 7822 3259             byte  50
0242 7823 ....             text  'You are about to lose changes to the current file!'
0243                       even
0244               
0245               txt.hint.unsaved
0246 7856 3F48             byte  63
0247 7857 ....             text  'HINT: Press F6 to proceed without saving or ENTER to save file.'
0248                       even
0249               
0250               txt.keys.unsaved
0251 7896 2846             byte  40
0252 7897 ....             text  'F9=Back    F6=Proceed    ENTER=Save file'
0253                       even
0254               
0255               
0256               ;--------------------------------------------------------------
...
0258               ;--------------------------------------------------------------
0259               txt.head.about
0260 78C0 0D41             byte  13
0261 78C1 ....             text  'About Stevie '
0262                       even
0263               
0264               txt.hint.about
0265 78CE 2C48             byte  44
0266 78CF ....             text  'HINT: Press F9 or ENTER to return to editor.'
0267                       even
0268               
0269               txt.keys.about
0270 78FC 1546             byte  21
0271 78FD ....             text  'F9=Back    ENTER=Back'
0272                       even
0273               
0274               
0275               ;--------------------------------------------------------------
...
0277               ;--------------------------------------------------------------
0278               txt.ioerr.load
0279 7912 2049             byte  32
0280 7913 ....             text  'I/O error. Failed loading file: '
0281                       even
0282               
0283               txt.ioerr.save
0284 7934 1F49             byte  31
0285 7935 ....             text  'I/O error. Failed saving file: '
0286                       even
0287               
0288               txt.io.nofile
0289 7954 2149             byte  33
0290 7955 ....             text  'I/O error. No filename specified.'
0291                       even
0292               
0293               
0294               
...
0297               ;--------------------------------------------------------------
0298               txt.cmdb.title
0299 7976 0E43             byte  14
0300 7977 ....             text  'Command buffer'
0301                       even
0302               
0303               txt.cmdb.prompt
0304 7986 013E             byte  1
0305 7987 ....             text  '>'
0306                       even
0307               
0308               
0309 7988 0C0A     txt.stevie         byte    12
0310                                  byte    10
0311 798A ....                        text    'stevie V0.1I'
0312 7996 0B00                        byte    11
0313                                  even
0314               
0315               txt.colorscheme
0316 7998 0D43             byte  13
0317 7999 ....             text  'Color scheme:'
0318                       even
0319               
**** **** ****     > mem.resident.3000.asm
...
0020                       ;------------------------------------------------------
0021 79A6 DEAD             data  >dead,>beef,>dead,>beef
     79A8 BEEF 
     79AA DEAD 
     79AC BEEF 
**** **** ****     > stevie_b0.asm.977512
0150               
0154 79AE 3944                   data $                ; Bank 0 ROM size OK.
0156                       ;-----------------------------------------------------------------------
0157                       ; Bank specific vector table
0158                       ;-----------------------------------------------------------------------
0162 79B0 3946                   data $                ; Bank 0 ROM size OK.
0164                       ;-------------------------------------------------------
...
0175      32E0     spvmod  equ   stevie.tx8030         ; Video mode.   See VIDTAB for details.
