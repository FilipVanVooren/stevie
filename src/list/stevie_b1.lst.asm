...
0008               ***************************************************************
0009               * File: stevie_b1.asm                 ; Version 200607-213410
0010               
0011               
0012               ***************************************************************
...
0007               *              (c)2018-2020 // Filip van Vooren
0008               ***************************************************************
0009               * File: equates.asm                 ; Version 200607-213410
0010               *--------------------------------------------------------------
...
0031 6015 ....             text  'STEVIE 200607-213410'
0032                       even
0033               
0041               
0042               *--------------------------------------------------------------
...
0260 21DB ....             text  'Build-ID  200607-213410'
0261                       even
0262               
**** **** ****     > runlib.asm
...
0349 2E18 06A0  32 runlic  bl    @vidtab               ; Load video mode table into VDP
     2E1A 22F2 
0350 2E1C 741C             data  spvmod                ; Equate selected video mode table
...
0084 60B4 06A0  32         bl    @cpym2m
     60B6 247A 
0085 60B8 7426                   data romsat,ramsat,4  ; Load sprite SAT
     60BA 8380 
     60BC 0004 
0086               
...
0123 6114 06A0  32         bl    @mkhook
     6116 2D40 
0124 6118 7088                   data hook.keyscan     ; Setup user hook
0125               
...
0026 670C 06A0  32         bl    @sams.layout
     670E 2582 
0027 6710 749A                   data mem.sams.layout.data
0028               
...
0165 6BA0 1317  14         jeq   edb.line.pack.exit    ; Nothing to copy if empty line
0166               
0167                       ;------------------------------------------------------
...
0190                       ;------------------------------------------------------
0191 6BBC A820  54 !       a     @rambuf+4,@edb.next_free.ptr
     6BBE 8394 
     6BC0 A208 
0192                                                      ; Add length of line
0193               
0194 6BC2 C120  34         mov   @edb.next_free.ptr,tmp0  ; \ Round up to next multiple of 16.
     6BC4 A208 
0195 6BC6 0504  16         neg   tmp0                     ; | tmp0 = tmp0 + (-tmp0 & 15)
0196 6BC8 0244  22         andi  tmp0,15                  ; | Hacker's Delight 2nd Edition
     6BCA 000F 
0197 6BCC A804  38         a     tmp0,@edb.next_free.ptr  ; / Chapter 2
     6BCE A208 
0198                       ;------------------------------------------------------
0199                       ; Exit
0200                       ;------------------------------------------------------
0201               edb.line.pack.exit:
0202 6BD0 C820  54         mov   @rambuf,@fb.column    ; Retrieve @fb.column
     6BD2 8390 
     6BD4 A10C 
0203 6BD6 0460  28         b     @poprt                ; Return to caller
     6BD8 222C 
0204               
0205               
0206               
...
0232 6BDA 0649  14         dect  stack
0233 6BDC C64B  30         mov   r11,*stack            ; Save return address
0234 6BDE 0649  14         dect  stack
0235 6BE0 C644  30         mov   tmp0,*stack           ; Push tmp0
0236 6BE2 0649  14         dect  stack
0237 6BE4 C645  30         mov   tmp1,*stack           ; Push tmp1
0238 6BE6 0649  14         dect  stack
0239 6BE8 C646  30         mov   tmp2,*stack           ; Push tmp2
0240                       ;------------------------------------------------------
0241                       ; Sanity check
0242                       ;------------------------------------------------------
0243 6BEA 8820  54         c     @parm1,@edb.lines     ; Beyond editor buffer ?
     6BEC 8350 
     6BEE A204 
0244 6BF0 1104  14         jlt   !
0245 6BF2 C80B  38         mov   r11,@>ffce            ; \ Save caller address
     6BF4 FFCE 
0246 6BF6 06A0  32         bl    @cpu.crash            ; / Crash and halt system
     6BF8 2030 
0247                       ;------------------------------------------------------
0248                       ; Save parameters
0249                       ;------------------------------------------------------
0250 6BFA C820  54 !       mov   @parm1,@rambuf
     6BFC 8350 
     6BFE 8390 
0251 6C00 C820  54         mov   @parm2,@rambuf+2
     6C02 8352 
     6C04 8392 
0252                       ;------------------------------------------------------
0253                       ; Calculate offset in frame buffer
0254                       ;------------------------------------------------------
0255 6C06 C120  34         mov   @fb.colsline,tmp0
     6C08 A10E 
0256 6C0A 3920  72         mpy   @parm2,tmp0           ; Offset is in tmp1!
     6C0C 8352 
0257 6C0E C1A0  34         mov   @fb.top.ptr,tmp2
     6C10 A100 
0258 6C12 A146  18         a     tmp2,tmp1             ; Add base to offset
0259 6C14 C805  38         mov   tmp1,@rambuf+6        ; Destination row in frame buffer
     6C16 8396 
0260                       ;------------------------------------------------------
0261                       ; Get pointer to line & page-in editor buffer page
0262                       ;------------------------------------------------------
0263 6C18 C120  34         mov   @parm1,tmp0
     6C1A 8350 
0264 6C1C 06A0  32         bl    @xmem.edb.sams.mappage
     6C1E 6724 
...
0269               
0270 6C20 C820  54         mov   @outparm2,@edb.sams.page
     6C22 8362 
     6C24 A212 
...
0274                       ;------------------------------------------------------
0275 6C26 C120  34         mov   @outparm1,tmp0        ; Get pointer to line
     6C28 8360 
0276 6C2A 1603  14         jne   !                     ; Check if pointer is set
0277 6C2C 04E0  34         clr   @rambuf+8             ; Set length=0
     6C2E 8398 
0278 6C30 100F  14         jmp   edb.line.unpack.clear
0279                       ;------------------------------------------------------
0280                       ; Get line length
0281                       ;------------------------------------------------------
0282 6C32 C154  26 !       mov   *tmp0,tmp1            ; Get line length
0283 6C34 C805  38         mov   tmp1,@rambuf+8        ; Save line length
     6C36 8398 
0284               
0285 6C38 05E0  34         inct  @outparm1             ; Skip line prefix
     6C3A 8360 
0286 6C3C C820  54         mov   @outparm1,@rambuf+4   ; Source memory address for block copy
     6C3E 8360 
     6C40 8394 
0287                       ;------------------------------------------------------
0288                       ; Sanity check on line length
0289                       ;------------------------------------------------------
0290 6C42 0285  22         ci    tmp1,80               ; \ Continue if length <= 80
     6C44 0050 
0291 6C46 1204  14         jle   edb.line.unpack.clear ; /
0292               
0293 6C48 C80B  38         mov   r11,@>ffce            ; \ Save caller address
     6C4A FFCE 
0294 6C4C 06A0  32         bl    @cpu.crash            ; / Crash and halt system
     6C4E 2030 
0295                       ;------------------------------------------------------
0296                       ; Erase chars from last column until column 80
0297                       ;------------------------------------------------------
0298               edb.line.unpack.clear:
0299 6C50 C120  34         mov   @rambuf+6,tmp0        ; Start of row in frame buffer
     6C52 8396 
0300 6C54 A120  34         a     @rambuf+8,tmp0        ; Skip until end of row in frame buffer
     6C56 8398 
0301               
0302 6C58 04C5  14         clr   tmp1                  ; Fill with >00
0303 6C5A C1A0  34         mov   @fb.colsline,tmp2
     6C5C A10E 
0304 6C5E 61A0  34         s     @rambuf+8,tmp2        ; Calculate number of bytes to clear
     6C60 8398 
0305 6C62 0586  14         inc   tmp2
0306               
0307 6C64 06A0  32         bl    @xfilm                ; Fill CPU memory
     6C66 2236 
...
0315 6C68 C1A0  34         mov   @rambuf+8,tmp2        ; Line length
     6C6A 8398 
0316 6C6C 130D  14         jeq   edb.line.unpack.exit  ; Exit if length = 0
0317 6C6E C120  34         mov   @rambuf+4,tmp0        ; Pointer to line in editor buffer
     6C70 8394 
0318 6C72 C160  34         mov   @rambuf+6,tmp1        ; Pointer to row in frame buffer
     6C74 8396 
0319                       ;------------------------------------------------------
0320                       ; Check before copy
0321                       ;------------------------------------------------------
0322               edb.line.unpack.copy:
0323 6C76 0286  22         ci    tmp2,80               ; Check line length
     6C78 0050 
0324 6C7A 1204  14         jle   !
0325 6C7C C80B  38         mov   r11,@>ffce            ; \ Save caller address
     6C7E FFCE 
0326 6C80 06A0  32         bl    @cpu.crash            ; / Crash and halt system
     6C82 2030 
0327                       ;------------------------------------------------------
0328                       ; Copy memory block
0329                       ;------------------------------------------------------
0330 6C84 06A0  32 !       bl    @xpym2m               ; Copy line to frame buffer
     6C86 2480 
...
0338 6C88 C1B9  30         mov   *stack+,tmp2          ; Pop tmp2
0339 6C8A C179  30         mov   *stack+,tmp1          ; Pop tmp1
0340 6C8C C139  30         mov   *stack+,tmp0          ; Pop tmp0
0341 6C8E C2F9  30         mov   *stack+,r11           ; Pop r11
0342 6C90 045B  20         b     *r11                  ; Return to caller
0343               
0344               
0345               
0346               ***************************************************************
...
0366 6C92 0649  14         dect  stack
0367 6C94 C64B  30         mov   r11,*stack            ; Push return address
0368 6C96 0649  14         dect  stack
0369 6C98 C644  30         mov   tmp0,*stack           ; Push tmp0
0370 6C9A 0649  14         dect  stack
0371 6C9C C645  30         mov   tmp1,*stack           ; Push tmp1
0372                       ;------------------------------------------------------
0373                       ; Initialisation
0374                       ;------------------------------------------------------
0375 6C9E 04E0  34         clr   @outparm1             ; Reset length
     6CA0 8360 
0376 6CA2 04E0  34         clr   @outparm2             ; Reset SAMS bank
     6CA4 8362 
0377                       ;------------------------------------------------------
0378                       ; Get length
0379                       ;------------------------------------------------------
0380 6CA6 06A0  32         bl    @idx.pointer.get      ; Get pointer to line
     6CA8 69E4 
0381                                                   ; \ i  parm1    = Line number
0382                                                   ; | o  outparm1 = Pointer to line
0383                                                   ; / o  outparm2 = SAMS page
0384               
0385 6CAA C120  34         mov   @outparm1,tmp0        ; Is pointer set?
     6CAC 8360 
0386 6CAE 1302  14         jeq   edb.line.getlength.exit
0387                                                   ; Exit early if NULL pointer
0388                       ;------------------------------------------------------
0389                       ; Process line prefix
0390                       ;------------------------------------------------------
0391 6CB0 C814  46         mov   *tmp0,@outparm1       ; Save length
     6CB2 8360 
0392                       ;------------------------------------------------------
0393                       ; Exit
0394                       ;------------------------------------------------------
0395               edb.line.getlength.exit:
0396 6CB4 C179  30         mov   *stack+,tmp1          ; Pop tmp1
0397 6CB6 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0398 6CB8 C2F9  30         mov   *stack+,r11           ; Pop r11
0399 6CBA 045B  20         b     *r11                  ; Return to caller
0400               
0401               
0402               
0403               ***************************************************************
...
0419 6CBC 0649  14         dect  stack
0420 6CBE C64B  30         mov   r11,*stack            ; Save return address
0421                       ;------------------------------------------------------
0422                       ; Calculate line in editor buffer
0423                       ;------------------------------------------------------
0424 6CC0 C120  34         mov   @fb.topline,tmp0      ; Get top line in frame buffer
     6CC2 A104 
0425 6CC4 A120  34         a     @fb.row,tmp0          ; Get current row in frame buffer
     6CC6 A106 
0426                       ;------------------------------------------------------
0427                       ; Get length
0428                       ;------------------------------------------------------
0429 6CC8 C804  38         mov   tmp0,@parm1
     6CCA 8350 
0430 6CCC 06A0  32         bl    @edb.line.getlength
     6CCE 6C92 
0431 6CD0 C820  54         mov   @outparm1,@fb.row.length
     6CD2 8360 
     6CD4 A108 
...
0437 6CD6 0460  28         b     @poprt                ; Return to caller
     6CD8 222C 
0438               
**** **** ****     > stevie_b1.asm.213410
...
0025               ********|*****|*********************|**************************
0026               cmdb.init:
0027 6CDA 0649  14         dect  stack
0028 6CDC C64B  30         mov   r11,*stack            ; Save return address
0029                       ;------------------------------------------------------
0030                       ; Initialize
0031                       ;------------------------------------------------------
0032 6CDE 0204  20         li    tmp0,cmdb.top         ; \ Set pointer to command buffer
     6CE0 D000 
0033 6CE2 C804  38         mov   tmp0,@cmdb.top.ptr    ; /
     6CE4 A300 
0034               
0035 6CE6 04E0  34         clr   @cmdb.visible         ; Hide command buffer
     6CE8 A302 
0036 6CEA 0204  20         li    tmp0,10
     6CEC 000A 
0037 6CEE C804  38         mov   tmp0,@cmdb.scrrows    ; Set current command buffer size
     6CF0 A304 
0038 6CF2 C804  38         mov   tmp0,@cmdb.default    ; Set default command buffer size
     6CF4 A306 
0039               
0040 6CF6 0204  20         li    tmp0,>1b02            ; Y=27, X=2
     6CF8 1B02 
0041 6CFA C804  38         mov   tmp0,@cmdb.cursor     ; Screen position of cursor in cmdb pane
     6CFC A308 
0042               
0043 6CFE 04E0  34         clr   @cmdb.lines           ; Number of lines in cmdb buffer
     6D00 A30E 
0044 6D02 04E0  34         clr   @cmdb.dirty           ; Command buffer is clean
     6D04 A310 
0045                       ;------------------------------------------------------
0046                       ; Clear command buffer
0047                       ;------------------------------------------------------
0048 6D06 06A0  32         bl    @film
     6D08 2230 
0049 6D0A D000             data  cmdb.top,>00,cmdb.size
     6D0C 0000 
     6D0E 1000 
...
0053                       ; Exit
0054                       ;------------------------------------------------------
0055 6D10 C2F9  30         mov   *stack+,r11           ; Pop r11
0056 6D12 045B  20         b     *r11                  ; Return to caller
0057               
0058               
0059               
0060               
...
0080               ********|*****|*********************|**************************
0081               cmdb.refresh:
0082 6D14 0649  14         dect  stack
0083 6D16 C64B  30         mov   r11,*stack            ; Save return address
0084 6D18 0649  14         dect  stack
0085 6D1A C644  30         mov   tmp0,*stack           ; Push tmp0
0086 6D1C 0649  14         dect  stack
0087 6D1E C645  30         mov   tmp1,*stack           ; Push tmp1
0088 6D20 0649  14         dect  stack
0089 6D22 C646  30         mov   tmp2,*stack           ; Push tmp2
0090                       ;------------------------------------------------------
0091                       ; Dump Command buffer content
0092                       ;------------------------------------------------------
0093 6D24 C820  54         mov   @wyx,@cmdb.yxsave     ; Save YX position
     6D26 832A 
     6D28 A30A 
0094               
0095 6D2A C820  54         mov   @cmdb.yxtop,@wyx
     6D2C A30C 
     6D2E 832A 
0096 6D30 06A0  32         bl    @yx2pnt               ; Get VDP PNT address for current YX pos.
     6D32 23F4 
0097               
0098 6D34 C160  34         mov   @cmdb.top.ptr,tmp1    ; Top of command buffer
     6D36 A300 
0099 6D38 0206  20         li    tmp2,9*80
     6D3A 02D0 
0100               
0101 6D3C 06A0  32         bl    @xpym2v               ; \ Copy CPU memory to VDP memory
     6D3E 2438 
...
0108                       ;------------------------------------------------------
0109 6D40 06A0  32         bl    @putat
     6D42 242A 
0110 6D44 1B01                   byte 27,1
0111 6D46 752E                   data txt.cmdb.prompt
0112               
0113 6D48 C820  54         mov   @cmdb.yxsave,@fb.yxsave
     6D4A A30A 
     6D4C A114 
0114 6D4E C820  54         mov   @cmdb.yxsave,@wyx
     6D50 A30A 
     6D52 832A 
...
0118                       ; Exit
0119                       ;------------------------------------------------------
0120 6D54 C1B9  30         mov   *stack+,tmp2          ; Pop tmp2
0121 6D56 C179  30         mov   *stack+,tmp1          ; Pop tmp1
0122 6D58 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0123 6D5A C2F9  30         mov   *stack+,r11           ; Pop r11
0124 6D5C 045B  20         b     *r11                  ; Return to caller
0125               
**** **** ****     > stevie_b1.asm.213410
...
0028 6D5E 0649  14         dect  stack
0029 6D60 C64B  30         mov   r11,*stack            ; Save return address
0030                       ;------------------------------------------------------
0031                       ; Initialisation
0032                       ;------------------------------------------------------
0033 6D62 04E0  34         clr   @fh.records           ; Reset records counter
     6D64 A42E 
0034 6D66 04E0  34         clr   @fh.counter           ; Clear internal counter
     6D68 A434 
0035 6D6A 04E0  34         clr   @fh.kilobytes         ; Clear kilobytes processed
     6D6C A432 
0036 6D6E 04C8  14         clr   tmp4                  ; Clear kilobytes processed display counter
0037 6D70 04E0  34         clr   @fh.pabstat           ; Clear copy of VDP PAB status byte
     6D72 A42A 
0038 6D74 04E0  34         clr   @fh.ioresult          ; Clear status register contents
     6D76 A42C 
0039               
0040 6D78 C120  34         mov   @edb.top.ptr,tmp0
     6D7A A200 
0041 6D7C 06A0  32         bl    @xsams.page.get       ; Get SAMS page
     6D7E 24DE 
0042                                                   ; \ i  tmp0  = Memory address
0043                                                   ; | o  waux1 = SAMS page number
0044                                                   ; / o  waux2 = Address of SAMS register
0045               
0046 6D80 C820  54         mov   @waux1,@fh.sams.page  ; Set current SAMS page
     6D82 833C 
     6D84 A438 
0047 6D86 C820  54         mov   @waux1,@fh.sams.hipage
     6D88 833C 
     6D8A A43A 
...
0051                       ;------------------------------------------------------
0052 6D8C C820  54         mov   @parm1,@fh.fname.ptr  ; Pointer to file descriptor
     6D8E 8350 
     6D90 A436 
0053 6D92 C820  54         mov   @parm2,@fh.callback1  ; Callback function "Open file"
     6D94 8352 
     6D96 A43C 
0054 6D98 C820  54         mov   @parm3,@fh.callback2  ; Callback function "Read line from file"
     6D9A 8354 
     6D9C A43E 
0055 6D9E C820  54         mov   @parm4,@fh.callback3  ; Callback function "Close" file"
     6DA0 8356 
     6DA2 A440 
0056 6DA4 C820  54         mov   @parm5,@fh.callback4  ; Callback function "File I/O error"
     6DA6 8358 
     6DA8 A442 
0057                       ;------------------------------------------------------
0058                       ; Sanity check
0059                       ;------------------------------------------------------
0060 6DAA C120  34         mov   @fh.callback1,tmp0
     6DAC A43C 
0061 6DAE 0284  22         ci    tmp0,>6000            ; Insane address ?
     6DB0 6000 
0062 6DB2 1114  14         jlt   fh.file.read.crash    ; Yes, crash!
0063               
0064 6DB4 0284  22         ci    tmp0,>7fff            ; Insane address ?
     6DB6 7FFF 
0065 6DB8 1511  14         jgt   fh.file.read.crash    ; Yes, crash!
0066               
0067 6DBA C120  34         mov   @fh.callback2,tmp0
     6DBC A43E 
0068 6DBE 0284  22         ci    tmp0,>6000            ; Insane address ?
     6DC0 6000 
0069 6DC2 110C  14         jlt   fh.file.read.crash    ; Yes, crash!
0070               
0071 6DC4 0284  22         ci    tmp0,>7fff            ; Insane address ?
     6DC6 7FFF 
0072 6DC8 1509  14         jgt   fh.file.read.crash    ; Yes, crash!
0073               
0074 6DCA C120  34         mov   @fh.callback3,tmp0
     6DCC A440 
0075 6DCE 0284  22         ci    tmp0,>6000            ; Insane address ?
     6DD0 6000 
0076 6DD2 1104  14         jlt   fh.file.read.crash    ; Yes, crash!
0077               
0078 6DD4 0284  22         ci    tmp0,>7fff            ; Insane address ?
     6DD6 7FFF 
0079 6DD8 1501  14         jgt   fh.file.read.crash    ; Yes, crash!
0080               
0081 6DDA 1004  14         jmp   fh.file.read.sams.load1
0082                                                   ; All checks passed, continue.
0083                                                   ;--------------------------
0084                                                   ; Check failed, crash CPU!
0085                                                   ;--------------------------
0086               fh.file.read.crash:
0087 6DDC C80B  38         mov   r11,@>ffce            ; \ Save caller address
     6DDE FFCE 
0088 6DE0 06A0  32         bl    @cpu.crash            ; / Crash and halt system
     6DE2 2030 
0089                       ;------------------------------------------------------
0090                       ; Callback "Before Open file"
0091                       ;------------------------------------------------------
0092               fh.file.read.sams.load1:
0093 6DE4 C120  34         mov   @fh.callback1,tmp0
     6DE6 A43C 
0094 6DE8 0694  24         bl    *tmp0                 ; Run callback function
0095                       ;------------------------------------------------------
0096                       ; Copy PAB header to VDP
0097                       ;------------------------------------------------------
0098               fh.file.read.sams.pabheader:
0099 6DEA 06A0  32         bl    @cpym2v
     6DEC 2432 
0100 6DEE 0A60                   data fh.vpab,fh.file.pab.header,9
     6DF0 6F3C 
     6DF2 0009 
...
0104                       ;------------------------------------------------------
0105 6DF4 0204  20         li    tmp0,fh.vpab + 9      ; VDP destination
     6DF6 0A69 
0106 6DF8 C160  34         mov   @fh.fname.ptr,tmp1    ; Get pointer to file descriptor
     6DFA A436 
0107 6DFC D195  26         movb  *tmp1,tmp2            ; Get file descriptor length
0108 6DFE 0986  56         srl   tmp2,8                ; Right justify
0109 6E00 0586  14         inc   tmp2                  ; Include length byte as well
0110 6E02 06A0  32         bl    @xpym2v               ; Append file descriptor to VDP PAB
     6E04 2438 
0111                       ;------------------------------------------------------
0112                       ; Load GPL scratchpad layout
0113                       ;------------------------------------------------------
0114 6E06 06A0  32         bl    @cpu.scrpad.pgout     ; \ Swap scratchpad memory (SPECTRA->GPL)
     6E08 2AC2 
0115 6E0A 3F00                   data scrpad.backup2   ; / 8300->2100, 2000->8300
0116                       ;------------------------------------------------------
0117                       ; Open file
0118                       ;------------------------------------------------------
0119 6E0C 06A0  32         bl    @file.open
     6E0E 2C10 
0120 6E10 0A60                   data fh.vpab          ; Pass file descriptor to DSRLNK
0121 6E12 21A0  38         coc   @wbit2,tmp2           ; Equal bit set?
     6E14 2026 
0122 6E16 1602  14         jne   fh.file.read.sams.record
0123 6E18 0460  28         b     @fh.file.read.sams.error
     6E1A 6F0A 
...
0129 6E1C 05A0  34         inc   @fh.records           ; Update counter
     6E1E A42E 
0130 6E20 04E0  34         clr   @fh.reclen            ; Reset record length
     6E22 A430 
0131               
0132 6E24 06A0  32         bl    @file.record.read     ; Read file record
     6E26 2C52 
0133 6E28 0A60                   data fh.vpab          ; \ i  p0   = Address of PAB in VDP RAM
...
0139               
0140 6E2A C804  38         mov   tmp0,@fh.pabstat      ; Save VDP PAB status byte
     6E2C A42A 
0141 6E2E C805  38         mov   tmp1,@fh.reclen       ; Save bytes read
     6E30 A430 
0142 6E32 C806  38         mov   tmp2,@fh.ioresult     ; Save status register contents
     6E34 A42C 
0143                       ;------------------------------------------------------
0144                       ; 1a: Calculate kilobytes processed
0145                       ;------------------------------------------------------
0146 6E36 A805  38         a     tmp1,@fh.counter
     6E38 A434 
0147 6E3A A160  34         a     @fh.counter,tmp1
     6E3C A434 
0148 6E3E 0285  22         ci    tmp1,1024
     6E40 0400 
0149 6E42 1106  14         jlt   !
0150 6E44 05A0  34         inc   @fh.kilobytes
     6E46 A432 
0151 6E48 0225  22         ai    tmp1,-1024            ; Remove KB portion and keep bytes
     6E4A FC00 
0152 6E4C C805  38         mov   tmp1,@fh.counter
     6E4E A434 
0153                       ;------------------------------------------------------
0154                       ; 1b: Load spectra scratchpad layout
0155                       ;------------------------------------------------------
0156 6E50 06A0  32 !       bl    @cpu.scrpad.backup    ; Backup GPL layout to @cpu.scrpad.tgt
     6E52 2A48 
0157 6E54 06A0  32         bl    @cpu.scrpad.pgin      ; \ Swap scratchpad memory (GPL->SPECTRA)
     6E56 2AE4 
0158 6E58 3F00                   data scrpad.backup2   ; / @scrpad.backup2 to >8300
0159                       ;------------------------------------------------------
0160                       ; 1c: Check if a file error occured
0161                       ;------------------------------------------------------
0162               fh.file.read.sams.check_fioerr:
0163 6E5A C1A0  34         mov   @fh.ioresult,tmp2
     6E5C A42C 
0164 6E5E 21A0  38         coc   @wbit2,tmp2           ; IO error occured?
     6E60 2026 
0165 6E62 1602  14         jne   fh.file.read.sams.check_setpage
0166                                                   ; No, goto (1d)
0167 6E64 0460  28         b     @fh.file.read.sams.error
     6E66 6F0A 
...
0173 6E68 C120  34         mov   @edb.next_free.ptr,tmp0
     6E6A A208 
0174                                                   ;--------------------------
0175                                                   ; Sanity check
0176                                                   ;--------------------------
0177 6E6C 0284  22         ci    tmp0,edb.top + edb.size
     6E6E D000 
0178                                                   ; Insane address ?
0179 6E70 15B5  14         jgt   fh.file.read.crash    ; Yes, crash!
0180                                                   ;--------------------------
0181                                                   ; Check overflow
0182                                                   ;--------------------------
0183 6E72 0244  22         andi  tmp0,>0fff            ; Get rid off highest nibble
     6E74 0FFF 
0184 6E76 A120  34         a     @fh.reclen,tmp0       ; Add length of line just read
     6E78 A430 
0185 6E7A 05C4  14         inct  tmp0                  ; +2 for line prefix
0186 6E7C 0284  22         ci    tmp0,>1000 - 16       ; 4K boundary reached?
     6E7E 0FF0 
0187 6E80 110E  14         jlt   fh.file.read.sams.process_line
0188                                                   ; Not yet so skip SAMS page switch
0189                       ;------------------------------------------------------
0190                       ; 1e: Increase SAMS page
0191                       ;------------------------------------------------------
0192 6E82 05A0  34         inc   @fh.sams.page         ; Next SAMS page
     6E84 A438 
0193 6E86 C820  54         mov   @fh.sams.page,@fh.sams.hipage
     6E88 A438 
     6E8A A43A 
0194                                                   ; Set highest SAMS page
0195 6E8C C820  54         mov   @edb.top.ptr,@edb.next_free.ptr
     6E8E A200 
     6E90 A208 
...
0199                       ;------------------------------------------------------
0200 6E92 C120  34         mov   @fh.sams.page,tmp0
     6E94 A438 
0201 6E96 C160  34         mov   @edb.top.ptr,tmp1
     6E98 A200 
0202 6E9A 06A0  32         bl    @xsams.page.set       ; Set SAMS page
     6E9C 2516 
...
0209 6E9E 0204  20         li    tmp0,fh.vrecbuf       ; VDP source address
     6EA0 0960 
0210 6EA2 C160  34         mov   @edb.next_free.ptr,tmp1
     6EA4 A208 
0211                                                   ; RAM target in editor buffer
0212               
0213 6EA6 C805  38         mov   tmp1,@parm2           ; Needed in step 4b (index update)
     6EA8 8352 
0214               
0215 6EAA C1A0  34         mov   @fh.reclen,tmp2       ; Number of bytes to copy
     6EAC A430 
0216 6EAE 1318  14         jeq   fh.file.read.sams.prepindex.emptyline
0217                                                   ; Handle empty line
0218                       ;------------------------------------------------------
0219                       ; 2a: Copy line from VDP to CPU editor buffer
0220                       ;------------------------------------------------------
0221                                                   ; Put line length word before string
0222 6EB0 DD46  32         movb  tmp2,*tmp1+           ; \ MSB to line prefix
0223 6EB2 06C6  14         swpb  tmp2                  ; |
0224 6EB4 DD46  32         movb  tmp2,*tmp1+           ; | LSB to line prefix
0225 6EB6 06C6  14         swpb  tmp2                  ; /
0226               
0227 6EB8 05E0  34         inct  @edb.next_free.ptr    ; Keep pointer synced with tmp1
     6EBA A208 
0228 6EBC A806  38         a     tmp2,@edb.next_free.ptr
     6EBE A208 
0229                                                   ; Add line length
0230               
0231 6EC0 06A0  32         bl    @xpyv2m               ; Copy memory block from VDP to CPU
     6EC2 245E 
...
0237                       ;------------------------------------------------------
0238 6EC4 C120  34         mov   @edb.next_free.ptr,tmp0  ; \ Round up to next multiple of 16.
     6EC6 A208 
0239 6EC8 0504  16         neg   tmp0                     ; | tmp0 = tmp0 + (-tmp0 & 15)
0240 6ECA 0244  22         andi  tmp0,15                  ; | Hacker's Delight 2nd Edition
     6ECC 000F 
0241 6ECE A804  38         a     tmp0,@edb.next_free.ptr  ; / Chapter 2
     6ED0 A208 
0242                       ;------------------------------------------------------
0243                       ; Step 3: Update index
0244                       ;------------------------------------------------------
0245               fh.file.read.sams.prepindex:
0246 6ED2 C820  54         mov   @edb.lines,@parm1     ; parm1 = Line number
     6ED4 A204 
     6ED6 8350 
0247                                                   ; parm2 = Must allready be set!
0248 6ED8 C820  54         mov   @fh.sams.page,@parm3  ; parm3 = SAMS page number
     6EDA A438 
     6EDC 8354 
0249               
0250 6EDE 1009  14         jmp   fh.file.read.sams.updindex
0251                                                   ; Update index
0252                       ;------------------------------------------------------
0253                       ; 3a: Special handling for empty line
0254                       ;------------------------------------------------------
0255               fh.file.read.sams.prepindex.emptyline:
0256 6EE0 C820  54         mov   @fh.records,@parm1    ; parm1 = Line number
     6EE2 A42E 
     6EE4 8350 
0257 6EE6 0620  34         dec   @parm1                ;         Adjust for base 0 index
     6EE8 8350 
0258 6EEA 04E0  34         clr   @parm2                ; parm2 = Pointer to >0000
     6EEC 8352 
0259 6EEE 0720  34         seto  @parm3                ; parm3 = SAMS not used >FFFF
     6EF0 8354 
0260                       ;------------------------------------------------------
0261                       ; 3b: Do actual index update
0262                       ;------------------------------------------------------
0263               fh.file.read.sams.updindex:
0264 6EF2 06A0  32         bl    @idx.entry.update     ; Update index
     6EF4 6992 
...
0270                                                   ; /               entry
0271               
0272 6EF6 05A0  34         inc   @edb.lines            ; lines=lines+1
     6EF8 A204 
0273                       ;------------------------------------------------------
0274                       ; Step 4: Callback "Read line from file"
0275                       ;------------------------------------------------------
0276               fh.file.read.sams.display:
0277 6EFA C120  34         mov   @fh.callback2,tmp0    ; Get pointer to "Loading indicator 2"
     6EFC A43E 
0278 6EFE 0694  24         bl    *tmp0                 ; Run callback function
0279                       ;------------------------------------------------------
0280                       ; 4a: Next record
0281                       ;------------------------------------------------------
0282               fh.file.read.sams.next:
0283 6F00 06A0  32         bl    @cpu.scrpad.pgout     ; \ Swap scratchpad memory (SPECTRA->GPL)
     6F02 2AC2 
0284 6F04 3F00                   data scrpad.backup2   ; / 8300->2100, 2000->8300
0285               
0286 6F06 0460  28         b     @fh.file.read.sams.record
     6F08 6E1C 
0287                                                   ; Next record
0288                       ;------------------------------------------------------
0289                       ; Error handler
...
0292 6F0A C120  34         mov   @fh.pabstat,tmp0      ; Get VDP PAB status byte
     6F0C A42A 
0293 6F0E 0984  56         srl   tmp0,8                ; Right align VDP PAB 1 status byte
0294 6F10 0284  22         ci    tmp0,io.err.eof       ; EOF reached ?
     6F12 0005 
0295 6F14 1309  14         jeq   fh.file.read.sams.eof
0296                                                   ; All good. File closed by DSRLNK
0297                       ;------------------------------------------------------
0298                       ; File error occured
0299                       ;------------------------------------------------------
0300 6F16 06A0  32         bl    @cpu.scrpad.pgin      ; \ Swap scratchpad memory (GPL->SPECTRA)
     6F18 2AE4 
0301 6F1A 3F00                   data scrpad.backup2   ; / >2100->8300
0302               
0303 6F1C 06A0  32         bl    @mem.sams.layout      ; Restore SAMS windows
     6F1E 6708 
0304                       ;------------------------------------------------------
0305                       ; Callback "File I/O error"
0306                       ;------------------------------------------------------
0307 6F20 C120  34         mov   @fh.callback4,tmp0    ; Get pointer to Callback "File I/O error"
     6F22 A442 
0308 6F24 0694  24         bl    *tmp0                 ; Run callback function
0309 6F26 1008  14         jmp   fh.file.read.sams.exit
0310                       ;------------------------------------------------------
0311                       ; End-Of-File reached
0312                       ;------------------------------------------------------
0313               fh.file.read.sams.eof:
0314 6F28 06A0  32         bl    @cpu.scrpad.pgin      ; \ Swap scratchpad memory (GPL->SPECTRA)
     6F2A 2AE4 
0315 6F2C 3F00                   data scrpad.backup2   ; / >2100->8300
0316               
0317 6F2E 06A0  32         bl    @mem.sams.layout      ; Restore SAMS windows
     6F30 6708 
0318                       ;------------------------------------------------------
0319                       ; Callback "Close file"
0320                       ;------------------------------------------------------
0321 6F32 C120  34         mov   @fh.callback3,tmp0    ; Get pointer to Callback "Close file"
     6F34 A440 
0322 6F36 0694  24         bl    *tmp0                 ; Run callback function
0323               *--------------------------------------------------------------
0324               * Exit
0325               *--------------------------------------------------------------
0326               fh.file.read.sams.exit:
0327 6F38 C2F9  30         mov   *stack+,r11           ; Pop r11
0328 6F3A 045B  20         b     *r11                  ; Return to caller
0329               
0330               
0331               
0332               ***************************************************************
...
0336 6F3C 0014             byte  io.op.open            ;  0    - OPEN
0337                       byte  io.ft.sf.ivd          ;  1    - INPUT, VARIABLE, DISPLAY
0338 6F3E 0960             data  fh.vrecbuf            ;  2-3  - Record buffer in VDP memory
0339 6F40 5000             byte  80                    ;  4    - Record length (80 chars max)
0340                       byte  00                    ;  5    - Character count
0341 6F42 0000             data  >0000                 ;  6-7  - Seek record (only for fixed recs)
0342 6F44 0000             byte  >00                   ;  8    - Screen offset (cassette DSR only)
0343                       ;------------------------------------------------------
...
0043                       ;-------------------------------------------------------
0044 6F80 0204  20         li    tmp0,fm.loadfile.cb.indicator1
     6F82 6FB4 
0045 6F84 C804  38         mov   tmp0,@parm2           ; Register callback 1
     6F86 8352 
0046               
0047 6F88 0204  20         li    tmp0,fm.loadfile.cb.indicator2
     6F8A 6FDC 
0048 6F8C C804  38         mov   tmp0,@parm3           ; Register callback 2
     6F8E 8354 
0049               
0050 6F90 0204  20         li    tmp0,fm.loadfile.cb.indicator3
     6F92 700E 
0051 6F94 C804  38         mov   tmp0,@parm4           ; Register callback 3
     6F96 8356 
0052               
0053 6F98 0204  20         li    tmp0,fm.loadfile.cb.fioerr
     6F9A 7040 
...
0089 6FB4 0649  14         dect  stack
0090 6FB6 C64B  30         mov   r11,*stack            ; Save return address
0091                       ;------------------------------------------------------
0092                       ; Show loading indicators and file descriptor
0093                       ;------------------------------------------------------
0094 6FB8 06A0  32         bl    @hchar
     6FBA 2762 
0095 6FBC 1D03                   byte 29,3,32,77
     6FBE 204D 
0096 6FC0 FFFF                   data EOL
0097               
0098 6FC2 06A0  32         bl    @putat
     6FC4 242A 
0099 6FC6 1D03                   byte 29,3
0100 6FC8 74DA                   data txt.loading      ; Display "Loading...."
0101               
0102 6FCA 06A0  32         bl    @at
     6FCC 266E 
0103 6FCE 1D0E                   byte 29,14            ; Cursor YX position
0104 6FD0 C160  34         mov   @parm1,tmp1           ; Get pointer to file descriptor
     6FD2 8350 
0105 6FD4 06A0  32         bl    @xutst0               ; Display device/filename
     6FD6 241A 
0106                       ;------------------------------------------------------
0107                       ; Exit
0108                       ;------------------------------------------------------
0109               fm.loadfile.cb.indicator1.exit:
0110 6FD8 0460  28         b     @poprt                ; Return to caller
     6FDA 222C 
0111               
0112               
0113               
...
0122 6FDC 0649  14         dect  stack
0123 6FDE C64B  30         mov   r11,*stack            ; Save return address
0124               
0125 6FE0 06A0  32         bl    @putnum
     6FE2 2A3E 
0126 6FE4 1D4B                   byte 29,75            ; Show lines read
0127 6FE6 A204                   data edb.lines,rambuf,>3020
     6FE8 8390 
     6FEA 3020 
0128               
0129 6FEC 8220  34         c     @fh.kilobytes,tmp4
     6FEE A432 
0130 6FF0 130C  14         jeq   fm.loadfile.cb.indicator2.exit
0131               
0132 6FF2 C220  34         mov   @fh.kilobytes,tmp4    ; Save for compare
     6FF4 A432 
0133               
0134 6FF6 06A0  32         bl    @putnum
     6FF8 2A3E 
0135 6FFA 1D38                   byte 29,56            ; Show kilobytes read
0136 6FFC A432                   data fh.kilobytes,rambuf,>3020
     6FFE 8390 
     7000 3020 
0137               
0138 7002 06A0  32         bl    @putat
     7004 242A 
0139 7006 1D3D                   byte 29,61
0140 7008 74E6                   data txt.kb           ; Show "kb" string
0141                       ;------------------------------------------------------
0142                       ; Exit
0143                       ;------------------------------------------------------
0144               fm.loadfile.cb.indicator2.exit:
0145 700A 0460  28         b     @poprt                ; Return to caller
     700C 222C 
0146               
0147               
0148               
...
0158 700E 0649  14         dect  stack
0159 7010 C64B  30         mov   r11,*stack            ; Save return address
0160               
0161               
0162 7012 06A0  32         bl    @hchar
     7014 2762 
0163 7016 1D03                   byte 29,3,32,50       ; Erase loading indicator
     7018 2032 
0164 701A FFFF                   data EOL
0165               
0166 701C 06A0  32         bl    @putnum
     701E 2A3E 
0167 7020 1D38                   byte 29,56            ; Show kilobytes read
0168 7022 A432                   data fh.kilobytes,rambuf,>3020
     7024 8390 
     7026 3020 
0169               
0170 7028 06A0  32         bl    @putat
     702A 242A 
0171 702C 1D3D                   byte 29,61
0172 702E 74E6                   data txt.kb           ; Show "kb" string
0173               
0174 7030 06A0  32         bl    @putnum
     7032 2A3E 
0175 7034 1D4B                   byte 29,75            ; Show lines read
0176 7036 A42E                   data fh.records,rambuf,>3020
     7038 8390 
     703A 3020 
0177                       ;------------------------------------------------------
0178                       ; Exit
...
0181 703C 0460  28         b     @poprt                ; Return to caller
     703E 222C 
0182               
0183               
0184               
...
0191 7040 0649  14         dect  stack
0192 7042 C64B  30         mov   r11,*stack            ; Save return address
0193               
0194 7044 06A0  32         bl    @hchar
     7046 2762 
0195 7048 1D00                   byte 29,0,32,50       ; Erase loading indicator
     704A 2032 
0196 704C FFFF                   data EOL
0197               
0198                       ;------------------------------------------------------
0199                       ; Display I/O error message
0200                       ;------------------------------------------------------
0201 704E 06A0  32         bl    @cpym2m
     7050 247A 
0202 7052 74F5                   data txt.ioerr+1
0203 7054 D000                   data cmdb.top
0204 7056 0029                   data 41               ; Error message
0205               
0206               
0207 7058 C120  34         mov   @edb.filename.ptr,tmp0
     705A A20E 
0208 705C D194  26         movb  *tmp0,tmp2            ; Get length byte
0209 705E 0986  56         srl   tmp2,8                ; Right align
0210 7060 0584  14         inc   tmp0                  ; Skip length byte
0211 7062 0205  20         li    tmp1,cmdb.top + 42    ; RAM destination address
     7064 D02A 
0212               
0213 7066 06A0  32         bl    @xpym2m               ; \ Copy CPU memory to CPU memory
     7068 2480 
...
0217               
0218               
0219 706A 0204  20         li    tmp0,txt.newfile      ; New file
     706C 7522 
0220 706E C804  38         mov   tmp0,@edb.filename.ptr
     7070 A20E 
0221               
0222 7072 0204  20         li    tmp0,txt.filetype.none
     7074 756E 
0223 7076 C804  38         mov   tmp0,@edb.filetype.ptr
     7078 A210 
0224                                                   ; Empty filetype string
0225               
0226 707A C820  54         mov   @cmdb.scrrows,@parm1
     707C A304 
     707E 8350 
0227 7080 06A0  32         bl    @pane.cmdb.show       ; Show command buffer pane
     7082 72CE 
0228                       ;------------------------------------------------------
0229                       ; Exit
0230                       ;------------------------------------------------------
0231               fm.loadfile.cb.fioerr.exit:
0232 7084 0460  28         b     @poprt                ; Return to caller
     7086 222C 
**** **** ****     > stevie_b1.asm.213410
...
0010               ****************************************************************
0011               hook.keyscan:
0012 7088 20A0  38         coc   @wbit11,config        ; ANYKEY pressed ?
     708A 2014 
0013 708C 160B  14         jne   hook.keyscan.clear_kbbuffer
0014                                                   ; No, clear buffer and exit
0015               *---------------------------------------------------------------
0016               * Identical key pressed ?
0017               *---------------------------------------------------------------
0018 708E 40A0  34         szc   @wbit11,config        ; Reset ANYKEY
     7090 2014 
0019 7092 8820  54         c     @waux1,@waux2         ; Still pressing previous key?
     7094 833C 
     7096 833E 
0020 7098 1309  14         jeq   hook.keyscan.bounce
0021               *--------------------------------------------------------------
0022               * New key pressed
0023               *--------------------------------------------------------------
0024               hook.keyscan.newkey:
0025 709A C820  54         mov   @waux1,@waux2         ; Save as previous key
     709C 833C 
     709E 833E 
0026 70A0 0460  28         b     @edkey.key.process    ; Process key
     70A2 611E 
0027               *--------------------------------------------------------------
0028               * Clear keyboard buffer if no key pressed
0029               *--------------------------------------------------------------
0030               hook.keyscan.clear_kbbuffer:
0031 70A4 04E0  34         clr   @waux1
     70A6 833C 
0032 70A8 04E0  34         clr   @waux2
     70AA 833E 
0033               *--------------------------------------------------------------
0034               * Delay to avoid key bouncing
0035               *--------------------------------------------------------------
0036               hook.keyscan.bounce:
0037 70AC 0204  20         li    tmp0,2000             ; Avoid key bouncing
     70AE 07D0 
0038                       ;------------------------------------------------------
0039                       ; Delay loop
0040                       ;------------------------------------------------------
0041               hook.keyscan.bounce.loop:
0042 70B0 0604  14         dec   tmp0
0043 70B2 16FE  14         jne   hook.keyscan.bounce.loop
0044               *--------------------------------------------------------------
0045               * Exit
0046               *--------------------------------------------------------------
0047 70B4 0460  28         b     @hookok               ; Return
     70B6 2C9A 
**** **** ****     > stevie_b1.asm.213410
...
0010               ***************************************************************
0011               task.vdp.panes:
0012 70B8 C120  34         mov   @fb.dirty,tmp0        ; Is frame buffer dirty?
     70BA A116 
0013 70BC 1342  14         jeq   task.vdp.panes.exit   ; No, skip update
0014 70BE C820  54         mov   @wyx,@fb.yxsave       ; Backup VDP cursor position
     70C0 832A 
     70C2 A114 
0015                       ;------------------------------------------------------
0016                       ; Determine how many rows to copy
0017                       ;------------------------------------------------------
0018 70C4 8820  54         c     @edb.lines,@fb.scrrows
     70C6 A204 
     70C8 A118 
0019 70CA 1103  14         jlt   task.vdp.panes.setrows.small
0020 70CC C160  34         mov   @fb.scrrows,tmp1      ; Lines to copy
     70CE A118 
0021 70D0 1003  14         jmp   task.vdp.panes.copy.framebuffer
0022                       ;------------------------------------------------------
0023                       ; Less lines in editor buffer as rows in frame buffer
0024                       ;------------------------------------------------------
0025               task.vdp.panes.setrows.small:
0026 70D2 C160  34         mov   @edb.lines,tmp1       ; Lines to copy
     70D4 A204 
0027 70D6 0585  14         inc   tmp1
0028                       ;------------------------------------------------------
0029                       ; Determine area to copy
0030                       ;------------------------------------------------------
0031               task.vdp.panes.copy.framebuffer:
0032 70D8 3960  72         mpy   @fb.colsline,tmp1     ; columns per line * rows on screen
     70DA A10E 
0033                                                   ; 16 bit part is in tmp2!
0034 70DC 04C4  14         clr   tmp0                  ; VDP target address (1nd line on screen!)
0035 70DE C160  34         mov   @fb.top.ptr,tmp1      ; RAM Source address
     70E0 A100 
0036                       ;------------------------------------------------------
0037                       ; Copy memory block
0038                       ;------------------------------------------------------
0039 70E2 06A0  32         bl    @xpym2v               ; Copy to VDP
     70E4 2438 
0040                                                   ; \ i  tmp0 = VDP target address
0041                                                   ; | i  tmp1 = RAM source address
0042                                                   ; / i  tmp2 = Bytes to copy
0043 70E6 04E0  34         clr   @fb.dirty             ; Reset frame buffer dirty flag
     70E8 A116 
0044                       ;-------------------------------------------------------
0045                       ; Draw EOF marker at end-of-file
0046                       ;-------------------------------------------------------
0047 70EA C120  34         mov   @edb.lines,tmp0
     70EC A204 
0048 70EE 6120  34         s     @fb.topline,tmp0      ; Y = @edb.lines - @fb.topline
     70F0 A104 
0049 70F2 0584  14         inc   tmp0                  ; Y = Y + 1
0050 70F4 8120  34         c     @fb.scrrows,tmp0      ; Hide if last line on screen
     70F6 A118 
0051 70F8 121F  14         jle   task.vdp.panes.draw.cmdb
0052                       ;-------------------------------------------------------
0053                       ; Do actual drawing of EOF marker
0054                       ;-------------------------------------------------------
0055               task.vdp.panes.draw_marker:
0056 70FA 0A84  56         sla   tmp0,8                ; Move LSB to MSB (Y), X=0
0057 70FC C804  38         mov   tmp0,@wyx             ; Set VDP cursor
     70FE 832A 
0058               
0059 7100 06A0  32         bl    @putstr
     7102 2418 
0060 7104 74C4                   data txt.marker       ; Display *EOF*
0061                       ;-------------------------------------------------------
0062                       ; Draw empty line after (and below) EOF marker
0063                       ;-------------------------------------------------------
0064 7106 06A0  32         bl    @setx
     7108 2684 
0065 710A 0005                   data  5               ; Cursor after *EOF* string
0066               
0067 710C C120  34         mov   @wyx,tmp0
     710E 832A 
0068 7110 0984  56         srl   tmp0,8                ; Right justify
0069 7112 0584  14         inc   tmp0                  ; One time adjust
0070 7114 8120  34         c     @fb.scrrows,tmp0      ; Don't spill on last line on screen
     7116 A118 
0071 7118 1303  14         jeq   !
0072 711A 0206  20         li    tmp2,colrow+colrow-5  ; Repeat count for 2 lines
     711C 009B 
0073 711E 1002  14         jmp   task.vdp.panes.draw_marker.empty.line
0074 7120 0206  20 !       li    tmp2,colrow-5         ; Repeat count for 1 line
     7122 004B 
0075                       ;-------------------------------------------------------
0076                       ; Draw 1 or 2 empty lines
0077                       ;-------------------------------------------------------
0078               task.vdp.panes.draw_marker.empty.line:
0079 7124 0604  14         dec   tmp0                  ; One time adjust
0080 7126 06A0  32         bl    @yx2pnt               ; Set VDP address in tmp0
     7128 23F4 
0081                                                   ; \ i  @wyx = Cursor position
0082                                                   ; / o  tmp0 = VDP address
0083               
0084 712A 0205  20         li    tmp1,32               ; Character to write (whitespace)
     712C 0020 
0085 712E 06A0  32         bl    @xfilv                ; Fill VDP memory
     7130 228E 
0086                                                   ; \ i  tmp0 = VDP destination
0087                                                   ; | i  tmp1 = byte to write
0088                                                   ; / i  tmp2 = Number of bytes to write
0089               
0090 7132 C820  54         mov   @fb.yxsave,@wyx       ; Restore cursor postion
     7134 A114 
     7136 832A 
0091                       ;-------------------------------------------------------
...
0095 7138 C120  34         mov   @cmdb.visible,tmp0    ; Show command buffer?
     713A A302 
0096 713C 1302  14         jeq   task.vdp.panes.exit   ; No, skip
0097 713E 06A0  32         bl    @pane.cmdb.draw       ; Draw command buffer
     7140 7296 
0098                       ;------------------------------------------------------
0099                       ; Exit task
0100                       ;------------------------------------------------------
0101               task.vdp.panes.exit:
0102 7142 06A0  32         bl    @pane.botline.draw    ; Draw status bottom line
     7144 7328 
0103 7146 0460  28         b     @slotok
     7148 2D16 
**** **** ****     > stevie_b1.asm.213410
...
0012 714A C120  34         mov   @tv.pane.focus,tmp0
     714C A016 
0013 714E 130A  14         jeq   !                     ; Frame buffer has focus
0014               
0015 7150 0284  22         ci    tmp0,pane.focus.cmdb
     7152 0001 
0016 7154 1304  14         jeq   task.vdp.copy.sat.cmdb
0017                                                   ; Command buffer has focus
0018                       ;------------------------------------------------------
0019                       ; Assert failed. Invalid value
0020                       ;------------------------------------------------------
0021 7156 C80B  38         mov   r11,@>ffce            ; \ Save caller address
     7158 FFCE 
0022 715A 06A0  32         bl    @cpu.crash            ; / Halt system.
     715C 2030 
0023                       ;------------------------------------------------------
0024                       ; Command buffer has focus, position cursor
0025                       ;------------------------------------------------------
0026               task.vdp.copy.sat.cmdb:
0027 715E C820  54         mov   @cmdb.cursor,@wyx     ; Position cursor in CMDB pane
     7160 A308 
     7162 832A 
0028                       ;------------------------------------------------------
0029                       ; Position cursor
0030                       ;------------------------------------------------------
0031 7164 E0A0  34 !       soc   @wbit0,config         ; Sprite adjustment on
     7166 202A 
0032 7168 06A0  32         bl    @yx2px                ; \ Calculate pixel position
     716A 2690 
0033                                                   ; | i  @WYX = Cursor YX
0034                                                   ; / o  tmp0 = Pixel YX
0035 716C C804  38         mov   tmp0,@ramsat          ; Set cursor YX
     716E 8380 
0036               
0037 7170 06A0  32         bl    @cpym2v               ; Copy sprite SAT to VDP
     7172 2432 
0038 7174 2180                   data sprsat,ramsat,4  ; \ i  tmp0 = VDP destination
     7176 8380 
     7178 0004 
...
0045 717A 0460  28         b     @slotok               ; Exit task
     717C 2D16 
**** **** ****     > stevie_b1.asm.213410
...
0012 717E 0560  34         inv   @fb.curtoggle          ; Flip cursor shape flag
     7180 A112 
0013 7182 1303  14         jeq   task.vdp.cursor.visible
0014 7184 04E0  34         clr   @ramsat+2              ; Hide cursor
     7186 8382 
0015 7188 1015  14         jmp   task.vdp.cursor.copy.sat
0016                                                    ; Update VDP SAT and exit task
0017               task.vdp.cursor.visible:
0018 718A C120  34         mov   @edb.insmode,tmp0      ; Get Editor buffer insert mode
     718C A20A 
0019 718E 130B  14         jeq   task.vdp.cursor.visible.overwrite_mode
0020                       ;------------------------------------------------------
0021                       ; Cursor in insert mode
0022                       ;------------------------------------------------------
0023               task.vdp.cursor.visible.insert_mode:
0024 7190 C120  34         mov   @tv.pane.focus,tmp0    ; Get pane with focus
     7192 A016 
0025 7194 1303  14         jeq   task.vdp.cursor.visible.insert_mode.fb
0026                                                    ; Framebuffer has focus
0027 7196 0284  22         ci    tmp0,pane.focus.cmdb
     7198 0001 
0028 719A 1302  14         jeq   task.vdp.cursor.visible.insert_mode.cmdb
0029                       ;------------------------------------------------------
0030                       ; Editor cursor (insert mode)
0031                       ;------------------------------------------------------
0032               task.vdp.cursor.visible.insert_mode.fb:
0033 719C 04C4  14         clr   tmp0                   ; Cursor editor insert mode
0034 719E 1005  14         jmp   task.vdp.cursor.visible.cursorshape
0035                       ;------------------------------------------------------
0036                       ; Command buffer cursor (insert mode)
0037                       ;------------------------------------------------------
0038               task.vdp.cursor.visible.insert_mode.cmdb:
0039 71A0 0204  20         li    tmp0,>0100             ; Cursor CMDB insert mode
     71A2 0100 
0040 71A4 1002  14         jmp   task.vdp.cursor.visible.cursorshape
0041                       ;------------------------------------------------------
0042                       ; Cursor in overwrite mode
0043                       ;------------------------------------------------------
0044               task.vdp.cursor.visible.overwrite_mode:
0045 71A6 0204  20         li    tmp0,>0200             ; Cursor overwrite mode
     71A8 0200 
0046                       ;------------------------------------------------------
0047                       ; Set cursor shape
0048                       ;------------------------------------------------------
0049               task.vdp.cursor.visible.cursorshape:
0050 71AA D804  38         movb  tmp0,@tv.curshape      ; Save cursor shape
     71AC A014 
0051 71AE C820  54         mov   @tv.curshape,@ramsat+2 ; Get cursor shape and color
     71B0 A014 
     71B2 8382 
0052                       ;------------------------------------------------------
0053                       ; Copy SAT
...
0056 71B4 06A0  32         bl    @cpym2v                ; Copy sprite SAT to VDP
     71B6 2432 
0057 71B8 2180                   data sprsat,ramsat,4   ; \ i  tmp0 = VDP destination
     71BA 8380 
     71BC 0004 
...
0062                       ;-------------------------------------------------------
0063 71BE 06A0  32         bl    @pane.botline.draw     ; Draw status bottom line
     71C0 7328 
0064                       ;------------------------------------------------------
0065                       ; Exit
0066                       ;------------------------------------------------------
0067               task.vdp.cursor.exit:
0068 71C2 0460  28         b     @slotok                ; Exit task
     71C4 2D16 
**** **** ****     > stevie_b1.asm.213410
0058               
...
0021 71C6 0649  14         dect  stack
0022 71C8 C64B  30         mov   r11,*stack            ; Push return address
0023 71CA 0649  14         dect  stack
0024 71CC C644  30         mov   tmp0,*stack           ; Push tmp0
0025               
0026 71CE C120  34         mov   @tv.colorscheme,tmp0  ; Load color scheme index
     71D0 A012 
0027 71D2 0284  22         ci    tmp0,tv.colorscheme.entries
     71D4 0004 
0028                                                   ; Last entry reached?
0029 71D6 1102  14         jlt   !
0030 71D8 04C4  14         clr   tmp0
0031 71DA 1001  14         jmp   pane.action.colorscheme.switch
0032 71DC 0584  14 !       inc   tmp0
0033                       ;-------------------------------------------------------
0034                       ; switch to new color scheme
0035                       ;-------------------------------------------------------
0036               pane.action.colorscheme.switch:
0037 71DE C804  38         mov   tmp0,@tv.colorscheme  ; Save index of color scheme
     71E0 A012 
0038 71E2 06A0  32         bl    @pane.action.colorscheme.load
     71E4 71EC 
0039                       ;-------------------------------------------------------
0040                       ; Exit
0041                       ;-------------------------------------------------------
0042               pane.action.colorscheme.cycle.exit:
0043 71E6 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0044 71E8 C2F9  30         mov   *stack+,r11           ; Pop R11
0045 71EA 045B  20         b     *r11                  ; Return to caller
0046               
0047               
0048               
0049               
...
0067 71EC 0649  14         dect  stack
0068 71EE C64B  30         mov   r11,*stack            ; Save return address
0069 71F0 0649  14         dect  stack
0070 71F2 C644  30         mov   tmp0,*stack           ; Push tmp0
0071 71F4 0649  14         dect  stack
0072 71F6 C645  30         mov   tmp1,*stack           ; Push tmp1
0073 71F8 0649  14         dect  stack
0074 71FA C646  30         mov   tmp2,*stack           ; Push tmp2
0075 71FC 0649  14         dect  stack
0076 71FE C647  30         mov   tmp3,*stack           ; Push tmp3
0077 7200 06A0  32         bl    @scroff               ; Turn screen off
     7202 262E 
0078                       ;-------------------------------------------------------
0079                       ; Get foreground/background color
0080                       ;-------------------------------------------------------
0081 7204 C120  34         mov   @tv.colorscheme,tmp0  ; Get color scheme index
     7206 A012 
0082 7208 0A14  56         sla   tmp0,1                ; Offset into color scheme data table
0083 720A 0224  22         ai    tmp0,tv.colorscheme.table
     720C 74BA 
0084                                                   ; Add base for color scheme data table
0085 720E C1D4  26         mov   *tmp0,tmp3            ; Get fg/bg color
0086                       ;-------------------------------------------------------
0087                       ; Dump cursor FG color to sprite table (SAT)
0088                       ;-------------------------------------------------------
0089 7210 C147  18         mov   tmp3,tmp1             ; Get work copy fg/bg color
0090 7212 0945  56         srl   tmp1,4                ; Move nibble to right
0091 7214 0245  22         andi  tmp1,>0f00
     7216 0F00 
0092 7218 D805  38         movb  tmp1,@ramsat+3        ; Update FG color in sprite table (SAT)
     721A 8383 
0093 721C D805  38         movb  tmp1,@tv.curshape+1   ; Save cursor color
     721E A015 
0094                       ;-------------------------------------------------------
0095                       ; Dump colors to VDP register 7 (text mode)
0096                       ;-------------------------------------------------------
0097 7220 C147  18         mov   tmp3,tmp1             ; Get work copy
0098 7222 0985  56         srl   tmp1,8                ; MSB to LSB
0099 7224 0265  22         ori   tmp1,>0700
     7226 0700 
0100 7228 C105  18         mov   tmp1,tmp0
0101 722A 06A0  32         bl    @putvrx               ; Write VDP register
     722C 232E 
0102                       ;-------------------------------------------------------
0103                       ; Dump colors for frame buffer pane (TAT)
0104                       ;-------------------------------------------------------
0105 722E 0204  20         li    tmp0,>1800            ; VDP start address (frame buffer area)
     7230 1800 
0106 7232 C147  18         mov   tmp3,tmp1             ; Get work copy fg/bg color
0107 7234 0985  56         srl   tmp1,8                ; MSB to LSB
0108 7236 0206  20         li    tmp2,29*80            ; Number of bytes to fill
     7238 0910 
0109 723A 06A0  32         bl    @xfilv                ; Fill colors
     723C 228E 
...
0115                       ;-------------------------------------------------------
0116 723E 0204  20         li    tmp0,>2110            ; VDP start address (bottom status line)
     7240 2110 
0117 7242 C147  18         mov   tmp3,tmp1             ; Get work copy fg/bg color
0118 7244 0245  22         andi  tmp1,>00ff            ; Only keep LSB
     7246 00FF 
0119 7248 0206  20         li    tmp2,80               ; Number of bytes to fill
     724A 0050 
0120 724C 06A0  32         bl    @xfilv                ; Fill colors
     724E 228E 
...
0128 7250 06A0  32         bl    @scron                ; Turn screen on
     7252 2636 
0129 7254 C1F9  30         mov   *stack+,tmp3          ; Pop tmp3
0130 7256 C1B9  30         mov   *stack+,tmp2          ; Pop tmp2
0131 7258 C179  30         mov   *stack+,tmp1          ; Pop tmp1
0132 725A C139  30         mov   *stack+,tmp0          ; Pop tmp0
0133 725C C2F9  30         mov   *stack+,r11           ; Pop R11
0134 725E 045B  20         b     *r11                  ; Return to caller
**** **** ****     > stevie_b1.asm.213410
...
0022 7262 C64B  30         mov   r11,*stack            ; Push return address
0023 7264 0649  14         dect  stack
0024 7266 C644  30         mov   tmp0,*stack           ; Push tmp0
0025                       ;-------------------------------------------------------
0026                       ; Read DV80 file
0027                       ;-------------------------------------------------------
0028 7268 0204  20         li    tmp0,fdname.clock
     726A 7634 
0029 726C C804  38         mov   tmp0,@parm1           ; Pointer to length-prefixed 'PI.CLOCK'
     726E 8350 
0030               
0031 7270 0204  20         li    tmp0,_pane.tipi.clock.cb.noop
     7272 7292 
0032 7274 C804  38         mov   tmp0,@parm2           ; Register callback 1
     7276 8352 
0033 7278 C804  38         mov   tmp0,@parm3           ; Register callback 2
     727A 8354 
0034 727C C804  38         mov   tmp0,@parm5           ; Register callback 4 (ignore IO errors)
     727E 8358 
0035               
0036 7280 0204  20         li    tmp0,_pane.tipi.clock.cb.datetime
     7282 7294 
0037 7284 C804  38         mov   tmp0,@parm4           ; Register callback 3
     7286 8356 
0038               
0039 7288 06A0  32         bl    @fh.file.read.sams    ; Read specified file with SAMS support
     728A 6D5E 
...
0070 7292 069B  24         bl    *r11                  ; Return to caller
0071               
0072               
0073               ***************************************************************
...
0083 7294 069B  24         bl    *r11                  ; Return to caller
**** **** ****     > stevie_b1.asm.213410
0062                                                   ; Colorscheme
0063               
...
0019               ********|*****|*********************|**************************
0020               pane.cmdb.draw:
0021 7296 0649  14         dect  stack
0022 7298 C64B  30         mov   r11,*stack            ; Save return address
0023                       ;------------------------------------------------------
0024                       ; Draw command buffer
0025                       ;------------------------------------------------------
0026 729A 06A0  32         bl    @cmdb.refresh          ; Refresh command buffer content
     729C 6D14 
0027               
0028 729E 06A0  32         bl    @vchar
     72A0 278A 
0029 72A2 1200                   byte 18,0,4,1          ; Top left corner
     72A4 0401 
0030 72A6 124F                   byte 18,79,5,1         ; Top right corner
     72A8 0501 
0031 72AA 1300                   byte 19,0,6,9          ; Left vertical double line
     72AC 0609 
0032 72AE 134F                   byte 19,79,7,9         ; Right vertical double line
     72B0 0709 
0033 72B2 1C00                   byte 28,0,8,1          ; Bottom left corner
     72B4 0801 
0034 72B6 1C4F                   byte 28,79,9,1         ; Bottom right corner
     72B8 0901 
0035 72BA FFFF                   data EOL
0036               
0037 72BC 06A0  32         bl    @hchar
     72BE 2762 
0038 72C0 1201                   byte 18,1,3,78         ; Horizontal top line
     72C2 034E 
0039 72C4 1C01                   byte 28,1,3,78         ; Horizontal bottom line
     72C6 034E 
0040 72C8 FFFF                   data EOL
0041                       ;------------------------------------------------------
0042                       ; Exit
0043                       ;------------------------------------------------------
0044               pane.cmdb.exit:
0045 72CA C2F9  30         mov   *stack+,r11           ; Pop r11
0046 72CC 045B  20         b     *r11                  ; Return
0047               
0048               
0049               ***************************************************************
0050               * pane.cmdb.show
...
0065               ********|*****|*********************|**************************
0066               pane.cmdb.show:
0067 72CE 0649  14         dect  stack
0068 72D0 C64B  30         mov   r11,*stack            ; Save return address
0069 72D2 0649  14         dect  stack
0070 72D4 C644  30         mov   tmp0,*stack           ; Push tmp0
0071                       ;------------------------------------------------------
0072                       ; Show command buffer pane
0073                       ;------------------------------------------------------
0074 72D6 C820  54         mov   @wyx,@cmdb.fb.yxsave
     72D8 832A 
     72DA A312 
0075                                                   ; Save YX position in frame buffer
0076               
0077 72DC C120  34         mov   @fb.scrrows.max,tmp0
     72DE A11A 
0078 72E0 6120  34         s     @cmdb.scrrows,tmp0
     72E2 A304 
0079 72E4 C804  38         mov   tmp0,@fb.scrrows      ; Resize framebuffer
     72E6 A118 
0080               
0081 72E8 05C4  14         inct  tmp0                  ; Line below cmdb top border line
0082 72EA 0A84  56         sla   tmp0,8                ; LSB to MSB (Y), X=0
0083 72EC 0584  14         inc   tmp0                  ; X=1
0084 72EE C804  38         mov   tmp0,@cmdb.yxtop      ; Set command buffer cursor
     72F0 A30C 
0085               
0086 72F2 0720  34         seto  @cmdb.visible         ; Show pane
     72F4 A302 
0087               
0088 72F6 0204  20         li    tmp0,pane.focus.cmdb  ; \ CMDB pane has focus
     72F8 0001 
0089 72FA C804  38         mov   tmp0,@tv.pane.focus   ; /
     72FC A016 
0090               
0091 72FE 0720  34         seto  @fb.dirty             ; Redraw framebuffer
     7300 A116 
0092               
...
0095                       ; Exit
0096                       ;------------------------------------------------------
0097 7302 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0098 7304 C2F9  30         mov   *stack+,r11           ; Pop r11
0099 7306 045B  20         b     *r11                  ; Return to caller
0100               
0101               
0102               
0103               ***************************************************************
...
0120               ********|*****|*********************|**************************
0121               pane.cmdb.hide:
0122 7308 0649  14         dect  stack
0123 730A C64B  30         mov   r11,*stack            ; Save return address
0124                       ;------------------------------------------------------
0125                       ; Hide command buffer pane
0126                       ;------------------------------------------------------
0127 730C C820  54         mov   @fb.scrrows.max,@fb.scrrows
     730E A11A 
     7310 A118 
0128                                                   ; Resize framebuffer
0129               
0130 7312 C820  54         mov   @cmdb.fb.yxsave,@wyx  ; Position cursor in framebuffer
     7314 A312 
     7316 832A 
0131               
0132 7318 04E0  34         clr   @cmdb.visible         ; Hide command buffer pane
     731A A302 
0133 731C 0720  34         seto  @fb.dirty             ; Redraw framebuffer
     731E A116 
0134 7320 04E0  34         clr   @tv.pane.focus        ; Framebuffer has focus!
     7322 A016 
0135               
...
0138                       ; Exit
0139                       ;------------------------------------------------------
0140 7324 C2F9  30         mov   *stack+,r11           ; Pop r11
0141 7326 045B  20         b     *r11                  ; Return to caller
**** **** ****     > stevie_b1.asm.213410
...
0021 7328 0649  14         dect  stack
0022 732A C64B  30         mov   r11,*stack            ; Save return address
0023 732C 0649  14         dect  stack
0024 732E C644  30         mov   tmp0,*stack           ; Push tmp0
0025               
0026 7330 C820  54         mov   @wyx,@fb.yxsave
     7332 832A 
     7334 A114 
0027                       ;------------------------------------------------------
...
0031 7336 06A0  32         bl    @putat
     7338 242A 
0032 733A 1D00                   byte  29,0
0033 733C 751E                   data  txt.bufnum
0034                       ;------------------------------------------------------
0035                       ; Show current file
0036                       ;------------------------------------------------------
0037               pane.botline.show_file:
0038 733E 06A0  32         bl    @at
     7340 266E 
0039 7342 1D03                   byte  29,3            ; Position cursor
0040 7344 C160  34         mov   @edb.filename.ptr,tmp1
     7346 A20E 
0041                                                   ; Get string to display
0042 7348 06A0  32         bl    @xutst0               ; Display string
     734A 241A 
0043               
0044 734C 06A0  32         bl    @at
     734E 266E 
0045 7350 1D23                   byte  29,35           ; Position cursor
0046               
0047 7352 C160  34         mov   @edb.filetype.ptr,tmp1
     7354 A210 
0048                                                   ; Get string to display
0049 7356 06A0  32         bl    @xutst0               ; Display Filetype string
     7358 241A 
0050                       ;------------------------------------------------------
0051                       ; Show text editing mode
0052                       ;------------------------------------------------------
0053               pane.botline.show_mode:
0054 735A C120  34         mov   @edb.insmode,tmp0
     735C A20A 
0055 735E 1605  14         jne   pane.botline.show_mode.insert
0056                       ;------------------------------------------------------
0057                       ; Overwrite mode
0058                       ;------------------------------------------------------
0059               pane.botline.show_mode.overwrite:
0060 7360 06A0  32         bl    @putat
     7362 242A 
0061 7364 1D32                   byte  29,50
0062 7366 74D0                   data  txt.ovrwrite
0063 7368 1004  14         jmp   pane.botline.show_changed
0064                       ;------------------------------------------------------
0065                       ; Insert  mode
0066                       ;------------------------------------------------------
0067               pane.botline.show_mode.insert:
0068 736A 06A0  32         bl    @putat
     736C 242A 
0069 736E 1D32                   byte  29,50
0070 7370 74D4                   data  txt.insert
0071                       ;------------------------------------------------------
0072                       ; Show if text was changed in editor buffer
0073                       ;------------------------------------------------------
0074               pane.botline.show_changed:
0075 7372 C120  34         mov   @edb.dirty,tmp0
     7374 A206 
0076 7376 1305  14         jeq   pane.botline.show_changed.clear
0077                       ;------------------------------------------------------
0078                       ; Show "*"
0079                       ;------------------------------------------------------
0080 7378 06A0  32         bl    @putat
     737A 242A 
0081 737C 1D36                   byte 29,54
0082 737E 74D8                   data txt.star
0083 7380 1001  14         jmp   pane.botline.show_linecol
0084                       ;------------------------------------------------------
0085                       ; Show "line,column"
0086                       ;------------------------------------------------------
0087               pane.botline.show_changed.clear:
0088 7382 1000  14         nop
0089               pane.botline.show_linecol:
0090 7384 C820  54         mov   @fb.row,@parm1
     7386 A106 
     7388 8350 
0091 738A 06A0  32         bl    @fb.row2line
     738C 67A6 
0092 738E 05A0  34         inc   @outparm1
     7390 8360 
0093                       ;------------------------------------------------------
0094                       ; Show line
0095                       ;------------------------------------------------------
0096 7392 06A0  32         bl    @putnum
     7394 2A3E 
0097 7396 1D40                   byte  29,64           ; YX
0098 7398 8360                   data  outparm1,rambuf
     739A 8390 
0099 739C 3020                   byte  48              ; ASCII offset
0100                             byte  32              ; Padding character
0101                       ;------------------------------------------------------
0102                       ; Show comma
0103                       ;------------------------------------------------------
0104 739E 06A0  32         bl    @putat
     73A0 242A 
0105 73A2 1D45                   byte  29,69
0106 73A4 74C2                   data  txt.delim
0107                       ;------------------------------------------------------
0108                       ; Show column
0109                       ;------------------------------------------------------
0110 73A6 06A0  32         bl    @film
     73A8 2230 
0111 73AA 8396                   data rambuf+6,32,12   ; Clear work buffer with space character
     73AC 0020 
     73AE 000C 
0112               
0113 73B0 C820  54         mov   @fb.column,@waux1
     73B2 A10C 
     73B4 833C 
0114 73B6 05A0  34         inc   @waux1                ; Offset 1
     73B8 833C 
0115               
0116 73BA 06A0  32         bl    @mknum                ; Convert unsigned number to string
     73BC 29C0 
0117 73BE 833C                   data  waux1,rambuf
     73C0 8390 
0118 73C2 3020                   byte  48              ; ASCII offset
0119                             byte  32              ; Fill character
0120               
0121 73C4 06A0  32         bl    @trimnum              ; Trim number to the left
     73C6 2A18 
0122 73C8 8390                   data  rambuf,rambuf+6,32
     73CA 8396 
     73CC 0020 
0123               
0124 73CE 0204  20         li    tmp0,>0200
     73D0 0200 
0125 73D2 D804  38         movb  tmp0,@rambuf+6        ; "Fix" number length to clear junk chars
     73D4 8396 
0126               
0127 73D6 06A0  32         bl    @putat
     73D8 242A 
0128 73DA 1D46                   byte 29,70
0129 73DC 8396                   data rambuf+6         ; Show column
0130                       ;------------------------------------------------------
0131                       ; Show lines in buffer unless on last line in file
0132                       ;------------------------------------------------------
0133 73DE C820  54         mov   @fb.row,@parm1
     73E0 A106 
     73E2 8350 
0134 73E4 06A0  32         bl    @fb.row2line
     73E6 67A6 
0135 73E8 8820  54         c     @edb.lines,@outparm1
     73EA A204 
     73EC 8360 
0136 73EE 1605  14         jne   pane.botline.show_lines_in_buffer
0137               
0138 73F0 06A0  32         bl    @putat
     73F2 242A 
0139 73F4 1D4B                   byte 29,75
0140 73F6 74CA                   data txt.bottom
0141               
0142 73F8 100B  14         jmp   pane.botline.exit
0143                       ;------------------------------------------------------
0144                       ; Show lines in buffer
0145                       ;------------------------------------------------------
0146               pane.botline.show_lines_in_buffer:
0147 73FA C820  54         mov   @edb.lines,@waux1
     73FC A204 
     73FE 833C 
0148 7400 05A0  34         inc   @waux1                ; Offset 1
     7402 833C 
0149 7404 06A0  32         bl    @putnum
     7406 2A3E 
0150 7408 1D4B                   byte 29,75            ; YX
0151 740A 833C                   data waux1,rambuf
     740C 8390 
0152 740E 3020                   byte 48
0153                             byte 32
0154                       ;------------------------------------------------------
0155                       ; Exit
0156                       ;------------------------------------------------------
0157               pane.botline.exit:
0158 7410 C820  54         mov   @fb.yxsave,@wyx
     7412 A114 
     7414 832A 
0159 7416 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0160 7418 C2F9  30         mov   *stack+,r11           ; Pop r11
0161 741A 045B  20         b     *r11                  ; Return
**** **** ****     > stevie_b1.asm.213410
...
0031               ***************************************************************
0032               stevie.tx8030:
0033 741C 04F0             byte  >04,>f0,>00,>3f,>02,>43,>05,SPFCLR,0,80
     741E 003F 
     7420 0243 
     7422 05F4 
     7424 0050 
0034               
0035               romsat:
0036 7426 0303             data  >0303,>0001             ; Cursor YX, initial shape and colour
     7428 0001 
0037               
0038               cursors:
0039 742A 0000             data  >0000,>0000,>0000,>001c ; Cursor 1 - Insert mode
     742C 0000 
     742E 0000 
     7430 001C 
0040 7432 1010             data  >1010,>1010,>1010,>1000 ; Cursor 2 - Insert mode
     7434 1010 
     7436 1010 
     7438 1000 
0041 743A 1C1C             data  >1c1c,>1c1c,>1c1c,>1c00 ; Cursor 3 - Overwrite mode
     743C 1C1C 
     743E 1C1C 
     7440 1C00 
0042               
0043               patterns:
0044 7442 0000             data  >0000,>ff00,>00ff,>0080 ; 01. Double line top + ruler
     7444 FF00 
     7446 00FF 
     7448 0080 
0045 744A 0080             data  >0080,>0000,>ff00,>ff00 ; 02. Ruler + double line bottom
     744C 0000 
     744E FF00 
     7450 FF00 
0046               patterns.box:
0047 7452 0000             data  >0000,>0000,>ff00,>ff00 ; 03. Double line bottom
     7454 0000 
     7456 FF00 
     7458 FF00 
0048 745A 0000             data  >0000,>0000,>ff80,>bfa0 ; 04. Top left corner
     745C 0000 
     745E FF80 
     7460 BFA0 
0049 7462 0000             data  >0000,>0000,>fc04,>f414 ; 05. Top right corner
     7464 0000 
     7466 FC04 
     7468 F414 
0050 746A A0A0             data  >a0a0,>a0a0,>a0a0,>a0a0 ; 06. Left vertical double line
     746C A0A0 
     746E A0A0 
     7470 A0A0 
0051 7472 1414             data  >1414,>1414,>1414,>1414 ; 07. Right vertical double line
     7474 1414 
     7476 1414 
     7478 1414 
0052 747A A0A0             data  >a0a0,>a0a0,>bf80,>ff00 ; 08. Bottom left corner
     747C A0A0 
     747E BF80 
     7480 FF00 
0053 7482 1414             data  >1414,>1414,>f404,>fc00 ; 09. Bottom right corner
     7484 1414 
     7486 F404 
     7488 FC00 
0054 748A 0000             data  >0000,>c0c0,>c0c0,>0080 ; 10. Double line top left corner
     748C C0C0 
     748E C0C0 
     7490 0080 
0055 7492 0000             data  >0000,>0f0f,>0f0f,>0000 ; 11. Double line top right corner
     7494 0F0F 
     7496 0F0F 
     7498 0000 
0056               
...
0064 749A 2000             data  >2000,>0002           ; >2000-2fff, SAMS page >02
     749C 0002 
0065 749E 3000             data  >3000,>0003           ; >3000-3fff, SAMS page >03
     74A0 0003 
0066 74A2 A000             data  >a000,>000a           ; >a000-afff, SAMS page >0a
     74A4 000A 
0067               
0068 74A6 B000             data  >b000,>0010           ; >b000-bfff, SAMS page >10
     74A8 0010 
0069                                                   ; \ The index can allocate
0070                                                   ; / pages >10 to >2f.
0071               
0072 74AA C000             data  >c000,>0030           ; >c000-cfff, SAMS page >30
     74AC 0030 
0073                                                   ; \ Editor buffer can allocate
0074                                                   ; / pages >30 to >ff.
0075               
0076 74AE D000             data  >d000,>000d           ; >d000-dfff, SAMS page >0d
     74B0 000D 
0077 74B2 E000             data  >e000,>000e           ; >e000-efff, SAMS page >0e
     74B4 000E 
0078 74B6 F000             data  >f000,>000f           ; >f000-ffff, SAMS page >0f
     74B8 000F 
0079               
0080               
0081               
...
0094 74BA F41C             data  >f41c                 ; White      | Dark blue  | Dark blue
0095 74BC F13A             data  >f13a                 ; White      | Black      | Black
0096 74BE 174B             data  >174b                 ; Black      | Cyan       | Cyan
0097 74C0 1F53             data  >1f53                 ; Black      | White      | White
0098               
**** **** ****     > stevie_b1.asm.213410
...
0006               ***************************************************************
0007               txt.delim
0008 74C2 012C             byte  1
0009 74C3 ....             text  ','
0010                       even
0011               
0012               txt.marker
0013 74C4 052A             byte  5
0014 74C5 ....             text  '*EOF*'
0015                       even
0016               
0017               txt.bottom
0018 74CA 0520             byte  5
0019 74CB ....             text  '  BOT'
0020                       even
0021               
0022               txt.ovrwrite
0023 74D0 034F             byte  3
0024 74D1 ....             text  'OVR'
0025                       even
0026               
0027               txt.insert
0028 74D4 0349             byte  3
0029 74D5 ....             text  'INS'
0030                       even
0031               
0032               txt.star
0033 74D8 012A             byte  1
0034 74D9 ....             text  '*'
0035                       even
0036               
0037               txt.loading
0038 74DA 0A4C             byte  10
0039 74DB ....             text  'Loading...'
0040                       even
0041               
0042               txt.kb
0043 74E6 026B             byte  2
0044 74E7 ....             text  'kb'
0045                       even
0046               
0047               txt.rle
0048 74EA 0352             byte  3
0049 74EB ....             text  'RLE'
0050                       even
0051               
0052               txt.lines
0053 74EE 054C             byte  5
0054 74EF ....             text  'Lines'
0055                       even
0056               
0057               txt.ioerr
0058 74F4 2921             byte  41
0059 74F5 ....             text  '! I/O error occured. Could not load file:'
0060                       even
0061               
0062               txt.bufnum
0063 751E 0223             byte  2
0064 751F ....             text  '#1'
0065                       even
0066               
0067               txt.newfile
0068 7522 0A5B             byte  10
0069 7523 ....             text  '[New file]'
0070                       even
0071               
0072               
0073               txt.cmdb.prompt
0074 752E 013E             byte  1
0075 752F ....             text  '>'
0076                       even
0077               
0078               txt.cmdb.hint
0079 7530 2348             byte  35
0080 7531 ....             text  'Hint: Type "help" for command list.'
0081                       even
0082               
0083               txt.cmdb.catalog
0084 7554 0C46             byte  12
0085 7555 ....             text  'File catalog'
0086                       even
0087               
0088               
0089               
0090               txt.filetype.dv80
0091 7562 0A44             byte  10
0092 7563 ....             text  'DIS/VAR80 '
0093                       even
0094               
0095               txt.filetype.none
0096 756E 0A20             byte  10
0097 756F ....             text  '          '
0098                       even
0099               
0100               
0101 757A 0C0A     txt.stevie         byte    12
0102                                  byte    10
0103 757C ....                        text    'stevie v1.00'
0104 7588 0B00                        byte    11
0105                                  even
0106               
0107               fdname1
0108 758A 0850             byte  8
0109 758B ....             text  'PI.CLOCK'
0110                       even
0111               
0112               fdname2
0113 7594 0E54             byte  14
0114 7595 ....             text  'TIPI.TIVI.NR80'
0115                       even
0116               
0117               fdname3
0118 75A4 0C44             byte  12
0119 75A5 ....             text  'DSK1.XBEADOC'
0120                       even
0121               
0122               fdname4
0123 75B2 1154             byte  17
0124 75B3 ....             text  'TIPI.TIVI.C99MAN1'
0125                       even
0126               
0127               fdname5
0128 75C4 1154             byte  17
0129 75C5 ....             text  'TIPI.TIVI.C99MAN2'
0130                       even
0131               
0132               fdname6
0133 75D6 1154             byte  17
0134 75D7 ....             text  'TIPI.TIVI.C99MAN3'
0135                       even
0136               
0137               fdname7
0138 75E8 1254             byte  18
0139 75E9 ....             text  'TIPI.TIVI.C99SPECS'
0140                       even
0141               
0142               fdname8
0143 75FC 1254             byte  18
0144 75FD ....             text  'TIPI.TIVI.RANDOM#C'
0145                       even
0146               
0147               fdname9
0148 7610 1254             byte  18
0149 7611 ....             text  'TIPI.TIVI.INVADERS'
0150                       even
0151               
0152               fdname0
0153 7624 0E54             byte  14
0154 7625 ....             text  'TIPI.TIVI.NR80'
0155                       even
0156               
0157               fdname.clock
0158 7634 0850             byte  8
0159 7635 ....             text  'PI.CLOCK'
0160                       even
0161               
0162               
0163               
...
0166               *---------------------------------------------------------------
0167               txt.fctn.0
0168 763E 0866             byte  8
0169 763F ....             text  'fctn + 0'
0170                       even
0171               
0172               txt.fctn.1
0173 7648 0866             byte  8
0174 7649 ....             text  'fctn + 1'
0175                       even
0176               
0177               txt.fctn.2
0178 7652 0866             byte  8
0179 7653 ....             text  'fctn + 2'
0180                       even
0181               
0182               txt.fctn.3
0183 765C 0866             byte  8
0184 765D ....             text  'fctn + 3'
0185                       even
0186               
0187               txt.fctn.4
0188 7666 0866             byte  8
0189 7667 ....             text  'fctn + 4'
0190                       even
0191               
0192               txt.fctn.5
0193 7670 0866             byte  8
0194 7671 ....             text  'fctn + 5'
0195                       even
0196               
0197               txt.fctn.6
0198 767A 0866             byte  8
0199 767B ....             text  'fctn + 6'
0200                       even
0201               
0202               txt.fctn.7
0203 7684 0866             byte  8
0204 7685 ....             text  'fctn + 7'
0205                       even
0206               
0207               txt.fctn.8
0208 768E 0866             byte  8
0209 768F ....             text  'fctn + 8'
0210                       even
0211               
0212               txt.fctn.9
0213 7698 0866             byte  8
0214 7699 ....             text  'fctn + 9'
0215                       even
0216               
0217               txt.fctn.a
0218 76A2 0866             byte  8
0219 76A3 ....             text  'fctn + a'
0220                       even
0221               
0222               txt.fctn.b
0223 76AC 0866             byte  8
0224 76AD ....             text  'fctn + b'
0225                       even
0226               
0227               txt.fctn.c
0228 76B6 0866             byte  8
0229 76B7 ....             text  'fctn + c'
0230                       even
0231               
0232               txt.fctn.d
0233 76C0 0866             byte  8
0234 76C1 ....             text  'fctn + d'
0235                       even
0236               
0237               txt.fctn.e
0238 76CA 0866             byte  8
0239 76CB ....             text  'fctn + e'
0240                       even
0241               
0242               txt.fctn.f
0243 76D4 0866             byte  8
0244 76D5 ....             text  'fctn + f'
0245                       even
0246               
0247               txt.fctn.g
0248 76DE 0866             byte  8
0249 76DF ....             text  'fctn + g'
0250                       even
0251               
0252               txt.fctn.h
0253 76E8 0866             byte  8
0254 76E9 ....             text  'fctn + h'
0255                       even
0256               
0257               txt.fctn.i
0258 76F2 0866             byte  8
0259 76F3 ....             text  'fctn + i'
0260                       even
0261               
0262               txt.fctn.j
0263 76FC 0866             byte  8
0264 76FD ....             text  'fctn + j'
0265                       even
0266               
0267               txt.fctn.k
0268 7706 0866             byte  8
0269 7707 ....             text  'fctn + k'
0270                       even
0271               
0272               txt.fctn.l
0273 7710 0866             byte  8
0274 7711 ....             text  'fctn + l'
0275                       even
0276               
0277               txt.fctn.m
0278 771A 0866             byte  8
0279 771B ....             text  'fctn + m'
0280                       even
0281               
0282               txt.fctn.n
0283 7724 0866             byte  8
0284 7725 ....             text  'fctn + n'
0285                       even
0286               
0287               txt.fctn.o
0288 772E 0866             byte  8
0289 772F ....             text  'fctn + o'
0290                       even
0291               
0292               txt.fctn.p
0293 7738 0866             byte  8
0294 7739 ....             text  'fctn + p'
0295                       even
0296               
0297               txt.fctn.q
0298 7742 0866             byte  8
0299 7743 ....             text  'fctn + q'
0300                       even
0301               
0302               txt.fctn.r
0303 774C 0866             byte  8
0304 774D ....             text  'fctn + r'
0305                       even
0306               
0307               txt.fctn.s
0308 7756 0866             byte  8
0309 7757 ....             text  'fctn + s'
0310                       even
0311               
0312               txt.fctn.t
0313 7760 0866             byte  8
0314 7761 ....             text  'fctn + t'
0315                       even
0316               
0317               txt.fctn.u
0318 776A 0866             byte  8
0319 776B ....             text  'fctn + u'
0320                       even
0321               
0322               txt.fctn.v
0323 7774 0866             byte  8
0324 7775 ....             text  'fctn + v'
0325                       even
0326               
0327               txt.fctn.w
0328 777E 0866             byte  8
0329 777F ....             text  'fctn + w'
0330                       even
0331               
0332               txt.fctn.x
0333 7788 0866             byte  8
0334 7789 ....             text  'fctn + x'
0335                       even
0336               
0337               txt.fctn.y
0338 7792 0866             byte  8
0339 7793 ....             text  'fctn + y'
0340                       even
0341               
0342               txt.fctn.z
0343 779C 0866             byte  8
0344 779D ....             text  'fctn + z'
0345                       even
0346               
0347               *---------------------------------------------------------------
...
0349               *---------------------------------------------------------------
0350               txt.fctn.dot
0351 77A6 0866             byte  8
0352 77A7 ....             text  'fctn + .'
0353                       even
0354               
0355               txt.fctn.plus
0356 77B0 0866             byte  8
0357 77B1 ....             text  'fctn + +'
0358                       even
0359               
0360               *---------------------------------------------------------------
...
0362               *---------------------------------------------------------------
0363               txt.ctrl.0
0364 77BA 0863             byte  8
0365 77BB ....             text  'ctrl + 0'
0366                       even
0367               
0368               txt.ctrl.1
0369 77C4 0863             byte  8
0370 77C5 ....             text  'ctrl + 1'
0371                       even
0372               
0373               txt.ctrl.2
0374 77CE 0863             byte  8
0375 77CF ....             text  'ctrl + 2'
0376                       even
0377               
0378               txt.ctrl.3
0379 77D8 0863             byte  8
0380 77D9 ....             text  'ctrl + 3'
0381                       even
0382               
0383               txt.ctrl.4
0384 77E2 0863             byte  8
0385 77E3 ....             text  'ctrl + 4'
0386                       even
0387               
0388               txt.ctrl.5
0389 77EC 0863             byte  8
0390 77ED ....             text  'ctrl + 5'
0391                       even
0392               
0393               txt.ctrl.6
0394 77F6 0863             byte  8
0395 77F7 ....             text  'ctrl + 6'
0396                       even
0397               
0398               txt.ctrl.7
0399 7800 0863             byte  8
0400 7801 ....             text  'ctrl + 7'
0401                       even
0402               
0403               txt.ctrl.8
0404 780A 0863             byte  8
0405 780B ....             text  'ctrl + 8'
0406                       even
0407               
0408               txt.ctrl.9
0409 7814 0863             byte  8
0410 7815 ....             text  'ctrl + 9'
0411                       even
0412               
0413               txt.ctrl.a
0414 781E 0863             byte  8
0415 781F ....             text  'ctrl + a'
0416                       even
0417               
0418               txt.ctrl.b
0419 7828 0863             byte  8
0420 7829 ....             text  'ctrl + b'
0421                       even
0422               
0423               txt.ctrl.c
0424 7832 0863             byte  8
0425 7833 ....             text  'ctrl + c'
0426                       even
0427               
0428               txt.ctrl.d
0429 783C 0863             byte  8
0430 783D ....             text  'ctrl + d'
0431                       even
0432               
0433               txt.ctrl.e
0434 7846 0863             byte  8
0435 7847 ....             text  'ctrl + e'
0436                       even
0437               
0438               txt.ctrl.f
0439 7850 0863             byte  8
0440 7851 ....             text  'ctrl + f'
0441                       even
0442               
0443               txt.ctrl.g
0444 785A 0863             byte  8
0445 785B ....             text  'ctrl + g'
0446                       even
0447               
0448               txt.ctrl.h
0449 7864 0863             byte  8
0450 7865 ....             text  'ctrl + h'
0451                       even
0452               
0453               txt.ctrl.i
0454 786E 0863             byte  8
0455 786F ....             text  'ctrl + i'
0456                       even
0457               
0458               txt.ctrl.j
0459 7878 0863             byte  8
0460 7879 ....             text  'ctrl + j'
0461                       even
0462               
0463               txt.ctrl.k
0464 7882 0863             byte  8
0465 7883 ....             text  'ctrl + k'
0466                       even
0467               
0468               txt.ctrl.l
0469 788C 0863             byte  8
0470 788D ....             text  'ctrl + l'
0471                       even
0472               
0473               txt.ctrl.m
0474 7896 0863             byte  8
0475 7897 ....             text  'ctrl + m'
0476                       even
0477               
0478               txt.ctrl.n
0479 78A0 0863             byte  8
0480 78A1 ....             text  'ctrl + n'
0481                       even
0482               
0483               txt.ctrl.o
0484 78AA 0863             byte  8
0485 78AB ....             text  'ctrl + o'
0486                       even
0487               
0488               txt.ctrl.p
0489 78B4 0863             byte  8
0490 78B5 ....             text  'ctrl + p'
0491                       even
0492               
0493               txt.ctrl.q
0494 78BE 0863             byte  8
0495 78BF ....             text  'ctrl + q'
0496                       even
0497               
0498               txt.ctrl.r
0499 78C8 0863             byte  8
0500 78C9 ....             text  'ctrl + r'
0501                       even
0502               
0503               txt.ctrl.s
0504 78D2 0863             byte  8
0505 78D3 ....             text  'ctrl + s'
0506                       even
0507               
0508               txt.ctrl.t
0509 78DC 0863             byte  8
0510 78DD ....             text  'ctrl + t'
0511                       even
0512               
0513               txt.ctrl.u
0514 78E6 0863             byte  8
0515 78E7 ....             text  'ctrl + u'
0516                       even
0517               
0518               txt.ctrl.v
0519 78F0 0863             byte  8
0520 78F1 ....             text  'ctrl + v'
0521                       even
0522               
0523               txt.ctrl.w
0524 78FA 0863             byte  8
0525 78FB ....             text  'ctrl + w'
0526                       even
0527               
0528               txt.ctrl.x
0529 7904 0863             byte  8
0530 7905 ....             text  'ctrl + x'
0531                       even
0532               
0533               txt.ctrl.y
0534 790E 0863             byte  8
0535 790F ....             text  'ctrl + y'
0536                       even
0537               
0538               txt.ctrl.z
0539 7918 0863             byte  8
0540 7919 ....             text  'ctrl + z'
0541                       even
0542               
0543               *---------------------------------------------------------------
...
0553               *---------------------------------------------------------------
0554               txt.enter
0555 792C 0565             byte  5
0556 792D ....             text  'enter'
0557                       even
0558               
**** **** ****     > stevie_b1.asm.213410
...
0103                       ; Movement keys
0104                       ;-------------------------------------------------------
0105 7932 0D00             data  key.enter, txt.enter, edkey.action.enter
     7934 792C 
     7936 6580 
0106 7938 0800             data  key.fctn.s, txt.fctn.s, edkey.action.left
     793A 7756 
     793C 617E 
0107 793E 0900             data  key.fctn.d, txt.fctn.d, edkey.action.right
     7940 76C0 
     7942 6194 
0108 7944 0B00             data  key.fctn.e, txt.fctn.e, edkey.action.up
     7946 76CA 
     7948 61AC 
0109 794A 0A00             data  key.fctn.x, txt.fctn.x, edkey.action.down
     794C 7788 
     794E 61FE 
0110 7950 8100             data  key.ctrl.a, txt.ctrl.a, edkey.action.home
     7952 781E 
     7954 626A 
0111 7956 8600             data  key.ctrl.f, txt.ctrl.f, edkey.action.end
     7958 7850 
     795A 6282 
0112 795C 9300             data  key.ctrl.s, txt.ctrl.s, edkey.action.pword
     795E 78D2 
     7960 6296 
0113 7962 8400             data  key.ctrl.d, txt.ctrl.d, edkey.action.nword
     7964 783C 
     7966 62E8 
0114 7968 8500             data  key.ctrl.e, txt.ctrl.e, edkey.action.ppage
     796A 7846 
     796C 6348 
0115 796E 9800             data  key.ctrl.x, txt.ctrl.x, edkey.action.npage
     7970 7904 
     7972 638A 
0116 7974 9400             data  key.ctrl.t, txt.ctrl.t, edkey.action.top
     7976 78DC 
     7978 63B6 
0117 797A 8200             data  key.ctrl.b, txt.ctrl.b, edkey.action.bot
     797C 7828 
     797E 63E2 
0118                       ;-------------------------------------------------------
0119                       ; Modifier keys - Delete
0120                       ;-------------------------------------------------------
0121 7980 0300             data  key.fctn.1, txt.fctn.1, edkey.action.del_char
     7982 7648 
     7984 6422 
0122 7986 0000             data  key.ctrl.k, txt.ctrl.k, edkey.action.del_eol
     7988 7882 
     798A 645A 
0123 798C 0700             data  key.fctn.3, txt.fctn.3, edkey.action.del_line
     798E 765C 
     7990 648E 
0124                       ;-------------------------------------------------------
0125                       ; Modifier keys - Insert
0126                       ;-------------------------------------------------------
0127 7992 0400             data  key.fctn.2, txt.fctn.2, edkey.action.ins_char.ws
     7994 7652 
     7996 64E6 
0128 7998 B900             data  key.fctn.dot, txt.fctn.dot, edkey.action.ins_onoff
     799A 77A6 
     799C 65EE 
0129 799E 0E00             data  key.fctn.5, txt.fctn.5, edkey.action.ins_line
     79A0 7670 
     79A2 653C 
0130                       ;-------------------------------------------------------
0131                       ; Other action keys
0132                       ;-------------------------------------------------------
0133 79A4 0500             data  key.fctn.plus, txt.fctn.plus, edkey.action.quit
     79A6 77B0 
     79A8 663E 
0134 79AA 0F00             data  key.fctn.9, txt.fctn.9, edkey.action.cmdb.toggle
     79AC 7698 
     79AE 664A 
0135 79B0 9A00             data  key.ctrl.z, txt.ctrl.z, pane.action.colorscheme.cycle
     79B2 7918 
     79B4 71C6 
0136                       ;-------------------------------------------------------
0137                       ; Editor/File buffer keys
0138                       ;-------------------------------------------------------
0139 79B6 B000             data  key.ctrl.0, txt.ctrl.0, edkey.action.buffer0
     79B8 77BA 
     79BA 6668 
0140 79BC B100             data  key.ctrl.1, txt.ctrl.1, edkey.action.buffer1
     79BE 77C4 
     79C0 666E 
0141 79C2 B200             data  key.ctrl.2, txt.ctrl.2, edkey.action.buffer2
     79C4 77CE 
     79C6 6674 
0142 79C8 B300             data  key.ctrl.3, txt.ctrl.3, edkey.action.buffer3
     79CA 77D8 
     79CC 667A 
0143 79CE B400             data  key.ctrl.4, txt.ctrl.4, edkey.action.buffer4
     79D0 77E2 
     79D2 6680 
0144 79D4 B500             data  key.ctrl.5, txt.ctrl.5, edkey.action.buffer5
     79D6 77EC 
     79D8 6686 
0145 79DA B600             data  key.ctrl.6, txt.ctrl.6, edkey.action.buffer6
     79DC 77F6 
     79DE 668C 
0146 79E0 B700             data  key.ctrl.7, txt.ctrl.7, edkey.action.buffer7
     79E2 7800 
     79E4 6692 
0147 79E6 9E00             data  key.ctrl.8, txt.ctrl.8, edkey.action.buffer8
     79E8 780A 
     79EA 6698 
0148 79EC 9F00             data  key.ctrl.9, txt.ctrl.9, edkey.action.buffer9
     79EE 7814 
     79F0 669E 
0149                       ;-------------------------------------------------------
0150                       ; End of list
0151                       ;-------------------------------------------------------
0152 79F2 FFFF             data  EOL                           ; EOL
0153               
0154               
0155               
0156               
...
0162                       ; Movement keys
0163                       ;-------------------------------------------------------
0164 79F4 0D00             data  key.enter, txt.enter, edkey.action.enter
     79F6 792C 
     79F8 6580 
0165 79FA 0800             data  key.fctn.s, txt.fctn.s, edkey.action.left
     79FC 7756 
     79FE 617E 
0166 7A00 0900             data  key.fctn.d, txt.fctn.d, edkey.action.right
     7A02 76C0 
     7A04 6194 
0167 7A06 0B00             data  key.fctn.e, txt.fctn.e, edkey.action.noop
     7A08 76CA 
     7A0A 6646 
0168 7A0C 0A00             data  key.fctn.x, txt.fctn.x, edkey.action.noop
     7A0E 7788 
     7A10 6646 
0169 7A12 8100             data  key.ctrl.a, txt.ctrl.a, edkey.action.noop
     7A14 781E 
     7A16 6646 
0170 7A18 8600             data  key.ctrl.f, txt.ctrl.f, edkey.action.noop
     7A1A 7850 
     7A1C 6646 
0171 7A1E 9300             data  key.ctrl.s, txt.ctrl.s, edkey.action.noop
     7A20 78D2 
     7A22 6646 
0172 7A24 8400             data  key.ctrl.d, txt.ctrl.d, edkey.action.noop
     7A26 783C 
     7A28 6646 
0173 7A2A 8500             data  key.ctrl.e, txt.ctrl.e, edkey.action.noop
     7A2C 7846 
     7A2E 6646 
0174 7A30 9800             data  key.ctrl.x, txt.ctrl.x, edkey.action.noop
     7A32 7904 
     7A34 6646 
0175 7A36 9400             data  key.ctrl.t, txt.ctrl.t, edkey.action.noop
     7A38 78DC 
     7A3A 6646 
0176 7A3C 8200             data  key.ctrl.b, txt.ctrl.b, edkey.action.noop
     7A3E 7828 
     7A40 6646 
0177                       ;-------------------------------------------------------
0178                       ; Modifier keys - Delete
0179                       ;-------------------------------------------------------
0180 7A42 0300             data  key.fctn.1, txt.fctn.1, edkey.action.del_char
     7A44 7648 
     7A46 6422 
0181 7A48 0000             data  key.ctrl.k, txt.ctrl.k, edkey.action.del_eol
     7A4A 7882 
     7A4C 645A 
0182 7A4E 0700             data  key.fctn.3, txt.fctn.3, edkey.action.noop
     7A50 765C 
     7A52 6646 
0183                       ;-------------------------------------------------------
0184                       ; Modifier keys - Insert
0185                       ;-------------------------------------------------------
0186 7A54 0400             data  key.fctn.2, txt.fctn.2, edkey.action.ins_char.ws
     7A56 7652 
     7A58 64E6 
0187 7A5A B900             data  key.fctn.dot, txt.fctn.dot, edkey.action.ins_onoff
     7A5C 77A6 
     7A5E 65EE 
0188 7A60 0E00             data  key.fctn.5, txt.fctn.5, edkey.action.noop
     7A62 7670 
     7A64 6646 
0189                       ;-------------------------------------------------------
0190                       ; Other action keys
0191                       ;-------------------------------------------------------
0192 7A66 0500             data  key.fctn.plus, txt.fctn.plus, edkey.action.quit
     7A68 77B0 
     7A6A 663E 
0193 7A6C 0F00             data  key.fctn.9, txt.fctn.9, edkey.action.cmdb.toggle
     7A6E 7698 
     7A70 664A 
0194 7A72 9A00             data  key.ctrl.z, txt.ctrl.z, pane.action.colorscheme.cycle
     7A74 7918 
     7A76 71C6 
0195                       ;-------------------------------------------------------
0196                       ; Editor/File buffer keys
0197                       ;-------------------------------------------------------
0198 7A78 B000             data  key.ctrl.0, txt.ctrl.0, edkey.action.buffer0
     7A7A 77BA 
     7A7C 6668 
0199 7A7E B100             data  key.ctrl.1, txt.ctrl.1, edkey.action.buffer1
     7A80 77C4 
     7A82 666E 
0200 7A84 B200             data  key.ctrl.2, txt.ctrl.2, edkey.action.buffer2
     7A86 77CE 
     7A88 6674 
0201 7A8A B300             data  key.ctrl.3, txt.ctrl.3, edkey.action.buffer3
     7A8C 77D8 
     7A8E 667A 
0202 7A90 B400             data  key.ctrl.4, txt.ctrl.4, edkey.action.buffer4
     7A92 77E2 
     7A94 6680 
0203 7A96 B500             data  key.ctrl.5, txt.ctrl.5, edkey.action.buffer5
     7A98 77EC 
     7A9A 6686 
0204 7A9C B600             data  key.ctrl.6, txt.ctrl.6, edkey.action.buffer6
     7A9E 77F6 
     7AA0 668C 
0205 7AA2 B700             data  key.ctrl.7, txt.ctrl.7, edkey.action.buffer7
     7AA4 7800 
     7AA6 6692 
0206 7AA8 9E00             data  key.ctrl.8, txt.ctrl.8, edkey.action.buffer8
     7AAA 780A 
     7AAC 6698 
0207 7AAE 9F00             data  key.ctrl.9, txt.ctrl.9, edkey.action.buffer9
     7AB0 7814 
     7AB2 669E 
0208                       ;-------------------------------------------------------
0209                       ; End of list
0210                       ;-------------------------------------------------------
0211 7AB4 FFFF             data  EOL                           ; EOL
**** **** ****     > stevie_b1.asm.213410
0069               
0073 7AB6 7AB6                   data $                ; Bank 1 ROM size OK.
0075               
0076               *--------------------------------------------------------------
...
0081      741C     spvmod  equ   stevie.tx8030         ; Video mode.   See VIDTAB for details.
