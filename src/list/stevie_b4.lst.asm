...
0008               ***************************************************************
0009               * File: stevie_b1.asm               ; Version 210327-48243
0010               *
0011               * Bank 4 "Janine"
0012               *
0013               ***************************************************************
...
0260 21D3 ....             text  'Build-ID  210327-48243'
0261                       even
0262               
**** **** ****     > runlib.asm
...
0019 34BD ....             text  'Build: 210327-48243 / 2018-2021 Filip Van Vooren / retroclouds on Atariage'
0020                       even
0021               
0022               ;--------------------------------------------------------------
...
0065 6036 06A0  32         bl    @cpu.crash            ; Should never get here
     6038 2026 
0066                       ;-----------------------------------------------------------------------
0067                       ; Keyboard actions - Framebuffer (2)
0068                       ;-----------------------------------------------------------------------
0069                       copy  "edkey.fb.tabs.asm"   ; Cursor tab handling
**** **** ****     > edkey.fb.tabs.asm
...
0008 603A 06A0  32          bl  @fb.tab.next           ; Jump to next tab position on line
     603C 6040 
0009                       ;------------------------------------------------------
0010                       ; Exit
0011                       ;------------------------------------------------------
0012               edkey.action.fb.tab.next.exit:
0013 603E 10FF  14         jmp  $
**** **** ****     > stevie_b4.asm.48243
0070                       ;-----------------------------------------------------------------------
0071                       ; Logic for Framebuffer (2)
0072                       ;-----------------------------------------------------------------------
0073                       copy  "fb.tabs.asm"         ; Move cursor to tab position
**** **** ****     > fb.tabs.asm
...
0004               ***************************************************************
0005               * fb.tab.next
0006               * Move cursor to next tab position
0007               ***************************************************************
0008               *  bl   @fb.tab.next
0009               *--------------------------------------------------------------
0010               * INPUT
...
0023               ********|*****|*********************|**************************
0024               fb.tab.next:
0025 6040 0649  14         dect  stack
0026 6042 C64B  30         mov   r11,*stack            ; Save return address
0027 6044 0649  14         dect  stack
0028 6046 C644  30         mov   tmp0,*stack           ; Push tmp0
0029 6048 0649  14         dect  stack
0030 604A C645  30         mov   tmp1,*stack           ; Push tmp1
0031 604C 0649  14         dect  stack
0032 604E C646  30         mov   tmp2,*stack           ; Push tmp2
0033                       ;-------------------------------------------------------
0034                       ; Initialize
0035                       ;-------------------------------------------------------
0036 6050 0204  20         li   tmp0,tv.tabs.table     ; Get pointer to tabs table
     6052 3484 
0037 6054 0206  20         li   tmp2,20                ; Up to 20 tabs supported
     6056 0014 
0038                       ;-------------------------------------------------------
0039                       ; Find next tab position
0040                       ;-------------------------------------------------------
0041               fb.tab.next.loop:
0042 6058 D174  28         movb *tmp0+,tmp1            ; \ Get tab position
0043 605A 0985  56         srl  tmp1,8                 ; / Right align
0044                       ;-------------------------------------------------------
0045                       ; Compare position
0046                       ;-------------------------------------------------------
0047 605C 8160  34         c    @fb.column,tmp1        ; Cursor > tab position?
     605E A10C 
0048 6060 1503  14         jgt  !                      ; Yes, next loop iteration
0049               
0050 6062 C805  38         mov  tmp1,@fb.column        ; Set cursor on tab position
     6064 A10C 
0051 6066 1002  14         jmp  fb.tab.next.exit       ; Exit
0052               
0053 6068 0606  14 !       dec  tmp2
0054 606A 16F6  14         jne  fb.tab.next.loop
0055                       ;------------------------------------------------------
0056                       ; Exit
0057                       ;------------------------------------------------------
0058               fb.tab.next.exit:
0059 606C C1B9  30         mov   *stack+,tmp2          ; Pop tmp2
0060 606E C179  30         mov   *stack+,tmp1          ; Pop tmp1
0061 6070 C139  30         mov   *stack+,tmp0          ; Pop tmp0
0062 6072 C2F9  30         mov   *stack+,r11           ; Pop R11
**** **** ****     > stevie_b4.asm.48243
0074                       ;-----------------------------------------------------------------------
...
0080                       ;-----------------------------------------------------------------------
0084 6074 6074                   data $                ; Bank 1 ROM size OK.
0086                       ;-------------------------------------------------------
...
0096      336E     spvmod  equ   stevie.tx8030         ; Video mode.   See VIDTAB for details.
