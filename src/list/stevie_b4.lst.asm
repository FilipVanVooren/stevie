...
0008               ***************************************************************
0009               * File: stevie_b4.asm               ; Version 220324-2030520
0010               *
0011               * Bank 4 "Janine"
...
0270 21ED   42             text  'Build-ID  220324-2030520'
     21EE 7569     
     21F0 6C64     
     21F2 2D49     
     21F4 4420     
...
0030 32BC 6008                   data bankid           ; / i  p2 = Source ROM bank for return
0031               
...
0056                       ;-----------------------------------------------------------------------
0057                       copy  "fb.null2char.asm"    ; Replace null characters in framebuffer row
     **** ****     > fb.null2char.asm
...
0078 609C 045B  20         b     *r11                  ; Return to caller
                   < stevie_b4.asm.39415
0058                       copy  "fb.tab.next.asm"     ; Move cursor to next tab position
     **** ****     > fb.tab.next.asm
...
0120 6126 045B  20         b     *r11                  ; Return to caller
                   < stevie_b4.asm.39415
0059                       copy  "fb.ruler.asm"        ; Setup ruler with tab positions in memory
     **** ****     > fb.ruler.asm
...
0028 6138 06A0  32         bl    @cpym2m
     613A 24EE     
0029 613C 397A                   data txt.ruler,fb.ruler.sit,80
     613E A31E     
     6140 0050     
...
0072 617E 045B  20         b     *r11                  ; Return
                   < stevie_b4.asm.39415
0060                       copy  "fb.colorlines.asm"   ; Colorize lines in framebuffer
     **** ****     > fb.colorlines.asm
...
0113 620C 045B  20         b     *r11                  ; Return
                   < stevie_b4.asm.39415
0061                       copy  "fb.vdpdump.asm"      ; Dump framebuffer to VDP SIT
     **** ****     > fb.vdpdump.asm
...
0074 625C 045B  20         b     *r11                  ; Return to caller
                   < stevie_b4.asm.39415
0062                       copy  "fb.scan.fname.asm"   ; Scan line for device & filename
     **** ****     > fb.scan.fname.asm
...
0064                       ; Stubs
0065                       ;-----------------------------------------------------------------------
0066                       copy  "rom.stubs.bank4.asm" ; Bank specific stubs
     **** ****     > rom.stubs.bank4.asm
0001               * FILE......: rom.stubs.bank4.asm
0002               * Purpose...: Bank 4 stubs for functions in other banks
                   < stevie_b4.asm.39415
0067                       copy  "rom.stubs.bankx.asm" ; Stubs to include in all banks > 0
     **** ****     > rom.stubs.bankx.asm
...
0074                       ;-----------------------------------------------------------------------
0078                       ;-----------------------------------------------------------------------
0079                       ; Show ROM bank in CPU crash screen
0080                       ;-----------------------------------------------------------------------
0081                       copy "rom.crash.asm"
     **** ****     > rom.crash.asm
...
0083                       ; Vector table
0084                       ;-----------------------------------------------------------------------
0085                       copy  "rom.vectors.bank4.asm"
...
0050 7FFE 2026     vec.32  data  cpu.crash             ;
                   < stevie_b4.asm.39415
0086                                                   ; Vector table bank 4
0087               *--------------------------------------------------------------
0088               * Video mode configuration
0089               *--------------------------------------------------------------
0090      00F4     spfclr  equ   >f4                   ; Foreground/Background color for font.
0091      0004     spfbck  equ   >04                   ; Screen background color.
0092      3654     spvmod  equ   stevie.80x30          ; Video mode.   See VIDTAB for details.
0093      000C     spfont  equ   fnopt3                ; Font to load. See LDFONT for details.
0094      0050     colrow  equ   80                    ; Columns per row
0095      0FC0     pctadr  equ   >0fc0                 ; VDP color table base
0096      1100     fntadr  equ   >1100                 ; VDP font start address (in PDT range)
0097      2180     sprsat  equ   >2180                 ; VDP sprite attribute table
0098      2800     sprpdt  equ   >2800                 ; VDP sprite pattern table
